...
// Função de arranque (AGORA ASSÍNCRONA)
...
async function initApp() {
    ...
    try {
        ...
        // CHECK NO SERVIDOR (IGNORA CACHE LOCAL)
        const snap = await getDocFromServer(docRef);

        // A. SITE NÃO EXISTE MAIS (EXCLUÍDO)
        if (!snap.exists()) {
            localStorage.clear(); 
        ...
        // Monitoramento em tempo real (Expulsa se deletar durante o uso)
        setInterval(async () => {
            try {
        ...
        return true; // RETORNA TRUE PARA LIBERAR O SITE

    } catch (error) {
        console.error("Bloqueio:", error);
        
        // Garante que o body esteja visível para mostrar a mensagem de erro
        document.body.style.display = 'block';
        
        if (error.message === "SITE_DELETED" || error.code === 'permission-denied') {
            exibirTelaMorte("404", "Esta loja foi encerrada.");
        } else if (error.message === "SITE_BLOCKED") {
            exibirTelaMorte("Suspenso", "Loja temporariamente indisponível.");
        } else {
            exibirTelaMorte("Erro", "Não foi possível carregar a loja.");
        }
        return false; // RETORNA FALSE PARA BLOQUEAR O RESTO
    }
}
