...
function loadAdminSales() {
    // 1. Conecta ao banco em tempo real
    ...
    onSnapshot(q, (snapshot) => {
        ...
        // 3. Conta APENAS os que não foram visualizados (!o.viewed)
        const newOrdersCount = state.orders.filter(o => !o.viewed).length;

        // 4. Atualiza a Bolinha no Botão
        const btnVendas = document.getElementById('admin-menu-sales');
        if (btnVendas) {
            if (newOrdersCount > 0) {
                btnVendas.innerHTML = `Vendas <span class="bg-red-600 text-white text-[10px] font-bold px-2 py-0.5 rounded-full animate-pulse">${newOrdersCount}</span>`;
        ...
        // 5. Renderiza a lista (Se a função existir)
...
window.filterAndRenderSales = () => {
    // 1. Onde vamos desenhar? (Se não achar a lista, aí sim pode parar)
    const list = document.getElementById('admin-sales-list');
    if (!list) return;

    // 2. Captura Inputs (Com segurança: se não existir, usa string vazia)
    ...
    // TRATAMENTO SEGURO (Evita o erro de tela branca)
    ...
    if (filtered.length === 0) {
        list.innerHTML = '<div class="text-gray-500 text-center py-8"><p>Nenhuma venda encontrada.</p></div>';
    } else {
        filtered.forEach(order => {
            // --- LÓGICA VISUAL DO PEDIDO NOVO ---
            const isNew = !order.viewed; // Se viewed é falso, é novo

            // Borda Verde + Fundo levemente verde
            const borderClass = isNew ? "border-green-500 border-l-4 bg-green-900/10" : "border-gray-800 bg-[#151720]";
            
            // Etiqueta "NOVO"
            const badgeHtml = isNew ? 
                `<span class="ml-2 bg-green-500 text-black text-[9px] font-bold px-2 py-0.5 rounded animate-pulse">NOVO</span>` : "";
            ...
            const idDisplay = (order.code || order.id || '').slice(0, 6);
...
window.openAdminOrderDetail = async (order) => {
    // 1. Marca como visualizado no banco (Isso limpa a notificação)
    if (!order.viewed) {
        try {
    ...
    // 2. Abre o seu modal de detalhes existente
    // (Ajuste o ID 'modal-admin-order' se o seu for diferente)
    const modal = document.getElementById('modal-admin-order');
    if (modal) {
        // Preencha os campos do modal aqui conforme sua lógica original
        // Exemplo:
        if(document.getElementById('admin-order-id-display')) 
           document.getElementById('admin-order-id-display').innerText = `#${order.code || order.id}`;
