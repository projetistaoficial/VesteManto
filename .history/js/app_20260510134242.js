...
window.aplicarCoresCategorias = (catName) => {
    ...
    // As exatas mesmas classes que usaremos no HTML
    const classesInativas = ['bg-white/10', 'border-transparent', 'text-[var(--txt-body)]'];
    const classesAtivas = ['bg-white/20', 'border-[var(--clr-accent)]', 'text-[var(--clr-accent)]'];
    
    todosBotoes.forEach(btn => {
        const isAtivo = btn.dataset.cat === catName || (catName !== '' && catName.startsWith(btn.dataset.cat + ' -'));
        
        if (isAtivo) {
            // Remove as inativas e coloca as ativas
            btn.classList.remove(...classesInativas);
            btn.classList.add(...classesAtivas);
        } else {
            // Remove as ativas e garante as inativas
            btn.classList.remove(...classesAtivas);
