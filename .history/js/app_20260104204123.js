...
function filterAndRenderSales() {
    // 1. Captura Inputs (SEU CÓDIGO ORIGINAL - MANTIDO)
    ...
    // 2. Filtragem (SEU CÓDIGO ORIGINAL - MANTIDO)
    ...
    // 3. Ordenação Inteligente (SEU CÓDIGO ORIGINAL - MANTIDO)
    filtered.sort((a, b) => {
        if (termCode) {
    ...
    // 4. Renderiza (AQUI ESTÁ A ALTERAÇÃO PARA O VISUAL "NOVO")
    const list = document.getElementById('orders-list') || document.getElementById('admin-sales-list');
    ...
    if (list) {
        ...
        if (filtered.length === 0) {
            ...
            filtered.forEach(order => {
                ...
                // Ao clicar, abre os detalhes (certifique-se que openAdminOrderDetail existe)
                div.onclick = () => {
                ...
                // Correção do erro ".slice": Converte para texto antes de cortar
                const codeRaw = order.code || order.id || '';
                const idDisplay = String(codeRaw).slice(0, 6);
                ...
                div.innerHTML = `
                    <div class="flex flex-col gap-1 relative z-10">
                        ...
                        <span class="text-xs font-bold text-yellow-500">${customerName}</span>
                    </div>
