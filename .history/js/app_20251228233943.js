...
function renderProductsList(products, preCalcMetrics = null) {
    ...
    const selectBtnClass = state.isSelectionMode ? 
        "text-red-400 hover:text-red-300 text-xs font-bold uppercase cursor-pointer" : 
        "text-yellow-500 hover:text-yellow-400 text-xs font-bold uppercase cursor-pointer";

    // Barra de Ações em Massa (CORREÇÃO: Só aparece se tiver itens e modo ativo)
    let bulkActionsHTML = '';
    if (state.isSelectionMode && state.selectedProducts.size > 0) {
        bulkActionsHTML = `
            <div class="flex items-center gap-2 animate-fade-in">
                <span class="text-white text-xs mr-2 font-bold bg-blue-900 px-2 py-1 rounded border border-blue-700">${state.selectedProducts.size} selecionados</span>
                <select id="bulk-category-select-dynamic" class="bg-gray-900 text-white text-[10px] border border-gray-600 rounded px-2 h-7 outline-none hidden md:block">
                    <option value="">Mover...</option>${state.categories.map(c => `<option value="${c.name}">${c.name}</option>`).join('')}
                </select>
                <button onclick="bulkMoveDynamic()" class="bg-blue-600 hover:bg-blue-500 text-white px-3 h-7 rounded text-[10px] uppercase font-bold hidden md:block">Mover</button>
    ...
    const getSortIcon = (key) => {
        if (state.sortConfig.key !== key) return '<i class="fas fa-sort text-gray-700 ml-1"></i>';
    ...
    // --- B. CABEÇALHO (CLICÁVEL) ---
    // Coluna 1 (Checkbox/All)
    const checkColContent = state.isSelectionMode 
        ? `<input type="checkbox" onchange="toggleSelectAll(this)" class="cursor-pointer rounded border-gray-600 bg-gray-800 text-yellow-500 focus:ring-0" title="Selecionar Todos">`
        : ``;

    const headerHTML = `
        <div class="hidden md:grid grid-cols-12 gap-2 bg-[#1f1f1f] text-gray-400 font-bold p-3 rounded-t-xl text-[10px] uppercase tracking-wider border-b border-gray-800 sticky top-0 z-20 select-none">
            <div class="col-span-1 text-center flex items-center justify-center h-full">
                ${checkColContent}
            </div>
            
            <div class="col-span-1 text-center border-r border-gray-700 cursor-pointer hover:text-white flex items-center justify-center" onclick="sortProducts('code')">
    ...
    // --- D. RECUPERA OU CALCULA MÉTRICAS (Fallback) ---
    // Se filterAndRenderProducts não passou as métricas (ex: carga inicial), calculamos aqui rapidinho (sem sort)
            ...
            <div class="relative z-10 p-3 transition-transform duration-200 ease-out prod-swipe-content ${bgClass} h-full flex flex-col md:grid md:grid-cols-12 gap-2 items-center">
                ...
                <div class="flex md:contents items-center justify-between w-full">
                    <div class="flex items-center gap-3 md:col-span-6 w-full">
                        ...
                        <div class="hidden md:flex md:col-span-1 items-center justify-center border-r border-gray-800 h-full">
                            <span class="text-base font-bold text-white font-mono">${codeStr}</span>
                        </div>
