...
function filterAndRenderSales() {
    ...
    let filtered = state.orders.filter(o => {
        ...
        let matchCode = true;
        if (termCode) {
            matchCode = String(o.code || o.id).includes(termCode);
        }
        ...
        if (dateStart || dateEnd) {
            // Tenta usar date ou createdAt (compatibilidade)
    ...
    // 4. Renderiza (COM O VISUAL DE PEDIDO NOVO)
    ...
    if (list) {
        ...
        if (filtered.length === 0) {
            ...
            filtered.forEach(order => {
                ...
                const isNew = !order.viewed; // Se viewed for false/undefined, é novo
                ...
                // Clique para abrir e marcar como visto
