...
function loadSettings() {
    ...
    onSnapshot(docRef, (docSnap) => {
        if (docSnap.exists()) {
            ...
            // Atualiza o checkbox se estiver no admin
            if (els.toggleStockGlobal) els.toggleStockGlobal.checked = state.globalSettings.allowNoStock;
        } else {
            console.log("Configurações não encontradas. Usando padrão (Sem gravação).");
            // ERRO ERA AQUI: Removemos o setDoc que travava o site para visitantes
            state.globalSettings = { allowNoStock: true }; // <--- Forçamos TRUE para os produtos aparecerem
        }
        ...
        console.warn("Erro ao ler settings (provavelmente permissão ou inexistente):", error);
        // Fallback para não travar a loja
