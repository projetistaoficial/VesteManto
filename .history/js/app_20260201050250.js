...
function fillProfileForm() {
    ...
    const setCheck = (id, val) => { 
        const el = document.getElementById(id); 
        if (el) el.checked = !!val; // Força booleano
    };
    ...
    // 3. PARCELAMENTO
    const inst = p.installments || { active: false, max: 12, freeUntil: 1, rate: 0 };
    
    setCheck('conf-card-active', inst.active);
    setVal('conf-card-max', inst.max);
    // Usa formatMoneyForInput para exibir "1,50" em vez de "1.5"
    setVal('conf-card-rate', typeof formatMoneyForInput === 'function' ? formatMoneyForInput(inst.rate) : inst.rate); 

    // Atualiza opções do select "Sem Juros" antes de definir o valor
    if (typeof updateFreeInstallmentsSelect === 'function') updateFreeInstallmentsSelect();
    setVal('conf-card-free', inst.freeUntil);

    // Controle visual da opacidade (habilita/desabilita visualmente)
    const elDetails = document.getElementById('conf-card-details');
    if (elDetails) {
        if (inst.active) elDetails.classList.remove('opacity-50', 'pointer-events-none');
        else elDetails.classList.add('opacity-50', 'pointer-events-none');
    }
...
async function saveStoreProfile() {
    ...
    try {
        // --- 1. HELPERS DE CAPTURA SEGURA ---
        const getValue = (id) => {
            const el = document.getElementById(id);
            return el ? el.value.trim() : '';
        };

        // Importante: Para Checkboxes usamos .checked, não .value
        const getChecked = (id) => {
            const el = document.getElementById(id);
        ...
        const data = {
            ...
            whatsapp: getValue('conf-store-wpp').replace(/\D/g, ''), // Remove formatação (só números)
            instagram: getValue('conf-store-insta'),
            facebook: getValue('conf-store-face'),
            address: getValue('conf-store-address'),
            description: getValue('conf-store-desc'),

            // LOGÍSTICA (CEP e RAIO)
            cep: getValue('conf-store-cep').replace(/\D/g, ''), 
            maxDistance: parseFloat(getValue('conf-max-dist').replace(',', '.')) || 0,

            // HORÁRIOS
            openingHours: {
                active: getChecked('conf-hours-active'),
                start: getValue('conf-hours-start'),
                end: getValue('conf-hours-end'),
                block: getChecked('conf-hours-block')
            },

            // PARCELAMENTO (Aqui estava o problema comum)
            installments: {
                active: getChecked('conf-card-active'), // Pega o checkbox corretamente
                max: parseInt(getValue('conf-card-max')) || 12,
                freeUntil: parseInt(getValue('conf-card-free')) || 1,
                // Trata taxa: troca vírgula por ponto
                rate: parseFloat(getValue('conf-card-rate').replace(',', '.')) || 0
            }
        };

        // --- 3. PRESERVA OUTRAS CONFIGURAÇÕES ---
        // (Para não apagar Pix e Formas de Pagamento que não estão neste formulário)
        if (state.storeProfile.paymentMethods) data.paymentMethods = state.storeProfile.paymentMethods;
        if (state.storeProfile.deliveryConfig) data.deliveryConfig = state.storeProfile.deliveryConfig;
        if (state.storeProfile.pixGlobal) data.pixGlobal = state.storeProfile.pixGlobal;
        if (state.storeProfile.settings) data.settings = state.storeProfile.settings;
        ...
        // Atualiza a vitrine (para mostrar/esconder parcelamento nos cards)
        renderCatalog(state.products);
        
        // Atualiza o visual da sidebar e header
