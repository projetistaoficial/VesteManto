...
window.saveProduct = async (e) => {
    ...
    try {
        ...
        if (!id) {
            ...
            if (index === -1) state.products.push(productData);
            else state.products[index] = productData;
        ...
        // ✨ FORÇA ATUALIZAÇÃO DO CAPITAL DE GIRO
        const elGiro = document.getElementById('stat-capital-giro');
        if (elGiro) {
            let capitalGiro = 0;
            state.products.forEach(p => {
                const stock = parseInt(p.stock) || 0;
                const cost = parseFloat(p.cost) || parseFloat(p.price) || 0;
                capitalGiro += (stock * cost);
            });
            elGiro.innerText = formatCurrency(capitalGiro);
        }
