...
async function autoSaveSettings(type) {
    ...
    // 2. PARCELAMENTO E PAGAMENTOS (AQUI ESTAVA O PROBLEMA)
        ...
        // Tratamento correto da vírgula para a taxa
        ...
        // Importante: Captura o estado ATUAL dos checkboxes
        const payConfig = {
            online: {
    ...
    try {
        ...
        // --- CORREÇÃO CRÍTICA: ATUALIZA O ESTADO LOCAL IMEDIATAMENTE ---
        // Isso faz o modal de checkout obedecer a mudança na hora
        ...
        if (dataToUpdate.paymentMethods) {
            ...
            console.log("State Local Atualizado (Pagamentos):", state.storeProfile.paymentMethods);
        }
...
window.openCheckoutModal = () => {
    ...
    // 2. RECUPERA CONFIGURAÇÕES (Leitura Segura)
    const pm = state.storeProfile.paymentMethods || {};
    const dConfig = state.storeProfile.deliveryConfig || {};

    // Verifica status (Online e Entrega)
    // Se for undefined (config antiga), considera ativo (true)
    ...
    // Regra: Entrega só aparece se o Meio de Pagamento estiver ON **E** a Logística estiver ON
    const deliveryOptionVisible = deliveryMethodActive && isLogisticsActive;

    console.log("CHECKOUT CONFIG:", { onlineActive, deliveryOptionVisible });

    // 3. REFERÊNCIAS AOS ELEMENTOS (IDs do seu HTML)
    ...
    if (labelOnline) {
        if (onlineActive) {
            ...
            labelOnline.style.setProperty('display', 'none', 'important'); // Força esconder
    ...
    // 5. AUTO-SELEÇÃO (Evita erro se a opção selecionada sumiu)
    
    // Se Online está visível, marca ele
    if (onlineActive && radioOnline) {
        radioOnline.checked = true;
    }
    // Se Online sumiu, mas Entrega está visível, marca Entrega
    else if (!onlineActive && deliveryOptionVisible && radioDelivery) {
        radioDelivery.checked = true;
    }
    // Se nenhum está visível, desmarca
    else {
