...
function setupEventListeners() {
    ...
    // --- CORREÇÃO 1: Remover 'window.' ---
    // A função handleCheckoutCep está neste arquivo, chame direto
    if (els.checkoutCep) {
        els.checkoutCep.addEventListener('blur', handleCheckoutCep);
    }
    ...
    if (btnBulkDel) btnBulkDel.onclick = async () => {
        ...
        try {
            ...
            await Promise.all(promises);
            state.selectedProducts.clear();
            // Verifique se updateBulkActionBar existe, senão remova essa linha
    ...
    if (btnBulkMove) btnBulkMove.onclick = async () => {
        ...
        try {
            ...
            await Promise.all(promises);
            state.selectedProducts.clear();
            // Verifique se updateBulkActionBar existe
            if (typeof updateBulkActionBar === 'function') updateBulkActionBar();
            filterAndRenderProducts();
            alert("Produtos movidos!");
        } catch (error) { alert("Erro ao mover: " + error.message); }
    ...
    // --- CORREÇÃO 2: Funções de Janela ---
    // Apenas atribua as funções locais para a janela para os onclicks do HTML funcionarem
    ...
    // ATENÇÃO: As funções abaixo PRECISAM estar no window pois são chamadas pelo HTML onclick="..."
    // Mas aqui dentro, usamos elas direto.
    
    // --- CORREÇÃO 3: BOTOES PRINCIPAIS (Use o nome da função sem window.) ---
    
    // Carrinho Desktop
    const btnCart = document.getElementById('cart-btn');
    if (btnCart) {
        btnCart.onclick = openCart; // <--- SEM 'window.'
    }

    // Carrinho Mobile
    const btnCartMob = document.getElementById('cart-btn-mobile');
    if (btnCartMob) {
        btnCartMob.onclick = openCart; // <--- SEM 'window.'
    }
    ...
    if (formProd) {
        formProd.onsubmit = async (e) => {
            e.preventDefault();
            // A função saveProduct precisa estar acessível aqui
            if(typeof saveProduct === 'function') saveProduct(e); 
            else window.saveProduct(e);
        };
    }
    ...
    if (chkDisablePix) {
        ...
        newDisable.addEventListener('change', () => togglePixGlobalUI()); // Sem window.
    }
...
window.calcCheckoutTotal = () => {
    ...
    if (detailsContainer) {
        ...
        if (discountPixDisplay > 0.01) {
            const labelPix = pixGlobal.active ? "Desconto Pix" : "Desconto Pix";
...
// =================================================================
// 🚨 ZONA DE CONEXÃO GLOBAL (COLE NO FINAL DO ARQUIVO APP.JS) 🚨
// =================================================================
// Isso torna as funções visíveis para os botões do HTML (onclick)

// 1. Navegação e Menus
if (typeof openCart === 'function') window.openCart = openCart;
if (typeof closeCartModal === 'function') window.closeCartModal = closeCartModal;
if (typeof toggleSidebar === 'function') window.toggleSidebar = toggleSidebar;
if (typeof showView === 'function') window.showView = showView;
if (typeof filterByCat === 'function') window.filterByCat = filterByCat;
...
if (typeof goToCheckoutView === 'function') window.goToCheckoutView = goToCheckoutView;
if (typeof backToOrderList === 'function') window.backToOrderList = backToOrderList;
if (typeof handleCheckoutCep === 'function') window.handleCheckoutCep = handleCheckoutCep;
if (typeof togglePaymentMode === 'function') window.togglePaymentMode = togglePaymentMode;
if (typeof toggleMethodSelection === 'function') window.toggleMethodSelection = toggleMethodSelection;
if (typeof calcCheckoutTotal === 'function') window.calcCheckoutTotal = calcCheckoutTotal;
...
if (typeof changeModalImage === 'function') window.changeModalImage = changeModalImage;
if (typeof setModalImage === 'function') window.setModalImage = setModalImage;
if (typeof removeTempImage === 'function') window.removeTempImage = removeTempImage;

// 4. Painel Admin (Produtos)
...
if (typeof confirmDeleteProduct === 'function') window.confirmDeleteProduct = confirmDeleteProduct;
if (typeof toggleProductSelection === 'function') window.toggleProductSelection = toggleProductSelection;
if (typeof toggleSelectAll === 'function') window.toggleSelectAll = toggleSelectAll;
if (typeof toggleSelectionMode === 'function') window.toggleSelectionMode = toggleSelectionMode;
if (typeof sortProducts === 'function') window.sortProducts = sortProducts;
if (typeof bulkMoveDynamic === 'function') window.bulkMoveDynamic = bulkMoveDynamic;

// 5. Painel Admin (Pedidos)
if (typeof openAdminOrderDetail === 'function') window.openAdminOrderDetail = openAdminOrderDetail;
if (typeof handleStatusChange === 'function') window.handleStatusChange = handleStatusChange;
if (typeof adminFinalizeOrder === 'function') window.adminFinalizeOrder = adminFinalizeOrder;
if (typeof adminCancelOrder === 'function') window.adminCancelOrder = adminCancelOrder;
if (typeof adminRefundOrder === 'function') window.adminRefundOrder = adminRefundOrder;
if (typeof toggleOrderAccordion === 'function') window.toggleOrderAccordion = toggleOrderAccordion;
if (typeof markAsViewed === 'function') window.markAsViewed = markAsViewed;

// 6. Configurações e Tema
if (typeof saveStoreProfile === 'function') window.saveStoreProfile = saveStoreProfile;
if (typeof cancelProfileEdit === 'function') window.cancelProfileEdit = cancelProfileEdit;
if (typeof saveSettingsManual === 'function') window.saveSettingsManual = saveSettingsManual;
if (typeof togglePixGlobalUI === 'function') window.togglePixGlobalUI = togglePixGlobalUI;
if (typeof savePixGlobal === 'function') window.savePixGlobal = savePixGlobal;
if (typeof cancelPixGlobal === 'function') window.cancelPixGlobal = cancelPixGlobal;
if (typeof loadTheme === 'function') window.loadTheme = loadTheme;
if (typeof saveThemeColors === 'function') window.saveThemeColors = saveThemeColors;
if (typeof resetThemeToDefault === 'function') window.resetThemeToDefault = resetThemeToDefault;
if (typeof previewTheme === 'function') window.previewTheme = previewTheme;
if (typeof updateStoreStatusUI === 'function') window.updateStoreStatusUI = updateStoreStatusUI;

// 7. Rastreio (Cliente)
if (typeof openTrackModal === 'function') window.openTrackModal = openTrackModal;
if (typeof showOrderListView === 'function') window.showOrderListView = showOrderListView;
if (typeof showOrderDetail === 'function') window.showOrderDetail = showOrderDetail;
if (typeof clientCancelOrder === 'function') window.clientCancelOrder = clientCancelOrder;
if (typeof retryWhatsapp === 'function') window.retryWhatsapp = retryWhatsapp;
if (typeof shareStoreLink === 'function') window.shareStoreLink = shareStoreLink;

// 8. DESTRAVAMENTO DE DADOS (Importante para estatísticas zeradas)
console.log("✅ Conectores Globais Ativados.");
if (state && state.user && typeof loadAdminSales === 'function') {
    console.log("🔄 Recarregando vendas para Admin...");
    loadAdminSales(); 
}