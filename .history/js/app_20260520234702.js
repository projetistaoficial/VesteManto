...
// Exemplo de como deve ser no seu app.js (ou onde você escuta o Firebase)
onSnapshot(collection(db, `sites/${state.siteId}/products`), (snapshot) => {
    state.products = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
    
    // ATUALIZE AQUI: Chame o inicializador do stats
    // Certifique-se de que 'state.orders' e 'state.dailyStats' já estejam disponíveis aqui
    if (typeof initStatsModule === 'function') {
        initStatsModule(state.products, state.orders, state.dailyStats);
    }
...
window.saveProduct = async (e) => {
    ...
    try {
        ...
        if (typeof updateStatsData === 'function') {
            updateStatsData(state.products, state.orders, state.dailyStats);
        }
