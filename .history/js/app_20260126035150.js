...
window.handleCheckoutCep = async () => {
    ...
    // 1. BLOQUEIO TOTAL INICIAL (Trava o botão assim que começa)
    if (btnFinish) {
    ...
    try {
        // Busca Endereço (Preenchimento dos campos)
        ...
        if (config.ownDelivery === true) {
            ...
            // CORREÇÃO CRÍTICA: Verifica se deu erro no cálculo
            if (dist === null || isNaN(dist)) {
                if (elDistDisplay) elDistDisplay.innerText = "Erro no cálculo";
                throw new Error("Não foi possível calcular a distância. Verifique se os CEPs estão corretos.");
            }
        ...
        checkoutState.isValidDelivery = false; // Garante que a venda continua bloqueada
