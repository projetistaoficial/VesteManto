...
window.filterByCat = (catName) => {
    // ========================================================
    // ✨ AQUI ESTÁ A CHAVE DA PAUSA DO CARROSSEL!
    // Se o nome for diferente de vazio (clicou numa categoria) = TRUE (Pausa)
    // Se o nome for vazio (clicou em "Todas") = FALSE (Volta a andar)
    // ========================================================
...
window.initCategoryCarousel = () => {
    ...
    // 1. MATA QUALQUER ANIMAÇÃO FANTASMA ANTERIOR
    // Se a tela recarregar as categorias, isso evita congelamento
    ...
    const playAutoScroll = () => {
        // Salva a ID da animação para podermos matar depois se necessário
        window.carouselAnimationId = requestAnimationFrame(playAutoScroll);

        // Se uma categoria estiver selecionada ou usuário estiver com a mão nela, congela.
        if (!isAutoScrolling || window.isCategorySelected) {
            return;
        }

        // Calcula o espaço máximo que dá pra rolar (Tamanho total - Tamanho visível)
        const maxScroll = slider.scrollWidth - slider.clientWidth;

        // Se a quantidade de categorias for pequena e couber tudo na tela, não rola
        if (maxScroll <= 0) return;

        // Bateu na direita? (Uso maxScroll - 2 de margem de segurança pro navegador não engasgar)
        if (slider.scrollLeft >= maxScroll - 2) {
            scrollDirection = -1; // Volta pra esquerda
        }
        // Bateu na esquerda?
        else if (slider.scrollLeft <= 0) {
            scrollDirection = 1; // Vai pra direita
        }
    ...
    // 2. GARANTE QUE OS EVENTOS DE MOUSE/DEDO SÓ SEJAM COLOCADOS 1 VEZ
    if (!slider.dataset.eventsSet) {
        ...
        slider.addEventListener('touchend', () => {
            // Espera 2 segundos após tirar o dedo para voltar a rodar
            setTimeout(() => { isAutoScrolling = true; }, 2000);
        });
    }
};