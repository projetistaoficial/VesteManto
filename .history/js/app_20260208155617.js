...
async function initApp() {
    // === 1. A CORTINA DE FERRO (IMEDIATA) ===
    // Salva o layout do site na memória antes de apagar
    const layoutOriginal = document.body.innerHTML;
    
    // Apaga TUDO visualmente. O usuário vê tela preta instantânea.
    ...
    try {
        // === 2. VERIFICAÇÃO NO SERVIDOR (A CHAVE) ===
        // getDocFromServer obriga o navegador a ir na internet conferir
        const docRef = doc(db, "sites", siteId);
        const snap = await getDocFromServer(docRef);

        // A. CENÁRIO: LOJA NÃO EXISTE (FOI DELETADA)
        if (!snap.exists()) {
            localStorage.clear(); // Limpa lixo antigo
            throw new Error("SITE_DELETED");
        }

        const data = snap.data();

        // B. CENÁRIO: LOJA BLOQUEADA
        if (data.status === 'bloqueado' || data.active === false) {
            throw new Error("SITE_BLOCKED");
        }

        // =========================================================
        // 3. SUCESSO: O SITE EXISTE! VAMOS RESTAURAR.
        // =========================================================
        
        // Devolve o HTML original que guardamos lá em cima
        document.body.innerHTML = layoutOriginal;
        document.body.style.backgroundColor = ''; // Remove fundo preto
        
        // Salva dados no estado
        state.storeProfile = data;

        // Carrega o sistema (agora que o HTML existe de novo)
        loadSiteStats();
        ...
        // Re-ativa os listeners (botões/carrinho) pois o HTML foi recriado
        setupEventListeners(); 
        setupKeyboardListeners();

        // --- FINALMENTE: ANIMAÇÃO DE ENTRADA ---
        // O CSS fará o site aparecer suavemente
        setTimeout(() => {
        ...
        // Renderiza a Tela de Morte (Substitui o HTML vazio)
        renderKillScreen(titulo, msg);
