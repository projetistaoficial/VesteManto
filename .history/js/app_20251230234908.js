...
function setupEventListeners() {
    ...
    // --- LOGICA DE PAGAMENTO (VALIDAÇÃO E UI) ---
    ...
    const validateAndSavePay = (e) => {
        // Regra: Pelo menos um deve estar ativo
        if (!checkOnlineActive.checked && !checkDeliveryActive.checked) {
            alert("⚠️ Atenção: Pelo menos uma forma de pagamento (Online ou Entrega) deve permanecer ativa.");
    ...
    if (checkOnlineActive) checkOnlineActive.addEventListener('change', validateAndSavePay);
    if (checkDeliveryActive) checkDeliveryActive.addEventListener('change', validateAndSavePay);

    // Listeners dos itens individuais (Pix, Cartão...)
    const subPayIds = [
        'conf-pay-online-pix', 'conf-pay-online-card',
        'conf-pay-delivery-pix', 'conf-pay-delivery-card', 'conf-pay-delivery-cash'
    ];
    subPayIds.forEach(id => {
        const el = document.getElementById(id);
        if (el) el.addEventListener('change', () => autoSaveSettings('installments'));
    });
}
...
function fillProfileForm() {
    ...
    // Helper para marcar checkbox seguro
    const setCheck = (id, val) => {
        const el = document.getElementById(id);
        if (el) el.checked = (val !== false); // Padrão true
    };
    ...
    // Ajusta opacidade inicial
    const groupOnline = document.getElementById('group-online-methods');
    const groupDelivery = document.getElementById('group-delivery-methods');
    if (groupOnline) groupOnline.className = (payConfig.online?.active !== false) ? "space-y-3 opacity-100" : "space-y-3 opacity-30 pointer-events-none";
    if (groupDelivery) groupDelivery.className = (payConfig.delivery?.active !== false) ? "space-y-3 opacity-100" : "space-y-3 opacity-30 pointer-events-none";
...
window.openCheckoutModal = () => {
    ...
    // 3. CONFIGURAÇÕES DE VISIBILIDADE (LÓGICA NOVA)
    // Recupera configurações de pagamento ou usa padrão (tudo ativo)
    const payConfig = state.storeProfile.paymentMethods || {
        online: { active: true },
        delivery: { active: true }
    };

    // Mantém compatibilidade com a config antiga de logística (se a loja não entrega, não pode pagar na entrega)
    const oldOwnDelivery = state.storeProfile.deliveryConfig?.ownDelivery;
    ...
    const lblOnline = radioOnline?.closest('label'); // O Label clicável do Online
    const radioDelivery = document.querySelector('input[name="pay-mode"][value="delivery"]');

    // --- A. Controla Visibilidade "Pagar Online" ---
    if (payConfig.online.active === false) {
        if (lblOnline) {
            lblOnline.classList.add('hidden');
            lblOnline.style.display = 'none'; // Força ocultação
        }
    } else {
        if (lblOnline) {
    ...
    // --- B. Controla Visibilidade "Pagar na Entrega" ---
    // Esconde se estiver desativado nas formas de pagamento OU se a entrega estiver desativada na logística
    if (payConfig.delivery.active === false || oldOwnDelivery === false) {
    ...
    // --- C. Seleção Automática (Garante que um válido esteja marcado) ---
    if (payConfig.online.active !== false) {
        // Se Online está ativo, ele é a prioridade
        if (radioOnline) radioOnline.checked = true;
    } else if (payConfig.delivery.active !== false) {
        // Se Online está off, mas Entrega está on, marca Entrega
        if (radioDelivery) radioDelivery.checked = true;
    }

    // 4. Atualiza Interface de Pagamento e Totais
    // Isso vai filtrar os sub-métodos (Pix/Cartão/Dinheiro) baseado na escolha acima
    if (typeof window.togglePaymentMode === 'function') window.togglePaymentMode();

    // Recalcula totais (usa a função calcCheckoutTotal que criamos anteriormente que suporta frete)
    if (typeof window.calcCheckoutTotal === 'function') window.calcCheckoutTotal();

    // 5. Troca a visualização do Modal (Navegação)
...
window.submitOrder = async () => {
    try {
        ...
        const order = {
            ...
            total: finalValue, // Valor final da tela (já inclui frete)
            ...
            // --- CORREÇÃO AQUI ---
            // Usar valueToSave (que respeita a regra Online/Entrega) e não shippingFee (que é fixo)
