...
function setupEventListeners() {
 ...
 // UPLOAD DE BANNER (FUNDO DO TOPO)
    ...
    if (bannerInput) {
        bannerInput.addEventListener('change', async (e) => {
            ...
            try {
                // Usa a mesma função de processar imagem da logo
                const base64 = await processImageFile(file);
                
                // Salva na memória temporária
                state.tempBanner = base64;

                // Atualiza o Preview na tela de edição
...
window.submitOrder = async () => {
    try {
        ...
        if (payMode === 'online' || true) {
            ...
            // 3. Valida se o número existe
            if (!targetNumber || targetNumber.length < 10) {
                alert("Aviso: O número de WhatsApp da loja não está configurado corretamente no Painel Admin.");
                openTrackModal();
                return; // Para aqui se não tiver número
            }

            // 4. Adiciona o 55 (Brasil) se o usuário esqueceu
            // Se tiver 10 ou 11 dígitos, provavelmente é DDD + Número (ex: 11999999999)
