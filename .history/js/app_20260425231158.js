...
// =================================================================
let activeAlertsListener = null; // Guarda o radar atual

function startAlertsListener() {
    // 1. Desliga radar antigo para não duplicar avisos
    if (activeAlertsListener) {
        activeAlertsListener();
        activeAlertsListener = null;
    }

    // 2. Trava de segurança: Só liga se for uma loja válida E se for um Vendedor/Admin
    if (!state.siteId || state.siteId === 'demo') return;
    if (!state.user) return; // Se for cliente final, para aqui e não liga o radar!

    console.log("📡 Ligando Radar de Avisos para o Admin...");

    const avisosRef = collection(db, `sites/${state.siteId}/avisos`);
    // Busca APENAS os não lidos, e ordena pelos mais novos
    const q = query(avisosRef, where("lido", "==", false));

    activeAlertsListener = onSnapshot(q, (snapshot) => {
        snapshot.docChanges().forEach(async (change) => {
            if (change.type === "added") {
                const aviso = change.doc.data();
                const avisoId = change.doc.id;

                // 3. Exibe o Modal (Apenas para quem está logado no Admin)
                if (typeof showSystemModal === 'function') {
                ...
                // 4. Marca como lido (Garante que não apareça de novo no próximo F5)
                try {
