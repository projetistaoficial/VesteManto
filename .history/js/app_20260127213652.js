...
async function updateRealTotals() {
    ...
    const dashTotalCost = document.getElementById('stat-cost-total'); // Elemento de Custo
    const dashTotalProfit = document.getElementById('stat-profit-total'); // Elemento de Lucro
    ...
    try {
        ...
        // Pedimos agora a soma do 'total' E do 'totalCost'
        const snapshot = await getAggregateFromServer(q, {
            totalPedidos: count(),
            faturamentoTotal: sum('total'),
            custoTotalAgregado: sum('totalCost') // <--- NOVO PEDIDO DE SOMA
        });

        const { totalPedidos, faturamentoTotal, custoTotalAgregado } = snapshot.data();

        // Calculamos o lucro na hora (Receita - Custo)
        ...
        if (dashTotalProfit) dashTotalProfit.innerText = formatCurrency(lucroLiquido);

        console.log("Financeiro Global:", { 
            Vendas: faturamentoTotal, 
            Custo: custoTotalAgregado, 
            Lucro: lucroLiquido 
        });
        ...
        if (error.message.includes("requires an index")) {
            console.warn("⚠️ IMPORTANTE: Abra o console do navegador e clique no link para criar o índice do campo 'totalCost'!");
        }
    }
}