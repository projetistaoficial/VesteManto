...
// 1. Função que monitora APENAS o estoque e disponibilidade (Leitura muito mais barata)
window.startStockMonitor = () => {
    // Escuta alterações na coleção de produtos, mas apenas nos campos de estoque e status
    // Se o seu Firebase permitir, o ideal é ter uma coleção separada de "estoque" com apenas {id, stock}.
    // Se não, o onSnapshot na coleção de produtos é o caminho.
    ...
    onSnapshot(q, (snapshot) => {
        snapshot.docChanges().forEach((change) => {
            if (change.type === "modified") {
                ...
                // Atualiza o produto na memória local instantaneamente
                const idx = state.products.findIndex(p => p.id === updatedProduct.id);
                if (idx !== -1) {
                    state.products[idx] = updatedProduct;
                }
                
                // Atualiza a vitrine para o cliente ver o "Esgotado" na hora
                if (typeof renderCatalog === 'function') {
                    renderCatalog(state.products);
                }
                
                console.log(`📦 Estoque atualizado em tempo real: ${updatedProduct.name}`);
            }
        });
    });
};
