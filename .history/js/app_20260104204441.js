...
function loadAdminSales() {
    ...
    onSnapshot(q, (snapshot) => {
        ...
        // 3. Conta APENAS os não visualizados (!o.viewed)
        const newOrdersCount = state.orders.filter(o => !o.viewed).length;

        // 4. Atualiza a Bolinha no Botão "Vendas"
        const btnVendas = document.getElementById('admin-menu-sales');
        if (btnVendas) {
            if (newOrdersCount > 0) {
                // Texto com a bolinha pulsando
                btnVendas.innerHTML = `Vendas <span class="bg-red-600 text-white text-[10px] font-bold px-2 py-0.5 rounded-full ml-1 animate-pulse">${newOrdersCount}</span>`;
                document.title = `(${newOrdersCount}) Painel Admin`;
            } else {
        ...
        // 5. Renderiza a lista e atualiza Dashboard
...
function filterAndRenderSales() {
    // 1. Captura Inputs (SEU CÓDIGO MANTIDO)
    ...
    // 2. Filtragem (SEU CÓDIGO MANTIDO)
    ...
    // 3. Ordenação Inteligente (SEU CÓDIGO MANTIDO)
    filtered.sort((a, b) => {
        if (termCode) {
    ...
    // 4. Renderiza (ALTERADO APENAS AQUI PARA INCLUIR O "NOVO")
    // Troquei a chamada renderSalesList(filtered) pelo loop manual abaixo:
    const list = document.getElementById('orders-list'); // Tentei manter o ID original, mas verifique se é 'orders-list' ou 'admin-sales-list'
    if (list) {
        list.innerHTML = '';

        if (filtered.length === 0) {
            list.innerHTML = '<div class="text-gray-500 text-center py-8"><i class="fas fa-inbox text-4xl mb-2 opacity-50"></i><p>Nenhum pedido encontrado.</p></div>';
        } else {
            filtered.forEach(order => {
                // --- AQUI ESTÁ A LÓGICA DO NOVO ---
                const isNew = !order.viewed; // Se não foi visto, é novo
                
                // Estilo: Borda verde se novo, cinza se normal
                const borderClass = isNew ? "border-green-500 border-l-4 bg-green-900/10" : "border-gray-800 bg-[#151720]";
                
                // Texto: Badge NOVO piscando
                const badgeHtml = isNew ? `<span class="ml-2 bg-green-500 text-black text-[9px] font-bold px-2 py-0.5 rounded animate-pulse">NOVO</span>` : "";
                // ----------------------------------

                // Correção do erro .slice (converte numero pra texto antes)
                const codeDisplay = String(order.code || order.id || "").slice(0, 6);
                
                const div = document.createElement('div');
                div.className = `p-4 rounded-xl border ${borderClass} flex justify-between items-center mb-2 cursor-pointer hover:bg-gray-800 transition relative`;
                
                // Função de clique
                div.onclick = () => { if(typeof openAdminOrderDetail === 'function') openAdminOrderDetail(order); };

                div.innerHTML = `
                    <div class="flex flex-col gap-1">
                        <div class="flex items-center">
                            <span class="font-bold text-white text-sm">#${codeDisplay}</span>
                            ${badgeHtml}
                        </div>
                        <span class="text-xs text-gray-400">${new Date(order.date).toLocaleString()}</span>
                        <span class="text-xs font-bold text-yellow-500">${order.customerName || (order.customer ? order.customer.name : 'Cliente')}</span>
                    </div>
                    
                    <div class="text-right">
                        <div class="font-bold text-green-400 text-sm">R$ ${parseFloat(order.total).toFixed(2)}</div>
