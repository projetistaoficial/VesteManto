...
function populateInstallments() {
    ...
    // 1. Calcula o Valor dos Produtos (Já com desconto do cupom)
    let totalProdutos = 0;
    state.cart.forEach(i => totalProdutos += i.price * i.qty);

    // Aplica Cupom sobre os produtos
    if (state.currentCoupon) {
        if (state.currentCoupon.type === 'percent') {
            totalProdutos -= totalProdutos * (state.currentCoupon.val / 100);
        } else {
            totalProdutos -= state.currentCoupon.val;
        }
    }
    
    // Garante que não fique negativo
    totalProdutos = Math.max(0, totalProdutos);

    // 2. Calcula o Frete Separadamente
    let valorFrete = 0;
    const dConfig = state.storeProfile.deliveryConfig || {};
    ...
    // Verifica se o frete deve ser cobrado (usando a validação de CEP existente)
    ...
    const maxParcelas = (instConfig.active && (totalProdutos + valorFrete) > 0) ? instConfig.max : 1;

    for (let i = 1; i <= maxParcelas; i++) {
        
        // A) Calcula o montante dos produtos (Com ou Sem Juros)
        let montanteProdutos = totalProdutos;
        let label = `${i}x Sem Juros`;

        // Aplica Juros APENAS nos produtos
        if (instConfig.active && i > instConfig.freeUntil && instConfig.rate > 0) {
            const taxa = instConfig.rate / 100;
            const fator = Math.pow(1 + taxa, i);
            
            // Fórmula Price (Juros Compostos) aplicada somente aos produtos
            const parcelaProdutos = totalProdutos * ((taxa * fator) / (fator - 1));
            
            montanteProdutos = parcelaProdutos * i;
            label = `${i}x (c/ juros)`;
        }

        // B) Soma o Frete (Sem juros) ao montante final
        const totalFinal = montanteProdutos + valorFrete;
        
        // C) Calcula o valor da parcela final para exibição
        const valorParcelaFinal = totalFinal / i;

        const option = document.createElement('option');
        option.value = i;
        // Salva o total final no dataset para o calcCheckoutTotal ler depois
        option.dataset.total = totalFinal.toFixed(2);
        option.text = `${label} de ${formatCurrency(valorParcelaFinal)}`;
        
        select.appendChild(option);
    }
    
    // Atualiza o total exibido no botão verde
    calcCheckoutTotal();
}
...
window.calcCheckoutTotal = () => {
    ...
    // A. PIX
    if (method === 'pix') {
        // 1. Calcula o valor total considerando preço de Pix de cada produto
        ...
        // O desconto do Pix é a diferença entre o Subtotal Original e a Soma dos Preços Pix
        discountPixDisplay = Math.max(0, subtotalDisplay - totalWithPixPrices);

        // 2. Aplica Cupom sobre o valor já reduzido do Pix
        if (state.currentCoupon) {
            if (state.currentCoupon.type === 'percent') {
    ...
    // B. CARTÃO DE CRÉDITO (ONLINE)
    else if (method === 'credit' && payMode === 'online') {
        // Aplica cupom sobre o subtotal normal
        if (state.currentCoupon) {
            if (state.currentCoupon.type === 'percent') {
        ...
        // Base para parcelas = (Subtotal - Cupom)
        let baseParaParcelas = Math.max(0, subtotalDisplay - discountCouponDisplay);

        const select = document.getElementById('checkout-installments');
        
        // Se tem parcelamento selecionado, pega o total COM juros do dropdown
        if (select && select.options.length > 0) {
            const selectedOpt = select.options[select.selectedIndex];
            if (selectedOpt && selectedOpt.dataset.total) {
                // O valor do dropdown já inclui o juros calculado sobre o produto
                let totalParcelado = parseFloat(selectedOpt.dataset.total);
                
                // Final = TotalParcelado + Frete
                // (O frete não sofre juros na nossa lógica corrigida anteriormente)
                finalTotal = totalParcelado + shippingDisplay;

                // Verifica se teve juros para exibir aviso
                const instConfig = state.storeProfile?.installments;
                const parcelas = parseInt(select.value);
    ...
    // C. OUTROS (Débito, Dinheiro, Cartão na Entrega)
    else {
        // Aplica cupom sobre o subtotal normal
        if (state.currentCoupon) {
            if (state.currentCoupon.type === 'percent') {
    ...
    // --- 4. RENDERIZAÇÃO DO DETALHAMENTO (HTML) ---
    ...
    if (detailsContainer) {
        ...
        if (shippingDisplay > 0) {
            ...
            `;
        } else if (typeof checkoutState !== 'undefined' && checkoutState.isValidDelivery) {
             // Opcional: Mostrar "Frete Grátis" se for entrega válida e valor 0
             /* html += `
    ...
    // Remove elementos antigos que não usamos mais nessa nova view
    if(els.labelPixDiscount) els.labelPixDiscount.innerText = '';
    if(els.installmentObs) els.installmentObs.innerText = '';
