...
function setupEventListeners() {
   ...
   // =================================================================
    // CONTROLE DE PAGAMENTOS (LISTENERS & TRAVAS) - VERSÃO BLINDADA
    // =================================================================
    ...
    if (elOnline) {
        ...
        const newOnline = elOnline.cloneNode(true);
        elOnline.parentNode.replaceChild(newOnline, elOnline);

        newOnline.addEventListener('change', async (e) => {
            const isActive = e.target.checked;
            
            // Segurança: Verifica se o outro está ativo
            const isDeliveryActive = document.getElementById('conf-pay-delivery-active').checked;
            if (!isActive && !isDeliveryActive) {
                showSystemModal("⚠️ Você precisa manter pelo menos uma forma de pagamento ativa.");
                e.target.checked = true; // Reverte visual
                return;
            }

            // ATUALIZAÇÃO OTIMISTA (AQUI ESTÁ A CORREÇÃO)
            // Atualiza a memória ANTES de salvar, para que o fillProfileForm não reverta
            if (!state.storeProfile.paymentMethods) state.storeProfile.paymentMethods = { online: {}, delivery: {} };
            if (!state.storeProfile.paymentMethods.online) state.storeProfile.paymentMethods.online = {};
            state.storeProfile.paymentMethods.online.active = isActive;
    ...
    // 2. PAGAMENTO NA ENTREGA (A LÓGICA DE DEPENDÊNCIA ESTÁ AQUI)
    const elDelivery = document.getElementById('conf-pay-delivery-active');
    if (elDelivery) {
        const newDelivery = elDelivery.cloneNode(true);
        elDelivery.parentNode.replaceChild(newDelivery, elDelivery);

        newDelivery.addEventListener('change', async (e) => {
            const isActive = e.target.checked;
            
            // Segurança
            const isOnlineActive = document.getElementById('conf-pay-online-active').checked;
            ...
            // ATUALIZAÇÃO OTIMISTA DO PAGAMENTO
            if (!state.storeProfile.paymentMethods) state.storeProfile.paymentMethods = { online: {}, delivery: {} };
            ...
            // LÓGICA DE VÍNCULO COM ENTREGA PRÓPRIA
            const ownCheck = document.getElementById('conf-own-delivery');
            if (ownCheck) {
                // Se DESLIGOU pagamento -> DESLIGA entrega
                if (!isActive) {
                    if (ownCheck.checked) {
                        ownCheck.checked = false; // Visual
                        
                        // Otimista Logística
                        if (state.storeProfile.deliveryConfig) state.storeProfile.deliveryConfig.ownDelivery = false;

                        // Trava código
                        const elReq = document.getElementById('conf-req-code');
                        if (elReq) { elReq.disabled = true; elReq.checked = false; elReq.closest('label')?.classList.add('opacity-50'); }

                        showSystemModal("⚠️ Ao remover o 'Pagamento na Entrega', a 'Entrega Própria' também foi desativada.");
                        await autoSaveSettings('orders'); // Salva Logística
                    }
                } 
                // Se LIGOU pagamento -> LIGA entrega
                else {
                    if (!ownCheck.checked) {
                        ownCheck.checked = true; // Visual
                        
                        // Otimista Logística
                        if (state.storeProfile.deliveryConfig) state.storeProfile.deliveryConfig.ownDelivery = true;

                        // Destrava código
                        const elReq = document.getElementById('conf-req-code');
                        if (elReq) { elReq.disabled = false; elReq.closest('label')?.classList.remove('opacity-50'); }
    ...
    if (elOwnDelivery) {
        ...
        newOwn.addEventListener('change', async (e) => {
            ...
            if (!isActive) {
                // Se desligar Entrega, tem que desligar Pagamento na Entrega
                const payCheck = document.getElementById('conf-pay-delivery-active');
                if (payCheck && payCheck.checked) {
                    // Mas só se o Online estiver ligado
                    const isOnlineActive = document.getElementById('conf-pay-online-active').checked;
                    if (!isOnlineActive) {
                        showSystemModal("⚠️ Ação Bloqueada: Você ficaria sem formas de pagamento. Ative o Online antes.");
                        e.target.checked = true; // Volta o check visual
                    ...
                    // Otimista Pagamento
                    if (state.storeProfile.paymentMethods?.delivery) state.storeProfile.paymentMethods.delivery.active = false;
                    
                    updatePaymentVisuals();
                    showSystemModal("ℹ️ O 'Pagamento na Entrega' foi desativado junto com a logística.");
                    await autoSaveSettings('payments');
                }
            } 
            // Se ligou entrega, liga pagamento
            else {
                const payCheck = document.getElementById('conf-pay-delivery-active');
                if (payCheck && !payCheck.checked) {
                    payCheck.checked = true;
                    if (state.storeProfile.paymentMethods?.delivery) state.storeProfile.paymentMethods.delivery.active = true;
                    updatePaymentVisuals();
                    await autoSaveSettings('payments');
                }
            }
    ...
    // Sub-opções (Pix, Crédito, Débito, Dinheiro)
    const subOptions = [
        'conf-pay-online-pix', 'conf-pay-online-credit', 'conf-pay-online-debit',
        'conf-pay-delivery-pix', 'conf-pay-delivery-credit', 'conf-pay-delivery-debit', 'conf-pay-delivery-cash'
    ];
    ...
    subOptions.forEach(id => {
        ...
        if(el) {
            ...
            newEl.addEventListener('change', () => {
                ...
                const groupClass = id.includes('online') ? 'sub-check-online' : 'sub-check-delivery';
...
function fillProfileForm() {
    ...
    // ... (Seus outros campos de nome, logo, etc) ...
    setVal('conf-store-name', p.name);
    // ...

    // --- REGRAS DE ENTREGA ---
    const dConfig = p.deliveryConfig || {};
    // Apenas MARCA ou DESMARCA. Não mexe em listeners!
    setCheck('conf-own-delivery', dConfig.ownDelivery);
    setCheck('conf-req-code', dConfig.reqCustomerCode);
    setVal('conf-cancel-time', dConfig.cancelTimeMin || 5);
    ...
    // Controle Visual Básico
    const elReq = document.getElementById('conf-req-code');
    if (elReq) {
        elReq.disabled = !dConfig.ownDelivery;
        if(dConfig.ownDelivery) elReq.closest('label')?.classList.remove('opacity-50', 'pointer-events-none');
        else elReq.closest('label')?.classList.add('opacity-50', 'pointer-events-none');
    }

    // --- PAGAMENTOS ---
    const pm = p.paymentMethods || {};
    setCheck('conf-pay-online-active', pm.online?.active !== false);
    setCheck('conf-pay-delivery-active', pm.delivery?.active !== false);
    
    // Atualiza visual sem recriar listeners
...
function setupDeliveryDependency() {
    const elOwnDelivery = document.getElementById('conf-own-delivery'); // Checkbox Entrega
    const elPayDelivery = document.getElementById('conf-pay-delivery-active'); // Checkbox Pagamento

    if (!elOwnDelivery || !elPayDelivery) return;

    // --- 1. MEXEU NA ENTREGA PRÓPRIA (Logística) ---
    const newOwn = elOwnDelivery.cloneNode(true);
    elOwnDelivery.parentNode.replaceChild(newOwn, elOwnDelivery);
    ...
    newOwn.addEventListener('change', async (e) => {
        ...
        // 1. ATUALIZA MEMÓRIA IMEDIATAMENTE (Corrige o bug do duplo clique)
        if (state.storeProfile.deliveryConfig) {
            state.storeProfile.deliveryConfig.ownDelivery = isActive;
        }

        // 2. Controle visual do código
        const elReq = document.getElementById('conf-req-code');
        if (elReq) {
            elReq.disabled = !isActive;
            if (!isActive) {
        ...
        // 3. Lógica de Dependência
        if (!isActive) {
            // Se desligou entrega, verifica se precisa derrubar o pagamento
            if (elPayDelivery.checked) {
                // Segurança: Online ligado?
                const onlineOn = document.getElementById('conf-pay-online-active')?.checked;
                if (!onlineOn) {
                    showSystemModal("⚠️ AÇÃO BLOQUEADA:\nO Pagamento Online já está desativado. Você não pode desligar a Entrega.", "warning");
                    e.target.checked = true; // Volta visualmente
                    state.storeProfile.deliveryConfig.ownDelivery = true; // Volta na memória
                    return; 
                }
                
                // Desliga Pagamento
                elPayDelivery.checked = false;
                
                // Atualiza Memória do Pagamento TAMBÉM
                if (state.storeProfile.paymentMethods?.delivery) {
                ...
                showSystemModal("ℹ️ Ao desativar a logística de entrega, o 'Pagamento na Entrega' também foi desativado.", "warning");
                await autoSaveSettings('payments');
            }
        } else {
            // Se ligou entrega, liga pagamento
            if (!elPayDelivery.checked) {
                elPayDelivery.checked = true;
                if (state.storeProfile.paymentMethods?.delivery) {
                    state.storeProfile.paymentMethods.delivery.active = true;
                }
                const group = document.getElementById('group-delivery-methods');
                if (group) group.className = "space-y-3 opacity-100";
                await autoSaveSettings('payments');
            }
        }
    ...
    // --- 2. MEXEU NO PAGAMENTO NA ENTREGA (Financeiro) ---
    ...
    newPay.addEventListener('change', async (e) => {
        ...
        if (!isActive && (!elOnline || !elOnline.checked)) {
            showSystemModal("⚠️ Você não pode desativar todas as formas de pagamento.", "warning");
            e.target.checked = true;
            return;
        }

        // 2. ATUALIZA MEMÓRIA IMEDIATAMENTE (A correção principal)
        ...
        // 4. Dependência Reversa (O seu problema específico)
        const ownCheck = document.getElementById('conf-own-delivery');

        if (ownCheck) {
            // Se desativou Pagamento -> Desativa Entrega
            if (!isActive) {
                if (ownCheck.checked) {
                    ownCheck.checked = false;
                    
                    // Atualiza memória da Entrega IMEDIATAMENTE
                    if (state.storeProfile.deliveryConfig) {
                        state.storeProfile.deliveryConfig.ownDelivery = false;
                    }
                    ...
                    showSystemModal("⚠️ Atenção:\n\nAo remover o 'Pagamento na Entrega', a 'Entrega Própria' também foi desativada.", "warning");
                    await autoSaveSettings('orders'); 
                }
            }
            // Se ativou Pagamento -> Ativa Entrega
            else {
                if (!ownCheck.checked) {
                    ownCheck.checked = true;
                    
                    if (state.storeProfile.deliveryConfig) {
                        state.storeProfile.deliveryConfig.ownDelivery = true;
                    }
