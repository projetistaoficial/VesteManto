...
function setupDeliveryDependency() {
    ...
    newOwn.addEventListener('change', async (e) => {
        ...
        // --- PREPARAÇÃO DOS DADOS (Sem salvar ainda) ---
        // Clona os objetos atuais da memória para não quebrar referências
        const newDeliveryConfig = { ...state.storeProfile.deliveryConfig };
        const newPaymentMethods = JSON.parse(JSON.stringify(state.storeProfile.paymentMethods || { online: {}, delivery: {} }));

        // Aplica a mudança da Entrega
        newDeliveryConfig.ownDelivery = isActive;

        let showWarning = false;

        // LÓGICA DE DEPENDÊNCIA
        if (!isActive) {
            // Se desligou Entrega -> Verifica se precisa desligar Pagamento
            const isPayActive = document.getElementById('conf-pay-delivery-active').checked;
            
            if (isPayActive) {
                const onlineOn = document.getElementById('conf-pay-online-active').checked;
                if (!onlineOn) {
                    alert("AÇÃO BLOQUEADA: Você ficaria sem formas de pagamento. Ative o Online antes.");
                    e.target.checked = true; // Reverte visual
                    return; 
                }
                
                // Desliga Pagamento nos dados
                if (newPaymentMethods.delivery) newPaymentMethods.delivery.active = false;
                
                // Desliga Pagamento Visualmente AGORA
                document.getElementById('conf-pay-delivery-active').checked = false;
                document.getElementById('group-delivery-methods').classList.add('opacity-30', 'pointer-events-none');
                
                showWarning = true;
            }
        } else {
            // Se ligou Entrega -> Liga Pagamento (Opcional)
            const isPayActive = document.getElementById('conf-pay-delivery-active').checked;
            if (!isPayActive) {
                if (newPaymentMethods.delivery) newPaymentMethods.delivery.active = true;
                
                // Liga Pagamento Visualmente AGORA
                document.getElementById('conf-pay-delivery-active').checked = true;
                document.getElementById('group-delivery-methods').classList.remove('opacity-30', 'pointer-events-none');
            }
        }

        // --- ATUALIZA MEMÓRIA IMEDIATAMENTE (CRUCIAL) ---
        state.storeProfile.deliveryConfig = newDeliveryConfig;
        state.storeProfile.paymentMethods = newPaymentMethods;

        // Trava/Destrava código visualmente
        const elReq = document.getElementById('conf-req-code');
        if (elReq) {
        ...
        if (showWarning) showSystemModal("ℹ️ O 'Pagamento na Entrega' foi desativado junto com a logística.");

        // --- SALVAMENTO ATÔMICO (UM ÚNICO TIRO NO BANCO) ---
        try {
            await setDoc(doc(db, `sites/${state.siteId}/settings`, 'profile'), {
                deliveryConfig: newDeliveryConfig,
    ...
    // =================================================================
    // 2. MEXEU NO PAGAMENTO NA ENTREGA (O PROBLEMA ERA AQUI)
    // =================================================================
    ...
    newPay.addEventListener('change', async (e) => {
        ...
        // Aplica mudança do Pagamento
        if (!newPaymentMethods.delivery) newPaymentMethods.delivery = {};
        newPaymentMethods.delivery.active = isActive;

        // Atualiza visual do grupo AGORA
        ...
        if (ownCheck) {
            if (!isActive) {
                ...
                if (ownCheck.checked) {
                    newDeliveryConfig.ownDelivery = false; // Dados
                    ownCheck.checked = false; // Visual
                    
                    // Trava código visualmente
                    const elReq = document.getElementById('conf-req-code');
                ...
                if (!ownCheck.checked) {
                    newDeliveryConfig.ownDelivery = true; // Dados
        ...
        if (showWarning) showSystemModal("⚠️ A 'Entrega Própria' também foi desativada.");
        if (showSuccess) showSystemModal("A 'Entrega Própria' foi ativada automaticamente.", "success");

        // --- SALVAMENTO ATÔMICO (A SOLUÇÃO FINAL) ---
        // Envia TUDO junto. O banco devolve TUDO junto. O fillProfileForm lê TUDO junto.
        // Sem chance de conflito.
        try {
            await setDoc(doc(db, `sites/${state.siteId}/settings`, 'profile'), {
                deliveryConfig: newDeliveryConfig,
                paymentMethods: newPaymentMethods
