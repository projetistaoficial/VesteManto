...
async function autoSaveSettings(type) {
    ...
    // 2. PARCELAMENTO E PAGAMENTOS
        ...
        // Tratamento seguro para taxa (evita erro de parse)
    ...
    try {
        ...
        // --- O PULO DO GATO: SE O CHECKOUT ESTIVER ABERTO, ATUALIZA A TELA ---
        const checkoutModal = document.getElementById('cart-modal');
        const viewCheckout = document.getElementById('view-checkout');
        
        if (checkoutModal && !checkoutModal.classList.contains('hidden') && viewCheckout && !viewCheckout.classList.contains('hidden')) {
             console.log("Atualizando visual do Checkout em tempo real...");
             window.openCheckoutModal(); // Recarrega a função de abrir para aplicar as regras de visibilidade
        }
...
window.openCheckoutModal = () => {
    ...
    // Verifica status (se undefined, assume true)
    ...
    console.log("CHECKOUT VISIBILIDADE:", { onlineActive, deliveryTotalActive });
    ...
    // 4. APLICA VISIBILIDADE (FORÇA BRUTA)
    if (labelOnline) {
        if (onlineActive) {
...
window.togglePaymentMode = () => {
    ...
    // Encontra o wrapper do cartão (div cinza)
    ...
    else if (current.closest('.hidden')) isInvalid = true; // Verifica se está visualmente oculto

    if (isInvalid) {
        // Busca o primeiro habilitado e visível
        const valid = document.querySelector('input[name="payment-method-selection"]:not(:disabled)');
        // Verifica visualmente se o pai não está hidden
...
window.toggleOnlinePayment = () => {
    // 1. Pega o checkbox original da configuração
    const checkbox = document.getElementById('conf-pay-online-active');
    
    if (checkbox) {
        // 2. Inverte o valor (se tá marcado, desmarca e vice-versa)
        checkbox.checked = !checkbox.checked;

        // 3. Notifica visualmente
        const status = checkbox.checked ? "ATIVADO" : "DESATIVADO";
        console.log(`Pagamento Online: ${status}`);

        // 4. Chama o salvamento (Isso atualiza o state.storeProfile e o Firebase)
        // Nota: 'installments' é o tipo que salva as configs de pagamento no seu código
        autoSaveSettings('installments');
    } else {
        alert("Erro: Elemento de configuração não encontrado (conf-pay-online-active).");
    }
};

window.toggleDeliveryPayment = () => {
    // 1. Pega o checkbox original
    const checkbox = document.getElementById('conf-pay-delivery-active');
    ...
    if (checkbox) {
        ...
        // 3. Notifica
        const status = checkbox.checked ? "ATIVADO" : "DESATIVADO";
        console.log(`Pagamento na Entrega: ${status}`);

        // 4. Salva e Atualiza
        autoSaveSettings('installments');
    } else {
        alert("Erro: Elemento de configuração não encontrado (conf-pay-delivery-active).");
    }
};