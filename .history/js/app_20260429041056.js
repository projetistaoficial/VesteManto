...
// 5. ATUALIZAÇÕES CRÍTICAS NO SALVAR E EDITAR PRODUTOS
window.saveProduct = async (e) => {
    ...
    try {
        ...
        const productData = {
            ...
            sizes: finalSizes,
            images: state.tempImages || [],
            paymentOptions: { pix: { active: document.getElementById('prod-pix-active').checked, val: parseFloat(document.getElementById('prod-pix-val').value.replace(/\./g, '').replace(',', '.')) || 0, type: document.getElementById('prod-pix-type').value } }
        };
...
window.editProduct = (id) => {
    ...
    // Verifica se o produto foi salvo no modelo de gradeado novo
    if (p.hasVariations && Array.isArray(p.sizes) && typeof p.sizes[0] === 'object') {
        chkVar.checked = true;
    ...
    toggleStockMode(); // Atualiza a tela
