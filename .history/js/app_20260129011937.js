...
function filterAndRenderSales(resetLimit = true) {
    // Se o argumento resetLimit for true, volta para 50 (usado nos filtros de data/nome)
    if (resetLimit) {
        salesRenderLimit = 50;
    }

    const container = document.getElementById('orders-list'); // Seu container de pedidos
    const loadMoreContainer = document.getElementById('load-more-container');
    
    // 1. FILTRAGEM (Mantém sua lógica atual de filtros)
    // Supondo que 'salesData' seja sua lista completa de vendas original
    let filtered = salesData.filter(o => {
        // ... (Aqui vai toda sua lógica de filtros de Data, Status, Nome, etc.) ...
        // ... Copie a lógica de filtros que você já corrigiu anteriormente ...
        return true; // (Simplificado para o exemplo)
    });

    // ---------------------------------------------------------
    // 2. ATUALIZAR TOTAIS GLOBAIS (Baseado na lista COMPLETA filtrada)
    // Isso garante que o topo da tela mostre o valor total real, não só dos 50
    // ---------------------------------------------------------
    const totalCount = filtered.length;
    const totalValue = filtered.reduce((acc, curr) => acc + (parseFloat(curr.total) || 0), 0);

    // Atualiza os elementos do HTML (Dashboard / Cabeçalho)
    const elTotalCount = document.getElementById('total-sales-count'); // Ajuste o ID conforme seu HTML
    const elTotalValue = document.getElementById('total-sales-value'); // Ajuste o ID conforme seu HTML
    
    if (elTotalCount) elTotalCount.innerText = totalCount;
    if (elTotalValue) elTotalValue.innerText = totalValue.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });

    // ---------------------------------------------------------
    // 3. PAGINAÇÃO E RENDERIZAÇÃO
    // ---------------------------------------------------------
    
    // Pega apenas os primeiros 'salesRenderLimit' itens para exibir
    const visibleItems = filtered.slice(0, salesRenderLimit);
    
    container.innerHTML = ''; // Limpa a lista atual

    if (visibleItems.length === 0) {
        container.innerHTML = '<div class="text-center p-4 text-gray-500">Nenhum pedido encontrado.</div>';
    ...
    visibleItems.forEach(order => {
        // Chame sua função que cria o HTML do card do pedido
        const cardHTML = createOrderCard(order); 
        container.insertAdjacentHTML('beforeend', cardHTML);
    });

    // ---------------------------------------------------------
    // 4. CONTROLE DO BOTÃO "CARREGAR MAIS"
    // ---------------------------------------------------------
    if (loadMoreContainer) {
        // Se a lista filtrada total for maior que o que estamos vendo, mostra o botão
        if (filtered.length > salesRenderLimit) {
            loadMoreContainer.classList.remove('hidden');
            // Opcional: Atualiza texto do botão ex: "Carregar mais (Restam 150)"
            const remaining = filtered.length - salesRenderLimit;
            // btnLoadMore.innerText = `Carregar mais (+${remaining})`; 
        } else {
