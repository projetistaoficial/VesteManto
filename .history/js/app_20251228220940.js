...
function loadAdminSales() {
    ...
    onSnapshot(q, (snapshot) => {
        // 1. Atualiza o estado com as vendas do banco
        state.orders = snapshot.docs.map(d => ({ id: d.id, ...d.data() }));
        
        // 2. Se estiver na tela de vendas, atualiza a lista de vendas
        if (typeof filterAndRenderSales === 'function') filterAndRenderSales();
        
        // 3. Atualiza o Dashboard (se existirem os elementos)
        if (typeof updateDashboardMetrics === 'function') updateDashboardMetrics();
        
        // 4. CRUCIAL: Atualiza a lista de PRODUTOS com as novas métricas de venda
        // Só executa se o elemento da lista de produtos existir na tela
        const productListEl = document.getElementById('admin-product-list');
        if (productListEl) {
            // Chama a função principal de renderização de produtos
            // Se você estiver usando a função filterAndRenderProducts (que chama a renderProductsList), use ela.
...
function renderProductsList(products) {
    ...
    // --- BARRA DE AÇÕES EM MASSA (Mantida) ---
    ...
    // --- CÁLCULO DE MÉTRICAS (CORRIGIDO: LÓGICA ESTRITA) ---
    const metricsMap = {};

    // AQUI ESTÁ O SEGREDO: Só conta se estiver nestes status.
    // "Aguardando aprovação" e "Cancelado" ESTÃO FORA.
    ...
    if (state.orders) {
        state.orders.forEach(order => {
            // Se o status NÃO estiver na lista acima, o pedido é ignorado para contagem
            if (validSaleStatuses.includes(order.status)) {
                ...
                order.items.forEach(item => {
                    ...
                    // Atualiza data se for mais recente
    ...
    // --- CABEÇALHO (Grid 12 colunas alinhado) ---
    const headerHTML = `
