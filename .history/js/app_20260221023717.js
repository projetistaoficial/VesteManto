...
// Listener do Campo CEP (handleCheckoutCep)
window.handleCheckoutCep = async () => {
    ...
    // Bloqueia botão ao iniciar a digitação
    if (btnFinish) {
    ...
    try {
        ...
        // Apenas se a loja ativou "Entrega Própria"
        if (config.ownDelivery === true) {
            if (!storeCep) throw new Error("CEP de origem da loja não configurado.");

            // Calcula a distância
            const dist = await calculateDist_FinalV3(storeCep, cep);

            if (dist === null || isNaN(dist)) {
                // CORREÇÃO DE SEGURANÇA: Se a API falhar e a loja EXIGE raio máximo, BLOQUEIA.
                if (maxDist > 0) {
                    if (elDistDisplay) {
                    ...
                    // Se a loja não tem limite de raio configurado, libera com aviso
                    if (elDistDisplay) {
            ...
            // Se a loja não ativou "Entrega Própria", a lógica de distância é ignorada
            if (typeof checkoutState !== 'undefined') checkoutState.isValidDelivery = true;
        }

        // Libera botão de finalizar se passar nos testes
        ...
        // Garante que o botão fique travado no erro
        if (btnFinish) {
            btnFinish.disabled = true;
            btnFinish.classList.add('opacity-50', 'cursor-not-allowed');
        }
