...
// ATUALIZADO: Agora busca também o Custo e calcula o Lucro
// VERSÃO DEFINITIVA (COM FALLBACK AUTOMÁTICO)
async function updateRealTotals() {
    ...
    // 1. VALORES PADRÃO (Baseados no que já está carregado na tela)
    // Se o servidor falhar, usamos isso para não mostrar ZERO.
    let totalPedidos = state.orders.length;
    let faturamentoTotal = state.orders.reduce((acc, o) => acc + (parseFloat(o.total) || 0), 0);
    
    // Custo só existe em pedidos novos, então somamos o que tiver
    let custoTotalAgregado = state.orders.reduce((acc, o) => acc + (parseFloat(o.totalCost) || 0), 0);
    
    let isServerData = false; // Flag para saber se conseguimos dados reais

    try {
        // 2. TENTATIVA DE BUSCAR NO SERVIDOR (Dados Globais)
        const salesRef = collection(db, `sites/${state.siteId}/sales`);

        // Tenta apenas Contagem e Total primeiro (mais chance de sucesso sem índices novos)
        const snapshot = await getAggregateFromServer(salesRef, {
            count: count(),
            sumTotal: sum('total')
        });

        // Se chegou aqui, funcionou! Substitui os valores locais pelos globais
        totalPedidos = snapshot.data().count;
        faturamentoTotal = snapshot.data().sumTotal;
        isServerData = true;

        // Tenta buscar o Custo separadamente (se falhar por falta de índice, não quebra o resto)
        try {
            const snapCost = await getAggregateFromServer(salesRef, { sumCost: sum('totalCost') });
            custoTotalAgregado = snapCost.data().sumCost;
        } catch (idxError) {
            console.warn("Índice de custo ainda não existe, usando valor local provisório.");
        }

    } catch (error) {
        console.error("⚠️ FALHA DE CONEXÃO/ÍNDICE:", error);
        
        // Se for bloqueio de AdBlock, avisa o usuário
        if (error.message && error.message.includes("BLOCKED")) {
            alert("⚠️ SEU ADBLOCK ESTÁ IMPEDINDO O CÁLCULO TOTAL.\n\nMostrando apenas os pedidos carregados na tela.");
        }
    }

    // 3. ATUALIZA A TELA (Seja com dados do Servidor ou Local)
    
    const lucroLiquido = faturamentoTotal - custoTotalAgregado;

    // Cabeçalho
    if (headerCount) {
        const searchInput = document.getElementById('filter-search-general');
        // Só atualiza o contador se não estiver filtrando
        if (!searchInput || searchInput.value === '') {
            headerCount.innerText = totalPedidos + (isServerData ? "" : "*");
        }
    }
    
    if (headerTotalValue) {
        headerTotalValue.innerText = formatCurrency(faturamentoTotal);
        // Muda o label para indicar se é Global ou Parcial
        const label = headerTotalValue.previousElementSibling;
        if (label && label.innerText.includes('LISTADO') || label.innerText.includes('GLOBAL')) {
            label.innerText = isServerData ? 'TOTAL GLOBAL' : 'TOTAL CARREGADO*';
        }
    }
    ...
    if (dashTotalCost) dashTotalCost.innerText = formatCurrency(custoTotalAgregado);
