...
function renderProductsList(products, preCalcMetrics = null) {
    ...
    // --- A. BARRA DE CONTROLES UNIFICADA ---
    const controlsBar = document.createElement('div');
    // Flex-wrap permite que quebre linha em telas pequenas se necessário
    controlsBar.className = "flex flex-wrap justify-between items-center mb-2 px-1 gap-2 min-h-[40px]";
    
    // 1. Botão de Selecionar / Cancelar (Lado Esquerdo)
    const selectBtnText = state.isSelectionMode ? 
        '<i class="fas fa-times mr-2"></i> Cancelar Seleção' : 
        '<i class="fas fa-check-square mr-2"></i> Selecionar Produtos';
    
    const selectBtnClass = state.isSelectionMode ? 
        "text-red-400 hover:text-red-300 text-xs font-bold uppercase cursor-pointer py-2 px-2" : 
        "text-yellow-500 hover:text-yellow-400 text-xs font-bold uppercase cursor-pointer py-2 px-2";

    // 2. Barra de Ações (Lado Direito - Só aparece se tiver selecionados)
    let bulkActionsHTML = '';
    
    // ESCONDE O CHECKBOX "SELECIONAR TODOS" SE NÃO ESTIVER NO MODO DE SELEÇÃO
    const showCheckAll = state.isSelectionMode;

    if (state.isSelectionMode && state.selectedProducts.size > 0) {
        bulkActionsHTML = `
            <div class="flex items-center gap-2 animate-fade-in bg-[#151720] border border-gray-700 rounded p-1">
                <span class="text-white text-[10px] font-bold bg-blue-600 px-2 py-1 rounded ml-1">${state.selectedProducts.size} item(s)</span>
                
                <select id="bulk-category-select-dynamic" class="bg-black text-white text-[10px] border border-gray-600 rounded px-2 h-7 outline-none hidden md:block">
                    <option value="">Mover para...</option>${state.categories.map(c => `<option value="${c.name}">${c.name}</option>`).join('')}
                </select>
                
                <button onclick="bulkMoveDynamic()" class="bg-blue-600 hover:bg-blue-500 text-white px-3 h-7 rounded text-[10px] uppercase font-bold hidden md:block transition">Mover</button>
                
                <button onclick="document.getElementById('btn-bulk-delete').click()" class="bg-red-600 hover:bg-red-500 text-white px-3 h-7 rounded text-[10px] uppercase font-bold transition flex items-center gap-1">
    ...
    // Adiciona a barra de controles no topo
    ...
    const checkColContent = state.isSelectionMode 
        ? `<input type="checkbox" onchange="toggleSelectAll(this)" class="cursor-pointer rounded border-gray-600 bg-gray-800 text-yellow-500 focus:ring-0 w-4 h-4" title="Selecionar Todos">`
        : ``;

    // Controle de colunas visíveis
    const checkColClass = state.isSelectionMode ? "col-span-1" : "hidden";
    // Se checkbox sumir, "Cód" ganha espaço ou ajustamos as outras
    // Vamos manter grid fixo de 12, mas a primeira coluna (checkbox) fica vazia ou oculta
    
    // Ajuste fino do Grid:
    // Com Checkbox: 1(Check) | 1(Cod) | 4(Prod) | ...
    // Sem Checkbox: 0(Check) | 2(Cod) | 4(Prod) | ... (Código fica maior)
    
    const codColClass = state.isSelectionMode ? "col-span-1" : "col-span-1"; 
    const prodColClass = state.isSelectionMode ? "col-span-4" : "col-span-5"; // Ganha 1 coluna se não tiver checkbox
    ...
    const headerHTML = `
        <div class="hidden md:grid grid-cols-12 gap-2 bg-[#1f1f1f] text-gray-400 font-bold p-3 rounded-t-xl text-[10px] uppercase tracking-wider border-b border-gray-800 sticky top-0 z-20 select-none items-center shadow-lg">
            ...
            <div class="${state.isSelectionMode ? 'col-span-1' : 'col-span-2'} text-center border-r border-gray-700 cursor-pointer hover:text-white flex items-center justify-center h-full" onclick="sortProducts('code')">
                Cód ${getSortIcon('code')}
            </div>
            
            <div class="col-span-4 pl-2 cursor-pointer hover:text-white flex items-center" onclick="sortProducts('product')">
    ...
    // --- D. RECUPERA OU CALCULA MÉTRICAS ---
