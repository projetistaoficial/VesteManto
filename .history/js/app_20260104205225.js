...
function renderSalesList(orders) {
    ...
    orders.forEach(o => {
        // --- NOVO CÓDIGO (Lógica do Visual Novo) ---
        const isNew = !o.viewed; // Se não tem 'viewed: true', é novo
        
        // Se for novo, adiciona borda verde. Se não, borda padrão.
        const borderStyle = isNew ? "border-green-500 border-l-4" : "border-gray-800";
        
        // Se for novo, adiciona o badge piscando
        const newBadge = isNew ? `<span class="ml-2 bg-green-600 text-white text-[9px] font-bold px-2 py-0.5 rounded animate-pulse shadow-lg">NOVO</span>` : "";
        // --------------------------------------------
        ...
        // 6. Monta Card
        // --- ALTERADO AQUI: Adicionado onclick para marcar como visto ---
        ...
        cardWrapper.innerHTML = `
            <div id="order-header-${o.id}" onclick="${clickAction}" 
                 class="bg-black border ${borderStyle} p-4 rounded-xl flex justify-between items-center cursor-pointer hover:border-gray-600 transition-all shadow-md relative z-10">
                <div class="flex items-center gap-4 flex-1">
                    <span class="text-yellow-500 font-bold text-xl tracking-wide flex items-center">
                        Pedidos #${o.code}
                        ${newBadge} </span>
...
// ADICIONE ESTA PEQUENA FUNÇÃO AUXILIAR NO FINAL DO ARQUIVO PARA FUNCIONAR O CLIQUE
window.markAsViewed = async (id) => {
    // Só tenta marcar se existir a variável global 'state' e ela tiver 'orders'
    if (window.state && window.state.orders) {
        const order = state.orders.find(o => o.id === id);
        // Só atualiza no banco se o pedido for novo (para economizar escrita no banco)
        if (order && !order.viewed) {
            try {
                // Atualiza no Firebase (o listener em tempo real vai atualizar a tela sozinho)
