...
function loadAdminSales() {
    ...
    onSnapshot(q, (snapshot) => {
        ...
        // 1. ATUALIZA A ABA DO NAVEGADOR
        updateBrowserTab(newOrdersCount);

        // 2. ATUALIZA O BOTÃO "VENDAS" NO MENU
        ...
        if (salesBtn) {
            if (newOrdersCount > 0) {
                ...
                salesBtn.innerHTML = `
                Vendas 
                <span class="bg-red-600 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full shadow-lg animate-pulse">
                    ${newOrdersCount}
                </span>
            `;
            } else {
