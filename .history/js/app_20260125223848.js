...
function populateInstallments() {
    ...
    // 1. Calcula o Valor dos Produtos (Base para Juros)
    let totalProdutos = 0;
    state.cart.forEach(i => totalProdutos += i.price * i.qty);

    // Aplica Cupom APENAS nos produtos
    if (state.currentCoupon) {
    ...
    // 2. Calcula o Frete (Somado POR FORA, sem juros)
    let valorFrete = 0;
    const dConfig = state.storeProfile.deliveryConfig || {};
    ...
    for (let i = 1; i <= maxParcelas; i++) {
        ...
        if (instConfig.active && i > instConfig.freeUntil && instConfig.rate > 0) {
            const taxa = instConfig.rate / 100; // ex: 50% vira 0.5
            const fator = Math.pow(1 + taxa, i); // (1.5)^16
            
            // Fórmula PMT = PV * [ i(1+i)^n / ((1+i)^n - 1) ]
            // Calcula o valor exato da parcela dos produtos
            const parcelaPrice = totalProdutos * ((taxa * fator) / (fator - 1));
            
            // O montante total é a parcela x número de vezes
            montanteProdutos = parcelaPrice * i;
            
            label = `${i}x (c/ juros)`;
        }

        // 4. Soma Final: (Produtos com/sem juros) + Frete "Limpo"
        const totalFinal = montanteProdutos + valorFrete;
        
        // 5. Parcela Final para Exibição
        const valorParcelaFinal = totalFinal / i;

        const option = document.createElement('option');
        option.value = i;
        
        // Salva o total no dataset com precisão de 2 casas
    ...
    // Atualiza o total verde no final
