    ...
    try {
        ...
        if (payMode === 'online') {
            ...
            if (targetNumber) {
                ...
                // --- SAFARI/IOS FIX ---
                // Safari blocks window.open inside async functions if too much time passes.
                // We try window.open first, and if it fails (returns null or is blocked), we redirect.
                const newWindow = window.open(url, '_blank');
                
                if (!newWindow || newWindow.closed || typeof newWindow.closed == 'undefined') {
                     // Fallback: Redirect current tab (Works on all mobile browsers)
