...
window.saveProduct = async (e) => {
    ...
    try {
        ...
        const pixValEl = document.getElementById('prod-pix-val');
        const pixTypeEl = document.getElementById('prod-pix-type');

        // ✨ ESCUDOS ANTI-NaN ✨
        // Evita que campos vazios ou com texto inválido virem NaN no banco
        const parseVal = (val) => {
        ...
        const parseValNull = (val) => {
            if (!val || val.trim() === "") return null; // Promo e Custo podem ser Null
            const num = parseFloat(val.replace(/\./g, '').replace(',', '.'));
            return isNaN(num) ? null : num;
        };

        let rawStock = (stockEl && stockEl.value.trim() !== "") ? parseInt(stockEl.value, 10) : 0;
        const safeStock = isNaN(rawStock) ? 0 : rawStock;

        // 4. Monta o Objeto Blindado
        const productData = {
            name: nameEl ? nameEl.value.trim() : 'Sem Nome',
            category: catEl ? catEl.value : "Geral",
            description: descEl ? descEl.value.trim() : '',
            
            price: priceEl ? parseVal(priceEl.value) : 0,
            promoPrice: promoEl ? parseValNull(promoEl.value) : null,
            stock: safeStock, // Zero forçado se deixado em branco
            cost: costEl ? parseValNull(costEl.value) : null,
            ...
            paymentOptions: {
                pix: {
                    active: pixActiveEl ? pixActiveEl.checked : false,
                    val: pixValEl ? parseVal(pixValEl.value) : 0,
                    type: pixTypeEl ? pixTypeEl.value : 'percent'
                }
            }
        };
