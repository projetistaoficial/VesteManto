...
async function fetchGlobalStats() {
    console.log(">>> 1. Iniciando fetchGlobalStats (Modo Blindado)...");
    ...
    try {
        ...
        console.log(`>>> DADOS REAIS: Pedidos=${totalPedidos}, Faturamento=${faturamentoTotal}`);

        // --- ATUALIZAÇÃO "TIRO DE CANHÃO" (Tenta todos os IDs possíveis) ---

        // 1. TOTAL DE PEDIDOS
        const idsCount = ['stat-sales-count', 'dash-confirmed-count', 'dash-total-items', 'orders-count'];
        idsCount.forEach(id => {
            const el = document.getElementById(id);
            if (el) el.innerText = totalPedidos;
        });

        // 2. FATURAMENTO TOTAL
        const idsTotal = ['stat-sales-total', 'dash-total-value', 'orders-filtered-total'];
        idsTotal.forEach(id => {
            const el = document.getElementById(id);
            if (el) {
                el.innerText = formatCurrency(faturamentoTotal);
                // Se for o cabeçalho da lista, ajusta o texto
