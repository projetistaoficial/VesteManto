...
function renderCatalog(productsToRender) {
    ...
    // RECUPERA CONFIGURAÇÕES GLOBAIS DE PIX
    ...
    filtered.forEach(p => {
        ...
        // --- LÓGICA DE EXIBIÇÃO DO PIX (HIERARQUIA) ---
        let pixHtml = '';

        // 1. Verifica se "Desativar Tudo" está OFF
        if (!pixGlobal.disableAll) {
            
            // A) Regra Global Ativa
            if (pixGlobal.active && pixGlobal.value > 0) {
                if (pixGlobal.mode === 'product') {
                    // MODO PRODUTO: Calcula e exibe o valor com desconto
...
window.openProductModal = (productId) => {
    ...
    // --- CÁLCULOS E VISUALIZAÇÃO DE PREÇO ---
    ...
    if (!pixGlobal.disableAll) {
        ...
        if (pixGlobal.active && pixGlobal.value > 0) {
            ...
            if (pixGlobal.mode === 'product') {
                // Modo Produto: Mostra valor calculado
                ...
                // Modo Total: Mostra aviso genérico
                pixHtml = `
                    <div class="flex items-center gap-2 text-sm text-gray-300">
                        <i class="fab fa-pix text-green-400 text-lg"></i>
                        <span><span class="text-green-400 font-bold">Desconto no Pix</span> aplicado no checkout</span>
                    </div>`;
            }

        } 
        // B) Individual (Fallback)
        else {
