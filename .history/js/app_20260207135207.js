...
async function initApp() {
    ...
    // 1. LIMPEZA NUCLEAR IMEDIATA
    // Remove todo o HTML do site para garantir que nada antigo apareça
    document.body.innerHTML = '';
    document.body.style.backgroundColor = '#000'; // Fundo preto enquanto carrega

    // Se não tiver ID na URL
    if (!siteId) {
        renderKillScreen("Loja não identificada", "Link inválido.");
        return;
    }
    ...
    try {
        // 2. O PORTEIRO (VERIFICAÇÃO NO SERVIDOR)
        const docRef = doc(db, "sites", siteId);
        
        // Pergunta ao Google: "Esse site existe?" (Ignora cache do celular)
        const snap = await getDocFromServer(docRef);

        // A. CENÁRIO: SITE EXCLUÍDO (404)
        if (!snap.exists()) {
            // Limpa dados locais do navegador do cliente para garantir
        ...
        // B. CENÁRIO: SITE BLOQUEADO PELO ADMIN
        if (data.status === 'bloqueado' || data.active === false) {
            throw new Error("SITE_BLOCKED");
        }

        // =================================================================
        // 3. SUCESSO: RECONSTRÓI O SITE
        // (O código só chega aqui se a loja existir e estiver ativa)
        // =================================================================
        
        // Restaura o HTML do site (Layout Base) se necessário, 
        // ou apenas libera o display se você usa um index.html fixo.
        // Como apagamos o innerHTML no começo, precisamos garantir que o layout base exista.
        // SE O SEU INDEX.HTML JÁ TEM O LAYOUT, MUDAMOS A ESTRATÉGIA PARA OPACIDADE:
        
        location.reload; // NÃO USAR RELOAD AQUI.
        
        // Vamos usar a estratégia de "Esconder e Mostrar" que é mais segura para SPAs:
        // (Nota: Ajustei o começo da função para usar opacity em vez de innerHTML vazio
        // para não quebrar seus elementos globais como 'product-modal', etc.)
        
    } catch (error) {
        console.error("BLOQUEIO FATAL:", error);
        
        // TELA DE MORTE (KILL SCREEN)
        // Isso substitui o site inteiro por uma mensagem de erro.
        document.body.innerHTML = `
            <div style="
                position: fixed; inset: 0; background: #000; color: #fff; 
                display: flex; flex-direction: column; align-items: center; justify-content: center; 
                z-index: 99999; font-family: sans-serif; text-align: center; padding: 20px;
            ">
                <div style="font-size: 4rem; color: #ef4444; margin-bottom: 20px;">
                    <i class="fas fa-ban"></i>
                </div>
                <h1 style="font-size: 2.5rem; font-weight: bold; margin: 0;">LOJA INDISPONÍVEL</h1>
                <p style="color: #9ca3af; margin-top: 15px; font-size: 1.2rem;">
                    ${error.message === 'SITE_BLOCKED' ? 'Acesso temporariamente suspenso.' : 'Esta loja não existe ou foi excluída.'}
                </p>
                <button onclick="window.location.reload()" style="
                    margin-top: 30px; padding: 12px 24px; background: #ef4444; color: white; 
        ...
        `;
        return; // <--- O CÓDIGO MORRE AQUI. NADA ABAIXO É EXECUTADO.
    }

    // =================================================================
    // 4. CARREGAMENTO DOS DADOS (SÓ RODA SE PASSOU PELO TRY)
    // =================================================================
    
    // Se chegou aqui, o site existe. Vamos "acender a luz".
    // Precisamos restaurar o body que escondemos (veja a lógica ajustada abaixo)
    
    // Salva perfil
    state.storeProfile = await getDoc(doc(db, "sites", siteId)).then(s => s.data());
    ...
    // Monitoramento Contínuo (Expulsa se deletar durante o uso)
    setInterval(async () => {
        try {
            // Verifica no servidor a cada 20 segundos
            const check = await getDocFromServer(doc(db, "sites", siteId));
            if (!check.exists() || check.data().status === 'bloqueado') {
                window.location.reload(); // Recarrega para cair na tela de erro
            }
        } catch(e) {}
    }, 20000);
...
// === HELPER PARA O AUTH LISTENER ===
function setupAuthListener() {
    onAuthStateChanged(auth, (user) => {
        state.user = user;
        const btnText = user ? 'Painel' : 'Área Admin';
        if (els.menuBtnAdmin) els.menuBtnAdmin.innerHTML = `<i class="fas fa-user-shield text-white"></i><span class="ml-2">${btnText}</span>`;
        if (getEl('btn-admin-login')) getEl('btn-admin-login').innerText = btnText;
...
// === AJUSTE CRÍTICO DE VISUALIZAÇÃO ===
// Coloque isso fora da função, logo no início do arquivo js/app.js
// Isso faz o site nascer invisível
