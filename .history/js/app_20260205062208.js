...
async function initApp() {
    // 1. Identificar e Validar Site na URL
    const params = new URLSearchParams(window.location.search);
    const siteId = params.get('site');

    // === TRAVA VISUAL ===
    // Esconde tudo imediatamente para não piscar conteúdo antigo
    ...
    try {
        ...
        // === CHECAGEM NO SERVIDOR (IGNORA CACHE) ===
        // Isso garante que se você deletou no painel, o site morre agora.
        ...
        if (!snap.exists()) {
            ...
            document.body.innerHTML = `
                <div style="height:100vh;display:flex;flex-direction:column;align-items:center;justify-content:center;background:#000;color:#ef4444;font-family:sans-serif;text-align:center;padding:20px;">
                    ...
                    <p style="color:#666;margin-top:10px;">Esta loja foi encerrada ou excluída.</p>
                </div>
            `;
            localStorage.removeItem('vestemanto_cart');
            return; // <--- CÓDIGO PARA AQUI
        }
        ...
        // === SUCESSO: SALVA DADOS E LIBERA VISUAL ===
        state.storeProfile = data; // Salva os dados frescos
        document.body.style.opacity = "1";
        document.body.style.pointerEvents = "auto";

    } catch (error) {
        console.error("Erro na verificação de segurança:", error);
        // Em caso de erro de rede (offline), libera o cache para não travar o cliente
        document.body.style.opacity = "1";
        document.body.style.pointerEvents = "auto";
        try {
            const cached = await getDoc(doc(db, "sites", siteId));
            if(cached.exists()) state.storeProfile = cached.data();
        } catch(e) {}
    }

    // ============================================================
    // 2. CARREGAMENTOS DO SISTEMA (SÓ RODA SE A LOJA EXISTIR)
    // ============================================================
    ...
    // loadStoreProfile(); <--- REMOVIDO pois já carregamos no início (linha 47)
    // Se tiver função de renderizar visual, chame aqui:
    ...
    // 3. Monitoramento em Tempo Real (A cada 30s verifica se foi excluído)
    setInterval(async () => {
        try {
            ...
            if (!check.exists() || check.data().status === 'bloqueado' || check.data().active === false) {
                window.location.reload(); // Força reload para cair na tela de bloqueio
            }
        } catch(e) {}
    }, 30000);
