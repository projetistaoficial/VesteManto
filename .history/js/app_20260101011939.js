...
function renderSalesList(orders) {
    ...
    orders.forEach(o => {
        ...
        // --- CÁLCULO E CLASSIFICAÇÃO DOS DESCONTOS (CORRIGIDO) ---
        const subTotalItens = o.items.reduce((acc, i) => acc + (i.price * i.qty), 0);
        const valFrete = o.shippingFee || 0;
        const valTotal = o.total || 0;
        
        // Desconto = (Produtos + Frete) - O que foi pago
        const valDescontoTotal = Math.max(0, (subTotalItens + valFrete) - valTotal);

        let discountHtml = '';
        
        // Se houver desconto real (> 5 centavos para margem de erro)
        if (valDescontoTotal > 0.05) {
            const isPix = (o.paymentMethod || '').toLowerCase().includes('pix');
            const hasCupom = !!o.cupom; // Verifica se existe código de cupom

            discountHtml += `<div class="mt-2 mb-2 border-y border-gray-700/50 py-2 space-y-1">`;

            // CENÁRIO 1: CUPOM + PIX (Exibe ambos, totaliza o valor)
            if (hasCupom && isPix) {
                discountHtml += `
                    <div class="flex justify-between text-xs text-gray-400">
                        <span>Cupom aplicado:</span>
                        <span class="text-yellow-500 font-bold uppercase tracking-wider">${o.cupom}</span>
                    </div>
                    <div class="flex justify-between text-xs text-gray-400">
                        <span>Método:</span>
                        <span class="text-green-400 font-bold">Desconto Pix Ativo</span>
                    </div>
                    <div class="flex justify-between text-xs text-green-500 font-bold pt-1 border-t border-gray-700/30 mt-1">
                        <span>Economia Total</span>
                        <span>- ${formatCurrency(valDescontoTotal)}</span>
                    </div>
                `;
            }
            // CENÁRIO 2: APENAS CUPOM
            else if (hasCupom) {
                discountHtml += `
                    <div class="flex justify-between text-xs text-green-500 font-bold">
                        <span>Cupom: <span class="uppercase text-yellow-500 ml-1">${o.cupom}</span></span>
                        <span>- ${formatCurrency(valDescontoTotal)}</span>
                    </div>
                `;
            }
            // CENÁRIO 3: APENAS PIX (ou outro desconto direto)
            else {
                 discountHtml += `
                    <div class="flex justify-between text-xs text-green-500 font-bold">
                        <span>${isPix ? 'Desconto no PIX' : 'Desconto'}</span>
                        <span>- ${formatCurrency(valDescontoTotal)}</span>
                    </div>
                `;
            }
