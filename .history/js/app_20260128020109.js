...
function calculateStatsMetrics() {
    ...
    if (state.products) {
        state.products.forEach(p => {
            if (p.stock > 0) {
                // Garante que é número para evitar NaN
    ...
    // 2. Filtragem Local (apenas para gráficos menores/status)
    if (!state.orders) return;

    let filteredOrders = state.orders;
    
    // Se o usuário selecionou um período específico (Hoje, Este Mês), filtra localmente
    ...
    // 3. Cálculos Locais (Contadores de Status)
    ...
    filteredOrders.forEach(o => {
        ...
        // Soma valores (apenas para uso local se o filtro NÃO for 'all')
    ...
    // --- CORREÇÃO PRINCIPAL ---
    // Se o filtro NÃO for 'all' (ex: vendo vendas de hoje), usa os dados calculados acima.
    if (state.statsFilterType !== 'all') {
        ...
        // Se for 'all' (Tudo), NÃO tocamos nos cards aqui (para não mostrar só os 100 primeiros).
        // Chamamos a função global para buscar o total real no servidor.
        if (typeof fetchGlobalStats === 'function') {
            setTimeout(fetchGlobalStats, 200);
        }
    }

    // Atualiza contadores de status (estes sempre usam a lista local carregada)
