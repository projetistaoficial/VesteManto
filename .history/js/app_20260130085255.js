...
function setupEventListeners() {
    console.log("🔧 Iniciando configuração dos botões...");

    // Helper seguro para adicionar eventos (Previne travamento se o ID não existir)
    const addSafeClick = (id, fn) => {
        const el = document.getElementById(id);
        if (el) {
            // Remove clones antigos para limpar a memória de eventos
    ...
    const addSafeEvent = (id, event, fn) => {
        const el = document.getElementById(id);
        if (el) el.addEventListener(event, fn);
    };

    // --- 1. CARRINHO E MENU (Prioridade Máxima) ---
    addSafeClick('cart-btn', openCart);
    addSafeClick('cart-btn-mobile', openCart);
    addSafeClick('close-cart', closeCartModal);
    addSafeClick('btn-go-checkout', goToCheckoutView);
    
    // Menu Lateral
    addSafeClick('mobile-menu-btn', toggleSidebar);
    addSafeClick('close-sidebar', toggleSidebar);
    addSafeClick('sidebar-overlay', toggleSidebar);
    
    // Botão de Rastreio (Motoquinha)
    addSafeClick('btn-track-icon', openTrackModal);

    // --- 2. ACORDEÕES (Menus que abrem/fecham) ---
    ...
    setupAccordion('btn-acc-orders', 'content-acc-orders', 'arrow-acc-orders'); // Faltava este

    // --- 3. CHECKOUT E ENDEREÇO ---
    addSafeEvent('checkout-cep', 'blur', window.handleCheckoutCep);
    addSafeClick('btn-finish-payment', window.submitOrder);
    
    // Inputs de Texto do Checkout (Validação)
    ['checkout-name', 'checkout-phone', 'checkout-number', 'checkout-street'].forEach(id => {
        addSafeEvent(id, 'input', validateCheckoutForm);
    });

    // Listener especial para bloqueio de pagamento (Toast)
    ...
    // --- 4. ADMIN: FILTROS E PRODUTOS ---
    addSafeEvent('admin-search-prod', 'input', filterAndRenderProducts);
    addSafeEvent('admin-filter-cat', 'change', filterAndRenderProducts);
    addSafeEvent('admin-sort-prod', 'change', filterAndRenderProducts);
    ...
    if (btnBulkMove) btnBulkMove.onclick = bulkMoveDynamic;

    // --- 5. CONFIGURAÇÕES (Parcelamento, Entrega, Pix) ---
    // Parcelamento Ativo
    ...
    addSafeEvent('checkout-installments', 'change', calcCheckoutTotal);
    
    // Pix Global UI
    addSafeEvent('conf-pix-disable-all', 'change', togglePixGlobalUI);
    const chkGlobalPix = document.getElementById('conf-pix-global-active');
    if (chkGlobalPix) {
        chkGlobalPix.addEventListener('change', (e) => {
            togglePixGlobalUI();
            if(e.target.checked) showSystemModal("Atenção: Padronização ativa sobrescreve descontos individuais.");
        });
    }

    // Configurações de Logística (AutoSave)
    addSafeEvent('conf-store-cep', 'blur', () => autoSaveSettings('logistics'));
    addSafeEvent('conf-max-dist', 'blur', () => autoSaveSettings('logistics'));
    addSafeEvent('conf-card-max', 'change', () => autoSaveSettings('installments'));
    addSafeEvent('conf-card-free', 'change', () => autoSaveSettings('installments'));
    addSafeEvent('conf-card-rate', 'blur', () => autoSaveSettings('installments'));

    // --- 6. FORMULÁRIO DE PRODUTO ---
    ...
    if (btnAddProd) {
        btnAddProd.onclick = () => {
            ...
            const checkNoStock = document.getElementById('prod-allow-no-stock');
    ...
    if(btnCancelProd) btnCancelProd.onclick = () => document.getElementById('product-form-modal').classList.add('hidden');

    const formProd = document.getElementById('form-product');
    if (formProd) formProd.onsubmit = window.saveProduct; // Usa a função global exposta

    // --- 7. LOGIN E SISTEMA ---
    addSafeClick('btn-logout', () => signOut(auth));
    addSafeClick('theme-toggle', () => toggleTheme(true));
    
    // Filtros Vitrine
    addSafeEvent('search-input', 'input', (e) => {
        const term = e.target.value.toLowerCase();
        const filtered = state.products.filter(p => p.name.toLowerCase().includes(term));
        renderCatalog(filtered);
    });
    
    addSafeEvent('category-filter', 'change', (e) => filterByCat(e.target.value));

    // Dashboard
    addSafeClick('dash-prev-date', () => { 
...
// Expõe para o HTML e para o resto do arquivo
...
function togglePixGlobalUI() {
    ...
    if (disableAll) {
        ...
        document.getElementById('conf-pix-global-active').checked = false;
    } else {
...
// --- EXPOR FUNÇÕES PARA O HTML (CORREÇÃO DE ESCOPO) ---

// Carrinho
window.openCart = openCart;
window.closeCartModal = closeCartModal;
window.showCartListView = showCartListView;
...
window.addToCart = addToCart; // Se usada no HTML