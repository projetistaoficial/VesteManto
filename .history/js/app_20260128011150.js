...
// NOVA ESTRATÉGIA: Carrega Totais Leves + Lista Pesada Limitada
function loadAdminSales() {
    if (state.salesUnsubscribe) state.salesUnsubscribe();

    // 1. LISTENER DA LISTA (Limitado a 100 para não travar) -> Preenche a Tabela
    const qList = query(
    ...
    state.salesUnsubscribe = onSnapshot(qList, (snapshot) => {
        // Salva os detalhes completos apenas dos 100 primeiros
        ...
        if (salesBtn) {
            salesBtn.innerHTML = newOrdersCount > 0 ? `Vendas (${newOrdersCount})` : 'Vendas';
        }
    });

    // 2. BUSCA DE TOTAIS (Leve e Única) -> Preenche o Dashboard
    // Não usamos onSnapshot aqui para economizar. Rodamos uma vez ao carregar.
    fetchGlobalStats();
}

// Busca apenas os campos necessários para somar (Economia de banda)
async function fetchGlobalStats() {
    try {
        const salesRef = collection(db, `sites/${state.siteId}/sales`);
        
        // Pega TODOS os documentos, mas o Firebase em JS baixa o doc todo.
        // Como são 315, ainda é tranquilo para leitura normal (não agregação).
        ...
        // --- ATUALIZA A TELA (Dashboard e Header) ---
        ...
        console.log("Estatísticas Globais (Leitura Direta):", totalPedidos, faturamentoTotal);
...
function calculateStatsMetrics() {
    ...
    let totalCreatedOrders = filteredOrders.length;
    ...
    // --- 4. RENDERIZAÇÃO NO DOM (A CORREÇÃO ESTÁ AQUI) ---
    
    // SE estiver vendo "Tudo", NÃO use os dados locais (que são limitados a 100).
    // Deixe o updateRealTotals() preencher isso com os dados do servidor.
    if (state.statsFilterType !== 'all') {
    ...
    // Estes abaixo continuam sendo atualizados localmente, pois o servidor 
    // não está configurado para contar status específicos (economia de cota).
    ...
    // --- 5. CHAMADA GLOBAL ---
    // Se for 'all', chama o servidor para buscar os números reais (315 pedidos, 5 milhões, etc)
    if (state.statsFilterType === 'all') {
        updateRealTotals();
    }
}
