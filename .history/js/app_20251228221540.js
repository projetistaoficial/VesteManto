...
function renderProductsList(products) {
    ...
    // LISTA ESTRITA: Só conta se estiver aqui.
    // "Aguardando aprovação" e "Cancelado" NÃO estão aqui, então não aparecem.
    const activeSaleStatuses = [
        'Aprovado', 
        'Preparando pedido', 
        'Saiu para entrega', 
        'Entregue', 
    ...
    if (state.orders) {
        state.orders.forEach(order => {
            // Se o status ATUAL do pedido estiver na lista acima, processa.
            if (activeSaleStatuses.includes(order.status)) {
