...
// Adicione ou substitua esta função no seu stats.js
export function updateStatsData(newProducts, newOrders, newDailyStats) {
    // Atualiza a memória interna do módulo de estatísticas
    statsState.cachedProducts = newProducts;
    
    // Recalcula tudo imediatamente
    calculateKPIs(newOrders, newDailyStats);
}
...
function calculateRankings(orders) {
    ...
    // 3. Pagamento
   // Define APENAS as categorias permitidas
    const payMap = {
        'Pix': { qty: 0, val: 0 },
    ...
    validOrders.forEach(o => {
        // Limpa o nome que veio do banco
        const name = normalizePaymentMethod(o.paymentMethod || '');
        
        // Só soma se o nome limpo existir no nosso mapa padrão
        // Isso ignora automaticamente "whatsapp", "outros", etc.
        if (name && payMap[name]) {
            payMap[name].qty++;
    ...
    // Transforma em lista e ordena por valor (R$)
    const rankedPay = Object.entries(payMap)
        .map(([name, data]) => ({ name, ...data }))
        .filter(item => item.qty > 0) // Remove os zerados para não poluir
