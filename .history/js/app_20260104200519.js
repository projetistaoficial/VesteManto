...
function loadAdminSales() {
    ...
    onSnapshot(q, (snapshot) => {
        // Mapeia os dados e salva no estado
        state.orders = snapshot.docs.map(d => ({ id: d.id, ...d.data() }));

        // --- CORREÇÃO 1: CALCULAR OS PEDIDOS NOVOS ---
        // Precisamos contar quantos pedidos não foram vistos ANTES de usar a variável
        let newOrdersCount = 0;
        state.orders.forEach(order => {
            // Se viewed for falso ou não existir, conta como novo
            if (order.viewed === false || order.viewed === undefined) {
                newOrdersCount++;
            }
        });
        // ---------------------------------------------

        // Atualiza Dashboard e Tabela de Vendas (se as funções existirem)
        if (typeof filterAndRenderSales === 'function') filterAndRenderSales();
        if (typeof updateDashboardMetrics === 'function') updateDashboardMetrics();

        // Atualiza produtos se a tabela estiver visível
        if (document.getElementById('admin-product-list')) {
            filterAndRenderProducts();
        }
        
        // Atualiza estatísticas globais (se a função existir)
        if (typeof updateStatsData === 'function') {
            updateStatsData(state.orders, state.products, state.dailyStats);
        }

        // --- CORREÇÃO 2: ATUALIZA A ABA DO NAVEGADOR (Sem função externa) ---
        if (newOrdersCount > 0) {
        ...
        // --- CORREÇÃO 3: ATUALIZA O BOTÃO "VENDAS" ---
        const salesBtn = document.getElementById('admin-menu-sales');

        if (salesBtn) {
            if (newOrdersCount > 0) {
                // Se tiver vendas novas, escreve "Vendas" + A Bolinha
                salesBtn.innerHTML = `
                Vendas 
                <span class="bg-red-600 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full shadow-lg animate-pulse ml-2">
                    ${newOrdersCount}
                </span>
            `;
            } else {
                // Se não tiver, escreve só "Vendas"
