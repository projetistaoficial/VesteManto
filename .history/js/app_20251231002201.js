...
function fillProfileForm() {
    ...
    const setCheck = (id, val) => {
        const el = document.getElementById(id);
        // Se val for undefined (primeira vez), assume true. Se for false, marca false.
        if (el) el.checked = (val !== false);
    };

    // 1. Carrega Mestres (Toggles)
    ...
    // 3. Atualiza visual (Opacidade) baseado no que foi carregado
    const groupOnline = document.getElementById('group-online-methods');
    const groupDelivery = document.getElementById('group-delivery-methods');

    if (groupOnline) {
        groupOnline.className = (payConfig.online?.active !== false)
            ? "space-y-3 opacity-100"
            : "space-y-3 opacity-30 pointer-events-none";
    }
    if (groupDelivery) {
        groupDelivery.className = (payConfig.delivery?.active !== false)
            ? "space-y-3 opacity-100"
            : "space-y-3 opacity-30 pointer-events-none";
    }
...
window.openCheckoutModal = () => {
    ...
    // 2. RECUPERA CONFIGURAÇÕES (Com log para debug)
    const payConfig = state.storeProfile.paymentMethods || {
        online: { active: true },
        delivery: { active: true }
    };
    
    // Log para você verificar no Console (F12) se está carregando false ou true
    console.log("Configuração Carregada:", payConfig);

    const oldOwnDelivery = state.storeProfile.deliveryConfig?.ownDelivery;

    // 3. REFERÊNCIAS AOS ELEMENTOS (USANDO IDS EXPLÍCITOS)
    const containerDelivery = document.getElementById('container-delivery-option'); 
    const labelOnline = document.getElementById('label-pay-online'); // <--- NOVO ID QUE CRIAMOS
    ...
    // --- A. LÓGICA DO "PAGAR ONLINE" ---
    if (payConfig.online && payConfig.online.active === false) {
        if (labelOnline) {
            labelOnline.classList.add('hidden');
            labelOnline.style.setProperty('display', 'none', 'important'); // Força bruta
        }
    } else {
        if (labelOnline) {
            labelOnline.classList.remove('hidden');
            labelOnline.style.display = 'flex'; // Restaura o flex
        }
    }

    // --- B. LÓGICA DO "PAGAR NA ENTREGA" ---
    const isDeliveryActive = (payConfig.delivery && payConfig.delivery.active !== false);
    // Compatibilidade: Se a loja desativou "Entrega Própria" na logística, também esconde
    const allowDelivery = isDeliveryActive && (oldOwnDelivery !== false);
    ...
    if (!allowDelivery) {
        ...
        if (containerDelivery) {
            ...
            containerDelivery.style.display = 'block';
        }
    }

    // --- C. AUTO-SELEÇÃO (CORRIGE O RADIO CHECKED) ---
    // Se o Online estiver visível, ele é o padrão.
    // Se o Online sumiu, marcamos a Entrega obrigatoriamente.
    
    const onlineIsVisible = (payConfig.online.active !== false);
    
    if (onlineIsVisible) {
        if (radioOnline) radioOnline.checked = true;
    } else {
        // Se online está oculto, força marcar a entrega
        if (radioDelivery && allowDelivery) radioDelivery.checked = true;
    }
