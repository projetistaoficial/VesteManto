...
async function initApp() {
    // 1. Identificar o Site na URL
    const params = new URLSearchParams(window.location.search);
    const siteId = params.get('site');

    // === TRAVA VISUAL ===
    // Deixa o site invisível enquanto verificamos. Isso impede que o conteúdo "pisque" antes de bloquear.
    document.body.style.opacity = "0";
    document.body.style.pointerEvents = "none"; // Impede cliques

    if (!siteId) {
        document.body.style.opacity = "1";
        document.body.innerHTML = '<div style="background:#000;height:100vh;display:flex;align-items:center;justify-content:center;color:#fff;"><h1>Loja não identificada.</h1></div>';
        return;
    }
    ...
    try {
        ...
        // === CHECAGEM NO SERVIDOR (A CHAVE DO SUCESSO) ===
        // getDocFromServer: Ignora o cache local e vai direto no banco de dados online.
        // Se foi deletado, ele vai descobrir AGORA.
        const snap = await getDocFromServer(docRef);

        // A. CENÁRIO: EXCLUÍDO (404)
        if (!snap.exists()) {
            document.body.style.opacity = "1";
            document.body.style.pointerEvents = "auto";
            document.body.innerHTML = `
                <div style="position:fixed; top:0; left:0; width:100%; height:100vh; display:flex; flex-direction:column; align-items:center; justify-content:center; background:#000; color:#ef4444; z-index:9999;">
                    <h1 style="font-size:4rem; margin:0; font-weight:bold;">404</h1>
                    <h2 style="color:white; text-transform:uppercase; margin-top:10px; font-family:sans-serif;">Loja Não Encontrada</h2>
                    <p style="color:#666; margin-top:10px; font-family:sans-serif;">Esta loja foi encerrada ou excluída.</p>
                </div>
            `;
            // Limpa qualquer dado salvo dessa loja
            localStorage.removeItem('vestemanto_cart');
            return; // <--- O CÓDIGO MORRE AQUI. Nada abaixo roda.
        }

        const data = snap.data();

        // B. CENÁRIO: BLOQUEADO (Pelo Admin)
        if (data.status === 'bloqueado' || data.active === false) {
            document.body.style.opacity = "1";
            document.body.style.pointerEvents = "auto";
            document.body.innerHTML = `
                <div style="position:fixed; top:0; left:0; width:100%; height:100vh; display:flex; flex-direction:column; align-items:center; justify-content:center; background:#111; color:#fff; z-index:9999; font-family:sans-serif;">
                    <i class="fas fa-lock" style="font-size:3rem; color:#ef4444; margin-bottom:20px;"></i>
                    <h2 style="text-transform:uppercase; letter-spacing:1px;">Acesso Suspenso</h2>
                    <p style="color:#888; margin-top:10px;">Esta loja está temporariamente indisponível.</p>
                </div>
            `;
            return; // <--- PARA AQUI TAMBÉM.
        }

        // === SUCESSO: LIBERA O SITE ===
        document.body.style.opacity = "1";
        document.body.style.pointerEvents = "auto";
        ...
        // Se der erro de internet (offline), mostramos o site (opcional) 
        // ou mostramos erro de conexão. Vou liberar para não prejudicar quem tem internet ruim.
        document.body.style.opacity = "1";
        document.body.style.pointerEvents = "auto";
        
        // Tenta carregar do cache apenas se deu erro de rede
        try {
            const cachedSnap = await getDoc(doc(db, "sites", siteId));
            if(cachedSnap.exists()) state.storeProfile = cachedSnap.data();
        } catch(e) {}
    }

    // ==========================================================
    // CARREGAMENTO NORMAL (Só roda se passou pelas travas acima)
    // ==========================================================
    ...
    loadCategories();
    loadProducts();
    // loadStoreProfile(); <--- REMOVIDO (Já carregamos no início)
    ...
    // Monitoramento contínuo: Se você excluir enquanto o cliente navega, 
    // ele será expulso em até 30 segundos.
    setInterval(async () => {
        try {
            // Verifica no servidor de novo
            const check = await getDocFromServer(doc(db, "sites", state.siteId));
            if (!check.exists() || check.data().status === 'bloqueado' || check.data().active === false) {
                window.location.reload(); // Recarrega para cair na tela de bloqueio
            }
        } catch(e) {}
    }, 30000);
