...
window.openCheckoutModal = () => {
    ...
    // Compatibilidade com trava antiga de logística
    const oldOwnDelivery = state.storeProfile.deliveryConfig?.ownDelivery;

    // 3. ELEMENTOS DO DOM (Referências)
    const containerDelivery = document.getElementById('container-delivery-option'); // Div inteira do "Pagar na Entrega"
    
    // Para o "Pagar Online", precisamos pegar o elemento pai <label> que envolve o input radio
    const radioOnline = document.querySelector('input[name="pay-mode"][value="online"]');
    const lblOnline = radioOnline ? radioOnline.closest('label') : null;
    
    const radioDelivery = document.querySelector('input[name="pay-mode"][value="delivery"]');

    // --- LÓGICA DE EXIBIÇÃO ---

    // A. Controla "Pagar Online"
    if (payConfig.online.active === false) {
        if (lblOnline) {
            lblOnline.classList.remove('flex'); // Remove flex para não atrapalhar o hidden
            lblOnline.classList.add('hidden');
            lblOnline.style.setProperty('display', 'none', 'important'); // Força bruta
        }
    } else {
        if (lblOnline) {
            lblOnline.classList.remove('hidden');
            lblOnline.classList.add('flex');
            lblOnline.style.display = ''; 
        }
    }

    // B. Controla "Pagar na Entrega"
    // Some se estiver desativado no financeiro OU desativado na logística
    const isDeliveryActive = (payConfig.delivery.active !== false) && (oldOwnDelivery !== false);
    ...
    // Se o que estava marcado sumiu, marca o outro
    const isOnlineVisible = (payConfig.online.active !== false);
    
    // Se online está visível, ele é preferencial. Se não, tenta entrega.
    if (isOnlineVisible) {
    ...
    // 4. Atualiza Interface e Navegação
