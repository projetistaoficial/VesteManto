    ...
    if (method === 'pix') {
        ...
        // 2. "Padronização Global" ATIVA
            ...
            if (pixGlobal.mode === 'product') {
                // MODO: Produto a Produto
                state.cart.forEach(item => {
                    let price = item.price;
                    let descVal = 0;

                    if (isFixed) {
                        // Desconto fixo por item (Cuidado para não negativar)
                        descVal = val; 
                    } else {
                        // Porcentagem
                        descVal = price * (val / 100);
                    }
                ...
                if (isFixed) {
                    // Desconto fixo no total bruto
                    totalDesc = val;
                } else {
                    // Porcentagem no total bruto
                    totalDesc = subtotalDisplay * (val / 100);
                }
        ...
        // 3. Padrão Individual (Mantém lógica antiga)
        else {
            state.cart.forEach(item => {
