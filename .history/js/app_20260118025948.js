...
function renderAdminCoupons() {
    ...
    els.couponListAdmin.innerHTML = state.coupons.map((c, index) => {
        ...
        const typeDisplay = c.type === 'percent' 
            ? `<span class="text-green-400 font-bold bg-green-900/20 px-1.5 rounded text-[10px]">${c.val}% OFF</span>` 
            : `<span class="text-green-400 font-bold bg-green-900/20 px-1.5 rounded text-[10px]">${formatCurrency(c.val)} OFF</span>`;

        let isExpired = false;
        let expiryDisplay = `<span class="text-[10px] text-green-500 font-bold flex items-center gap-1"><i class="fas fa-infinity text-[8px]"></i> Permanente</span>`;

        if (c.expiryDate) {
            const expiryDate = new Date(c.expiryDate);
            const now = new Date();
            // Formatação curta para mobile (DD/MM HH:mm)
            const dateStr = expiryDate.toLocaleString('pt-BR', { day: '2-digit', month: '2-digit', hour: '2-digit', minute:'2-digit' });

            if (now > expiryDate) {
                isExpired = true;
                // Layout de expirado ajustado para quebrar linha
                expiryDisplay = `<span class="text-[9px] text-red-400 font-bold bg-red-900/20 px-1.5 py-0.5 rounded border border-red-900/50 block mt-1 w-fit">EXPIRADO: ${dateStr}</span>`;
            } else {
                expiryDisplay = `<span class="text-[10px] text-gray-400">Expira: <span class="text-white font-bold">${dateStr}</span></span>`;
            }
        }
        ...
        return `
            <div id="coupon-item-${index}" 
                 ...
                 class="${bgClass} border-l-4 ${borderClass} p-3 rounded-lg flex justify-between items-center shadow-sm mb-2 cursor-pointer transition select-none group relative overflow-hidden">
                
                ${isFocused ? '<div class="absolute -left-2 top-1/2 -translate-y-1/2 text-yellow-500 text-xs"><i class="fas fa-caret-right"></i></div>' : ''}

                <div class="flex flex-col flex-1 min-w-0 pr-2 pointer-events-none">
                    <span class="text-yellow-500 font-bold text-base tracking-wider truncate group-hover:text-white transition">${c.code}</span>
                ...
                <button onclick="event.stopPropagation(); deleteCoupon('${c.id}')" 
                        class="w-8 h-8 shrink-0 flex items-center justify-center bg-red-600/10 text-red-500 border border-red-600/30 hover:bg-red-600 hover:text-white rounded transition z-20 active:scale-95">
                    <i class="fas fa-trash-alt text-xs"></i>
                </button>
            </div>
        `;
