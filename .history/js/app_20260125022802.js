...
async function autoSaveSettings(type) {
    ...
    if (type === 'logistics') {
        ...
        // Corrige leitura da taxa com virgula
        const rateStr = document.getElementById('conf-card-rate').value;
        const rate = parseFloat(rateStr.replace(/\./g, '').replace(',', '.')) || 0;

        // --- CAPTURA CONFIGURAÇÕES DE PAGAMENTO ---
        const payConfig = {
            online: {
                active: document.getElementById('conf-pay-online-active').checked,
                pix: document.getElementById('conf-pay-online-pix').checked,
                card: document.getElementById('conf-pay-online-card').checked
            },
            delivery: {
                active: document.getElementById('conf-pay-delivery-active').checked,
        ...
        const shipVal = parseFloat(shipValRaw.replace(/[^\d,]/g, '').replace(',', '.')) || 0;
...
window.openCheckoutModal = () => {
    ...
    // Se undefined, assume TRUE por padrão para não quebrar lojas antigas
    ...
    // Regra: Entrega só aparece se o Meio Financeiro estiver ON E a Logística estiver ON
    ...
    // 4. VISIBILIDADE DOS BOTÕES PRINCIPAIS
    if (labelOnline) {
        if (onlineActive) {
    ...
    // 6. CHAMA O FILTRO DE SUB-OPÇÕES (Onde estava o erro do Pix)
...
window.togglePaymentMode = () => {
    ...
    // RECUPERA CONFIGURAÇÕES DA MEMÓRIA
    const pm = state.storeProfile.paymentMethods || { 
    ...
    // Cartão: Input value="card" (Está dentro de uma div wrapper no seu HTML)
    const radioCard = document.querySelector('input[name="payment-method-selection"][value="card"]');
    // Para o cartão, queremos esconder a DIV container dele, que tem a borda e o fundo cinza
    const divCardContainer = radioCard ? radioCard.closest('.bg-gray-800') : null;

    // Dinheiro: Input value="cash" (Dentro da div container-cash-option)
    ...
    // --- FUNÇÃO HELPER PARA ESCONDER/MOSTRAR ---
    const setVisible = (el, visible, radio) => {
        if (!el) return;
        if (visible) {
            el.classList.remove('hidden');
            el.style.display = ''; 
    ...
    if (mode === 'delivery') {
        ...
        // Regras para Entrega
        // Pix na entrega
        const pixEnabled = pm.delivery?.pix !== false; // Padrão true
        setVisible(lblPix, pixEnabled, radioPix);

        // Cartão na entrega
        const cardEnabled = pm.delivery?.card !== false;
        setVisible(divCardContainer, cardEnabled, radioCard);

        // Dinheiro na entrega (Sempre exibe se for delivery e estiver ativo)
        const cashEnabled = pm.delivery?.cash !== false;
        setVisible(containerCash, cashEnabled, radioCash);
        ...
        // Regras para Online
        // Pix Online
        const pixEnabled = pm.online?.pix !== false;
        setVisible(lblPix, pixEnabled, radioPix);

        // Cartão Online
        const cardEnabled = pm.online?.card !== false;
        setVisible(divCardContainer, cardEnabled, radioCard);

        // Dinheiro (Nunca existe no online)
        setVisible(containerCash, false, radioCash);
    }

    // --- AUTO-CORREÇÃO DA SELEÇÃO DO RADIO ---
    // Se a opção marcada ficou oculta, marca a primeira visível
    const currentChecked = document.querySelector('input[name="payment-method-selection"]:checked');
    let needsChange = false;

    if (!currentChecked) needsChange = true;
    else if (currentChecked.disabled) needsChange = true;
    else {
        // Verifica se o elemento visual está hidden
        const parent = currentChecked.closest('.hidden'); // Procura classe hidden nos pais
        if (parent) needsChange = true;
    }

    if (needsChange) {
        const availableRadios = document.querySelectorAll('input[name="payment-method-selection"]:not(:disabled)');
        // Filtra apenas os que não têm pais com display:none
        for (let radio of availableRadios) {
            // Verifica visualmente se está visível
    ...
    // Chama ajustes de UI (Parcelas/Troco)
