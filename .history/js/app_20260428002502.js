...
// =================================================================
// 📦 MÓDULO DE PRODUTOS E ESTOQUE HÍBRIDO (GRADE VS GERAL)
// =================================================================

// 1. Variável de Memória para os Tamanhos
state.tempVariations = [];

// 2. Controle Visual do Checkbox de Estoque
...
// 3. Funções do Mini-Modal de Tamanhos (Admin)
window.openVariationModal = () => {
    document.getElementById('var-modal-name').value = '';
    document.getElementById('var-modal-stock').value = '';
    document.getElementById('variation-input-modal').classList.remove('hidden');
...
// 4. Salvar Produto (Tratando Estoque Híbrido)
window.saveProduct = async (e) => {
    ...
    try {
        ...
        if (hasVariations) {
            if (state.tempVariations.length === 0) return showToast("Adicione pelo menos um tamanho no +.","error");
            
            finalSizes = [...state.tempVariations];
            finalStock = finalSizes.reduce((acc, val) => acc + parseInt(val.stock), 0);
        } else {
            const elSizesSimple = document.getElementById('prod-sizes-simple');
            const sizesRaw = elSizesSimple ? elSizesSimple.value : "";
            finalSizes = sizesRaw.split(',').map(s => s.trim().toUpperCase()).filter(s => s);
...
window.editProduct = (id) => {
    ...
    if (p.hasVariations && Array.isArray(p.sizes) && p.sizes.length > 0 && typeof p.sizes[0] === 'object') {
        ...
        const sizeStr = Array.isArray(p.sizes) ? p.sizes.map(s => typeof s === 'object' ? s.name : s).join(', ') : '';
        const elSizesSimple = document.getElementById('prod-sizes-simple');
...
// 6. Abrir Modal de Compra do Cliente (Vitrine)
...
// Atribuição global obrigatória (para o onClick do HTML funcionar)
