...
function setupEventListeners() {
    ...
    // 2. Correção: Salvamento ao sair do campo de valor (blur)
    if (inputPixVal) {
        inputPixVal.addEventListener('blur', () => {
...
async function autoSaveSettings(type) {
    ...
    try {
        ...
        // 2. PARCELAMENTO E PAGAMENTOS (CORRIGIDO)
            ...
            // 1º Passo: Definir payConfig (ANTES de usar)
            const payConfig = {
                online: {
            ...
            // 3º Passo: Montar o objeto ÚNICO final
            dataToUpdate = {
        ...
        // Atualiza chaves específicas se existirem no update
        if (dataToUpdate.paymentMethods) state.storeProfile.paymentMethods = dataToUpdate.paymentMethods;
        if (dataToUpdate.pixGlobal) state.storeProfile.pixGlobal = dataToUpdate.pixGlobal; // <--- Importante atualizar memória
        ...
        if (!document.getElementById('view-checkout').classList.contains('hidden')) {
            ...
            // Também recalcula totais pois o Pix Global pode ter mudado valores
