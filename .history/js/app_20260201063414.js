...
// --- FUNÇÃO PARA LIGAR OS CAMPOS DE PEDIDO E FRETE ---
function setupOrderSettings() {
    // Lista de campos que devem salvar nas configurações de PEDIDOS
    const orderInputs = [
        { id: 'conf-req-code', event: 'change' },      // Checkbox Pedir Código
        { id: 'conf-shipping-rule', event: 'change' }, // Select Regra Frete
        { id: 'conf-shipping-value', event: 'blur' },  // Input Valor Frete
        { id: 'conf-cancel-time', event: 'blur' }      // Input Tempo Cancelamento
    ];
    ...
    orderInputs.forEach(item => {
        ...
        if (el) {
            ...
            newEl.addEventListener(item.event, () => {
                // Se for valor de frete, formata visualmente se precisar
                if (item.id === 'conf-shipping-value' && typeof window.formatMoneyForInput === 'function') {
                    // Opcional: formatação visual
                }
                // CHAMA O SALVAMENTO
            ...
            // Para inputs de texto/numero, salva também no Enter
            if(item.event === 'blur') {
                newEl.addEventListener('keydown', (e) => {
                    if(e.key === 'Enter') newEl.blur();
                });
            }
        }
    });
