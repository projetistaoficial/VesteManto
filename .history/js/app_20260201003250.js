...
function setupEventListeners() {
    ...
    // --- CORREÇÃO 1: Removido 'window.' (Chama a função local direto) ---
    if (els.checkoutCep) {
    ...
    // ====================================================================
    // CORREÇÃO CRÍTICA AQUI: Removido 'window.' das atribuições
    // ====================================================================
    ...
    if (els.menuLinkHome) {
        els.menuLinkHome.onclick = (e) => {
            ...
            showView('catalog');
            filterByCat('');
            if (window.innerWidth < 1024) toggleSidebar(); // <--- SEM 'window.'
        };
    }
    
    if (els.menuBtnAdmin) els.menuBtnAdmin.onclick = () => { 
        toggleSidebar(); // <--- SEM 'window.'
        if (state.user) { showView('admin'); } else { getEl('login-modal').showModal(); } 
    };
...
// =================================================================
// 🚨 CONECTOR DE SEGURANÇA (COLE NO FINAL DO ARQUIVO) 🚨
// =================================================================
// Necessário para botões criados via HTML (innerHTML) funcionarem.
...
// Força atualização da tela se já estiver logado
