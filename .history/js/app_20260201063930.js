...
// --- CONFIGURAÇÃO ESPECIAL DE PEDIDOS E FRETE ---
function setupOrderSettings() {
    console.log("🔧 Iniciando configuração de Pedidos e Frete...");

    // 1. MAPEAMENTO DOS ELEMENTOS
    const elOwn = document.getElementById('conf-own-delivery');       // Check: Entrega Própria
    const elPayDel = document.getElementById('conf-pay-delivery-active'); // Check: Pagamento na Entrega
    const elReq = document.getElementById('conf-req-code');           // Check: Pedir Código
    const elRule = document.getElementById('conf-shipping-rule');     // Select: Regra de Frete
    const elVal = document.getElementById('conf-shipping-value');     // Input: Valor do Frete
    const elTime = document.getElementById('conf-cancel-time');       // Input: Tempo Cancelamento

    // Se os principais não existirem, para tudo
    if (!elOwn || !elPayDel) {
        console.error("Erro Crítico: Checkboxes de Entrega/Pagamento não encontrados no HTML.");
        return;
    }

    // 2. LÓGICA DE DEPENDÊNCIA (AVISOS E TRAVAS)
    
    // --> Quando muda a ENTREGA PRÓPRIA
    elOwn.addEventListener('change', async (e) => {
        const isActive = e.target.checked;
        
        // Visual do Código do Cliente
        if (elReq) {
        ...
        // TRAVA DE SEGURANÇA
        if (!isActive) {
            // Se tentar desligar a entrega...
            if (elPayDel.checked) {
                const onlineOn = document.getElementById('conf-pay-online-active')?.checked;
                
                // Se online também estiver desligado, PROÍBE
                if (!onlineOn) {
                    alert("⚠️ AÇÃO BLOQUEADA:\n\nVocê não pode desativar a Entrega Própria.\nMotivo: O Pagamento Online já está desativado e o cliente precisa de uma forma para receber/pagar.");
                    e.target.checked = true; // Força voltar para marcado
                    return; 
                }
                
                // Se online estiver ligado, permite mas desliga o pagamento na entrega
                const confirmOff = confirm("⚠️ Ao desativar a Entrega Própria, o 'Pagamento na Entrega' também será desativado.\n\nDeseja continuar?");
                if (!confirmOff) {
                    e.target.checked = true;
                    return;
                }

                elPayDel.checked = false;
                await autoSaveSettings('payments'); // Salva o pagamento desligado
            }
        } else {
            // Se ligou a entrega, liga o pagamento na entrega (conveniência)
            if (!elPayDel.checked) {
                elPayDel.checked = true;
                await autoSaveSettings('payments');
            }
        }

        // Salva a alteração da ENTREGA
        await autoSaveSettings('orders');
    });

    // --> Quando muda o PAGAMENTO NA ENTREGA
    elPayDel.addEventListener('change', async (e) => {
        const isActive = e.target.checked;
        
        if (!isActive) {
            // Só pode desligar se o online estiver ativo
            const onlineOn = document.getElementById('conf-pay-online-active')?.checked;
            if (!onlineOn) {
                alert("⚠️ AÇÃO BLOQUEADA:\nVocê precisa manter pelo menos uma forma de pagamento ativa.");
                e.target.checked = true;
                return;
            }
        }

        // Se ativar pagamento na entrega, PRECISA ativar a logística
        if (isActive && !elOwn.checked) {
            elOwn.checked = true;
            alert("ℹ️ A Entrega Própria foi ativada automaticamente para permitir o Pagamento na Entrega.");
            await autoSaveSettings('orders');
        }
    ...
    // 3. LISTENERS PARA SALVAR (Inputs Órfãos)
    
    // Checkbox: Pedir Código
    if (elReq) {
        // Remove listeners antigos para evitar duplicação
        const newReq = elReq.cloneNode(true);
        elReq.parentNode.replaceChild(newReq, elReq);
        
        newReq.addEventListener('change', () => {
            console.log("Salvando código cliente...");
            autoSaveSettings('orders');
        });
    }

    // Select: Regra de Frete
    if (elRule) {
        elRule.addEventListener('change', () => autoSaveSettings('orders'));
    }

    // Input: Valor do Frete (Ao sair do campo)
    if (elVal) {
        elVal.addEventListener('blur', () => {
            // Formatação visual simples (opcional)
            let val = elVal.value.replace(',', '.');
            if (val && !isNaN(val)) elVal.value = parseFloat(val).toFixed(2).replace('.', ',');
            
            autoSaveSettings('orders');
        });
    }

    // Input: Tempo de Cancelamento
    if (elTime) {
        elTime.addEventListener('blur', () => autoSaveSettings('orders'));
    }

    console.log("✅ Configuração de Pedidos e Frete concluída.");
}
...
const state = {
    ...
    globalSettings: { allowNoStock: true }, // <--- Mude para TRUE
    cardSelections: {},
...
const startApplication = () => {
    console.log("🚀 Iniciando App.js..."); // Log para confirmar que rodou
    initApp();
    setupEventListeners();
    setupKeyboardListeners();
};

// Verifica se o site AINDA está carregando ou se JÁ carregou
if (document.readyState === 'loading') {
    // Se ainda está carregando, espera
    document.addEventListener('DOMContentLoaded', startApplication);
} else {
    // Se já carregou (nosso caso), roda imediatamente!
    startApplication();
}
...
function loadSettings() {
    // Tenta ler o arquivo de configurações da loja
    const docRef = doc(db, `sites/${state.siteId}/settings`, 'general');

    // Configura o ouvinte
    onSnapshot(docRef, (docSnap) => {
        if (docSnap.exists()) {
            // CENÁRIO IDEAL: Achou a configuração no banco
            ...
            // CENÁRIO DE ERRO (O SEU CASO): A coleção settings não existe
            console.warn("⚠️ Settings não encontrados. Usando modo de emergência na memória.");

            // AQUI ESTAVA O ERRO ANTES: Não tente fazer setDoc/gravar aqui.
            // Apenas defina na memória para o site abrir.
            state.globalSettings = { allowNoStock: true };
        }

        // DESTRAMENTO: Carrega dependências de entrega e força o desenho dos produtos
        if (typeof setupDeliveryDependency === 'function') setupDeliveryDependency();
        renderCatalog(state.products);

    }, (error) => {
        // CENÁRIO DE PERMISSÃO NEGADA
        console.error("Erro de permissão em Settings:", error);

        // Define padrão e força abertura
        state.globalSettings = { allowNoStock: true };
        renderCatalog(state.products);
    });
}
...
function setupEventListeners() {
    ...
    // Lista de campos que acionam o salvamento de 'orders'
    const orderInputs = [
        { id: 'conf-req-code', event: 'change' },      // Checkbox
        { id: 'conf-shipping-rule', event: 'change' }, // Select
        { id: 'conf-shipping-value', event: 'blur' },  // Input Texto (ao sair)
        { id: 'conf-cancel-time', event: 'blur' }      // Input Numero (ao sair)
    ];
    ...
    orderInputs.forEach(item => {
        ...
        if (el) {
            el.addEventListener(item.event, () => {
                // Se for valor de frete, formata visualmente 0,00 se tiver função
                if(item.id === 'conf-shipping-value' && typeof formatMoneyInput === 'function') {
                    // (Opcional) Lógica de máscara se existir
                }
                autoSaveSettings('orders');
            });
            
            // Para inputs de texto, salva também no Enter
            if(item.event === 'blur') {
                el.addEventListener('keydown', (e) => {
...
async function autoSaveSettings(type) {
    ...
    try {
        // --- 1. LOGÍSTICA BÁSICA (CEP/RAIO) ---
        if (type === 'logistics') {
            const elCep = document.getElementById('conf-store-cep');
            const elDist = document.getElementById('conf-max-dist');
            dataToUpdate = {
                cep: elCep ? elCep.value.replace(/\D/g, '') : '',
                maxDistance: elDist ? parseFloat(elDist.value.replace(',', '.')) : 0
            };
            message = 'Raio e CEP salvos!';
        }

        // --- 2. CONFIGURAÇÕES DE PEDIDO (FRETE, CÓDIGO, TEMPO, ENTREGA PRÓPRIA) ---
        // <--- AQUI ESTAVA FALTANDO A LÓGICA DO FRETE E CÓDIGO --->
        else if (type === 'orders') {
            // Inputs
            const elOwn = document.getElementById('conf-own-delivery');
            ...
            const shippingValue = elVal ? parseFloat(elVal.value.replace(',', '.')) || 0 : 0;
            const cancelTime = elTime ? parseInt(elTime.value) || 5 : 5;

            // Estrutura mista: Atualiza deliveryConfig e settings ao mesmo tempo
            dataToUpdate = {
                deliveryConfig: {
                    ownDelivery: elOwn ? elOwn.checked : true,
                    shippingRule: elRule ? elRule.value : 'none', // none, both, online, delivery
                    shippingValue: shippingValue
                },
                settings: {
                    reqClientCode: elReq ? elReq.checked : false,
                    cancellationTime: cancelTime
                }
            };
            message = 'Configurações de Pedido Salvas!';
        }
        
        // --- 3. PARCELAMENTO ---
        else if (type === 'installments') {
            const elActive = document.getElementById('conf-card-active');
            const elMax = document.getElementById('conf-card-max');
            const elFree = document.getElementById('conf-card-free');
            const elRate = document.getElementById('conf-card-rate');
            ...
            dataToUpdate = {
                installments: {
                    active: elActive ? elActive.checked : false,
                    max: elMax ? parseInt(elMax.value) : 12,
                    freeUntil: elFree ? parseInt(elFree.value) : 1,
                    rate: rate
                }
            };
            message = 'Parcelamento atualizado!';
        }
        ...
        // --- ATUALIZA MEMÓRIA LOCAL (Crucial para não precisar de F5) ---
        if (!state.storeProfile) state.storeProfile = {};
        
        // Atualiza DeliveryConfig
...
window.calcCheckoutTotal = () => {
    ...
    const payModeEl = document.querySelector('input[name="pay-mode"]:checked');
    const payMode = payModeEl ? payModeEl.value : null; // 'online' ou 'delivery'
    ...
    // Configurações de Cupom e Pix
    const pixGlobal = state.storeProfile?.pixGlobal || { active: false, value: 0, type: 'percent', mode: 'product' };
    ...
    // 2. LÓGICA DE FRETE (CORRIGIDA)
    // Só cobra se: O endereço for válido (dentro do raio) E o valor for maior que 0
    if (typeof checkoutState !== 'undefined' && checkoutState.isValidDelivery && shipValue > 0) {

        let applyShipping = false;

        if (shipRule === 'both') {
            applyShipping = true; // Cobra sempre
        }
        else if (shipRule === 'online' && payMode === 'online') {
            applyShipping = true; // Cobra só se pagar online
        }
        else if (shipRule === 'delivery' && payMode === 'delivery') {
            applyShipping = true; // Cobra só se pagar na entrega
        }

        if (applyShipping) {
            shippingFee = shipValue;
        }
    }

    // 3. LÓGICA DE DESCONTOS (Pix e Cupom)
    const methodEl = document.querySelector('input[name="payment-method-selection"]:checked');
    const method = methodEl ? methodEl.value : '';

    // Se for Pix, calcula desconto
    if (method === 'pix') {
        let totalPixBase = 0; // Valor base para cálculo do desconto

        if (pixGlobal.active && !pixGlobal.disableAll) {
            if (pixGlobal.mode === 'total') {
                // Desconto no total
                const desc = pixGlobal.type === 'percent'
                    ? subtotal * (pixGlobal.value / 100)
                    : pixGlobal.value;
                discountPix = desc;
            } else {
                // Modo Produto (já calculado no preço unitário visual, mas aqui precisamos somar a economia)
                state.cart.forEach(item => {
                    const originalTotal = item.price * item.qty;

                    // Recalcula o preço Pix deste item
                    let pricePix = item.price;
                    const desc = pixGlobal.type === 'percent'
                        ? item.price * (pixGlobal.value / 100)
                        : pixGlobal.value;
                    pricePix = Math.max(0, item.price - desc);

                    const pixTotal = pricePix * item.qty;
                    discountPix += (originalTotal - pixTotal);
                });
            }
        }
        // Fallback: Pix Individual do Produto
        else {
            state.cart.forEach(item => {
                const prod = state.products.find(p => p.id === item.id);
                if (prod && prod.paymentOptions?.pix?.active) {
                    const cfg = prod.paymentOptions.pix;
                    const desc = cfg.type === 'percent'
                        ? item.price * (cfg.val / 100)
                        : cfg.val;
                    discountPix += (desc * item.qty);
                }
            });
        }
    }

    // Cupom (Aplica sobre o valor já com desconto Pix, se houver)
    const baseForCoupon = Math.max(0, subtotal - discountPix);
    if (state.currentCoupon) {
        if (state.currentCoupon.type === 'percent') {
            discountCoupon = baseForCoupon * (state.currentCoupon.val / 100);
        } else {
    ...
    // 4. TOTAL FINAL
    // (Subtotal + Frete) - Descontos
    // Juros de cartão entram aqui se a lógica exigir, mas por padrão é isso:
    let finalTotal = (subtotal + shippingFee) - discountPix - discountCoupon;
    finalTotal = Math.max(0, finalTotal);

    // 5. RENDERIZAÇÃO
    const elBreakdown = document.getElementById('checkout-details-breakdown');
    if (elBreakdown) {
        let html = `<div class="flex justify-between text-gray-400"><span>Subtotal</span><span>${formatCurrency(subtotal)}</span></div>`;

        if (shippingFee > 0) {
            html += `<div class="flex justify-between text-yellow-500 font-bold"><span>Taxa de Entrega</span><span>+ ${formatCurrency(shippingFee)}</span></div>`;
        }
        if (discountPix > 0.01) {
            html += `<div class="flex justify-between text-green-500"><span>Desconto Pix</span><span>- ${formatCurrency(discountPix)}</span></div>`;
        }
        if (discountCoupon > 0.01) {
            html += `<div class="flex justify-between text-green-500"><span>Cupom</span><span>- ${formatCurrency(discountCoupon)}</span></div>`;
        }
...
function setupDeliveryDependency() {
    const elOwn = document.getElementById('conf-own-delivery'); // Entrega Própria
    const elPayDel = document.getElementById('conf-pay-delivery-active'); // Pagamento na Entrega

    if (!elOwn || !elPayDel) return;

    // --- 1. MEXEU NA ENTREGA PRÓPRIA ---
    elOwn.addEventListener('change', async (e) => {
        const isActive = e.target.checked;
        
        // Controle visual do "Código de Segurança" (só faz sentido se tiver entrega)
        const elReq = document.getElementById('conf-req-code');
        if (elReq) {
            if (!isActive) {
                elReq.checked = false;
        ...
        if (!isActive) {
            // Se estou DESLIGANDO a entrega, preciso desligar o pagamento na entrega?
            if (elPayDel.checked) {
                // Só posso desligar se o ONLINE estiver ligado
                const onlineOn = document.getElementById('conf-pay-online-active')?.checked;
                
                if (!onlineOn) {
                    alert("⚠️ AÇÃO BLOQUEADA:\n\nVocê não pode desativar a Entrega Própria agora.\nMotivo: O Pagamento Online já está desativado.\n\nAtive o Pagamento Online primeiro.");
                    e.target.checked = true; // Cancela a ação (volta o check)
                    return; 
                }
                
                // Se passou, desliga o pagamento na entrega também
                elPayDel.checked = false;
                showToast("Pagamento na entrega desativado (Sem logística).", "info");
                await autoSaveSettings('payments');
            }
        } else {
            // Se estou LIGANDO a entrega, ligo o pagamento na entrega por conveniência
            if (!elPayDel.checked) {
                elPayDel.checked = true;
                await autoSaveSettings('payments');
            }
        ...
        // Salva a alteração da ENTREGA/FRETE
        await autoSaveSettings('orders');
    });

    // --- 2. MEXEU NO PAGAMENTO NA ENTREGA ---
    elPayDel.addEventListener('change', async (e) => {
        const isActive = e.target.checked;
        
        // Tenta desligar pagamento na entrega
        if (!isActive) {
            const onlineOn = document.getElementById('conf-pay-online-active')?.checked;
            if (!onlineOn) {
                alert("⚠️ AÇÃO BLOQUEADA:\n\nVocê precisa manter pelo menos uma forma de pagamento ativa (Online ou Na Entrega).");
                e.target.checked = true; // Cancela
                return;
            }
        }

        // Se LIGAR pagamento na entrega, força LIGAR a logística (Entrega Própria)
        if (isActive && !elOwn.checked) {
            elOwn.checked = true;
            showToast("Entrega Própria ativada para permitir este pagamento.", "info");
            await autoSaveSettings('orders'); // Salva entrega
        }

        await autoSaveSettings('payments'); // Salva pagamento
    });
}
