...
async function initApp() {
    console.log("🚀 Iniciando App V2 (Versão Blindada)...");

    const params = new URLSearchParams(window.location.search);
    const siteId = params.get('site');

    // 1. Oculta tudo imediatamente
    ...
    try {
        console.log(`🔍 Verificando ID: ${siteId} no servidor...`);
        
        const docRef = doc(db, "sites", siteId);
        
        // Tenta buscar no servidor ignorando cache
        ...
        // A. CENÁRIO: EXCLUÍDO
        if (!snap.exists()) {
            console.error("❌ O documento não existe! (Site Excluído)");
        ...
        // B. CENÁRIO: BLOQUEADO
        if (data.status === 'bloqueado' || data.status === 'excluido' || data.active === false) {
    
    // Se for 'excluido', mostramos a tela de 404 igual se não existisse
    if (data.status === 'excluido') {
        throw new Error("SITE_DELETED"); // Joga para o catch que exibe a tela 404
    }

    // Se for apenas bloqueado
    throw new Error("SITE_BLOCKED");
        ...
        console.error("🔥 CAIU NO BLOQUEIO:", error);
        ...
        let msg = "Loja Indisponível";
        
        // Se for erro de permissão, é porque foi deletado e as regras bloquearam (o que é bom)
        if (error.code === 'permission-denied' || error.message === "SITE_DELETED") {
            msg = "404 - LOJA NÃO ENCONTRADA (EXCLUÍDA)";
        } else if (error.message === "SITE_BLOCKED") {
            msg = "ACESSO SUSPENSO";
        } else {
            msg = "ERRO DE CONEXÃO OU CONFIGURAÇÃO";
        }

        document.body.innerHTML = `
            <div style="height:100vh; display:flex; flex-direction:column; align-items:center; justify-content:center; color:red; text-align:center;">
                <h1 style="font-size:30px;">${msg}</h1>
                <p style="color:white; margin-top:10px;">O sistema impediu o carregamento.</p>
    ...
    // Se chegou aqui, é porque passou. Carrega o resto.
