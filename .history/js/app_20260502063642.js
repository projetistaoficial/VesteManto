...
function togglePixGlobalUI() {
    const elDisableAll = document.getElementById('conf-pix-disable-all');
    const elGlobalActive = document.getElementById('conf-pix-global-active');

    // ✨ BLINDAGEM 2: Se esses elementos ainda não existem no HTML, aborta em silêncio sem gerar erro vermelho!
    if (!elDisableAll || !elGlobalActive) return;

    const disableAll = elDisableAll.checked;
    const globalActive = elGlobalActive.checked;
