...
function populateInstallments() {
    ...
    // 1. Calcula o Valor dos Produtos (Base de Cálculo da Tabela Price)
    let totalProdutos = 0;
    ...
    // 2. Calcula o Frete Separadamente (Ficará FORA da Tabela Price)
    let valorFrete = 0;
    const dConfig = state.storeProfile.deliveryConfig || {};
    ...
    for (let i = 1; i <= maxParcelas; i++) {
        ...
        // A) Calcula o montante dos produtos
        let montanteProdutos = totalProdutos;
        let label = `${i}x Sem Juros`;

        // Aplica Juros (TABELA PRICE)
        if (instConfig.active && i > instConfig.freeUntil && instConfig.rate > 0) {
            const taxa = instConfig.rate / 100; // Taxa unitária (ex: 0.05 para 5%)
            const fator = Math.pow(1 + taxa, i); // (1+i)^n
            
            // --- FÓRMULA TABELA PRICE (PMT) ---
            // PMT = VP * [ i(1+i)^n / ((1+i)^n - 1) ]
            const parcelaPrice = totalProdutos * ((taxa * fator) / (fator - 1));
            
            // O montante total dos produtos será a parcela Price x numero de parcelas
            montanteProdutos = parcelaPrice * i;
            
            label = `${i}x (c/ juros)`;
        }

        // B) Soma o Frete (Sem juros, divisão simples) ao montante final
        // O frete é apenas somado ao total financiado pela Price
        const totalFinal = montanteProdutos + valorFrete;
        
        // C) Calcula o valor da parcela final para exibição (Produto Price + Parcela Frete)
