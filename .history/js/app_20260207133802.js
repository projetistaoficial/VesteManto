...
async function deleteCurrentClient() {
    // 1. Segurança: Verifica se tem um ID selecionado
    if (!currentDocId) return;

    // 2. Confirmação Dupla
    const confirmacao = prompt(`ATENÇÃO: Isso apagará a loja "${currentDocId}" e tornará o link inacessível.\n\nDigite DELETAR para confirmar:`);
    
    if (confirmacao !== "DELETAR") {
        return alert("Ação cancelada. O site não foi excluído.");
    }

    try {
        // 3. EXCLUSÃO REAL DO DOCUMENTO PAI
        // Ao deletar isso, o snap.exists() na loja vai retornar FALSE imediatamente.
        await deleteDoc(doc(db, "sites", currentDocId));

        // 4. LIMPEZA DE MEMÓRIA (O PASSO MAIS IMPORTANTE)
        // Isso impede que o site "ressuscite" se você clicar em salvar algo sem querer.
        const idDeletado = currentDocId; // Guarda só para o log
        currentDocId = null; 
        
        // 5. Resetar formulários para não sobrar dados visuais
        document.querySelectorAll('#client-modal input').forEach(i => i.value = '');
        
        // 6. Fechar modal e recarregar lista
        closeClientModal();
        await loadClients(); 

        alert(`Sucesso! O site '${idDeletado}' foi deletado totalmente.`);
...
PASSO 2: Bloqueio Total na Loja (js/app.js)
Agora, na loja do cliente, vamos garantir que se o documento principal foi deletado, a loja não carrega, mesmo que tenham sobrado produtos órfãos no banco de dados.

Use esta versão da função initApp. O segredo aqui é o getDocFromServer.

Substitua a função initApp no js/app.js:

JavaScript
// Certifique-se de importar getDocFromServer no topo do arquivo (vindo do firebase-config.js)

async function initApp() {
    const params = new URLSearchParams(window.location.search);
    const siteId = params.get('site');

    // 1. TRAVA VISUAL IMEDIATA
    document.body.style.opacity = "0";
    document.body.style.pointerEvents = "none";

    // Se não tem ID na URL, nem tenta buscar
    if (!siteId) {
        exibirTelaMorte("URL Inválida", "Nenhuma loja especificada.");
        return;
    }
    ...
    try {
        ...
        // 2. VERIFICAÇÃO RIGOROSA NO SERVIDOR
        // O "getDocFromServer" ignora o cache do celular e pergunta pro Google agora.
        // Se você deletou no painel, o google vai responder: exists = false.
        const snap = await getDocFromServer(docRef);

        // A. O SITE FOI DELETADO?
        // No Firestore, se você deleta o documento "pai", snap.exists() é falso.
        // Não importa se sobraram produtos órfãos, a loja é considerada inexistente.
        if (!snap.exists()) {
            console.error("Site não encontrado no servidor.");
            throw new Error("SITE_DELETED");
        }

        const data = snap.data();

        // B. O SITE ESTÁ BLOQUEADO?
        if (data.status === 'bloqueado' || data.active === false) {
            throw new Error("SITE_BLOCKED");
        }

        // === SUCESSO: O SITE EXISTE E ESTÁ ATIVO ===
        state.storeProfile = data;
        document.body.style.opacity = "1";
        document.body.style.pointerEvents = "auto";

    } catch (error) {
        console.error("Bloqueio Fatal:", error);
        
        // Limpa cache local do navegador do cliente para garantir
        localStorage.removeItem('vestemanto_cart'); 
        
        // Exibe a tela de erro 404
        if (error.message === "SITE_DELETED" || error.code === 'permission-denied') {
            exibirTelaMorte("404", "Esta loja foi excluída permanentemente.");
        } else if (error.message === "SITE_BLOCKED") {
            exibirTelaMorte("Acesso Suspenso", "Esta loja está temporariamente indisponível.");
        } else {
            exibirTelaMorte("Erro de Conexão", "Não foi possível carregar a loja.");
        }
        return; // MATA O CÓDIGO AQUI. Nada abaixo roda.
    }

    // ============================================================
    // CARREGAMENTO DOS MÓDULOS (SÓ RODA SE A LOJA EXISTIR)
    // ============================================================
    loadSiteStats();
    ...
    setInterval(async () => {
        try {
            const check = await getDocFromServer(doc(db, "sites", state.siteId));
            if (!check.exists()) window.location.reload();
        } catch(e) {}
    }, 30000);

    // Auth e Timers...
    setupAuthListener(); // (Suas funções auxiliares de auth)
    checkActiveOrders();
}

// Função auxiliar para criar a tela preta de erro
function exibirTelaMorte(titulo, mensagem) {
    document.body.style.opacity = "1";
    document.body.innerHTML = `
        <div style="position:fixed; top:0; left:0; width:100%; height:100vh; display:flex; flex-direction:column; align-items:center; justify-content:center; background:#000; color:#ef4444; z-index:99999; font-family:sans-serif; text-align:center;">
            <h1 style="font-size:5rem; margin:0; font-weight:bold;">${titulo}</h1>
            <h2 style="color:white; margin-top:10px; text-transform:uppercase;">Loja Indisponível</h2>
            <p style="color:#888; margin-top:20px; max-width:80%;">${mensagem}</p>
        </div>
    `;
}
