...
async function initApp() {
    ...
    // Verifica se a loja existe ou foi excluída ANTES de carregar qualquer coisa
    
    const params = new URLSearchParams(window.location.search);
    const siteId = params.get('site');

    if (!siteId) {
        document.body.innerHTML = '<div style="display:flex;height:100vh;align-items:center;justify-content:center;background:#000;color:#fff;"><h1>Loja não identificada.</h1></div>';
        return; // Para tudo aqui
    }

    // Salva o ID no estado global
    state.siteId = siteId;

    try {
        // Busca o documento da loja no Firestore
        const docRef = doc(db, "sites", siteId);
        const snap = await getDoc(docRef);

        // A. CENÁRIO: LOJA EXCLUÍDA
        if (!snap.exists()) {
            document.body.innerHTML = `
                <div style="height:100vh;display:flex;flex-direction:column;align-items:center;justify-content:center;background:#000;color:#fff;font-family:sans-serif;text-align:center;">
                    ...
                    <h2 style="text-transform:uppercase;letter-spacing:2px;">Loja Não Encontrada</h2>
                    <p style="color:#666;margin-top:10px;">Esta loja foi excluída permanentemente.</p>
                </div>`;
            return; // O comando return IMPEDE que o resto do código rode
        }

        const data = snap.data();

        // B. CENÁRIO: LOJA BLOQUEADA OU PAUSADA (OPCIONAL, SE QUISER BLOQUEAR ACESSO TOTAL)
        if (data.status === 'bloqueado' || data.active === false) {
            document.body.innerHTML = `
                <div style="height:100vh;display:flex;flex-direction:column;align-items:center;justify-content:center;background:#111;color:#fff;font-family:sans-serif;text-align:center;">
                    <h1 style="font-size:3rem;color:#ef4444;margin-bottom:20px;"><i class="fas fa-lock"></i></h1>
                    <h2>Acesso Temporariamente Bloqueado</h2>
                    <p style="color:#888;">Entre em contato com a administração.</p>
                </div>`;
            return; // Para tudo aqui também
        }

        // Se passar por aqui, a loja existe e está ativa.
        // Agora atualizamos o perfil localmente com os dados frescos que acabamos de baixar
        state.storeProfile = data; 

    } catch (error) {
        console.error("Erro de verificação inicial:", error);
        // Se der erro de rede, talvez queira deixar passar ou mostrar erro
        // Por segurança, vamos parar se for erro crítico
        if (error.code === 'permission-denied') return; 
    }

    // === 2. CARREGAMENTO NORMAL (SEU CÓDIGO ORIGINAL) ===
    // Só executa se passar pelas verificações acima
    ...
    loadCategories();
    loadProducts();
    // loadStoreProfile(); // <-- NÃO PRECISA MAIS, JÁ CARREGAMOS NO INÍCIO (linha 47)
    
    // Atualiza a UI com os dados que já pegamos
    ...
    // Checa status a cada 60 segundos (para bloquear em tempo real se o dono bloquear)
    setInterval(async () => {
        try {
            const check = await getDoc(doc(db, "sites", state.siteId));
            if (!check.exists() || check.data().status === 'bloqueado' || check.data().active === false) {
                window.location.reload(); // Recarrega a página para cair na trava do início
            }
        } catch(e) {}
    }, 60000);
