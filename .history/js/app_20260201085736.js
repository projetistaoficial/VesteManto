...
function setupEventListeners() {
  ...
  // =================================================================
    // LÓGICA DE PAGAMENTO E ENTREGA (Sincronizada)
    // =================================================================
    ...
    // --- 2. PAGAMENTO NA ENTREGA (O VILÃO) ---
    if (elDelivery) {
        ...
        newDelivery.addEventListener('change', async (e) => {
            ...
            // 1. ATUALIZA MEMÓRIA DO PAGAMENTO IMEDIATAMENTE
            if (!state.storeProfile.paymentMethods) state.storeProfile.paymentMethods = { online: {}, delivery: {} };
            ...
            // 2. VÍNCULO COM ENTREGA PRÓPRIA
            if (ownCheck) {
                // Se DESLIGOU pagamento -> Desliga Entrega
                if (!isActive && ownCheck.checked) {
                    ownCheck.checked = false; // Visual
                    
                    // ATUALIZA MEMÓRIA DA LOGÍSTICA IMEDIATAMENTE
                    ...
                    showSystemModal("⚠️ Atenção:\n\nAo remover o 'Pagamento na Entrega', a 'Entrega Própria' também foi desativada.");
                    await autoSaveSettings('orders'); // Salva Logística
                }
                
                // Se LIGOU pagamento -> Liga Entrega
                else if (isActive && !ownCheck.checked) {
    ...
    // --- 3. ENTREGA PRÓPRIA (CHECKBOX LOGÍSTICA) ---
    if (elOwnDelivery) {
        ...
        newOwn.addEventListener('change', async (e) => {
            ...
            // ATUALIZA MEMÓRIA LOGÍSTICA IMEDIATAMENTE
            if (!state.storeProfile.deliveryConfig) state.storeProfile.deliveryConfig = {};
            ...
            if (!isActive) {
                ...
                if (payCheck && payCheck.checked) {
                    ...
                    payCheck.checked = false; // Visual
                    
                    // ATUALIZA MEMÓRIA PAGAMENTO IMEDIATAMENTE
                ...
                // Se ligar Entrega, liga Pagamento
                if (payCheck && !payCheck.checked) {
    ...
    // Sub-opções (Validação de grupo vazio)
    const subOptions = [
