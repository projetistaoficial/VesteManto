...
// --- CORREÇÃO 3: Status da Loja ---
function updateStoreStatusUI() {
    const status = getStoreStatus(); // Certifique-se que getStoreStatus também é uma function
    ...
    if (badgeBtn) {
        if (!state.storeProfile.openingHours?.active) {
            ...
            // Lógica visual simplificada para garantir funcionamento
            let labelText = status.isOpen ? "Aberto" : "Fechado";
            let colorClass = status.isOpen ? "text-green-400 border-green-500/30 bg-green-500/10" : "text-red-400 border-red-500/30 bg-red-500/10";
            
            badgeBtn.className = `flex items-center justify-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider mt-2 border transition hover:opacity-80 mx-auto cursor-pointer ${colorClass}`;
            badgeBtn.innerHTML = `<span>${labelText}</span>`;
            
            badgeBtn.onclick = () => alert(status.isOpen ? "Estamos abertos!" : `Fechado. Abre às ${status.start}`);
        }
    }

    // Modal de Bloqueio
    if (!status.isOpen && status.block && modalBlock && !state.user) {
