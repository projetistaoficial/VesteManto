...
// =================================================================
// CÓDIGO CORRIGIDO: CÁLCULO DE FRETE (BRASIL API) E CHECKOUT
// =================================================================
...
// 2. Função de Distância usando BrasilAPI (Sem bloqueio CORS)
async function calculateDistanceByCEP(cepOrigin, cepDest) {
    const getCoords = async (cep) => {
        try {
            const cleanCep = cep.replace(/\D/g, '');
...
// 3. Listener do Campo de CEP (Visual e Validação)
window.handleCheckoutCep = async () => {
    const cepInput = document.getElementById('checkout-cep');
    if (!cepInput) return;
    ...
    try {
        // Busca dados de endereço (ViaCEP é ótimo para textos)
        const viaCepRes = await fetch(`https://viacep.com.br/ws/${cep}/json/`);
        const data = await viaCepRes.json();
        ...
        if (config.ownDelivery === true) {
            ...
            if (dist === null || isNaN(dist)) {
                throw new Error("Não foi possível validar a localização para entrega.");
            }
...
// 4. Função Submit Order (Com trava de segurança)
