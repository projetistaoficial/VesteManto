...
window.toggleCatDropdown = (parentName, event) => {
    ...
    dropdown.innerHTML = subs.map(sub => {
        ...
        return `
            <button onclick="filterByCat('${safeSubName}'); fecharCatDropdown()" class="block w-full text-left px-5 py-2.5 text-xs font-bold text-[var(--clr-te)] hover:bg-white/10 hover:text-white transition-colors outline-none">
                ${shortName}
            </button>
        `;
    }).join('');
