...
function setupEventListeners() {
    ...
    // CÁLCULO DE ENTREGA NO CHECKOUT ---
    if (els.checkoutCep) {
...
window.handleCheckoutCep = async () => {
    ...
    // 1. BLOQUEIO IMEDIATO DO BOTÃO (Correção Principal)
    if (btnFinish) {
        btnFinish.disabled = true;
        btnFinish.classList.add('opacity-50', 'cursor-not-allowed');
    }

    // Reseta estado global de entrega
    ...
    if (elDistDisplay) {
        elDistDisplay.innerText = "Calculando..."; // Feedback visual
    ...
    try {
        ...
        // --- VALIDAÇÃO DE DISTÂNCIA E FRETE ---
        ...
        // Se Entrega Própria estiver ativa e tiver CEP da loja configurado
        if (config.ownDelivery === true) {
            
            if (!storeCep) throw new Error("CEP da loja não configurado no Admin.");

            // Calcula Distância
            const dist = await calculateDistanceByCEP(storeCep, cep);

            // CORREÇÃO: Se dist for null (erro API), lança erro para impedir a venda
            if (dist === null) {
                throw new Error("Não foi possível calcular a distância. Verifique o CEP.");
            }

            checkoutState.distance = dist; // Salva para uso futuro
            ...
            if (elDistDisplay) {
                ...
                if (maxDist > 0 && dist > maxDist) {
                    ...
                    throw new Error(`Entrega indisponível para sua região.\nDistância: ${dist.toFixed(1)}km (Máx: ${maxDist}km)`);
                } else {
                    // LIBERA SE ESTIVER DENTRO
                    elDistDisplay.classList.add('text-green-500');
                }
            }
        }

        // SE CHEGOU AQUI SEM ERROS, LIBERA A VENDA
        ...
        console.error("Erro CEP/Distância:", err);
...
async function calculateDistanceByCEP(cepOrigin, cepDest) {
    // Função auxiliar para buscar coordenadas via BrasilAPI
    const getCoords = async (cep) => {
        try {
            // Limpa o CEP
            const cleanCep = cep.replace(/\D/g, '');
            
            // Busca na BrasilAPI (V2 inclui coordenadas)
            ...
            // BrasilAPI retorna location: { type: 'Point', coordinates: { longitude: -46..., latitude: -23... } }
            // As vezes retorna coordinates como array ou objeto, vamos garantir
            ...
            console.warn(`BrasilAPI falhou para ${cep}, tentando Nominatim...`);
            // Fallback para Nominatim (caso BrasilAPI caia)
            return getCoordsNominatim(cep);
        }
    };

    // Fallback antigo (Nominatim) - Só usa se BrasilAPI falhar
    const getCoordsNominatim = async (c) => {
        try {
            const url = `https://nominatim.openstreetmap.org/search?format=json&q=${c},+Brazil&limit=1`;
            const r = await fetch(url); // Removi o header User-Agent que estava causando erro CORS
    ...
    if (!c1 || !c2) return null; // Se não achar coordenada de um dos dois, retorna erro
