        ...
        if (!pixGlobal.disableAll) {
            ...
            if (pixGlobal.active && pixGlobal.value > 0) {
                ...
                if (pixGlobal.mode === 'total') {
                    // MODO TOTAL: Exibe APENAS a porcentagem de desconto, sem preço calculado
                    pixHtml = `<p class="text-green-500 text-[10px] font-bold mt-1"><i class="fas fa-tag mr-1"></i>${pixGlobal.value}% OFF no Pix (Total)</p>`;
                } else {
                    // MODO PRODUTO: Calcula (pois a regra é item a item)
