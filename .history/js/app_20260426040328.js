        ...
        onAuthStateChanged(auth, (user) => {
           ...
           // --- DECISÃO DE TELA E RADAR ---
            if (state.user) {
                ...
                // Ao chamar showView('admin'), ele já vai ligar o radar de avisos internamente!
                if (typeof showView === 'function') showView('admin');
                if (typeof filterAndRenderProducts === 'function') filterAndRenderProducts();

            } else {
                // É um cliente ou visitante deslogado, vai para a vitrine
                // O showView('catalog') vai matar o radar internamente.
...
function showView(viewName) {
   ...
   // =========================================================
    ...
    if (viewName === 'admin') {
        ...
        // ✨ LIGA O RADAR: Só quando entra fisicamente na tela
        if (typeof loadAvisos === 'function') loadAvisos();
        if (typeof processarAvisos === 'function') processarAvisos();
    }
        ...
        // ✨ FAXINA: Força a ocultação do ícone de sino se ele estiver ativo
        const alertIcon = document.getElementById('icone-avisos');
        if (alertIcon) alertIcon.classList.add('hidden');
        
        // ✨ FAXINA: Mata o ouvinte do Firebase para poupar dados e não dar vazamentos
        if (window.activeAvisosListener) {
            window.activeAvisosListener(); // Desconecta
