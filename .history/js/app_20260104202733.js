...
function loadAdminSales() {
    ...
    onSnapshot(q, (snapshot) => {
        // 2. Salva os pedidos na memória
        state.orders = snapshot.docs.map(d => ({ id: d.id, ...d.data() }));

        // 3. Conta quantos pedidos novos existem (viewed é falso ou não existe)
        const newOrdersCount = state.orders.filter(o => !o.viewed).length;

        // 4. Atualiza o Botão da Aba "Vendas"
        const btnVendas = document.getElementById('admin-menu-sales');
        if (btnVendas) {
            if (newOrdersCount > 0) {
                // Mostra: Vendas (3)
                btnVendas.innerHTML = `Vendas <span class="bg-red-600 text-white text-[10px] px-2 py-0.5 rounded-full ml-1 animate-pulse">${newOrdersCount}</span>`;
            } else {
                // Mostra: Vendas
...
window.filterAndRenderSales = () => {
    ...
    if (!list) return; // Se não estiver na tela de admin, para.

    // 1. Limpa a lista atual
    list.innerHTML = '';

    // 2. Pega os pedidos da memória (ou lista vazia se der erro)
    const orders = state.orders || [];

    if (orders.length === 0) {
        list.innerHTML = '<p class="text-gray-500 text-center py-4">Nenhum pedido encontrado.</p>';
        return;
    }

    // 3. Desenha cada pedido
    orders.forEach(order => {
        // --- LÓGICA DO NOVO ---
        // Se 'viewed' for falso, é NOVO.
        const isNew = !order.viewed;

        // Estilos: Se for novo, borda verde e fundo levemente verde. Se não, cinza padrão.
        const borderClass = isNew ? "border-green-500 border-l-4 bg-green-900/10" : "border-gray-800 bg-[#151720]";
        
        // Etiqueta HTML: Só aparece se for novo
        const badgeHtml = isNew ? 
            `<span class="ml-2 bg-green-600 text-white text-[9px] px-2 py-0.5 rounded font-bold animate-pulse">NOVO</span>` : "";
        ...
        // Ao clicar, chama a função para abrir e marcar como visto
        ...
        div.innerHTML = `
            <div class="flex flex-col gap-1">
                <div class="flex items-center">
                    <span class="font-bold text-white text-sm">#${order.code || order.id.slice(0,4)}</span>
                    ${badgeHtml}
                </div>
                <span class="text-xs text-gray-400">${dateStr}</span>
                <span class="text-xs font-bold text-yellow-500">${order.customerName || 'Cliente'}</span>
            </div>
            
            <div class="text-right">
                <div class="font-bold text-green-400 text-sm">R$ ${totalStr}</div>
                <span class="text-[10px] bg-gray-700 px-2 py-1 rounded text-gray-300 uppercase">${order.status || 'Pendente'}</span>
            </div>
        `;
