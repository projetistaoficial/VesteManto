...
function loadAdminSales() {
    // 1. Limpa ouvinte anterior
    if (state.salesUnsubscribe) state.salesUnsubscribe();

    // 2. Query LIMITADA (100 itens) para a LISTA (Aba Vendas)
    const q = query(
    ...
    state.salesUnsubscribe = onSnapshot(q, (snapshot) => {
        // A. Salva os dados na memória (apenas os 100 mais recentes)
        state.orders = snapshot.docs.map(d => ({ id: d.id, ...d.data() }));

        // B. Notificações (Bolinha vermelha)
        ...
        // C. RENDERIZA A LISTA (Isso resolve o problema da tela zerada)
        // Chamamos a função que desenha a tabela baseada no state.orders que acabamos de carregar
        if (typeof filterAndRenderSales === 'function') {
            filterAndRenderSales();
        }

        // D. BUSCA OS TOTAIS GLOBAIS (Isso resolve as estatísticas erradas)
        // Chama a função que vai no servidor pegar o total REAL (315) e preencher os cards
        // IMPORTANTE: Só chama se não estiver filtrando por texto
        const searchInput = document.getElementById('filter-search-general');
        if (!searchInput || searchInput.value === '') {
             updateRealTotals(); 
        }

        // E. Atualiza Métricas Locais (Gráficos menores, porcentagens)
        if (typeof updateStatsData === 'function') {
            updateStatsData(state.orders, state.products, state.dailyStats);
        }
        // Se updateStatsData chamar calculateStatsMetrics, o Passo 1 impede que ela estrague os totais.

        // F. Botão Carregar Mais
