...
// Função para configurar os Listeners de Dependência (CORRIGIDA)
function setupDeliveryDependency() {
    ...
    // =================================================================
    // Clonamos para limpar ouvintes antigos e evitar duplicação
    ...
    newOwn.addEventListener('change', async (e) => {
        ...
        // CENÁRIO: DESATIVAR ENTREGA
        if (!isActive) {
            // Se o Pagamento na Entrega estiver ligado, temos que desligar também
            if (elPayDelivery.checked) {
                // Antes, verifica se o Online está ligado (para não ficar sem nenhum pagamento)
                const onlineOn = document.getElementById('conf-pay-online-active')?.checked;
                
                if (!onlineOn) {
                    showSystemModal("⚠️ AÇÃO BLOQUEADA:\n\nO Pagamento Online já está desativado.\nVocê não pode desligar a Entrega agora pois ficaria sem formas de pagamento.", "warning");
                    e.target.checked = true; // Cancela a ação
                    return; 
                }
        ...
        // CENÁRIO: ATIVAR ENTREGA
        else {
            // Se ativou a entrega, ativamos o pagamento por conveniência
            if (!elPayDelivery.checked) {
    ...
    newPay.addEventListener('change', async (e) => {
        ...
        // TRAVA: Pelo menos um pagamento ativo
        ...
        if (ownCheck) {
            // CENÁRIO: DESATIVOU O PAGAMENTO NA ENTREGA
            if (!isActive) {
                if (ownCheck.checked) {
                    // Desativa a Entrega Própria também
                    ownCheck.checked = false;

                    // Trava input de código
                    const elReq = document.getElementById('conf-req-code');
                    if (elReq) {
                        elReq.disabled = true;
                        elReq.checked = false;
                    ...
                    // *** O AVISO QUE FALTAVA ***
                    showSystemModal("⚠️ Atenção:\n\nAo remover o 'Pagamento na Entrega', a 'Entrega Própria' da loja também foi desativada.", "warning");
                    
                    await autoSaveSettings('orders'); // Salva a mudança na entrega
                }
            }
            // CENÁRIO: ATIVOU O PAGAMENTO NA ENTREGA
            else {
                if (!ownCheck.checked) {
                    ...
                    showSystemModal("A 'Entrega Própria' foi ativada automaticamente para permitir este pagamento.", "success");
                    await autoSaveSettings('orders');
                }
            }
        }

        await autoSaveSettings('payments'); // Salva a mudança no pagamento
    });
}
