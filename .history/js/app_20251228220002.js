...
function renderProductsList(products) {
            ...
            bulkBar.innerHTML = `
                <div class="flex flex-col md:flex-row items-center justify-between w-full bg-blue-900/50 p-2 rounded border border-blue-500/30 mb-4 gap-2">
                    <div class="flex items-center gap-3">
                        <button onclick="clearProductSelection()" class="w-8 h-8 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center transition">
    ...
    // AGORA INCLUÍMOS TODOS OS STATUS RELEVANTES PARA HISTÓRICO
    // Mesmo se cancelar, o registro de que houve uma venda/tentativa permanece.
    ...
    if (state.orders) {
        state.orders.forEach(order => {
            // Verifica se o pedido tem um status que conta para histórico
            if (historyStatuses.includes(order.status)) {
                
                // Usa a data do pedido (criação) para ordenar a movimentação
                const orderDate = new Date(order.date);
                
                order.items.forEach(item => {
                    if (!metricsMap[item.id]) {
                        metricsMap[item.id] = { qtd: 0, lastDate: null };
                    }
                    
                    // Soma a quantidade vendida (mesmo que cancelada, para registro histórico)
                    metricsMap[item.id].qtd += (parseInt(item.qty) || 0);

                    // Atualiza a data se essa venda for mais recente
