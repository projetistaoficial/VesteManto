...
// Função para configurar os Listeners de Dependência (CORRIGIDA DEFINITIVA)
function setupDeliveryDependency() {
    const elOwn = document.getElementById('conf-own-delivery'); // Entrega
    const elPayDel = document.getElementById('conf-pay-delivery-active'); // Pagamento
    ...
    newOwn.addEventListener('change', async (e) => {
        ...
        // 1. Atualiza Memória da Entrega IMEDIATAMENTE
        ...
        // Se desligou entrega, desliga pagamento
        if (!isActive) {
            ...
            if (payCheck && payCheck.checked) {
                ...
                if (!onlineOn) {
                    alert("Você não pode desligar a entrega pois o pagamento online já está desativado.");
                    e.target.checked = true;
                    state.storeProfile.deliveryConfig.ownDelivery = true;
                    return; 
                }

                // Desliga visualmente o pagamento
                payCheck.checked = false;
                
                // Atualiza Memória do Pagamento IMEDIATAMENTE
                ...
                if (group) group.classList.add('opacity-30', 'pointer-events-none');
            ...
            if (payCheck && !payCheck.checked) {
                ...
                // Atualiza Memória do Pagamento
                if (state.storeProfile.paymentMethods?.delivery) {
                    state.storeProfile.paymentMethods.delivery.active = true;
                }
                
                const group = document.getElementById('group-delivery-methods');
                if (group) group.classList.remove('opacity-30', 'pointer-events-none');
    ...
    // --- 2. MEXEU NO PAGAMENTO NA ENTREGA (AQUI ESTAVA O PROBLEMA) ---
    ...
    newPay.addEventListener('change', async (e) => {
        ...
        // 1. ATUALIZA A MEMÓRIA DO PAGAMENTO AGORA (Antes de qualquer lógica)
        // Isso impede que o check volte quando a entrega for salva
        ...
        if (!isActive && !onlineOn) {
            ...
            state.storeProfile.paymentMethods.delivery.active = true; // Volta memória
            return;
        }

        // 3. Atualiza Visual do Grupo
        const group = document.getElementById('group-delivery-methods');
        if (group) {
        ...
        // 4. Lógica da Entrega Própria (Dependência Reversa)
        const ownCheck = document.getElementById('conf-own-delivery'); 
        ...
        if (ownCheck) {
            ...
            if (!isActive && ownCheck.checked) {
                ...
                showSystemModal("⚠️ A 'Entrega Própria' também foi desativada.");
                
                // Salva a Entrega (Isso vai disparar o fillProfileForm, mas como a memória do pagamento já está atualizada no passo 1, o check não vai voltar)
                await autoSaveSettings('orders');
            }
        ...
        // 5. Por fim, salva o pagamento no banco
        await autoSaveSettings('payments');
    });
}
