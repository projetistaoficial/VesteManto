...
function calculateStatsMetrics() {
    // 1. Capital de Giro (Estoque) - Sempre calcula com base nos produtos carregados
    let capitalGiro = 0;
    if (state.products) {
    ...
    // Se for filtro de período, aplica o filtro nas ordens locais
    ...
    filteredOrders.forEach(o => {
        ...
        // Soma valores apenas de pedidos válidos
    ...
    // Se NÃO for 'all' (ou seja, é um dia/mês específico), usamos os dados locais calculados acima
    if (state.statsFilterType !== 'all') {
        ...
        // Se for 'all' (Visão Geral), chamamos a função global para preencher os cards com os dados do servidor
        // O setTimeout ajuda a garantir que essa chamada ocorra após qualquer renderização pendente
        // e sobrescreva os dados locais limitados.
        setTimeout(fetchGlobalStats, 50);
    }

    // Atualiza contadores secundários (estes ficam baseados no carregamento local)
