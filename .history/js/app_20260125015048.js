...
window.openCheckoutModal = () => {
    ...
    // 2. LEITURA "FORÇA BRUTA" DAS CONFIGURAÇÕES
    // Acessamos o state com verificações de segurança para não quebrar
    const profile = state.storeProfile || {};
    const pm = profile.paymentMethods || {};
    const dc = profile.deliveryConfig || {};

    // --- LÓGICA DE ATIVAÇÃO ---
    // Regra: Se for undefined, considera TRUE (ativado). Só é false se for explicitamente false.
    
    // A) Pagamento Online
    const isOnlineActive = (pm.online && pm.online.active === false) ? false : true;
    
    // B) Método de Pagamento na Entrega (Dinheiro/Maquininha)
    const isDeliveryMethodActive = (pm.delivery && pm.delivery.active === false) ? false : true;
    
    // C) Logística Própria (Se a loja faz entrega)
    const isLogisticsActive = (dc.ownDelivery === true); // Padrão é false se undefined

    // REGRA COMBINADA: A opção "Pagar na Entrega" só aparece se o FINANCEIRO aceita E a LOGÍSTICA entrega.
    const showDeliveryOption = isDeliveryMethodActive && isLogisticsActive;

    // --- LOGS PARA VOCÊ VER NO CONSOLE (F12) ---
    console.log("1. Estado da Configuração:");
    console.log("   • Online Ativo (Financeiro):", isOnlineActive);
    console.log("   • Entrega Ativa (Financeiro):", isDeliveryMethodActive);
    console.log("   • Entrega Própria (Logística):", isLogisticsActive);
    console.log("   --------------------------------");
    console.log("   RESULTADO VISUAL ESPERADO:");
    console.log("   • Botão Online deve aparecer?", isOnlineActive);
    console.log("   • Botão Entrega deve aparecer?", showDeliveryOption);

    // 3. MANIPULAÇÃO DO DOM (ELEMENTOS HTML)
    const elBtnOnline = document.getElementById('label-pay-online');
    const elBtnDelivery = document.getElementById('container-delivery-option');
    ...
    // A) Aplicar Visibilidade ONLINE
    if (elBtnOnline) {
        if (isOnlineActive) {
            elBtnOnline.classList.remove('hidden');
            elBtnOnline.style.display = 'flex'; // Força display flex
            if (radioOnline) radioOnline.disabled = false;
        } else {
            elBtnOnline.classList.add('hidden');
            elBtnOnline.style.display = 'none'; // Força display none
            if (radioOnline) radioOnline.disabled = true;
        }
    } else {
        console.error("ERRO CRÍTICO: ID 'label-pay-online' não encontrado no HTML.");
    }

    // B) Aplicar Visibilidade ENTREGA
    if (elBtnDelivery) {
        if (showDeliveryOption) {
            elBtnDelivery.classList.remove('hidden');
            elBtnDelivery.style.display = 'block'; // Força display block
            if (radioDelivery) radioDelivery.disabled = false;
        } else {
            elBtnDelivery.classList.add('hidden');
            elBtnDelivery.style.display = 'none'; // Força display none
            if (radioDelivery) radioDelivery.disabled = true;
        }
    } else {
        console.error("ERRO CRÍTICO: ID 'container-delivery-option' não encontrado no HTML.");
    }

    // 4. CORREÇÃO DA SELEÇÃO (AUTO-SELECT)
    // Impede que uma opção invisível fique marcada
    
    // Se Online está visível, marcamos ele por padrão
    if (isOnlineActive && radioOnline) {
        radioOnline.checked = true;
    } 
    // Se Online sumiu, mas Entrega está visível, marcamos Entrega
    else if (!isOnlineActive && showDeliveryOption && radioDelivery) {
        radioDelivery.checked = true;
    } 
    // Se nenhum está visível (Erro de config), desmarca tudo
    else {
        if (radioOnline) radioOnline.checked = false;
        if (radioDelivery) radioDelivery.checked = false;
    }

    // 5. ATUALIZAÇÕES FINAIS (Chama as lógicas de UI dependentes)
    // Pequeno delay para garantir que o navegador processou o display:none
    setTimeout(() => {
