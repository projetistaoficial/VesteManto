...
async function autoSaveSettings(type) {
    console.log(`[AutoSave] Disparado para: ${type}`);
    const docRef = doc(db, `sites/${state.siteId}/settings`, 'profile');
    let dataToUpdate = {};
    let message = '';

    try {
        // --- TIPO: ORDERS (FRETE, CÓDIGO, TEMPO, ENTREGA) ---
        if (type === 'orders') {
            const elOwn = document.getElementById('conf-own-delivery');
            ...
            const shippingVal = elVal ? parseFloat(elVal.value.replace('.', '').replace(',', '.')) || 0 : 0;
        ...
        // --- TIPO: INSTALLMENTS (PARCELAMENTO) ---
        ...
        // --- TIPO: PAYMENTS (PAGAMENTO) ---
            ...
            message = 'Pagamentos Salvos!';
        }

        // 1. SALVA NO FIREBASE
        await setDoc(docRef, dataToUpdate, { merge: true });

        // 2. ATUALIZA MEMÓRIA LOCAL (STATE)
        if (!state.storeProfile) state.storeProfile = {};
        
        // Merge profundo manual para não apagar o resto do objeto
        ...
        if (dataToUpdate.settings) {
            state.storeProfile.settings = { ...state.storeProfile.settings, ...dataToUpdate.settings };
        }
        if (dataToUpdate.paymentMethods) {
            state.storeProfile.paymentMethods = dataToUpdate.paymentMethods;
        }
        if (dataToUpdate.installments) {
            state.storeProfile.installments = dataToUpdate.installments;
        }
