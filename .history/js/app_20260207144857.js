...
// =================================================================
// 3. INICIALIZAÇÃO BLINDADA (COM LOGS)
// =================================================================
...
const startApplication = async () => {
    ...
    console.log(`🚀 [${agora}] INICIANDO SISTEMA V3...`);

    // 1. CORTINA DE FERRO: Esconde tudo via JS imediatamente
    document.body.style.display = 'none';
    document.body.style.backgroundColor = 'black';

    // 2. Limpeza de Cache de PWA (Service Workers)
    if ('serviceWorker' in navigator) {
        ...
        } catch(e) { /* ignora erro de sw */ }
    }

    // 3. O PORTEIRO: Chama initApp e ESPERA (await) a resposta
    // Se initApp retornar FALSE, o código para e não carrega listeners
    const acessoPermitido = await initApp();

    if (acessoPermitido) {
        console.log("✅ [SISTEMA] Acesso Autorizado. Carregando interface...");
        
        // Só carrega os eventos se o site existir
        setupEventListeners();
        ...
        console.warn("⛔ [SISTEMA] Acesso Negado. Interface abortada.");
        // O initApp já cuidou de mostrar a tela de erro (Kill Screen)
        // Não fazemos nada aqui, o site continua com display: none ou com a tela de erro
    }
};
...
async function initApp() {
    ...
    // 1. Matar Service Workers (Cache PWA persistente)
    if ('serviceWorker' in navigator) {
    ...
    // Se não tiver ID, mostra erro fatal
    if (!siteId) {
        matarSite("Loja não identificada", "Link inválido.");
        return;
    }
    ...
    try {
        ...
        // 2. CHECK NO SERVIDOR (IGNORA CACHE LOCAL)
        // Isso é obrigatório. Se você deletou no painel, o Google vai retornar false aqui.
        const snap = await getDocFromServer(docRef);

        // --- MOMENTO DA VERDADE ---

        // A. NÃO EXISTE (FOI DELETADO)
        if (!snap.exists()) {
            console.error("⛔ SITE DELETADO DETECTADO");
        ...
        // B. ESTÁ BLOQUEADO/INATIVO
        if (data.status === 'bloqueado' || data.active === false || data.status === 'excluido') {
            throw new Error("SITE_BLOCKED");
        }

        // =========================================================
        // 3. SUCESSO: O SITE EXISTE! LIBERA O ACESSO.
        // =========================================================
        console.log("✅ Acesso permitido. Carregando...");
        ...
        // --- LEVANTA A CORTINA (REMOVE O BLOQUEIO VISUAL) ---
        document.body.classList.add('acesso-liberado');

        // Monitoramento contínuo (Segurança em tempo real)
        setInterval(async () => {
            try {
        ...
        console.error("⛔ BLOQUEIO ATIVADO:", error);
        
        let titulo = "ERRO 404";
        let msg = "Esta loja não existe mais.";

        if (error.message === "SITE_BLOCKED") {
            titulo = "ACESSO SUSPENSO";
            msg = "Loja temporariamente indisponível.";
        }

        // CHAMA A FUNÇÃO QUE DESTRÓI O SITE
        matarSite(titulo, msg);
        return; // PARA TUDO AQUI
    }
}

// FUNÇÃO DESTRUIDORA (Substitui o site por tela de erro)
function matarSite(titulo, msg) {
    document.body.innerHTML = ''; // Limpa todo o HTML original
    document.body.classList.add('acesso-liberado'); // Libera para mostrar o erro
    document.body.style.backgroundColor = '#000';
    
    document.body.innerHTML = `
        <div style="
            position: fixed; inset: 0; background: #000; color: #ef4444; 
            display: flex; flex-direction: column; align-items: center; justify-content: center; 
            z-index: 99999; font-family: sans-serif; text-align: center; padding: 20px;
        ">
            <h1 style="font-size: 4rem; margin-bottom: 20px; font-weight: bold;">${titulo}</h1>
            <p style="color: #fff; font-size: 1.5rem;">${msg}</p>
...
function renderKillScreen(titulo, msg) {
    ...
    document.body.innerHTML = `
        <div style="
            ...
            z-index: 99999; font-family: sans-serif; text-align: center; padding: 20px;
        ">
            <h1 style="font-size: 3rem; margin-bottom: 10px; font-weight: bold;">${titulo}</h1>
            <p style="color: #fff; font-size: 1.2rem;">${msg}</p>
            <button onclick="window.location.reload()" style="
...
// Helper caso precise chamar de fora
function showKillScreen(t, m) {
    document.body.innerHTML = `<div style="background:#000;height:100vh;color:red;display:flex;align-items:center;justify-content:center;"><h1>${t}</h1><p>${m}</p></div>`;
    document.body.classList.add('loaded');
}
