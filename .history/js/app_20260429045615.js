...
// =================================================================
// =================================================================
// 8. EVENT LISTENERS (CORRIGIDO - LIMPEZA DE CÓDIGO FANTASMA)
// =================================================================
function setupEventListeners() {
    ...
    // ✨ CORREÇÃO AQUI: Acionando o openNewProductModal() perfeitamente ✨
...
window.saveProduct = async (e) => {
    ...
    try {
        ...
        // Pega se o Switch "Ativar estoque gradeado" está ligado
        const isGraded = document.getElementById('prod-has-variations').checked;
        let finalStock = 0;
        let finalSizes = [];

        // ✨ VALIDAÇÃO DE ESTOQUE GRADEADO
        if (isGraded) {
            ...
            finalSizes = [...state.tempVariations]; // Salva a lista de caixinhas criadas
            finalStock = finalSizes.reduce((acc, val) => acc + parseInt(val.stock), 0); // Soma o total
        } else {
            // Estoque Geral Padrão
        ...
        const productData = {
            ...
            hasVariations: isGraded, // Manda pro banco se tem variação
            stock: finalStock,       // Manda o estoque total
            sizes: finalSizes,       // Manda o array de tamanhos
        ...
        if (!id) {
            ...
            showToast(`Produto #${nextCode} criado!`, 'success');
        } else {
...
window.editProduct = (id) => {
    ...
    // ✨ CARREGA A GRADE DE ESTOQUE DO BANCO PARA A TELA
    const chkVar = document.getElementById('prod-has-variations');
    
    if (p.hasVariations && Array.isArray(p.sizes) && p.sizes.length > 0) {
        chkVar.checked = true;
        
        // Garante compatibilidade caso existam produtos antigos no banco salvos apenas com letras "P, M, G"
    ...
    toggleStockMode(); // Atualiza a caixinha preta baseada no switch
