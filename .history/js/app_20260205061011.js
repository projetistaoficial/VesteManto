...
async function initApp() {
    ...
    try {
        ...
        // O segredo está aqui: { source: 'server' }
        // Isso obriga o navegador a ir na internet conferir. Se foi deletado, ele vai saber agora.
        let snap;
        try {
            snap = await getDoc(docRef); // Tenta normal primeiro
            // Se quiser ser muito agressivo use: await getDocFromCache ou getDocFromServer
            // Mas o getDoc padrão já atualiza se tiver rede.
            // Vamos forçar a verificação do dado "real":
            if (snap.metadata.fromCache && snap.exists()) {
                // Se veio do cache, vamos tentar confirmar no servidor em background ou forçar reload se crítico
                // Mas para exclusão, a verificação abaixo costuma bastar se a rede estiver ok.
            }
        } catch (e) {
            // Se falhar rede, tenta cache, mas avisa
            console.log("Offline ou erro de rede.");
        }

        // 2. VERIFICAÇÃO FATAL: SE NÃO EXISTE, MATA A TELA
        if (!snap || !snap.exists()) {
            document.body.innerHTML = ''; // Limpa qualquer coisa que tenha carregado
            document.body.style.backgroundColor = "black";
            document.body.innerHTML = `
                <div style="height:100vh;display:flex;flex-direction:column;align-items:center;justify-content:center;color:#ef4444;font-family:sans-serif;text-align:center;padding:20px;">
                    <h1 style="font-size:4rem;margin:0;">404</h1>
                    <h2 style="color:white;text-transform:uppercase;margin-top:10px;">Loja Não Encontrada</h2>
                    <p style="color:#666;margin-top:10px;">Esta loja foi encerrada ou não existe mais.</p>
                    <button onclick="window.location.reload()" style="margin-top:20px;padding:10px 20px;border:1px solid #333;background:transparent;color:#fff;cursor:pointer;border-radius:5px;">Tentar Novamente</button>
                </div>
            `;
            // Limpa dados locais para evitar fantasmas
            localStorage.removeItem('vestemanto_cart'); 
            return; // <--- O CÓDIGO PARA AQUI. Nada abaixo é executado.
        }
        ...
        if (data.status === 'bloqueado' || data.active === false) {
            document.body.innerHTML = `
                <div style="height:100vh;display:flex;flex-direction:column;align-items:center;justify-content:center;background:#111;color:#fff;font-family:sans-serif;text-align:center;">
                    ...
                    <h2 style="text-transform:uppercase;">Loja Indisponível</h2>
                    <p style="color:#888;">Acesso temporariamente suspenso.</p>
                </div>
            `;
        ...
        // Atualizamos o perfil com o dado fresco
        state.storeProfile = data;

    } catch (error) {
        console.error("Erro crítico na inicialização:", error);
        // Opcional: Mostrar tela de erro técnico ou tentar carregar cache se for erro de internet
    }

    // ============================================================
    // A PARTIR DAQUI CARREGA O RESTO (SÓ SE A LOJA EXISTIR)
    // ============================================================
    ...
    // loadStoreProfile(); // REMOVER ESSA LINHA (Já carregamos lá em cima)
    if(typeof renderStoreProfile === 'function') renderStoreProfile(); // Renderiza o que baixamos
    ...
    // Monitoramento em tempo real (mata a sessão se você excluir enquanto o cliente usa)
    setInterval(async () => {
        try {
            const check = await getDoc(doc(db, "sites", state.siteId));
            if (!check.exists()) {
                window.location.reload(); // Recarrega para cair na tela 404
            }
        } catch(e) {}
