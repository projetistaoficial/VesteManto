...
window.openCheckoutModal = () => {
    ...
    // 2. RECUPERA CONFIGURAÇÕES DO STATE (Blindado)
    const profile = state.storeProfile || {};
    const pm = profile.paymentMethods || {};
    const dc = profile.deliveryConfig || {};

    // --- LEITURA ESTRITA DAS CONFIGURAÇÕES ---
    // Se o valor for estritamente false, é false. Se for undefined ou true, é true.
    // Isso evita que configurações antigas quebrem o site.
    const onlineActive = (pm.online && pm.online.active === false) ? false : true;
    const deliveryMethodActive = (pm.delivery && pm.delivery.active === false) ? false : true;
    
    // Entrega Própria (Logística)
    const ownDeliveryActive = (dc.ownDelivery === true);

    // REGRA DE NEGÓCIO: 
    // "Pagar na Entrega" só aparece se o método financeiro estiver ON **E** a Logística Própria estiver ON.
    const showDeliveryOption = deliveryMethodActive && ownDeliveryActive;

    console.log("--- DEBUG CHECKOUT ---");
    console.log("Online Ativo?", onlineActive);
    console.log("Método Entrega Ativo?", deliveryMethodActive);
    console.log("Logística Própria Ativa?", ownDeliveryActive);
    console.log("RESULTADO: Mostrar opção entrega?", showDeliveryOption);
    console.log("----------------------");
    ...
    if (labelOnline) {
        if (onlineActive) {
            ...
            labelOnline.style.display = ''; // Remove inline styles antigos
            if(radioOnline) radioOnline.disabled = false;
        } else {
            labelOnline.classList.add('hidden');
            labelOnline.style.setProperty('display', 'none', 'important'); // Força total
    ...
    if (containerDelivery) {
        if (showDeliveryOption) {
            ...
            containerDelivery.style.setProperty('display', 'none', 'important'); // Força total
            if(radioDelivery) radioDelivery.disabled = true;
        }
    }

    // 5. AUTO-SELEÇÃO (Corrige qual bolinha vem marcada)
    // Evita que o usuário entre no modal com uma opção oculta marcada
    
    // Se Online estiver disponível, marca ele como prioridade
    if (onlineActive && radioOnline) {
        radioOnline.checked = true;
    } 
    // Se Online NÃO estiver disponível, mas Entrega SIM, marca Entrega
    else if (!onlineActive && showDeliveryOption && radioDelivery) {
        radioDelivery.checked = true;
    }
    // Se NENHUM estiver disponível (Erro de config), desmarca tudo
    else {
        if (radioOnline) radioOnline.checked = false;
        if (radioDelivery) radioDelivery.checked = false;
    }

    // 6. ATUALIZAÇÕES FINAIS (Chama as lógicas de UI dependentes)
    ...
    // 7. EXIBE AS TELAS (Código original mantido)
