...
function renderCategories() {
    ...
    // 1. Preenche Selects (Filtros Simples)
    ...
    const buildHtml = (node, level = 0) => {
        ...
        keys.forEach(key => {
            ...
            // Escapa aspas para o JS não quebrar
            const safePath = item._path.replace(/'/g, "\\'"); 

            // Estilos
            const paddingLeft = level * 12; 
            const baseClass = "block w-full text-left py-2 px-2 text-sm transition rounded hover:bg-gray-800 cursor-pointer";
            const textClass = level === 0 ? "text-yellow-500 font-bold uppercase tracking-wide" : "text-gray-300 font-medium";

            if (hasChildren) {
                html += `
                    <details class="group mb-1">
                        <summary class="list-none flex items-center justify-between cursor-pointer rounded hover:bg-gray-800 transition pr-2">
                            <span class="${textClass} py-2 px-2 flex-1" 
                                  style="padding-left:${paddingLeft > 0 ? paddingLeft + 'px' : '8px'}"
                        ...
                        <div class="border-l border-gray-800 ml-3">
                            ${buildHtml(item._children, level + 1)}
                        </div>
                    </details>
                `;
            } else {
                // Item final (sem filhos)
                html += `
                    <div class="${baseClass} ${textClass}" 
                         style="padding-left:${paddingLeft > 0 ? paddingLeft + 'px' : '8px'}"
                         onclick="filterByCat('${safePath}')">
                        ${level > 0 ? '<span class="text-gray-600 mr-1">↳</span>' : ''} ${key}
                    </div>
                `;
            }
        });
    ...
    els.sidebarCategories.innerHTML = `
        <button class="w-full text-left py-3 px-3 mb-2 text-white bg-blue-900/50 hover:bg-blue-900 border border-blue-800 rounded font-bold uppercase text-xs tracking-wider transition" onclick="filterByCat('')">
