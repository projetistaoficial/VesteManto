...
// =================================================================
// CÁLCULO DE FRETE E GEOLOCALIZAÇÃO (VERSÃO BRASIL API V2)
// =================================================================
...
// 2. Busca Coordenadas (Latitude/Longitude) via BrasilAPI
async function calculateDistanceByCEP(cepOrigin, cepDest) {
    const getCoords = async (cep) => {
        try {
            const cleanCep = cep.replace(/\D/g, '');
            
            // BrasilAPI V2 fornece coordenadas exatas
            const res = await fetch(`https://brasilapi.com.br/api/cep/v2/${cleanCep}`);
            
            if (!res.ok) {
                console.warn(`BrasilAPI erro para CEP ${cleanCep}`);
                return null;
            }

            const data = await res.json();
            
            // Verifica se a API retornou location
            ...
            console.error("Erro requisição coordenadas:", e);
            return null;
        }
    };

    // Executa as duas buscas simultaneamente
    const [c1, c2] = await Promise.all([getCoords(cepOrigin), getCoords(cepDest)]);

    // Se falhar alguma, retorna null
    if (!c1 || !c2) return null;

    return getDistanceFromLatLonInKm(c1.lat, c1.lon, c2.lat, c2.lon);
}

// 3. Listener e Lógica Visual (handleCheckoutCep)
window.handleCheckoutCep = async () => {
    ...
    if (elDistDisplay) {
        elDistDisplay.innerText = "Calculando raio de entrega...";
    ...
    try {
        // A. Busca Endereço Textual (Rua, Bairro) - ViaCEP é melhor pra isso
        ...
        // Só calcula se for Entrega Própria
        if (config.ownDelivery === true) {
            
            if (!storeCep) throw new Error("CEP da loja não configurado.");

            // Chama a função da BrasilAPI
            const dist = await calculateDistanceByCEP(storeCep, cep);

            // SE DER ERRO NO CÁLCULO (Retorno null)
            if (dist === null || isNaN(dist)) {
                // Aqui decidimos: Bloquear ou Liberar com Aviso?
                // Vamos liberar com aviso laranja para não perder venda por erro de API, 
                // mas você saberá que não foi validado.
                if (elDistDisplay) {
                    elDistDisplay.innerText = "Distância não verificada (Erro API)";
                    elDistDisplay.classList.remove('text-yellow-500');
                    elDistDisplay.classList.add('text-orange-500');
                }
                checkoutState.isValidDelivery = true; // Libera, mas com risco
            } 
            else {
                // CÁLCULO COM SUCESSO
                ...
                if (elDistDisplay) {
                    ...
                    const maxText = maxDist.toFixed(1).replace('.', ',');

                    if (maxDist > 0 && dist > maxDist) {
                        // CASO 1: LONGE DEMAIS (BLOQUEIA)
                        elDistDisplay.innerText = `Entrega indisponível: ${distText}km (Limite: ${maxText}km)`;
                        elDistDisplay.classList.remove('text-yellow-500');
                        elDistDisplay.classList.add('text-red-500');
                        
                        checkoutState.isValidDelivery = false; // Bloqueia lógico
                        // Força erro para cair no catch e manter botão travado
                        throw new Error(`Endereço fora da área de entrega (${distText}km).`);
                    } else {
                        // CASO 2: DENTRO DO RAIO (LIBERA)
                        elDistDisplay.innerText = `✓ Endereço atendido (${distText}km da loja)`;
                        elDistDisplay.classList.remove('text-yellow-500');
            ...
            // Se não for entrega própria, libera direto
            checkoutState.isValidDelivery = true;
        }

        // LIBERA O BOTÃO SE FOR VÁLIDO
        ...
        console.error("Erro CEP/Validação:", err);
        ...
        // Se foi erro de cálculo/distância, o texto no display já foi setado acima ou aqui
        if (elDistDisplay && elDistDisplay.innerText === "Calculando...") {
             elDistDisplay.innerText = "Erro ao validar endereço";
        }
