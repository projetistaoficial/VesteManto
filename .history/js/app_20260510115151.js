...
window.initCategoryCarousel = () => {
    ...
    let speed = 1; // 1 Pixel INTEIRO (Evita que o navegador trave o scroll)
    ...
    const playAutoScroll = () => {
        ...
        // SALVA ONDE ESTAVA
        let prevScroll = slider.scrollLeft;
        
        // TENTA DAR O PASSO
        slider.scrollLeft += (scrollDirection * speed);
        
        // VERIFICA SE SAIU DO LUGAR
        // Se tentou andar e continuou no mesmo pixel, é porque bateu na parede!
        if (slider.scrollLeft === prevScroll) {
            scrollDirection *= -1; // Inverte a direção (1 vira -1, e -1 vira 1)
            slider.scrollLeft += (scrollDirection * speed * 2); // Dá um empurrãozinho pra desgrudar
        }
    };
