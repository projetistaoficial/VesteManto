...
async function initApp() {
    ...
    // Apaga todo o HTML visual para garantir que nada antigo apareça (menu, rodapé, etc)
    // Deixamos apenas os scripts rodando no fundo.
    const appContent = document.body.innerHTML; // Salva o HTML original na memória
    document.body.innerHTML = ''; 
    document.body.style.backgroundColor = '#000'; // Fundo preto de carregamento
    ...
    try {
        console.log("🔒 Verificando existência da loja...");
        const docRef = doc(db, "sites", siteId);
        
        // 2. PERGUNTA AO SERVIDOR (IGNORA O CACHE DO CELULAR)
        // Isso obriga o navegador a ir na internet conferir.
        const snap = await getDocFromServer(docRef);

        // A. CENÁRIO: SITE EXCLUÍDO (404)
        if (!snap.exists()) {
            console.error("Site deletado do banco de dados.");
            // Limpa dados locais para evitar conflito futuro
        ...
        // Restaura o HTML original que apagamos no começo
        document.body.style.backgroundColor = ''; // Remove fundo preto
        document.body.innerHTML = appContent;     // Devolve o site
        
        // Re-conecta os elementos do DOM nas variáveis (pois o HTML foi recriado)
        // Isso é crucial porque perdemos as referências ao fazer innerHTML = ''
        // (Recomendação: recarregar a página seria mais seguro, mas vamos tentar manter SPA)
        ...
        // Re-ativa os listeners (botões, carrinho) pois o HTML foi resetado
        setupEventListeners(); 
        setupKeyboardListeners();
        ...
        renderKillScreen(
            error.message === 'SITE_BLOCKED' ? "ACESSO SUSPENSO" : "LOJA NÃO ENCONTRADA",
            error.message === 'SITE_BLOCKED' ? "Esta loja está temporariamente indisponível." : "Esta loja foi excluída permanentemente."
        );
        return; // MATA O CÓDIGO AQUI.
    }
}

// Função auxiliar para desenhar a tela de erro
function renderKillScreen(titulo, msg) {
    ...
    document.body.innerHTML = `
        ...
        ">
            ...
            <h1 style="font-size: 2.5rem; font-weight: bold; margin: 0;">${titulo}</h1>
    ...
    `;
    // Adiciona FontAwesome caso tenha sido apagado
