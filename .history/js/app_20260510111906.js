...
function renderCategories() {
    ...
    // 2. Renderiza as Pílulas Dinâmicas no Carrossel
    ...
    if (scrollContainer) {
        ...
        // Filtra apenas Categorias Principais (Sem o ' - ')
        ...
        principais.forEach(c => {
            ...
            // Procura se essa categoria tem filhas (Subcategorias)
            const hasSubs = state.categories.some(sub => sub.name.startsWith(c.name + ' - '));
            
            const coresBase = isAtivo ? 'bg-brand-pink text-white shadow-lg' : 'bg-[#1a1c23] text-gray-400 hover:bg-[#252836] hover:text-white';

            if (hasSubs) {
                // Botão DIVIDIDO (Nome + Seta)
                pillsHtml += `
                    <div class="categoria-btn flex items-center h-full rounded-full transition-all shrink-0 ${coresBase}" data-cat="${safeName}">
                        <button onclick="filterByCat('${safeName}')" class="px-4 h-full font-bold text-xs rounded-l-full outline-none">
                            ${c.name}
                        </button>
                        <button onclick="toggleCatDropdown('${safeName}', event)" class="px-3 h-full border-l border-white/10 flex items-center justify-center rounded-r-full hover:bg-white/10 outline-none">
                ...
                // Botão SIMPLES
                pillsHtml += `
                    <button onclick="filterByCat('${safeName}')" data-cat="${safeName}" class="categoria-btn px-4 h-full rounded-full font-bold text-xs transition-all shrink-0 ${coresBase}">
                        ${c.name}
                    </button>
                `;
            }
        ...
        // Inicia o loop e arraste do carrossel
...
window.filterByCat = (catName) => {
    ...
    // ===============================================
    // Muda a cor visualmente na hora para o efeito Translúcido
    ...
    todosBotoes.forEach(btn => {
        if (btn.dataset.cat === catName || catName.startsWith(btn.dataset.cat + ' -')) {
            // Fica Rosa Translúcido
            btn.classList.remove('bg-white/10', 'text-gray-300', 'border-white/10', 'hover:bg-white/20', 'hover:text-white');
            ...
            // Volta para Cinza Translúcido
            btn.classList.remove('bg-brand-pink/80', 'text-white', 'border-brand-pink/50');
            btn.classList.add('bg-white/10', 'text-gray-300', 'border-white/10', 'hover:bg-white/20', 'hover:text-white');
        } else {
            // Regra do botão principal "Todas"
            if (catName === '') {
                btn.classList.remove('bg-white/10', 'text-gray-300', 'border-white/10', 'hover:bg-white/20', 'hover:text-white');
                btn.classList.add('bg-brand-pink/80', 'text-white', 'border-brand-pink/50');
            } else {
                btn.classList.remove('bg-brand-pink/80', 'border-brand-pink/50');
                btn.classList.add('bg-white/10', 'text-gray-300', 'border-white/10', 'hover:bg-white/20', 'hover:text-white');
            }
        }
    });
