...
window.saveProduct = async (e) => {
    ...
    try {
        ...
        // ✨ CORREÇÃO: Força a recarga dos produtos atualizando a memória local na mesma hora
...
async function processStockUpdate(items, operation) {
    ...
    for (const item of items) {
        ...
        if (pSnap.exists()) {
            ...
            stockChanged = true; // Avisa que atualizou no banco
        }
    }
    
    // ✨ CORREÇÃO CRÍTICA: Se o estoque foi alterado, força a atualização da RAM e da tabela na mesma hora!
    if (stockChanged) {
        await loadProducts(true);
        if (typeof filterAndRenderProducts === 'function') filterAndRenderProducts();
    }
