...
function setupDeliveryDependency() {
    ...
    // --- 1. CONFIGURA CHECKBOX DE ENTREGA ---
    // Clona para remover qualquer conflito anterior (Garante 1 clique único)
    ...
    newOwn.addEventListener('change', async (e) => {
        ...
        // [FIX IMEDIATO] Atualiza a memória AGORA para o check não voltar
        ...
        // Lógica: Se desligou Entrega -> Desliga Pagamento
        if (!isActive) {
            if (elPayDel.checked) {
                // Segurança: Online está ligado?
                ...
                if (!onlineOn) {
                    ...
                    e.target.checked = true; 
                    state.storeProfile.deliveryConfig.ownDelivery = true; // Reverte memória
                    return; 
                }

                // Desliga visualmente o pagamento
                const currentPayEl = document.getElementById('conf-pay-delivery-active');
                currentPayEl.checked = false;
                
                // Desliga na memória o pagamento
        ...
        // Lógica: Se ligou Entrega -> Liga Pagamento (Opcional, mas recomendado)
        else {
            const currentPayEl = document.getElementById('conf-pay-delivery-active');
            if (!currentPayEl.checked) {
                currentPayEl.checked = true;
                if (state.storeProfile.paymentMethods?.delivery) {
    ...
    // --- 2. CONFIGURA CHECKBOX DE PAGAMENTO ---
    ...
    newPay.addEventListener('change', async (e) => {
        ...
        // [FIX IMEDIATO] Atualiza a memória AGORA
        ...
        if (!isActive && !onlineOn) {
            ...
            e.target.checked = true;
            state.storeProfile.paymentMethods.delivery.active = true; // Reverte memória
            return;
        }
        ...
        // Vínculo Reverso (Se mexer no Pagamento, mexe na Entrega)
        const currentOwn = document.getElementById('conf-own-delivery'); 
        
        if (currentOwn) {
            // Se desligou Pagamento -> Desliga Entrega
            if (!isActive && currentOwn.checked) {
                currentOwn.checked = false;
            ...
            else if (isActive && !currentOwn.checked) {
                currentOwn.checked = true;
                // Atualiza memória da Entrega
