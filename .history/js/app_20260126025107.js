...
async function calculateDistanceByCEP(cepOrigin, cepDest) {
    const getCoords = async (c) => {
        try {
            // TENTATIVA 1: Busca específica por postalcode (Mais precisa)
            let url = `https://nominatim.openstreetmap.org/search?format=json&postalcode=${c}&country=Brazil&limit=1`;
            let r = await fetch(url, { headers: { 'User-Agent': 'VesteMantoApp/1.0' } });
            let d = await r.json();

            // TENTATIVA 2: Se falhar, busca genérica por query (Mais abrangente)
            if (!d || d.length === 0) {
                console.warn(`Tentativa 1 falhou para CEP ${c}, tentando busca genérica...`);
                url = `https://nominatim.openstreetmap.org/search?format=json&q=${c},+Brazil&limit=1`;
                r = await fetch(url, { headers: { 'User-Agent': 'VesteMantoApp/1.0' } });
                d = await r.json();
            }
            ...
            console.error(`Não foi possível encontrar coordenadas para o CEP: ${c}`);
            return null;
    ...
    if (!c1 || !c2) return null; // Retorna null se não achar um dos dois
