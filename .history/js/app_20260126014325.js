...
function renderCatalog(productsToRender) {
    ...
    // Configurações
    const pixGlobal = state.storeProfile.pixGlobal || { disableAll: false, active: false, value: 0, mode: 'product' };
    ...
    filtered.forEach(p => {
        ...
        // --- LÓGICA DE EXIBIÇÃO DO PIX (ATUALIZADA) ---
        let pixHtml = '';
        ...
        if (!pixGlobal.disableAll) {
            ...
            if (pixGlobal.active && pixGlobal.value > 0) {
                ...
                if (pixGlobal.mode === 'total') {
                    // MODO TOTAL: Exibe APENAS a mensagem fixa solicitada
                    pixHtml = `<p class="text-green-500 text-[10px] font-bold mt-1"><i class="fas fa-tag mr-1"></i>Desconto no Pix</p>`;
                } else {
                    // MODO PRODUTO: Calcula e mostra o valor com desconto
                    const valDesconto = currentPrice * (pixGlobal.value / 100);
                    const finalPix = currentPrice - valDesconto;
...
function setupEventListeners() {
   ...
   // --- REGRAS PIX GLOBAIS (CORRIGIDO) ---
    ...
    // Inputs
    const radiosPixMode = document.getElementsByName('conf-pix-mode');
    const radiosPixType = document.getElementsByName('conf-pix-type'); // <--- NOVO
    const inputPixVal = document.getElementById('conf-pix-global-value');

    // 1. Checkboxes Principais
    if (chkDisablePix) {
        // Remove listener anterior para evitar duplicação (cloneNode trick)
    ...
    if (chkGlobalPix) {
        ...
        newGlobal.addEventListener('change', (e) => {
            togglePixGlobalUI();
            if (e.target.checked) {
                showSystemModal("⚠️ ATENÇÃO: PADRONIZAÇÃO ATIVA\n\nDescontos individuais dos produtos serão IGNORADOS.");
            }
            autoSaveSettings('installments');
        });
    }

    // 2. CORREÇÃO: Salvar ao sair do campo de valor (blur)
    if (inputPixVal) {
        // Remove listener antigo
        const newVal = inputPixVal.cloneNode(true);
        inputPixVal.parentNode.replaceChild(newVal, inputPixVal);
        
        newVal.addEventListener('blur', () => {
            console.log("Campo valor Pix alterado -> Salvando...");
            autoSaveSettings('installments');
        });
    }

    // 3. Listeners para os Radios (Tipo e Modo)
    radiosPixMode.forEach(r => r.addEventListener('change', () => autoSaveSettings('installments')));
    radiosPixType.forEach(r => r.addEventListener('change', () => autoSaveSettings('installments')));
}
...
window.openProductModal = (productId) => {
    ...
    if (!pixGlobal.disableAll) {
        ...
        if (pixGlobal.active && pixGlobal.value > 0) {
            ...
            if (pixGlobal.mode === 'total') {
                // MODO TOTAL: Exibe apenas a informação do desconto, SEM CÁLCULO
                pixHtml = `
                <div class="flex flex-col gap-1 mt-1">
                    <div class="flex items-center gap-2 text-sm text-gray-300">
                        <i class="fab fa-pix text-green-400 text-lg"></i>
                        <span class="text-green-400 font-bold">${pixGlobal.value}% de Desconto</span>
                    </div>
                    <p class="text-[10px] text-gray-500 pl-6 italic">* Aplicado no valor total da venda.</p>
                </div>`;
            } else {
                // MODO PRODUTO: Exibe o preço calculado
                const valDesconto = priceFinal * (pixGlobal.value / 100);
                const finalPix = priceFinal - valDesconto;

                pixHtml = `
                <div class="flex items-center gap-2 text-sm text-gray-300 mt-1">
                    <i class="fab fa-pix text-green-400 text-lg"></i>
                    <span><b>${formatCurrency(finalPix)}</b> no Pix <span class="text-green-400 text-[10px] font-bold bg-green-900/30 px-1.5 py-0.5 rounded ml-1">${pixGlobal.value}% OFF</span></span>
                </div>`;
            }
...
async function autoSaveSettings(type) {
    ...
    try {
        ...
        // 2. PARCELAMENTO E PAGAMENTOS (AQUI ESTÁ A ATUALIZAÇÃO DO PIX)
            ...
            // 2º Passo: Definir pixGlobal (COM O CAMPO TYPE NOVO)
            const pixGlobal = {
                ...
                type: document.querySelector('input[name="conf-pix-type"]:checked')?.value || 'percent', // <--- NOVO
            ...
            // 3º Passo: Montar o objeto final
            dataToUpdate = {
            ...
            message = 'Configurações de pagamento e Pix salvas!';
        }
        // 3. PEDIDOS
        else if (type === 'orders') {
        ...
        if (dataToUpdate.pixGlobal) state.storeProfile.pixGlobal = dataToUpdate.pixGlobal;
...
window.calcCheckoutTotal = () => {
    ...
    // A. PIX (Aqui entra a nova lógica)
    if (method === 'pix') {
        
        let totalWithPixPrices = 0;

        // 1. Verifica se "Desativar Tudo" está ligado
        if (pixGlobal.disableAll) {
            // Se bloqueado, o preço Pix é igual ao preço normal (sem desconto)
            totalWithPixPrices = subtotalDisplay;
        }
        // 2. Verifica se "Padronização Global" está ativa
        else if (pixGlobal.active && pixGlobal.value > 0) {
            
            if (pixGlobal.mode === 'product') {
                // MODO: Aplicar individualmente (simula desconto em cada item)
                state.cart.forEach(item => {
                    // Ignora config do produto, usa global
                    let price = item.price; 
                    const descVal = price * (pixGlobal.value / 100);
                    price = Math.max(0, price - descVal);
                    totalWithPixPrices += price * item.qty;
                });
            } else {
                // MODO: Total (Aplica % sobre a soma bruta)
                const totalDesc = subtotalDisplay * (pixGlobal.value / 100);
                totalWithPixPrices = Math.max(0, subtotalDisplay - totalDesc);
            }

        } 
        // 3. Padrão (Usa configurações individuais dos produtos)
        else {
            state.cart.forEach(item => {
                const prod = state.products.find(p => p.id === item.id);
                let price = item.price;
                
                // Lê do cadastro do produto
        ...
        // Calcula quanto foi economizado no Pix
        discountPixDisplay = Math.max(0, subtotalDisplay - totalWithPixPrices);

        // Aplica Cupom sobre o valor Pix
        if (state.currentCoupon) {
            if (state.currentCoupon.type === 'percent') {
    ...
    // B. CRÉDITO
        ...
        if (select && select.options.length > 0) {
            ...
            if (opt && opt.dataset.total) {
                finalTotal = parseFloat(opt.dataset.total); // Já tem frete se aplicável
                
                // Se o dataset.total já tem frete, e shippingDisplay > 0, precisamos cuidar para não somar 2x no visual.
                // Na lógica atual de populateInstallments, o frete já está embutido.
                // Mas para o 'finalTotal' ser correto aqui, usamos o valor do dropdown.
                // O shippingDisplay serve apenas para exibir na lista detalhada.
            }
        } else {
    ...
    // --- RENDERIZAÇÃO (Mantida igual, apenas atualiza valores) ---
    const detailsContainer = document.getElementById('checkout-details-breakdown');
    if (detailsContainer) {
        let html = `
            <div class="flex justify-between text-gray-400"><span>Subtotal</span><span>${formatCurrency(subtotalDisplay)}</span></div>
        `;
        if (discountPixDisplay > 0.01) {
            // Adiciona aviso se for Global
