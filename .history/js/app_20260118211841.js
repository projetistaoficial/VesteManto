...
window.openProductModal = (productId) => {
    // ... (parte inicial: busca produto, imagens, config do modal... mantém igual) ...
    ...
    // 2. IMAGENS (Mantém igual)
    let images = p.images || [];
    if (images.length === 0) images = ['https://placehold.co/600'];
    updateCarouselUI(images); // Chama sua função existente de carrossel

    // 3. TEXTOS E PREÇOS (Mantém igual com a lógica De/Por e Pix)
    ...
    const pricePix = priceFinal * 0.95; 
    const priceInstallment = priceFinal / 12;
    ...
    // 4. ESTRUTURA DE ROLAGEM (AQUI ESTÁ A MUDANÇA SOLICITADA)
    const rightCol = card.children[2]; 

    if (rightCol) {
        // MUDANÇA 1: O Scroll agora é aqui no PAI (Coluna Direita inteira)
        // Adicionamos 'overflow-y-auto' e 'custom-scrollbar' aqui.
        rightCol.className = "w-full md:w-1/2 flex flex-col h-full bg-gray-900 overflow-y-auto custom-scrollbar relative";
        ...
        // B. Miolo (Texto/Tamanhos)
        if (rightCol.children[1]) {
            const scrollContent = rightCol.children[1];
            // MUDANÇA 2: Removemos 'overflow-y-auto' daqui.
            // Mantemos 'flex-1' para empurrar o rodapé para baixo se o texto for curto.
            scrollContent.className = "px-6 md:px-8 py-6 space-y-6 flex-1";
        }
    }

    // 5. TAMANHOS (Mantém igual)
    const sizesDiv = getEl('modal-sizes');
    const sizesWrapper = getEl('modal-sizes-wrapper');
    let selectedSizeInModal = 'U';

    if (sizesDiv) {
        // ... (Lógica de renderização dos botões de tamanho mantém igual) ...
        sizesDiv.innerHTML = ''; 
        if (p.sizes && p.sizes.length > 0) {
            ...
            p.sizes.forEach(s => {
                ...
                btn.innerText = s;
                btn.onclick = () => { /* ... lógica de clique ... */ }; // (Resumido para não ocupar espaço, mantém sua lógica)
    ...
    if (btnAdd) {
        // MUDANÇA 3: O container do botão flui com a página.
        // 'mt-auto' garante que ele fique no final se o texto for pequeno.
        // Se o texto for grande, ele será empurrado para baixo e você precisará rolar para vê-lo.
