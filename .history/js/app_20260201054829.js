...
window.applyCheckoutVisibility = () => {
    ...
    // Entrega só aparece se estiver ativa no financeiro E na logística
    const showDelivery = deliveryActive && logisticsActive;

    // Elementos DOM
    const lblOnline = document.getElementById('label-pay-online');
    const divDelivery = document.getElementById('container-delivery-option');
    const rOnline = document.querySelector('input[name="pay-mode"][value="online"]');
    const rDelivery = document.querySelector('input[name="pay-mode"][value="delivery"]');

    // 1. Aplica Visibilidade MACRO
    if (lblOnline) {
        if (onlineActive) {
            lblOnline.classList.remove('hidden');
            if(rOnline) rOnline.disabled = false;
        } else {
            lblOnline.classList.add('hidden');
            if(rOnline) rOnline.disabled = true;
        }
    }

    if (divDelivery) {
        if (showDelivery) {
            divDelivery.classList.remove('hidden');
            if(rDelivery) rDelivery.disabled = false;
        } else {
            divDelivery.classList.add('hidden');
            if(rDelivery) rDelivery.disabled = true;
        }
    }

    // 2. Auto-seleção se a opção atual sumiu
    if (rOnline && rOnline.checked && !onlineActive) {
        if (showDelivery && rDelivery) rDelivery.checked = true;
        else rOnline.checked = false;
    }
    if (rDelivery && rDelivery.checked && !showDelivery) {
        if (onlineActive && rOnline) rOnline.checked = true;
        else rDelivery.checked = false;
    }
    // Se nada marcado, força o primeiro disponível
    if ((!rOnline || !rOnline.checked) && (!rDelivery || !rDelivery.checked)) {
        if (onlineActive && rOnline) rOnline.checked = true;
        else if (showDelivery && rDelivery) rDelivery.checked = true;
    }

    // 3. Visibilidade MICRO (Sub-opções)
    const mode = document.querySelector('input[name="pay-mode"]:checked')?.value;
    const configGroup = mode === 'online' ? pm.online : pm.delivery;

    // Mapa de sub-opções
    const subs = {
        pix: document.querySelector(`input[name="payment-method-selection"][value="pix"]`),
        credit: document.querySelector(`input[name="payment-method-selection"][value="credit"]`),
        debit: document.querySelector(`input[name="payment-method-selection"][value="debit"]`),
        cash: document.querySelector(`input[name="payment-method-selection"][value="cash"]`)
    };

    const getParent = (el) => el ? (el.closest('label') || el.parentElement) : null;

    // Helper para esconder
    const toggleSub = (key, isVisible) => {
        const el = subs[key];
        const container = getParent(el);
        // Container especial para Crédito/Dinheiro se houver
        const specialContainer = document.getElementById(`container-${key}-option`); 
        
        const target = specialContainer || container;

        if (target) {
            if (isVisible !== false) { // Se true ou undefined (padrão ativo)
    ...
    if (configGroup) {
        toggleSub('pix', configGroup.pix);
        toggleSub('credit', configGroup.credit);
        toggleSub('debit', configGroup.debit);
        if (mode === 'delivery') toggleSub('cash', configGroup.cash);
        else toggleSub('cash', false); // Dinheiro nunca no online
    }

    // 4. Auto-seleção de sub-opção
    const currentSub = document.querySelector('input[name="payment-method-selection"]:checked');
    if (!currentSub || currentSub.disabled || (getParent(currentSub) && getParent(currentSub).classList.contains('hidden'))) {
        const firstValid = document.querySelector('input[name="payment-method-selection"]:not(:disabled)');
        if (firstValid && getParent(firstValid) && !getParent(firstValid).classList.contains('hidden')) {
