...
async function autoSaveSettings(type) {
    ...
    try {
        ...
        // ... (suas linhas de atualização do state aqui) ...
        ...
        // --- CORREÇÃO: REDESENHA O CHECKOUT SE ESTIVER ABERTO ---
        const viewCheckout = document.getElementById('view-checkout');
        if (viewCheckout && !viewCheckout.classList.contains('hidden')) {
            applyCheckoutVisibility(); // <--- CHAMA A NOVA FUNÇÃO AQUI
        }
...
// Função para configurar os Listeners de Dependência
function setupDeliveryDependency() {
    ...
    // 1. QUANDO CLICA EM "ENTREGA PRÓPRIA"
    // (Removemos o listener antigo para evitar conflito e adicionamos este novo)
    // Nota: Como addEventListener acumula, vamos garantir que a lógica esteja aqui.

    // Clone para remover listeners antigos (truque rápido)
    ...
    newOwn.addEventListener('change', (e) => {
        ...
        // Lógica visual padrão (Codigo de segurança)
        const elReq = document.getElementById('conf-req-code');
        if (elReq) {
            if (isActive) {
                elReq.disabled = false;
        ...
        // LÓGICA NOVA: Se desativar entrega, desativa pagamento
        if (!isActive) {
    ...
    // 2. QUANDO CLICA EM "PAGAMENTO NA ENTREGA"
    ...
    newPay.addEventListener('change', (e) => {
        ...
        if (ownCheck) {
            if (isActive) {
                // SE ATIVAR: Ativa Entrega Própria também
                if (!ownCheck.checked) {
                    ownCheck.checked = true;
                    // Ativa visualmente o campo de código
                    const elReq = document.getElementById('conf-req-code');
                    if (elReq) {
                        elReq.disabled = false;
                        elReq.closest('label')?.classList.remove('opacity-50', 'pointer-events-none');
                ...
                // SE DESATIVAR: Desativa Entrega Própria também
                if (ownCheck.checked) {
                    ownCheck.checked = false;
                    // Desativa visualmente o campo de código
                    const elReq = document.getElementById('conf-req-code');
                    if (elReq) {
                        elReq.disabled = true;
                        elReq.checked = false;
                    ...
                    // Aviso Opcional (se quiser avisar que desligou a logística também)
                    // showSystemModal("Entrega própria desativada junto com o pagamento.");
