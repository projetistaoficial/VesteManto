...
window.handleCheckoutCep = async () => {
    ...
    // 1. TRAVA O BOTÃO ASSIM QUE COMEÇA A CALCULAR
    if (btnFinish) {
    ...
    try {
        // 2. Busca dados do endereço (ViaCEP é ótimo para preencher rua/bairro)
        ...
        // 3. Validação de Distância (Lógica da Loja)
        ...
        if (config.ownDelivery === true) {
            ...
            // Chama a função nova da BrasilAPI
            const dist = await calculateDistanceByCEP(storeCep, cep);

            // Se der erro no cálculo (distância nula ou NaN)
            if (dist === null || isNaN(dist)) {
                throw new Error("Não foi possível validar a distância deste CEP. Verifique se está correto.");
            }
            ...
            if (elDistDisplay) {
                ...
                if (maxDist > 0 && dist > maxDist) {
                    ...
                    throw new Error(`Entrega indisponível: Acima do raio de ${maxDist}km.`);
                } else {
                    // LIBERA
        ...
        // SUCESSO: Tudo válido, libera o botão
        ...
        checkoutState.isValidDelivery = false; // Mantém bloqueado

        if (elErrorMsg) elErrorMsg.innerText = err.message;
        if (elErrorDiv) elErrorDiv.classList.remove('hidden');
        if (elDistDisplay) elDistDisplay.innerText = "Erro no cálculo";
        
        // Mantém botão travado visualmente
        if (btnFinish) {
            btnFinish.disabled = true;
            btnFinish.classList.add('opacity-50', 'cursor-not-allowed');
        }
        ...
        // Recalcula totais (para somar ou remover frete se necessário)
...
async function calculateDistanceByCEP(cepOrigin, cepDest) {
    const getCoords = async (cep) => {
        try {
            ...
            // Usa BrasilAPI V2 (gratuita e não bloqueia)
            ...
            // Verifica se a API retornou as coordenadas
    ...
    // Se não encontrou coordenadas de algum dos dois, retorna null (erro)
...
// --- FUNÇÕES AUXILIARES DE CÁLCULO (Substitua as antigas) ---
...
// Nova função usando BrasilAPI (Resolve o erro 403/CORS)
async function calculateDistanceByCEP(cepOrigin, cepDest) {
    const getCoords = async (cep) => {
        try {
            const cleanCep = cep.replace(/\D/g, '');
            // Busca na BrasilAPI V2
            const res = await fetch(`https://brasilapi.com.br/api/cep/v2/${cleanCep}`);

            if (!res.ok) return null; // Se não achar, retorna nulo
            ...
            console.warn("Erro BrasilAPI:", e);
            return null;
        }
    };

    // Executa busca simultânea
    const [c1, c2] = await Promise.all([getCoords(cepOrigin), getCoords(cepDest)]);
...
window.submitOrder = async () => {
    try {
        ...
        // --- Monta texto do pagamento (Lógica Original) ---
        let paymentDetails = "";
        let paymentMsgShort = "";
        ...
        // Código de Segurança e Cancelamento (Usando a variável dConfig do topo - SEM DUPLICAR)
        ...
        // Frete (Usa a variável dConfig do topo - SEM DUPLICAR)
        const shipRule = dConfig.shippingRule || 'none';
        const shipValue = parseFloat(dConfig.shippingValue) || 0;
        ...
        // Envio para WhatsApp (Lógica Original)
        if (payMode === 'online') {
