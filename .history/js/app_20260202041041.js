...
function loadStoreProfile() {
    ...
    try {
        ...
        // 1. VERIFICAÇÃO DE EXCLUSÃO (Site não existe mais)
        if (!docSnap.exists()) {
            document.body.innerHTML = `
                <div style="height:100vh; display:flex; flex-direction:column; align-items:center; justify-content:center; background:#111; color:#fff; font-family:sans-serif;">
                    ...
                    <p style="color:#888;">Esta loja pode ter sido excluída ou o endereço está incorreto.</p>
                </div>
            `;
            // Para tudo! Não carrega o resto do site
            throw new Error("Site not found"); 
        }

        const data = docSnap.data();

        // 2. VERIFICAÇÃO DE BLOQUEIO/PAUSA (Site existe, mas está ativo: false)
        if (data.active === false) {
            document.body.innerHTML = `
                <div style="height:100vh; display:flex; flex-direction:column; align-items:center; justify-content:center; background:#0f172a; color:#fff; font-family:sans-serif; text-align:center; padding:20px;">
                    ...
                    <h1 style="font-size:2rem; font-weight:bold; margin-bottom:0.5rem;">Loja Temporariamente Indisponível</h1>
                    <p style="color:#94a3b8; max-width:400px; line-height:1.5;">
                        O acesso a esta loja está suspenso no momento. Entre em contato com a administração para regularizar.
                    </p>
                </div>
            `;
            // Para tudo!
        ...
        // ... (Se passou daqui, carrega o site normalmente: state.storeProfile = data ...)
        state.storeProfile = data; 
        // ... resto do seu código ...

    } catch (error) {
        console.error("Erro ao carregar loja:", error);
        // Se não for um erro de bloqueio/404 que nós mesmos lançamos, mostre erro genérico
        if (error.message !== "Site blocked" && error.message !== "Site not found") {
