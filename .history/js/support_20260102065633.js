...
// =================================================================
// 13. SISTEMA DE SUPORTE (ÁREA RESTRITA)
// =================================================================

// Variável de controle de acesso (reseta ao recarregar a página)
let isSupportAuthenticated = false;

// Defina sua senha mestra aqui (ou busque do Firebase se preferir algo mais seguro)
const MASTER_SUPPORT_KEY = "suporte123"; // <--- TROQUE POR SUA SENHA SECRETA

// 1. Interceptador de Abas (Substitui a lógica simples de abas)
function setupTabSystem() {
    document.querySelectorAll('.tab-btn').forEach(btn => {
        btn.onclick = (e) => {
            const targetId = btn.dataset.tab;

            // Lógica Especial para a Aba Suporte
            if (targetId === 'tab-suporte') {
                if (!isSupportAuthenticated) {
                    openSupportAuth(); // Pede senha
                    return; // Para a execução, não troca a aba ainda
                }
            }

            // Lógica Padrão de Troca de Abas
            ...
            document.querySelectorAll('.tab-btn').forEach(b => {
                ...
                // Adiciona estilo inativo (se não for o de suporte que é vermelho)
                if (!b.dataset.tab.includes('suporte')) b.classList.add('text-gray-400');
            });
...
// Chame essa função nova no initApp() ou substitua o bloco antigo de tabs no setupEventListeners
// Para garantir, chame setupTabSystem() DENTRO de setupEventListeners(), substituindo o código antigo das abas.

// 2. Funções do Modal de Senha
window.openSupportAuth = () => {
...
window.closeSupportAuth = () => {
    const modal = document.getElementById('modal-support-auth');
    if (modal) modal.classList.add('hidden');
};

window.verifySupportAccess = () => {
    const input = document.getElementById('support-password-input');
    ...
    if (pass === MASTER_SUPPORT_KEY) {
        ...
        closeSupportAuth();
        showToast("Modo Suporte Ativado!", "success");
        
        // Simula o clique na aba para abrir agora que está liberado
...
// Listener para tecla Enter no modal
...
// A. Limpar Cache (LocalStorage)
window.emergencyClearCache = () => {
    if(!confirm("Isso irá deslogar sua conta e limpar o carrinho local.\nContinuar?")) return;
    
    // Limpa chaves específicas para não perder configurações vitais se houver
    localStorage.removeItem('cart');
    localStorage.removeItem('site_orders_history');
    // localStorage.clear(); // Use com cautela se quiser limpar TUDO
    
    alert("Cache limpo. A página será recarregada.");
    window.location.reload();
};

// B. Resetar Monitoramento de Pedidos
window.emergencyResetOrders = () => {
    // Para os listeners ativos
    ...
    showToast("Monitoramento resetado. Recarregando lista...", "info");
...
window.runDiagnostics = () => {
    ...
    const info = {
        AppVersion: '1.0.0',
        User: auth.currentUser ? auth.currentUser.email : 'Não logado',
        SiteID: state.siteId,
        TotalProdutos: state.products.length,
        TotalPedidosMemoria: state.orders.length,
        TemaAtual: localStorage.getItem('theme') || 'Padrão',
        Carrinho: state.cart.length + ' itens',
        DataHora: new Date().toLocaleString()
    };

    box.innerHTML = `<pre class="text-[10px]">${JSON.stringify(info, null, 2)}</pre>`;
};
