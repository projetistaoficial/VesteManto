...
function calculateStatsMetrics() {
    ...
    // Regra: Estoque Atual * Custo. Se não tiver custo cadastrado, usa o valor de venda.
    let capitalGiro = 0;
    if (state.products) {
        state.products.forEach(p => {
            // Pega o estoque real (soma os tamanhos se for gradeado)
            let currentStock = 0;
            if (p.hasVariations && Array.isArray(p.sizes)) {
                currentStock = p.sizes.reduce((sum, s) => sum + (parseInt(s.stock) || 0), 0);
            } else {
                currentStock = parseInt(p.stock) || parseInt(p.generalStock) || 0;
            }

            if (currentStock > 0) {s
                // Tenta puxar o Custo. Se for zero ou vazio, usa Promoção, senão Preço Normal.
                let val = parseFloat(p.cost);
                if (!val || val <= 0) val = parseFloat(p.promoPrice);
                if (!val || val <= 0) val = parseFloat(p.price);
                if (!val || val <= 0) val = 0;

                capitalGiro += currentStock * val;
            }
        });
    }
    
    // Atualiza o DOM com segurança
    const elGiro = els.statCapitalGiro || document.getElementById('stat-capital-giro');
    if (elGiro) elGiro.innerText = formatCurrency(capitalGiro);
