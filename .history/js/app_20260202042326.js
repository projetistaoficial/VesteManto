...
async function loadStoreProfile() {
    try {
        // 1. CHECAGEM DE SEGURANÇA NO SERVIDOR (Ignora Cache Local)
        // Isso garante que se você bloqueou há 1 segundo, a loja vê agora.
        ...
        try {
            ...
            // Se estiver sem internet, tenta o cache
            siteSnap = await getDoc(siteRef);
        }

        // =========================================================
        // A. SITE EXCLUÍDO (Não existe documento)
        // =========================================================
        if (!siteSnap.exists()) {
            document.body.innerHTML = `
                <div class="flex flex-col items-center justify-center h-screen bg-black text-white text-center p-6">
                    <h1 class="text-6xl font-bold text-red-600 mb-4">404</h1>
                    <h2 class="text-2xl font-bold mb-2">Loja Não Encontrada</h2>
                    <p class="text-gray-500">Esta loja foi desativada permanentemente ou o link está incorreto.</p>
                </div>
            `;
            throw new Error("SITE_DELETED"); // Para a execução
        }

        const siteData = siteSnap.data();

        // =========================================================
        // B. SITE PAUSADO (Status amarelo)
        // =========================================================
        if (siteData.status === 'pausado') {
            document.body.innerHTML = `
                <div class="flex flex-col items-center justify-center h-screen bg-gray-900 text-white text-center p-6">
                    <div class="text-6xl mb-6 text-yellow-500">
                        <i class="far fa-clock"></i>
                    </div>
                    <h1 class="text-3xl font-bold text-white mb-3">Loja em Pausa</h1>
                    <p class="text-gray-400 max-w-md mx-auto">
                        Esta loja está temporariamente indisponível para manutenção ou ajustes administrativos.
                        <br><br>
                        Por favor, retorne em breve.
                    </p>
                </div>
            `;
            throw new Error("SITE_PAUSED");
        }

        // =========================================================
        // C. SITE BLOQUEADO (Status vermelho ou active: false)
        // =========================================================
        if (siteData.status === 'bloqueado' || siteData.active === false) {
            document.body.innerHTML = `
                <div class="flex flex-col items-center justify-center h-screen bg-gray-900 text-white text-center p-6">
                    ...
                    <h1 class="text-3xl font-bold text-white mb-3">Acesso Suspenso</h1>
                    <p class="text-gray-400 max-w-md mx-auto">
                        O acesso a esta loja encontra-se bloqueado administrativamente.
        ...
        onSnapshot(settingsRef, (docSnap) => {
            if (docSnap.exists()) {
                ...
                state.storeProfile = { name: siteData.name || "Loja", installments: { active: false } };
            }
            ...
            if (state.products && state.products.length > 0) renderCatalog(state.products);
            if (typeof updateStoreStatusUI === 'function') updateStoreStatusUI();
        });

    } catch (error) {
        if (["SITE_DELETED", "SITE_PAUSED", "SITE_BLOCKED"].includes(error.message)) return;
