...
window.handleCheckoutCep = async () => {
    ...
    // 1. BLOQUEIO INICIAL (Enquanto calcula)
    if (btnFinish) {
    ...
    try {
        // Busca Endereço (ViaCEP para preencher campos)
        ...
        if (config.ownDelivery === true) {
            ...
            // SE FALHAR O CÁLCULO (Retornar null ou NaN)
            if (dist === null || isNaN(dist)) {
                throw new Error("Não foi possível calcular a distância da loja. Verifique o CEP.");
            }
        ...
        if (elDistDisplay) elDistDisplay.innerText = "Erro no cálculo"; // Evita mostrar NaN
