...
window.aplicarCoresCategorias = (catName) => {
    ...
    todosBotoes.forEach(btn => {
        // Verifica se o botão é o selecionado
        const isAtivo = btn.dataset.cat === catName || (catName !== '' && catName.startsWith(btn.dataset.cat + ' -'));
        
        if (isAtivo) {
            // ESTADO ATIVO: Fundo colorido (Acento), Borda colorida e Texto Escuro
            btn.style.backgroundColor = 'var(--clr-accent)';
            btn.style.borderColor = 'var(--clr-accent)';
            btn.style.color = '#0a0a0a'; // Texto sempre legível no fundo colorido
            btn.style.opacity = '1';
        } else {
            // ESTADO INATIVO: Fundo da Loja (Main), Sem Borda e Texto Variável
