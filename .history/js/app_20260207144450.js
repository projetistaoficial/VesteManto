...
const startApplication = async () => {
    ...
    // 1. Esconde TUDO imediatamente via JS (Segurança visual)
    document.body.style.display = 'none';
    document.body.style.backgroundColor = 'black';

    // 2. Remove Cache/Service Workers antigos (Evita site zumbi)
    ...
    // 3. O PORTEIRO: Verifica se pode entrar
    // O await aqui é o segredo. Nada roda abaixo enquanto isso não responder.
    const accessGranted = await initApp(); 

    // 4. Se o acesso foi permitido, carrega o resto
    if (accessGranted) {
        console.log("🔓 Acesso permitido. Carregando interface...");
        setupEventListeners();
        ...
        // Adiciona classe para animação CSS se houver
        document.body.classList.add('loaded'); 
    }
    // Se accessGranted for false, o initApp já matou a tela com exibirTelaMorte
};
