...
function renderProductsList(products, preCalcMetrics = null) {
    ...
    // --- 2. HEADER DA TABELA ---
    const headerHTML = `
        <div class="hidden md:grid grid-cols-12 gap-2 bg-[#1f1f1f] text-gray-400 font-bold p-3 rounded-t-xl text-[10px] uppercase tracking-wider border-b border-gray-800 sticky top-0 z-20 items-center">
            <div class="col-span-1 text-center border-r border-gray-700 text-yellow-500">Nº</div>
            <div class="col-span-2 pl-2 border-r border-gray-700">Cód / Status</div>
            <div class="col-span-6 pl-2">Produto</div>
    ...
    scrollContainer.className = "max-h-[65vh] overflow-y-auto overflow-x-hidden border-x border-b border-gray-800 rounded-b-xl bg-[#0f111a] custom-scrollbar";
    ...
    products.forEach((p, index) => {
        ...
        let bgClass = 'bg-[#151720] hover:bg-[#1c1f2b]';
        if (isInactive) bgClass = 'bg-[#2a1313] hover:bg-[#351818]';

        let controlColHTML = '';
        if (state.isReorderMode) {
            controlColHTML = `
                <div class="flex flex-col items-center justify-center gap-0.5 w-12 border-r border-gray-800 shrink-0 bg-black/20">
                    <button onclick="event.stopPropagation(); moveProductInReorder('${p.id}', -1)" class="text-gray-500 hover:text-yellow-500 p-1" ${isFirst ? 'disabled style="opacity:0.2"' : ''}><i class="fas fa-caret-up"></i></button>
                    <span class="text-xs font-black text-yellow-500 leading-none">${positionNum}</span>
                    <button onclick="event.stopPropagation(); moveProductInReorder('${p.id}', 1)" class="text-gray-500 hover:text-yellow-500 p-1" ${isLast ? 'disabled style="opacity:0.2"' : ''}><i class="fas fa-caret-down"></i></button>
                </div>
            `;
        } else {
            controlColHTML = `
                <div class="flex items-center justify-center w-12 border-r border-gray-800 shrink-0">
                    <span class="text-xs font-bold text-gray-500">${positionNum}</span>
                </div>
            `;
        }

        const row = document.createElement('div');
        row.className = `border-b border-gray-800 last:border-0 select-none cursor-pointer ${bgClass}`;
        row.ondblclick = () => editProduct(p.id);

        row.innerHTML = `
            <div class="p-2 flex md:grid md:grid-cols-12 gap-2 md:items-center">
                
                <div class="flex items-center gap-2 md:col-span-9 w-full">
                    ${controlColHTML}
                    
                    <div class="hidden md:flex flex-col w-20 items-center justify-center border-r border-gray-800 shrink-0">
                        <span class="text-[11px] font-bold text-white opacity-70">#${p.code || '-'}</span>
                        ${isInactive ? '<span class="text-[8px] bg-red-600 text-white px-1 mt-0.5 rounded uppercase font-bold">Inativo</span>' : ''}
                    </div>
                    ...
                    <div class="flex items-center gap-3 flex-1 min-w-0 px-2">
                        ...
                        <div class="flex flex-col truncate">
                            <span class="text-gray-200 font-bold text-sm truncate">${p.name}</span>
                            <span class="text-gray-500 text-[10px] truncate">${p.category || 'Geral'}</span>
                        </div>
                    </div>
                </div>

                <div class="hidden md:block col-span-2 text-center text-gray-400 text-xs font-bold">${p.stock} un.</div>
