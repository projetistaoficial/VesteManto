        ...
        onAuthStateChanged(auth, (user) => {
            ...
            if (state.user) {
                ...
                if (window.activeAvisosListener) {
                    window.activeAvisosListener(); // Desliga o ouvinte do Firebase
                    window.activeAvisosListener = null;
                }
                state.unreadAvisos = []; // Limpa avisos da memória para garantir
...
function showView(viewName) {
    ...
    if (viewName === 'admin') {
        ...
        // ✨ GATILHO: Dispara mensagens pendentes assim que a tela abre ✨
        if (typeof window.processarAvisos === 'function') window.processarAvisos();
    }
...
window.loadAvisos = () => {
    // Evita criar múltiplos radares se a função for chamada várias vezes
    ...
    window.activeAvisosListener = onSnapshot(q, (snapshot) => {
        // 1. Guarda as mensagens não lidas na memória do site
        state.unreadAvisos = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));

        // 2. Tenta processar (exibir modal e ícone)
...
// Nova função que decide se o modal pode pular na tela ou não
window.processarAvisos = () => {
    // 1. TRAVA ABSOLUTA: Se não tiver logado, morre aqui.
    if (!state.user) return;
    
    // 2. TRAVA VISUAL: A tela do Admin está aberta no HTML?
    const viewAdmin = document.getElementById('view-admin');
    const isPainelAberto = viewAdmin && !viewAdmin.classList.contains('hidden');

    // ✨ SE O PAINEL ADMIN NÃO ESTIVER ABERTO, ESCONDE TUDO E ABORTA ✨
    // Isso garante que mesmo que o vendedor logado esteja testando a vitrine, ele não seja interrompido!
    if (!isPainelAberto) {
        try {
            const alertIcon = document.getElementById('icone-avisos');
            if (alertIcon) alertIcon.classList.add('hidden'); // Esconde o ícone na vitrine
        } catch(e) {}
        return; 
    }

    // Se chegou até aqui, O PAINEL ADMIN ESTÁ ABERTO!
    const unreadCount = state.unreadAvisos ? state.unreadAvisos.length : 0;

    // 3. Atualiza o Ícone na tela do admin
    try {
        const alertIcon = document.getElementById('icone-avisos');
        if (alertIcon) {
            if (unreadCount > 0) {
    ...
    // 4. Exibe o Pop-up das mensagens (Apenas os que não foram vistos)
