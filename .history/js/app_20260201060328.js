...
// Usamos () => para esperar a função existir antes de tentar chamar
...
async function autoSaveSettings(type) {
    ...
    try {
        ...
        // --- 2. PARCELAMENTO ---
            ...
            let rate = 0;
            if (elRate) {
                const cleanRate = elRate.value.replace(/[^\d,.]/g, '').replace(',', '.');
                rate = parseFloat(cleanRate) || 0;
            }
        ...
        // --- 3. FORMAS DE PAGAMENTO (AQUI ESTAVA O PROBLEMA) ---
        else if (type === 'payments') {
            const getChk = (id) => {
                const el = document.getElementById(id);
                return el ? el.checked : true; // Padrão true se não achar
            };
...
window.applyCheckoutVisibility = () => {
    // 1. Carrega configurações (com fallback seguro)
    ...
    // 2. Define Estados Mestres
    const onlineActive = pm.online?.active !== false;
    const deliveryPaymentActive = pm.delivery?.active !== false;
    
    // REGRA DE OURO: A entrega própria precisa ser TRUE para a aba aparecer
    const logisticsActive = dConfig.ownDelivery === true;

    // A aba "Na Entrega" só aparece se: (Pagamento Ativo) E (Logística Ativa)
    ...
    // 4. Aplica Visibilidade (Aba Online)
    if (labelOnline) {
        if (onlineActive) {
    ...
    // 5. Aplica Visibilidade (Aba Entrega)
    ...
    // 6. Auto-Correção da Seleção (Se a aba atual sumiu, troca para a outra)
    if (radioOnline && radioOnline.checked && !onlineActive) {
        if (showDeliveryTab && radioDelivery) radioDelivery.checked = true;
        else radioOnline.checked = false;
    }
    if (radioDelivery && radioDelivery.checked && !showDeliveryTab) {
        if (onlineActive && radioOnline) radioOnline.checked = true;
        else radioDelivery.checked = false;
    }
    
    // Se nada estiver marcado, força o primeiro disponível
    if ((!radioOnline || !radioOnline.checked) && (!radioDelivery || !radioDelivery.checked)) {
        if (onlineActive && radioOnline) radioOnline.checked = true;
        else if (showDeliveryTab && radioDelivery) radioDelivery.checked = true;
    }

    // 7. Atualiza UI interna (esconde/mostra Pix, Cartão, Troco)
...
function setupDeliveryDependency() {
    ...
    // Listener da Entrega Própria
    elOwn.addEventListener('change', async (e) => {
        const isActive = e.target.checked;
        
        // 1. Lógica do Código de Segurança
        const elReq = document.getElementById('conf-req-code');
        if (elReq) {
        ...
        // 2. Dependência com Pagamento
        if (!isActive) {
            // Se desligou entrega, TEM QUE desligar pagamento na entrega
            if (elPayDel.checked) {
                // Verifica se o online está ligado antes de desligar este
                const onlineOn = document.getElementById('conf-pay-online-active')?.checked;
                if (!onlineOn) {
                    alert("Você não pode desligar a entrega própria pois o pagamento online já está desativado.");
                    e.target.checked = true; // Cancela
                    return; 
                }
                
                elPayDel.checked = false;
                // Salva a alteração do PAGAMENTO
                await autoSaveSettings('payments');
            }
        } else {
            // Se ligou entrega, liga pagamento (conveniência)
            if (!elPayDel.checked) {
        ...
        // 3. Salva a alteração da LOGÍSTICA (Entrega Própria)
    ...
    // Listener do Pagamento na Entrega
    elPayDel.addEventListener('change', async (e) => {
        ...
        if (!isActive) {
            const onlineOn = document.getElementById('conf-pay-online-active')?.checked;
            if (!onlineOn) {
                alert("Pelo menos uma forma de pagamento deve ficar ativa.");
                e.target.checked = true;
                return;
            }
        }

        // Vínculo Reverso: Se ligar pagamento, liga entrega
        if (isActive && !elOwn.checked) {
            elOwn.checked = true;
            await autoSaveSettings('orders'); // Salva entrega
        }
        // Se desligar pagamento, NÃO desliga entrega (pode ter entrega grátis/outros meios)
