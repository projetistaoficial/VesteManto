...
window.moveProductOrder = (id, direction) => {
    // 1. Acha o índice do produto na lista GERAL (state.products)
    // Se você estiver usando filtros, é melhor mover baseado na lista atual da tela
    const currentIndex = state.products.findIndex(p => p.id === id);
    if (currentIndex === -1) return;

    const targetIndex = currentIndex + direction;

    // 2. Trava se tentar subir o primeiro ou descer o último
    if (targetIndex < 0 || targetIndex >= state.products.length) return;

    // 3. Troca de posição no Array (Swap)
    ...
    // 4. Redesenha a tabela IMEDIATAMENTE
    filterAndRenderProducts();
};
