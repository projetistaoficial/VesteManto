...
// =================================================================
// 🚀 ATUALIZAÇÃO EM LOTE DE STATUS (ATIVO/INATIVO)
// =================================================================
window.bulkChangeProductStatus = async (isActive) => {
    const count = state.selectedProducts.size;
    const actionText = isActive ? 'ATIVAR' : 'INATIVAR';

    if (!confirm(`Tem certeza que deseja ${actionText} ${count} produto(s)?`)) return;

    try {
        // Mostra que está carregando
        document.body.style.cursor = 'wait';
        
        // Dispara a atualização de todos os produtos ao mesmo tempo no Firebase
        const promises = Array.from(state.selectedProducts).map(id => {
            return updateDoc(doc(db, `sites/${state.siteId}/products`, id), { active: isActive });
        });
        
        await Promise.all(promises);
        
        // Atualiza a memória RAM local instantaneamente para a tela não piscar
        ...
        showToast(`Produtos atualizados com sucesso!`, 'success');
