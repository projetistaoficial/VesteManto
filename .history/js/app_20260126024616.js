...
window.submitOrder = async () => {
    try {
        ...
        // --- TRAVA DE SEGURANÇA DE ENTREGA (NOVO) ---
        const payMode = document.querySelector('input[name="pay-mode"]:checked')?.value;
        const dConfig = state.storeProfile?.deliveryConfig || {};
        
        // Se for Entrega Própria e a validação de distância falhou
        if (dConfig.ownDelivery === true && checkoutState.isValidDelivery === false) {
            return alert("⛔ ENDEREÇO INVÁLIDO OU FORA DA ÁREA\n\nPor favor, verifique o CEP. Não realizamos entregas neste endereço.");
        }
        // ----------------------------------------------
        ...
        // ... (RESTO DA FUNÇÃO MANTIDO IGUAL AO ANTERIOR) ...
        // Copie o restante da função submitOrder que você já tinha a partir daqui.
        // Apenas certifique-se de que a parte de cima (TRAVA DE SEGURANÇA) foi adicionada.
