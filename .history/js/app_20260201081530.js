...
function setupDeliveryDependency() {
    ...
    newOwn.addEventListener('change', async (e) => {
        ...
        // 1. Controle visual do "Código de Segurança"
        const elReq = document.getElementById('conf-req-code');
        if (elReq) {
        ...
        if (!isActive) {
            // Se DESLIGAR a entrega, verifica se precisa desligar o pagamento na entrega
            if (elPayDelivery.checked) {
                // Segurança: O Online está ligado?
                const onlineOn = document.getElementById('conf-pay-online-active')?.checked;
                
                if (!onlineOn) {
                    showSystemModal("⚠️ AÇÃO BLOQUEADA:\n\nVocê não pode desligar a Entrega Própria pois o Pagamento Online já está desativado.\nAtive o Online primeiro.", "warning");
                    e.target.checked = true; // Cancela a ação
                    return; 
                }
                
                // Desliga o pagamento visualmente
                ...
                // ATUALIZAÇÃO OTIMISTA (Evita que o check volte sozinho)
            ...
            // Se LIGAR a entrega, liga o pagamento por conveniência
            if (!elPayDelivery.checked) {
        ...
        // Atualização Otimista da Logística
    ...
    newPay.addEventListener('change', async (e) => {
        const isActive = e.target.checked; // O estado que o usuário clicou
        const elOnline = document.getElementById('conf-pay-online-active');

        // VALIDAÇÃO: Se tentar desligar e o online já estiver desligado
        if (!isActive && (!elOnline || !elOnline.checked)) {
            showSystemModal("⚠️ Você não pode desativar todas as formas de pagamento.", "warning");
            e.target.checked = true; // Força visualmente para TRUE
            return;
        }
        ...
        // DEPENDÊNCIA REVERSA (O problema estava aqui)
        const ownCheck = document.getElementById('conf-own-delivery');

        if (ownCheck) {
            // CENÁRIO: DESATIVOU O PAGAMENTO
            if (!isActive) {
                if (ownCheck.checked) {
                    // Desativa a Entrega Própria visualmente
