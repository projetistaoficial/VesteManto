...
window.filterAndRenderSales = () => {
    // --- CORREÇÃO DO SUMIÇO: Tenta achar a lista pelos dois IDs comuns ---
    const list = document.getElementById('admin-sales-list') || document.getElementById('orders-list');
    
    if (!list) return; // Se não achar nenhum dos dois, sai (provavelmente não está na tela certa)
    ...
    if (filtered.length === 0) {
        ...
        filtered.forEach(order => {
            ...
            const isNew = !order.viewed; // Se viewed for falso, é novo
            ...
            const customerDisplay = order.customer?.name || 'Cliente';
