...
function renderStoreProfile() {
    ...
    // --- 0. ATUALIZA NAVBAR (CORREÇÃO: Isso faltava para exibir a logo no topo) ---
    ...
    if (navLogo && navText) {
        if (p.logo) {
            ...
            // Mantém o estilo de duas cores se não tiver nome definido
    ...
    // --- 1. ATUALIZA SIDEBAR (Mantém como estava) ---
    ...
    // Se você tiver a logo pequena na sidebar (rodapé), atualiza ela também
    ...
    if (homeLogo && homeTitle) {
        if (p.logo) {
            // Se tem logo, mostra imagem e esconde texto
            homeLogo.src = p.logo;
            homeLogo.classList.remove('hidden');
            homeTitle.classList.add('hidden');
        } else {
            // Se não tem logo, mostra o nome da loja em texto grande
    ...
    // Links sociais da sidebar (mantém lógica antiga)
    ...
    // Tenta pegar pelo ID ou nome da variável do els
    updateLink(els.linkWhatsapp || document.getElementById('sidebar-link-wpp'), p.whatsapp, 'https://wa.me/');
    updateLink(els.linkInstagram || document.getElementById('sidebar-link-insta'), p.instagram, 'https://instagram.com/');
    updateLink(els.linkFacebook || document.getElementById('sidebar-link-facebook'), p.facebook);
