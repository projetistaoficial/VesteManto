...
function populateInstallments() {
    ...
    // --- 3. ADICIONA O FRETE AO CÁLCULO BASE ---
    // Importante: O juros do cartão incide sobre (Produto + Frete)
    const dConfig = state.storeProfile.deliveryConfig || {};
    ...
    // Verifica se o frete deve ser cobrado
    if (typeof checkoutState !== 'undefined' && checkoutState.isValidDelivery && shipValue > 0) {
        ...
        // Se for entrega, não entra aqui pois esta função é só para 'credit' online
    ...
    for (let i = 1; i <= maxParcelas; i++) {
        ...
        // Aplica Juros (Tabela Price Simples ou Composta conforme configurado)
        // Aqui usamos Juros Compostos padrão de mercado
        if (instConfig.active && i > instConfig.freeUntil && instConfig.rate > 0) {
            const taxa = instConfig.rate / 100;
            const fator = Math.pow(1 + taxa, i);
            
            // Fórmula da Parcela com Juros Compostos: PMT = VP * [i(1+i)^n] / [(1+i)^n - 1]
        ...
        option.value = i;
        // Salva o total já calculado com juros no dataset para o calcCheckoutTotal ler
        option.dataset.total = finalVal.toFixed(2);
        option.text = `${label} de ${formatCurrency(valorParcela)}`;
        select.appendChild(option);
    }
    
    // Atualiza o total exibido assim que preencher
    calcCheckoutTotal();
}
