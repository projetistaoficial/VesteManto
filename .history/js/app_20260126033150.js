...
// --- FUNÇÕES MATEMÁTICAS (Coloque estas antes das funções de cálculo) ---
...
// --- FUNÇÃO DE BUSCA DE DISTÂNCIA (BrasilAPI V2) ---
async function calculateDistanceByCEP(cepOrigin, cepDest) {
    ...
    const getCoords = async (cep) => {
        try {
            ...
            // Se der erro (404, 500), retorna nulo
            if (!res.ok) {
                console.warn(`BrasilAPI erro ${res.status} para CEP ${cleanCep}`);
                return null;
            }

            const data = await res.json();

            // Verifica se a resposta tem as coordenadas
            if (data.location && data.location.coordinates) {
                const lat = parseFloat(data.location.coordinates.latitude);
                const lon = parseFloat(data.location.coordinates.longitude);
                
                // Validação extra se é número
    ...
    console.log(`Calculando distância: ${cepOrigin} -> ${cepDest}`);

    // Executa as buscas
    const [c1, c2] = await Promise.all([getCoords(cepOrigin), getCoords(cepDest)]);

    // Se falhar qualquer um dos dois, retorna null
    if (!c1) { console.error("Falha ao obter coordenadas da LOJA"); return null; }
    if (!c2) { console.error("Falha ao obter coordenadas do CLIENTE"); return null; }

    return getDistanceFromLatLonInKm(c1.lat, c1.lon, c2.lat, c2.lon);
}

// --- LISTENER DO INPUT CEP (Manipulação da UI) ---
window.handleCheckoutCep = async () => {
    ...
    // 1. BLOQUEIA BOTÃO IMEDIATAMENTE
    if (btnFinish) {
    ...
    // Se CEP incompleto, para e não faz nada
    ...
    if (elDistDisplay) {
        elDistDisplay.innerText = "Verificando entrega...";
        elDistDisplay.classList.remove('hidden', 'text-red-500', 'text-green-500');
        elDistDisplay.classList.add('text-yellow-500');
    }

    try {
        // A. Busca dados de endereço (Logradouro/Bairro) via ViaCEP (Melhor para textos)
        ...
        // B. LÓGICA DE DISTÂNCIA (Apenas se for Entrega Própria)
        ...
        if (config.ownDelivery === true) {
            ...
            if (!storeCep) throw new Error("O CEP da loja não está configurado no Painel Admin.");

            // Chama a função corrigida acima
            const dist = await calculateDistanceByCEP(storeCep, cep);

            // Se retornar null, a API falhou em achar latitude/longitude
            if (dist === null) {
                throw new Error("Não foi possível validar a localização deste CEP para cálculo de frete.");
            }
            ...
            if (elDistDisplay) {
                ...
                if (maxDist > 0 && dist > maxDist) {
                    ...
                    throw new Error(`Entrega indisponível: Endereço a ${dist.toFixed(1)}km (Limite: ${maxDist}km).`);
                } else {
                    // LIBERA
        ...
        if (elDistDisplay) elDistDisplay.innerText = "Localização não validada";
