...
// ===============================================
// 1. SINCRONIZADOR VISUAL (Blindado com Style Inline)
// ===============================================
// ===============================================
// 1. SINCRONIZADOR VISUAL (Fundo Translúcido e Ativo)
// ===============================================
window.aplicarCoresCategorias = (catName) => {
    const todosBotoes = document.querySelectorAll('.categoria-btn');
    
    // Agrupamos as cores para não ter erro na hora de trocar
    const classesInativo = ['bg-white/10', 'text-gray-300', 'border-transparent', 'hover:bg-white/20', 'hover:text-white'];
    const classesAtivo = ['bg-[var(--clr-accent)]', 'text-[#0a0a0a]', 'border-[var(--clr-accent)]', 'shadow-lg'];
    
    todosBotoes.forEach(btn => {
        // Remove qualquer sujeira de estilo inline que tenha ficado da tentativa anterior
        btn.removeAttribute('style'); 

        // Verifica se este botão é a categoria selecionada
        const isAtivo = btn.dataset.cat === catName || (catName !== '' && catName.startsWith(btn.dataset.cat + ' -'));
        
        if (isAtivo) {
            // ATIVO: Fica com a cor de destaque da loja
            btn.classList.remove(...classesInativo);
            btn.classList.add(...classesAtivo);
        } else {
            // INATIVO: Fica com fundo de vidro translúcido (bg-white/10)
...
function renderCategories() {
    ...
    if (scrollContainer) {
        ...
        // Se a categoria sumiu do banco, volta pra todas
        if (activeCat !== '') {
        ...
        categoriasParaMostrar.forEach(c => {
            ...
            // Note que aqui as classes Tailwind são apenas para estrutura (tamanho, arredondamento)
            // As cores serão forçadas pela função aplicarCoresCategorias logo abaixo
            const classesEstrutura = "categoria-btn flex items-center h-full rounded-full transition-all shrink-0 border font-black tracking-wide text-xs outline-none shadow-sm";
            ...
            if (hasSubs) {
                pillsHtml += `
                    <div class="${classesEstrutura}" data-cat="${safeName}">
                        ...
                        <button onclick="toggleCatDropdown('${safeName}', event)" class="px-3 h-full border-l border-black/10 flex items-center justify-center rounded-r-full outline-none">
        ...
        // ✨ O SEGREDO: Aplica as cores via JS imediatamente após desenhar o HTML
        setTimeout(() => {
