...
window.openAdminOrderDetail = async (order) => {
    ...
    if (!order.viewed) {
        try {
            ...
            // O onSnapshot vai perceber essa mudança automaticamente e:
            // 1. Remover a etiqueta "NOVO"
            // 2. Diminuir o contador lá em cima
        } catch (error) {
            console.error("Erro ao marcar visualização:", error);
        }
    }

    // 2. Abre o Modal (Seu código original de abrir modal continua aqui)
    const modal = document.getElementById('modal-admin-order');
    if(modal) modal.classList.remove('hidden');
    
    // ... Preencha os campos do modal aqui como você já fazia ...
    document.getElementById('admin-order-id-display').innerText = `#${order.id}`;
    // etc...
};
