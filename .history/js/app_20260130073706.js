...
function loadSettings() {
    ...
    // O erro estava aqui: O código original tentava fazer "setDoc" se não achasse os dados.
    // Visitantes não têm permissão para "setDoc", então o site travava.
    
    onSnapshot(docRef, (docSnap) => {
        if (docSnap.exists()) {
            state.globalSettings = docSnap.data();
            // Atualiza o checkbox se estiver no painel admin
            if (els.toggleStockGlobal) els.toggleStockGlobal.checked = state.globalSettings.allowNoStock;
        } else {
            console.log("Configurações não encontradas ou acesso restrito. Usando padrão.");
            
            // --- CORREÇÃO: Usamos memória local em vez de tentar gravar no banco ---
            state.globalSettings = { allowNoStock: true }; // Força TRUE para os produtos aparecerem
        }
        
        // Continua o carregamento da loja
        if (typeof setupDeliveryDependency === 'function') setupDeliveryDependency();
        renderCatalog(state.products);
    }, (error) => {
        // Se der erro de permissão (comum em novas lojas), cai aqui
        console.warn("Usando configurações padrão devido a erro de acesso:", error);
