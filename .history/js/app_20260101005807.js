...
window.handleCheckoutCep = async () => {
    ...
    try {
        ...
        // --- CORREÇÃO AQUI ---
        // 1. Recalcula o total (para somar o frete no valor final)
        calcCheckoutTotal();
        
        // 2. Recalcula as parcelas (para que o select inclua o frete na divisão)
        if (typeof populateInstallments === 'function') {
            populateInstallments();
        }
        
        // 3. Atualiza visual do botão e textos
...
function populateInstallments() {
    ...
    // 1. Calcula o Total Base (Produtos - Cupom)
    let totalBase = 0;
    ...
    // Só soma se CEP for válido e a regra de frete se aplicar
    ...
    // Se já havia uma seleção (ex: usuário estava em 3x), tenta manter
    // Caso contrário, o select volta para 1x, mas agora com o valor correto
}
