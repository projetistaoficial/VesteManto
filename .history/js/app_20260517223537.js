...
async function submitOrder() {
    try {
        ...
        for (const item of state.cart) {
            // Busca o dado FRESQUINHO direto do servidor do Google ignorando qualquer cache
            const prodRef = doc(db, `sites/${state.siteId}/products`, item.id);
            const snap = await getDocFromServer(prodRef); 
            
            if (!snap.exists()) {
                outOfStockItems.push(`- ${item.name} (Produto foi excluído da loja)`);
                continue;
            }
            ...
            // Se a loja não permite estoque negativo, fazemos a checagem matemática
            if (!allowNegative) {
                let estoqueReal = 0;
                ...
                // Se o cliente quer comprar MAIS do que tem no servidor naquele exato segundo
                if (estoqueReal < item.qty) {
                    outOfStockItems.push(`- ${item.name} ${item.size !== 'U' ? '('+item.size+')' : ''}: Restam apenas ${estoqueReal} unid.`);
                }
            }
        }

        // Se encontrou algum produto esgotado, BARRA A VENDA AQUI MESMO!
        if (outOfStockItems.length > 0) {
            if (btnSubmit) { btnSubmit.disabled = false; btnSubmit.innerText = "Confirmar Pedido"; }
            
            alert("⛔ ATENÇÃO: Ocorreu uma mudança no estoque enquanto você finalizava a compra!\n\nOs seguintes itens acabaram de esgotar ou não têm quantidade suficiente:\n\n" + outOfStockItems.join('\n') + "\n\nPor favor, feche esta tela, retorne ao carrinho e ajuste as quantidades para prosseguir.");
            return; // 🛑 Interrompe a função, a venda NÃO é criada!
        }
        // =================================================================
