...
async function initApp() {
    ...
    try {
        ...
        // 2. PERGUNTA AO SERVIDOR (IGNORA CACHE)
        // Se você deletou no painel, isso aqui vai dar false AGORA.
        const snap = await getDocFromServer(docRef);

        // A. SITE NÃO EXISTE MAIS
        if (!snap.exists()) {
            // Limpa qualquer lixo local do navegador do cliente
        ...
        // Monitoramento em tempo real (A cada 15s)
        setInterval(async () => {
            try {
...
function exibirTelaMorte(titulo, msg) {
    ...
    document.body.innerHTML = `
        <div style="position:fixed;top:0;left:0;width:100%;height:100vh;background:#000;color:red;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:9999;">
            <h1 style="font-size:4rem;margin:0;">${titulo}</h1>
            <p style="color:white;margin-top:10px;">${msg}</p>
        </div>
    `;
