...
function calculateStatsMetrics() {
    ...
    if (state.products) {
        state.products.forEach(p => {
            if (p.stock > 0) {
                const val = parseFloat(p.promoPrice || p.price || 0);
                capitalGiro += p.stock * val;
            }
        });
    }
    // CORRIGIDO: st-capital
    const elCapital = document.getElementById('st-capital');
    if (elCapital) elCapital.innerText = formatCurrency(capitalGiro);

    // 2. Se for visão geral, chama global e sai
    if (state.statsFilterType === 'all') {
        fetchGlobalStats();
        return;
    }
    ...
    // --- ATUALIZA COM OS IDs CERTOS (st-*) ---
    const elCount = document.getElementById('st-sales-count');
    const elTotal = document.getElementById('st-sales-val');
    const elCost = document.getElementById('st-cost');
    const elProfit = document.getElementById('st-profit');
    
    if (elCount) elCount.innerText = totalSalesCount;
    if (elTotal) elTotal.innerText = formatCurrency(totalSalesValue);
    if (elCost) elCost.innerText = formatCurrency(totalCost);
    
    if (elProfit) {
        elProfit.innerText = formatCurrency(totalProfit);
        elProfit.className = totalProfit >= 0 ? "text-3xl font-bold text-green-500" : "text-3xl font-bold text-red-500";
    }

    // Status secundários (IDs corrigidos do seu HTML)
    ...
    if (elRefunded) elRefunded.innerText = countRefunded;
    ...
    const elRateApprov = document.getElementById('st-rate-approval');
    if (elRateApprov) elRateApprov.innerText = Math.round(approvalRate) + '%';
...
// NOVA FUNÇÃO: Calcula estatísticas globais (Adicione no final do arquivo app.js)
// ATUALIZAÇÃO FORÇA BRUTA: Garante que os números apareçam
async function fetchGlobalStats() {
    console.log(">>> [1/3] Iniciando busca global...");
    ...
    try {
        ...
        querySnapshot.forEach((doc) => {
            const data = doc.data();
            totalPedidos++;
            faturamentoTotal += parseFloat(data.total || 0);
            custoTotal += parseFloat(data.totalCost || 0);
        });

        const lucroLiquido = faturamentoTotal - custoTotal;

        console.log(`>>> [2/3] Dados calculados: Pedidos=${totalPedidos} | Total=${formatCurrency(faturamentoTotal)}`);

        // FUNÇÃO INTERNA: Atualiza qualquer coisa que pareça ser o contador
        const forceUpdate = () => {
            // Lista de todos os IDs possíveis que seu template usa
            const targets = {
                count: ['stat-sales-count', 'dash-confirmed-count', 'dash-total-items', 'orders-count'],
                total: ['stat-sales-total', 'dash-total-value', 'orders-filtered-total'],
                cost:  ['stat-cost-total'],
                profit:['stat-profit-total']
            };

            // Atualiza Contadores (Pedidos)
            targets.count.forEach(id => {
                // querySelectorAll pega TODOS os elementos, inclusive duplicatas (mobile/desktop)
                document.querySelectorAll(`#${id}`).forEach(el => el.innerText = totalPedidos);
            });

            // Atualiza Faturamento
            targets.total.forEach(id => {
                document.querySelectorAll(`#${id}`).forEach(el => {
                    el.innerText = formatCurrency(faturamentoTotal);
                    // Ajusta label "Total Listado" para "Global" se for o da lista
                    if (id === 'orders-filtered-total' && el.previousElementSibling) {
                        el.previousElementSibling.innerText = 'TOTAL GLOBAL';
                    }
                });
            });

            // Atualiza Custo
            targets.cost.forEach(id => {
                document.querySelectorAll(`#${id}`).forEach(el => el.innerText = formatCurrency(custoTotal));
            });

            // Atualiza Lucro (com cor)
            targets.profit.forEach(id => {
                document.querySelectorAll(`#${id}`).forEach(el => {
                    el.innerText = formatCurrency(lucroLiquido);
                    el.className = lucroLiquido >= 0 
        ...
        // EXECUTAR 3 VEZES: Agora, daqui a 500ms e daqui a 1s (Para vencer qualquer conflito de carregamento)
        forceUpdate();
        setTimeout(forceUpdate, 500);
        setTimeout(forceUpdate, 1000);

    } catch (error) {
        console.error(">>> Erro fatal nas estatísticas:", error);
    }
}