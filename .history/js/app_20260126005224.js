...
window.calcCheckoutTotal = () => {
    ...
    // Configs de Pix (Novas - Default type='percent')
    const pixGlobal = state.storeProfile?.pixGlobal || { disableAll: false, active: false, value: 0, mode: 'product', type: 'percent' };
    ...
    // A. PIX (Lógica Atualizada com Tipo Fixo/Percentual)
    if (method === 'pix') {
    ...
    // B. CRÉDITO
        ...
        if (select && select.options.length > 0) {
            ...
            if (opt && opt.dataset.total) {
                ...
                // O dataset.total já inclui o frete (se aplicável) e juros
            }
        } else {
