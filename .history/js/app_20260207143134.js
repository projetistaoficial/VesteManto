...
async function initApp() {
    ...
    // 1. CHECAGEM RÁPIDA DE URL
    if (!siteId) {
        showKillScreen("Loja não identificada", "Link inválido.");
        return;
    }
    ...
    try {
        // 2. VERIFICAÇÃO RIGOROSA NO SERVIDOR (SEM CACHE)
        // Isso vai confirmar se a loja foi deletada agora.
        const docRef = doc(db, "sites", siteId);
        const snap = await getDocFromServer(docRef);

        // --- AQUI É O PONTO CRÍTICO ---
        
        // CENÁRIO A: LOJA NÃO EXISTE (FOI DELETADA)
        if (!snap.exists()) {
            localStorage.clear(); // Limpa qualquer lixo do navegador
            throw new Error("SITE_DELETED");
        }

        const data = snap.data();

        // CENÁRIO B: LOJA BLOQUEADA
        if (data.status === 'bloqueado' || data.active === false) {
            throw new Error("SITE_BLOCKED");
        }

        // =========================================================
        // 3. SUCESSO: O SITE EXISTE! VAMOS MOSTRAR.
        // =========================================================
        ...
        // --- FINALMENTE: LEVANTA A CORTINA ---
        document.body.classList.add('loaded'); // O CSS fará o fade-in
        ...
        // Se cair aqui, o site NUNCA vai aparecer.
        ...
        document.body.innerHTML = `
            ...
            ">
                <h1 style="font-size: 3rem; margin-bottom: 10px;">${titulo}</h1>
                <p style="color: #fff; font-size: 1.2rem;">${msg}</p>
            </div>
        `;
        
        // Força a visibilidade da mensagem de erro
