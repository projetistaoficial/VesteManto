...
window.openAdminOrderDetail = async (order) => {
    ...
    // Preenche Itens e Painel Financeiro
    if (itemsEl) {
        let totalCost = 0; // Armazena o custo total dos produtos

        // A. Renderiza a lista normal de itens
        let itemsHtmlStr = (order.items || []).map(i => {
            const itemCost = parseFloat(i.cost) || 0;
            const itemRev = parseFloat(i.price) * parseInt(i.qty);
            totalCost += itemCost * parseInt(i.qty);
            ...
            <div class="flex justify-between py-2 border-b border-gray-700 text-sm">
                ...
                <div class="text-white">${formatCurrency(itemRev)}</div>
            </div>
        `}).join('');

        // Adiciona Frete na lista normal se houver
        ...
        // B. LÓGICA DO RESUMO FINANCEIRO (LUCRO)
        const liquidProfit = order.total - totalCost;
        const profitMargin = order.total > 0 ? ((liquidProfit / order.total) * 100).toFixed(1) : 0;

        const financeDetailsHtml = (order.items || []).map(i => {
             const c = parseFloat(i.cost) || 0;
             const rev = parseFloat(i.price) * parseInt(i.qty);
             const prof = rev - (c * parseInt(i.qty));
             return `
                <div class="flex justify-between items-center text-xs border-b border-gray-800/50 pb-2 mb-2 last:border-0 last:pb-0 last:mb-0">
                    <span class="text-gray-300 truncate pr-2 flex-1">${i.qty}x ${i.name}</span>
        ...
        const financePanelHTML = `
            <div class="mt-4 pt-4 border-t border-gray-700">
                <button type="button" onclick="document.getElementById('admin-finance-panel-${order.id}').classList.toggle('hidden')" class="w-full bg-blue-900/10 hover:bg-blue-900/30 text-blue-400 border border-blue-900/50 py-2 rounded-lg text-xs font-bold transition flex justify-center items-center gap-2 outline-none">
                    <i class="fas fa-hand-holding-usd"></i> Ver Resumo Financeiro
                </button>
                
                <div id="admin-finance-panel-${order.id}" class="hidden mt-3 bg-[#0a0c13] p-4 rounded-xl border border-blue-900/30 shadow-inner">
                    ...
                    <div class="mt-3 pt-3 border-t border-blue-900/30 flex flex-col gap-1.5 text-xs">
                         ...
                         <div class="flex justify-between items-end mt-2 pt-2 border-t border-gray-800/50">
                             <span class="text-gray-500 font-bold uppercase text-[10px] tracking-wider">Lucro Líquido:</span>
        ...
        // Aplica a lista normal + o painel financeiro na tela
        itemsEl.innerHTML = itemsHtmlStr + financePanelHTML;
    }
