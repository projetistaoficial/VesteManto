...
window.openCheckoutModal = () => {
    ...
    // 2. Leitura das Configurações
    const pm = state.storeProfile.paymentMethods || {};
    
    // Se não estiver definido, considera TRUE (ativado)
    ...
    console.log("Abrindo Checkout -> Online:", onlineActive, " | Delivery:", deliveryActive);
    ...
    // --- A. OCULTAR PAGAMENTO ONLINE (FORÇA BRUTA) ---
    if (!onlineActive) {
        if (labelOnline) {
            labelOnline.classList.remove('flex'); // Remove flex para não brigar
            labelOnline.classList.add('hidden');
            labelOnline.style.cssText = 'display: none !important'; // CSS direto
        }
    } else {
        if (labelOnline) {
            labelOnline.classList.remove('hidden');
            labelOnline.classList.add('flex');
            labelOnline.style.cssText = ''; // Limpa estilo inline
        }
    }

    // --- B. OCULTAR PAGAMENTO ENTREGA (FORÇA BRUTA) ---
    // Só mostra se: (Ativado no Financeiro) E (Ativado na Logística)
    const showDelivery = deliveryActive && oldOwnDelivery;

    if (!showDelivery) {
        if (containerDelivery) {
            containerDelivery.classList.add('hidden');
            containerDelivery.style.cssText = 'display: none !important';
        }
    } else {
        if (containerDelivery) {
            containerDelivery.classList.remove('hidden');
            containerDelivery.style.cssText = '';
        }
    }

    // --- C. SELEÇÃO AUTOMÁTICA ---
    // Garante que a bolinha (radio) vá para uma opção que esteja visível
    if (onlineActive) {
