...
function renderSalesList(orders) {
    ...
    orders.forEach(o => {
        ...
        // --- CÁLCULO E LISTAGEM DOS DESCONTOS (ATUALIZADO) ---
        const subTotalItens = o.items.reduce((acc, i) => acc + (i.price * i.qty), 0);
        const valFrete = o.shippingFee || 0;
        const valTotal = o.total || 0;
        
        // O desconto é a diferença entre (Produtos + Frete) e o que foi Pago
        const valDescontoTotal = Math.max(0, (subTotalItens + valFrete) - valTotal);

        let discountHtml = '';
        
        // Só exibe bloco de descontos se houver alguma economia real (> 5 centavos)
        if (valDescontoTotal > 0.05) {
            const isPix = (o.paymentMethod || '').toLowerCase().includes('pix');
            const hasCupom = !!o.cupom;

            discountHtml += `<div class="mt-1 mb-2 border-y border-gray-700/50 py-1 space-y-0.5">`;

            // 1. Linha do Cupom (se existir)
            if (hasCupom) {
                discountHtml += `
                    <div class="flex justify-between text-xs text-green-400">
                        <span>Cupom: <span class="uppercase font-bold tracking-wider">${o.cupom}</span></span>
                        <i class="fas fa-check text-[10px]"></i>
                    </div>
                `;
            }

            // 2. Linha do Pix (se for Pix)
            if (isPix) {
                discountHtml += `
                    <div class="flex justify-between text-xs text-green-400">
            ...
            // 3. Total do Desconto (Soma tudo)
            discountHtml += `
                <div class="flex justify-between text-xs text-green-500 font-bold pt-1 mt-1 border-t border-gray-700/30">
