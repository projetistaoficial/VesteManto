...
function setupEventListeners() {
    ...
    // ====================================================================
    // AQUI ESTÁ A CORREÇÃO REAL: CARRINHO E MENU LATERAL (CLONE NODE)
    // ====================================================================
    ...
    // Botões e Menu Lateral (Sidebar)
    const btnMob = document.getElementById('mobile-menu-btn');
    if (btnMob) {
        const newBtnMobMenu = btnMob.cloneNode(true);
        btnMob.parentNode.replaceChild(newBtnMobMenu, btnMob);
        newBtnMobMenu.addEventListener('click', toggleSidebar);
    }

    const btnCloseSide = document.getElementById('close-sidebar');
    if (btnCloseSide) {
        const newBtnClose = btnCloseSide.cloneNode(true);
        btnCloseSide.parentNode.replaceChild(newBtnClose, btnCloseSide);
        newBtnClose.addEventListener('click', toggleSidebar);
    }
    ...
    // Config Logistica e Parcelamento (Listeners)
    ...
    if (elCardMax) {
        elCardMax.addEventListener('input', () => { if (typeof updateFreeInstallmentsSelect === 'function') updateFreeInstallmentsSelect(); });
    ...
    if (elCardRate) {
        elCardRate.type = 'text'; elCardRate.inputMode = 'numeric';
    ...
    if (checkOwnDelivery && checkReqCode) {
        ...
        if (inputCancelTime) inputCancelTime.addEventListener('blur', () => autoSaveSettings('orders'));
    }
    ...
    if (btnFinishPayment) btnFinishPayment.onclick = window.submitOrder;
...
function addToCart(product, size) {
    ...
    if (!status.isOpen && status.block) {
        ...
        if (!state.user) {
            ...
            windowupdateStoreStatusUI(); // Força o modal a aparecer caso não tenha aparecido
