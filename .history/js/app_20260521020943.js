...
async function loadCoupons() {
    ...
    onSnapshot(q, async (snapshot) => {
        ...
        // 1. SE FOR ADMIN: Dispara a rotação e o aviso
        if (state.user && typeof checkAndRotateExpiredOfferedCoupon === 'function') {
            // Await aqui é importante para garantir que o banco seja limpo 
            // antes de renderizar a lista no admin
            await checkAndRotateExpiredOfferedCoupon();
        }

        // 2. RENDER ADMIN (Se a aba estiver aberta)
        const viewAdmin = document.getElementById('view-admin');
        if (viewAdmin && viewAdmin.offsetParent !== null) {
            renderAdminCoupons();
        }

        // 3. EXIBIÇÃO CLIENTE (Vitrine)
        // A função showOfferedCoupon agora tem uma trava interna que ignora vencidos
...
window.showOfferedCoupon = () => {
    ...
    const now = new Date();
    const offered = state.coupons.find(c => {
        const isMarkedAsOffered = c.isOffered === true;
        const isNotExpired = !c.expiryDate || new Date(c.expiryDate) > now;
        return isMarkedAsOffered && isNotExpired;
    });
...
window.checkAndRotateExpiredOfferedCoupon = async () => {
    // 1. Encontra o cupom oferecido
    const offered = state.coupons.find(c => c.isOffered === true);
    if (!offered || !offered.expiryDate) return;

    const expiry = new Date(offered.expiryDate);
    const now = new Date();

    // 2. Se NÃO expirou, não faz nada
    if (now <= expiry) return;

    // 3. SE EXPIROU: O sistema vai rotacionar AGORA (mesmo que seja um cliente comum)
    console.log("Cupom expirado detectado. Iniciando rotação automática...");

    const nextCoupon = state.coupons.find(c => {
        if (c.id === offered.id) return false;
