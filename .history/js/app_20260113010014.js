...
function updateCartUI() {
    ...
    // 2. Renderiza Itens (COM O NOVO VISUAL)
    let subtotal = 0;
    ...
    state.cart.forEach((item, index) => {
        ...
        // Lógica de Imagem: Tenta pegar do item ou busca no produto original
        ...
        const imageHTML = imgUrl 
            ...
            : `<div class="w-full h-full flex items-center justify-center bg-gray-800 text-gray-600"><i class="fas fa-box-open text-2xl"></i></div>`;

        const div = document.createElement('div');
        // Novo Layout de Card
        div.className = "group flex items-center gap-4 bg-[#151720] p-3 rounded-xl border border-gray-800 shadow-sm relative overflow-hidden transition hover:border-gray-700";
        
        div.innerHTML = `
            <div class="w-20 h-20 shrink-0 rounded-lg overflow-hidden border border-gray-700 bg-black relative">
                ${imageHTML}
            </div>
                ...
                <div class="flex justify-between items-start">
                    <div class="pr-6">
                        ...
                        <div class="flex items-center gap-2 mt-0.5">
                            ${item.size !== 'U' ? `<span class="text-[10px] bg-gray-800 border border-gray-600 px-1.5 rounded text-gray-400 font-mono">${item.size}</span>` : ''}
                        </div>
                    </div>
                    
                    <button onclick="changeQty(${index}, -${item.qty})" class="text-gray-500 hover:text-red-500 transition p-2 -mr-2 -mt-2 rounded-full hover:bg-red-500/10">
                        <i class="fas fa-trash-alt text-sm"></i>
                    </button>
                </div>

                <div class="flex justify-between items-end">
                    <span class="text-yellow-500 font-bold font-mono text-sm tracking-wide">
                        ${formatCurrency(item.price * item.qty)}
                    </span>

                    <div class="flex items-center bg-black border border-gray-700 rounded-lg h-8">
                        <button onclick="changeQty(${index}, -1)" 
                            class="w-8 h-full flex items-center justify-center text-gray-400 hover:text-white hover:bg-gray-800 rounded-l-lg transition active:bg-gray-700">
                        ...
                        <button onclick="changeQty(${index}, 1)" 
                            class="w-8 h-full flex items-center justify-center text-gray-400 hover:text-white hover:bg-gray-800 rounded-r-lg transition active:bg-gray-700">
    ...
    // Aqui usamos o HTML responsivo que criamos antes para o campo de cupom
