...
async function loadStoreProfile() {
    // 1. VERIFICAÇÃO DE SEGURANÇA (Lê o documento Pai primeiro)
    try {
        const siteRef = doc(db, "sites", state.siteId);
        const siteSnap = await getDoc(siteRef);

        // A. Se o site não existe (foi excluído no Master)
        if (!siteSnap.exists()) {
            document.body.innerHTML = `
                <div style="height:100vh; display:flex; flex-direction:column; align-items:center; justify-content:center; background:#111; color:#fff; font-family:sans-serif;">
                    ...
                    <h2 style="font-size:1.5rem;">Loja não encontrada</h2>
                    <p style="color:#888;">Verifique o endereço digitado.</p>
                </div>
            `;
            return; // Para tudo
        }

        // B. Se o site está bloqueado/pausado
        const siteData = siteSnap.data();
        if (siteData.active === false) {
            document.body.innerHTML = `
                <div style="height:100vh; display:flex; flex-direction:column; align-items:center; justify-content:center; background:#0f172a; color:#fff; font-family:sans-serif; text-align:center; padding:20px;">
                    <div style="font-size:4rem; margin-bottom:1rem;">🔒</div>
                    <h1 style="font-size:2rem; font-weight:bold; margin-bottom:0.5rem;">Loja Indisponível</h1>
                    <p style="color:#94a3b8; max-width:400px;">
                        O acesso a esta loja está suspenso temporariamente.
                    </p>
                </div>
            `;
            return; // Para tudo
    ...
    // 2. CARREGA AS CONFIGURAÇÕES VISUAIS (Se passou na segurança)
    ...
    onSnapshot(settingsRef, (docSnap) => {
        if (docSnap.exists()) {
            ...
            // Define padrão se não existir configuração visual ainda
            state.storeProfile = { 
                name: "Minha Loja", // Nome padrão visual
                installments: { active: false } 
            };
        }

        renderStoreProfile(); // Atualiza Sidebar (Logo, cores)
        
        // Se estiver no painel admin, preenche os inputs
        if (typeof fillProfileForm === 'function') fillProfileForm();    

        // Configura dependências de entrega/pagamento se a função existir
        if (typeof setupDeliveryDependency === 'function') setupDeliveryDependency();

        // --- ATUALIZAÇÕES CRÍTICAS ---
        // Força a vitrine a se redesenhar (ex: recalcular parcelamento com juros novos)
