...
window.openCheckoutModal = () => {
    ...
    // 3. ✨ O SEGREDO: Inicializa o filtro imediatamente ao abrir
    applyCheckoutVisibility(); 
    togglePaymentMode(); // Isso remove as formas desativadas no segundo 0.1

    // 4. Bloqueia o botão de finalizar até validar endereço
...
window.togglePaymentMode = () => {
    ...
    // Mapeia todas de forma independente
    ...
    const forceUpdateVis = (obj, shouldShow) => {
        ...
        if (shouldShow) {
            ...
            obj.wrapper.style.display = 'flex'; // Força visualização flex para o design
    ...
    if (mode === 'delivery') {
        ...
        // Cada um decide sua vida sozinho:
        ...
        forceUpdateVis(cash, false); // Dinheiro nunca online
    }

    // Auto-selecionar a primeira disponível se a atual sumiu
    const current = document.querySelector('input[name="payment-method-selection"]:checked');
    if (!current || current.disabled || current.closest('.hidden')) {
        const firstVisible = document.querySelector('input[name="payment-method-selection"]:not(:disabled)');
        if (firstVisible) {
