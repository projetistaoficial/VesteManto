...
async function autoSaveSettings(type) {
    ...
    // Referência ao documento de perfil
    const docRef = doc(db, `sites/${state.siteId}/settings`, 'profile');
    let dataToUpdate = {};
    let message = '';

    try {
        // --- CASO 1: LOGÍSTICA (CEP e RAIO) ---
        if (type === 'logistics') {
            ...
            // Trata vírgula para ponto (ex: 5,5 km -> 5.5)
            const dist = elDist ? parseFloat(elDist.value.replace(',', '.')) : 0;
        ...
        // --- CASO 2: PARCELAMENTO ---
            ...
            const active = elActive ? elActive.checked : false;
            const max = elMax ? parseInt(elMax.value) : 12;
            const freeUntil = elFree ? parseInt(elFree.value) : 1;
            
            let rate = 0;
            if (elRate) {
                // Trata vírgula para ponto
        ...
        // Atualiza Vitrine (se mudou regras de negócio)
...
window.applyCheckoutVisibility = () => {
    ...
    // Se não tiver nada salvo, assume que tudo é TRUE (ativo)
    ...
    // Regra: Entrega só aparece se Financeiro ON + Logística ON
    ...
    // 1. Aplica Visibilidade MACRO (Abas Principais)
    if (labelOnline) {
        if (onlineActive) {
    ...
    // --- AUTO-CORREÇÃO DA SELEÇÃO DE ABA ---
    // Se a aba selecionada foi escondida, muda para a outra
    if (radioOnline && radioOnline.checked && !onlineActive) {
        if (showDeliveryOption && radioDelivery) radioDelivery.checked = true;
        else radioOnline.checked = false;
    }
    if (radioDelivery && radioDelivery.checked && !showDeliveryOption) {
        if (onlineActive && radioOnline) radioOnline.checked = true;
        else radioDelivery.checked = false;
    }
    
    // Se nenhuma estiver marcada e o online estiver ativo, marca ele por padrão
    if (radioOnline && !radioOnline.checked && (!radioDelivery || !radioDelivery.checked) && onlineActive) {
        radioOnline.checked = true;
    }


    // --- 2. APLICA VISIBILIDADE MICRO (SUB-MÉTODOS: PIX, CARTÃO...) ---
    // Descobre qual aba está ativa agora
    const currentMode = document.querySelector('input[name="pay-mode"]:checked')?.value;

    // Elementos de Opção (Radio Buttons)
    const rPix = document.querySelector('input[name="payment-method-selection"][value="pix"]');
    const rCredit = document.querySelector('input[name="payment-method-selection"][value="credit"]');
    const rDebit = document.querySelector('input[name="payment-method-selection"][value="debit"]');
    const rCash = document.querySelector('input[name="payment-method-selection"][value="cash"]');

    // Containers (Para esconder o botão inteiro)
    // Nota: O HTML precisa estar estruturado para pegarmos o container pai.
    // Assumindo que o input está dentro de um <label> ou <div> que deve sumir.
    const getContainer = (el) => el ? el.closest('label') || el.closest('.payment-option') : null;

    const cPix = getContainer(rPix);
    const cCredit = document.getElementById('container-credit-option') || getContainer(rCredit);
    const cDebit = document.getElementById('container-debit-option') || getContainer(rDebit);
    const cCash = document.getElementById('container-cash-option') || getContainer(rCash);

    // Função Helper para mostrar/esconder
    const setVisible = (container, input, isVisible) => {
        if (!container) return;
        if (isVisible) {
            container.classList.remove('hidden');
            container.style.display = ''; // Limpa inline styles
            ...
            if (input) {
                input.disabled = true;
                input.checked = false; // Desmarca se esconder
            }
        }
    };

    // Aplica as regras baseadas no modo atual
    if (currentMode === 'online') {
        const conf = pm.online || {};
        setVisible(cPix, rPix, conf.pix !== false);
        setVisible(cCredit, rCredit, conf.credit !== false);
        setVisible(cDebit, rDebit, conf.debit !== false);
        setVisible(cCash, rCash, false); // Dinheiro nunca no online
    } 
    else if (currentMode === 'delivery') {
        const conf = pm.delivery || {};
        setVisible(cPix, rPix, conf.pix !== false);
        setVisible(cCredit, rCredit, conf.credit !== false);
        setVisible(cDebit, rDebit, conf.debit !== false);
        setVisible(cCash, rCash, conf.cash !== false);
    }

    // --- 3. AUTO-CORREÇÃO DA SELEÇÃO DE MÉTODO ---
    // Se o método selecionado sumiu, desmarca ou seleciona o primeiro disponível
    const selectedMethod = document.querySelector('input[name="payment-method-selection"]:checked');
    
    // Se não tem nada selecionado OU o selecionado está desabilitado/escondido
    if (!selectedMethod || selectedMethod.disabled || selectedMethod.closest('.hidden')) {
        // Tenta achar o primeiro visível e habilitado
        const firstAvailable = document.querySelector('input[name="payment-method-selection"]:not(:disabled)');
        
        // Verifica se o pai dele não está hidden
        if (firstAvailable && !firstAvailable.closest('.hidden')) {
            firstAvailable.checked = true;
            // Dispara a lógica de UI (abrir parcelamento, troco, etc)
