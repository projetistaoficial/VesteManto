...
function renderSalesList(orders) {
    ...
    orders.forEach(o => {
        ...
        if (valDescontoTotal > 0.05) {
            ...
            // 1. Exibe o CUPOM (Se existir)
            if (hasCupom) {
                discountHtml += `
                    <div class="flex justify-between text-xs text-gray-300">
                        <span>Cupom aplicado:</span>
                        <span class="text-yellow-500 font-bold uppercase tracking-wider border border-yellow-500/30 px-1 rounded bg-yellow-500/10">${o.cupom}</span>
                    </div>
                `;
            }

            // 2. Exibe o PIX (Se for Pix e houver desconto)
            // Se tiver cupom E pix, mostra os dois. Se só tiver um, mostra só um.
            if (isPix) {
                discountHtml += `
                    <div class="flex justify-between text-xs text-gray-300">
                        <span>Desconto via Pix:</span>
                        <span class="text-green-400 font-bold"><i class="fas fa-check-circle"></i> Aplicado</span>
                    </div>
                `;
            }

            // 3. Exibe o Valor Total Economizado
            discountHtml += `
