...
window.updateStoreStatusUI = () => {
    const status = getStoreStatus();
    const badgeBtn = document.getElementById('store-status-badge'); // Agora é um botão/badge
    ...
    // 1. ATUALIZA BADGE NA SIDEBAR (Visual novo)
    if (badgeBtn) {
        ...
        if (!state.storeProfile.openingHours?.active) {
            ...
            // Define o clique para mostrar o horário
            badgeBtn.onclick = () => {
    ...
    if (!status.isOpen && status.block && modalBlock) {
        if (!state.user) {
            ...
            // Fecha outros modais para evitar conflito visual
    ...
    // Se chegou aqui, não é para bloquear a tela
    if(modalBlock) {
