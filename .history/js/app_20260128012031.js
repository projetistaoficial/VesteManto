...
function loadAdminSales() {
    ...
    // 1. QUERY DA LISTA (Limitada a 100 para ser rápida e não travar a tela)
    const qList = query(
    ...
    state.salesUnsubscribe = onSnapshot(qList, (snapshot) => {
        // Carrega na memória apenas os 100 pedidos mais recentes
        ...
        // --- [CRUCIAL] O BOTÃO CARREGAR MAIS ---
        // Você precisa dessa linha para conseguir ver além dos 100 primeiros
        renderLoadMoreButton(snapshot.size);

        // --- ATUALIZA ESTATÍSTICAS GLOBAIS ---
        // Chame direto (sem o if). A função fetchGlobalStats já cuida de não estragar o cabeçalho,
        // mas garante que os Cards de Estatística (R$ Totais) sejam atualizados.
        fetchGlobalStats(); 
    });
}
...
async function fetchGlobalStats() {
    try {
        ...
        const snapshot = await getDocs(collection(db, `sites/${state.siteId}/sales`));
        ...
        snapshot.forEach(doc => {
            const data = doc.data();
            // Conta TUDO, independente do status (para total de pedidos)
            totalPedidos++; 
            
            // Regra de Negócio: Somar Faturamento apenas de pedidos válidos?
            // Se quiser somar tudo (bruto), mantenha assim. 
            // Se quiser somar só 'Confirmados', adicione um if(data.status === '...') aqui.
            // Pelo seu print, parece que você quer o valor total bruto:
            faturamentoTotal += parseFloat(data.total || 0);
            
            // Custo (agora somando o campo totalCost que criamos)
        ...
        if (elProfit) {
            ...
            elProfit.className = lucroLiquido >= 0 ? "text-3xl font-bold text-green-500" : "text-3xl font-bold text-red-500";
        }
...
function calculateStatsMetrics() {
    ...
    // 2. Filtragem Local (apenas para gráficos de pizza/status)
    ...
    filteredOrders.forEach(o => {
        if (o.status === 'Reembolsado') countRefunded++;
        if (o.status.includes('Cancelado')) countCancelled++;
        if (o.status === 'Aguardando aprovação' || o.status === 'Pendente') countPending++;
        
        // Soma valores locais (apenas para uso quando NÃO for 'all')
        if (o.status === 'Confirmado' || o.status === 'Entregue' || o.status === 'Concluído') {
            totalSalesCount++;
            totalSalesValue += parseFloat(o.total || 0);
            totalCost += parseFloat(o.totalCost || 0); // Pega o custo salvo no pedido
        }
    });
    ...
    // Se o filtro for 'all' (Tudo), PROIBIMOS essa função de tocar nos cards principais.
    // Assim, o fetchGlobalStats() (que traz os 317 pedidos) não é sobrescrito.
    if (state.statsFilterType !== 'all') {
        ...
        // Se for 'all', forçamos a busca global para garantir que os dados venham do servidor
        // Adicionei um pequeno delay para garantir que a UI não pisque
    ...
    // Atualiza KPIs percentuais com base no que temos
