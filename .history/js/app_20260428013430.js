...
window.openProductModal = (productId) => {
    ...
    // 🧹 LIMPEZA: Remove o código ruim da tentativa anterior que fez a foto sumir
    const badStyle = document.getElementById('style-modal-fullscreen');
    ...
    // ✨ 1. MODAL: Travado (overflow-hidden) para a imagem ficar fixa
    ...
    // ✨ 2. COLUNA DA IMAGEM: Ocupa toda a altura no PC
    if (imgCol) {
        imgCol.className = "w-full md:w-1/2 h-[40vh] md:h-full relative shrink-0 overflow-hidden bg-[#0a0c13]";
    }

    // ✨ 3. COLUNA DIREITA (Textos): Rolável e independente
    if (rightCol) {
    ...
    // ✨ 4. FORÇA A IMAGEM A PREENCHER TUDO SEM SOBRAS (object-cover seguro via JS)
    const imgInterna = document.getElementById('modal-img');
    if (imgInterna) {
        imgInterna.style.width = '100%';
        imgInterna.style.height = '100%';
        imgInterna.style.objectFit = 'cover';
        imgInterna.style.objectPosition = 'top center';
    }

    // Posiciona as miniaturas na parte de baixo flutuando
    const thumbnails = document.getElementById('modal-thumbnails');
    if (thumbnails) {
        thumbnails.className = "absolute bottom-4 left-0 w-full flex justify-center z-10 gap-2 px-4";
    }
