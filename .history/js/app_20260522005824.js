...
function renderCatalog(productsToRender) {
    ...
    filtered.sort((a, b) => {
        ...
        // Facilita o cálculo pegando o preço final (promocional ou normal)
        const priceA = parseFloat(a.promoPrice || a.price) || 0;
        const priceB = parseFloat(b.promoPrice || b.price) || 0;

        // O SEGREDO ESTÁ AQUI: Tratando cada tipo de filtro
        switch (sortMode) {
            case 'menor_preco':
            case 'lowest_price':
            case 'price_asc':
                return priceA - priceB; // Do mais barato pro mais caro

            case 'maior_preco':
            case 'highest_price':
            case 'price_desc':
                return priceB - priceA; // Do mais caro pro mais barato

            case 'nome':
            case 'name_asc':
                return a.name.localeCompare(b.name); // Ordem Alfabética (A-Z)

            case 'lancamento':
            case 'newest':
            case 'date_desc':
                // Tenta usar a data de criação. Se não tiver, usa o código do produto (maior = mais novo)
            ...
            case 'vitrine':
            default:
                // 1. REGRA SOBERANA: Ordem Manual (Arraste)
                ...
                // 2. SE NÃO TIVER ORDEM MANUAL, aplica a hierarquia: Destaque > Promoção > Novo
