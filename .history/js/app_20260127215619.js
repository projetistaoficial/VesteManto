...
// VERSÃO BLINDADA: Faz 3 consultas separadas para não exigir índices complexos
async function updateRealTotals() {
    ...
    try {
        ...
        // Fazer a soma separada evita o erro "requires an index with fields [total, totalCost]"
        
        // 1. Busca Contagem
        const snapCount = await getAggregateFromServer(salesRef, { count: count() });
        const totalPedidos = snapCount.data().count;

        // 2. Busca Soma de Vendas (Simples)
        const snapTotal = await getAggregateFromServer(salesRef, { sumVal: sum('total') });
        const faturamentoTotal = snapTotal.data().sumVal;

        // 3. Busca Soma de Custos (Simples)
        const snapCost = await getAggregateFromServer(salesRef, { sumCost: sum('totalCost') });
        const custoTotalAgregado = snapCost.data().sumCost;
        ...
        if (headerTotalValue) {
            ...
            // Ajusta o texto da label se necessário
            const label = headerTotalValue.previousElementSibling;
            if (label && label.innerText.includes('LISTADO')) {
                label.innerText = 'TOTAL GLOBAL';
            }
        }
        ...
        console.log("Totais atualizados (Modo Seguro):", { totalPedidos, faturamentoTotal, custoTotalAgregado });

    } catch (error) {
        console.error("Erro ao buscar totais:", error);
        
        // Verifica se foi bloqueado por extensão
        if (error.message && error.message.includes("Failed to fetch")) {
             if (headerTotalValue) headerTotalValue.innerText = "Desative o AdBlock";
             alert("⚠️ Parece que uma extensão (AdBlock) está bloqueando o banco de dados. Por favor, desative-a para este site.");
        } else {
             if (headerTotalValue) headerTotalValue.innerText = "Erro";
        }
    }
}