...
function renderCategories() {
    ...
    if (scrollContainer) {
        ...
        categoriasParaMostrar.forEach(c => {
            ...
            // Estrutura Base: Fonte grossa, vidro fosco (backdrop-blur) e borda (para não tremer a tela)
            const classesEstrutura = "categoria-btn flex items-center h-full rounded-full transition-all shrink-0 border backdrop-blur-md font-black tracking-wide text-xs outline-none";
            ...
            if (hasSubs) {
                pillsHtml += `
                    <div class="${classesEstrutura}" data-cat="${safeName}">
                        ...
                        <button onclick="toggleCatDropdown('${safeName}', event)" class="px-3 h-full border-l border-white/10 flex items-center justify-center rounded-r-full hover:bg-black/20 outline-none transition">
