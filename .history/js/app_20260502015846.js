...
// =================================================================
// 🛒 1. ABERTURA DO CHECKOUT (ORDEM DE RENDERIZAÇÃO CORRIGIDA)
// =================================================================
window.openCheckoutModal = () => {
    ...
    // ✨ 2. O SEGREDO: Mostra a tela PRIMEIRO para o HTML "acordar"
    ...
    // 3. Trava a sessão de pagamento antes do usuário interagir
    ...
    // ✨ 4. Agora sim, com a tela aberta, aplicamos as regras visuais rigorosamente!
...
// =================================================================
// 💳 2. CONTROLE DE MODO DE PAGAMENTO (TOTALMENTE INDEPENDENTE)
// =================================================================
window.togglePaymentMode = () => {
    let modeEl = document.querySelector('input[name="pay-mode"]:checked');
    if (!modeEl) return;
    ...
    const getWrapper = (val) => {
        ...
        const wrapper = document.getElementById(`container-${val}-option`) || radio.closest('label')?.parentElement || radio.parentElement;
    ...
    const forceUpdateVis = (obj, shouldShow) => {
        if (!obj || !obj.wrapper) return;
        if (shouldShow) {
            obj.wrapper.classList.remove('hidden');
            obj.wrapper.style.display = 'flex'; // Garante o alinhamento do Tailwind
    ...
    // ✨ CADEIA QUEBRADA: Nenhuma forma de pagamento interfere na outra agora!
    if (mode === 'delivery') {
        if (lblMethod) lblMethod.innerText = "Pagarei na entrega com:";
        const pDel = pm.delivery || {};
        
        forceUpdateVis(pix, pDel.pix !== false);
        forceUpdateVis(credit, pDel.credit !== false);
        forceUpdateVis(debit, pDel.debit !== false);
        forceUpdateVis(cash, pDel.cash !== false);
    } else {
        // ONLINE
        if (lblMethod) lblMethod.innerText = "Pagar agora com:";
        const pOn = pm.online || {};
        
        forceUpdateVis(pix, pOn.pix !== false);
        forceUpdateVis(credit, pOn.credit !== false);
        forceUpdateVis(debit, pOn.debit !== false);
        forceUpdateVis(cash, false); // Dinheiro NUNCA existe online
    }

    // Auto-Correção: Pula para a primeira opção válida se a atual foi desativada
    let current = document.querySelector('input[name="payment-method-selection"]:checked') || document.querySelector('input[name="payment-method"]:checked');
    let isInvalid = (!current || current.disabled || current.closest('.hidden'));
...
// =================================================================
// 💰 3. CÁLCULO DE TOTAL (COM REGRA DE FRETE BLINDADA EM PT/EN)
// =================================================================
window.calcCheckoutTotal = () => {
    ...
    // ✨ LÓGICA DE FRETE BLINDADA: Aceita as configurações tanto em Inglês quanto Português
    if (typeof checkoutState !== 'undefined' && checkoutState.isValidDelivery && shipValue > 0) {
        let applyFrete = false;
        
        if (shipRule === 'both' || shipRule === 'todos') {
            applyFrete = true;
        } else if ((shipRule === 'online' || shipRule === 'online') && payMode === 'online') {
            applyFrete = true;
        } else if ((shipRule === 'delivery' || shipRule === 'entrega') && payMode === 'delivery') {
            applyFrete = true;
        }
    ...
    if (detailsContainer) {
        ...
        // Reseta textos auxiliares
        if (els.labelPixDiscount) els.labelPixDiscount.innerText = discountPixDisplay > 0 ? `Economia de ${formatCurrency(discountPixDisplay)}` : '';
...
// =================================================================
// 🚀 4. SUBMIT ORDER (REPASSANDO A REGRA DE FRETE BLINDADA)
// =================================================================
async function submitOrder() {
    try {
        ...
        // ✨ APLICAÇÃO DE FRETE BLINDADA PARA O SALVAMENTO NO BANCO
        const shipRule = dConfig.shippingRule || 'none';
        const shipValue = parseFloat(dConfig.shippingValue) || 0;
        let valueToSave = 0;
        ...
        if (typeof checkoutState !== 'undefined' && checkoutState.isValidDelivery && shipValue > 0) {
            let applyFrete = false;
            if (shipRule === 'both' || shipRule === 'todos') applyFrete = true;
            else if (shipRule === 'online' && payMode === 'online') applyFrete = true;
            else if ((shipRule === 'delivery' || shipRule === 'entrega') && payMode === 'delivery') applyFrete = true;
