...
function loadSettings() {
    // Tenta buscar as configurações da loja específica
    const docRef = doc(db, `sites/${state.siteId}/settings`, 'general');
    
    onSnapshot(docRef, (docSnap) => {
        if (docSnap.exists()) {
            // Se achou no banco, usa a regra do dono da loja
            state.globalSettings = docSnap.data();
            
            // Atualiza checkbox visual se estiver no painel
            if (els.toggleStockGlobal) els.toggleStockGlobal.checked = state.globalSettings.allowNoStock;
        } else {
            // Se não achou, NÃO TENTA GRAVAR. Apenas libera a visualização.
            console.log("Configuração não encontrada, usando padrão liberado.");
            state.globalSettings = { allowNoStock: true }; 
        }
        
        // Se houver lógica de entrega, carrega
        if (typeof setupDeliveryDependency === 'function') setupDeliveryDependency();
        
        // IMPORTANTE: Força redesenhar os produtos agora que temos a regra
        renderCatalog(state.products);

    }, (error) => {
        // Se der erro de permissão (comum), libera o site para não ficar preto
        console.warn("Usando configuração padrão (Fallback):", error);
        state.globalSettings = { allowNoStock: true };
        renderCatalog(state.products);
    });
}
