...
// ✨ ÍCONES ATUALIZADOS COM OPÇÕES DE E-COMMERCE
...
window.renderIconSelector = (selectedIcon = 'fa-file-alt') => {
    ...
    // 1. Força a caixa a ter apenas UMA LINHA e rolagem lateral suave no celular
    container.className = "flex flex-nowrap overflow-x-auto gap-1.5 sm:gap-2 bg-[#151720] p-2 rounded-md border border-gray-700 items-center hide-scroll";

    // 2. Cria os botões menores (w-6 h-6 no celular e w-7 h-7 no PC) com 'shrink-0' para não amassarem
    container.innerHTML = availableIcons.map(icon => {
        ...
        return `
            <button type="button" onclick="selectTopicIcon('${icon}')" class="shrink-0 w-6 h-6 sm:w-7 sm:h-7 rounded border flex items-center justify-center transition ${color} text-[10px] sm:text-xs outline-none">
                <i class="fas ${icon}"></i>
            </button>
        `;
    }).join('');
...
window.checkTopicChanges = () => {
    ...
    // ✨ CORREÇÃO: Garante que os botões não fiquem travados!
    ...
    if (isEditing) {
        // MODO EDIÇÃO: Mostra sempre o Cancelar e a Caixa de texto
        descContainer.classList.remove('hidden');
        btnCancelar.classList.remove('hidden');

        if (hasChanged && hasText) {
            // Se alterou algo, MOSTRA o botão Salvar
            ...
            // Se NÃO alterou nada, ESCONDE o botão Salvar
            btnAcao.classList.add('hidden');
        }
    } else {
        // MODO CRIAÇÃO (NOVO)
        if (hasText) {
            // Começou a digitar: Mostra Cancelar e muda o botão pra Criar
            btnCancelar.classList.remove('hidden');
            btnAcao.classList.remove('hidden');
...
window.handleTopicAction = async () => {
    ...
    // Se clicar em Novo, apenas abre o formulário
    if (acaoTexto === "NOVO") {
    ...
    try {
        ...
        // ✨ CORREÇÃO: O bloco FINALLY sempre executa (dando certo ou erro), 
        // destravando os botões para o lojista continuar usando!
        btnAcao.disabled = false;
        document.getElementById('btn-cancelar-topico').disabled = false;
    }
};
...
window.resetTopicForm = () => {
    ...
    // ✨ CORREÇÃO: Garante o desbloqueio ao limpar
    ...
    checkTopicChanges(); // Força a interface a voltar ao normal
    renderAdminTopics();
};
