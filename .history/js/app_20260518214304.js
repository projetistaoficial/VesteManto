...
function renderProductsList(products, preCalcMetrics = null) {
    ...
    // --- 1. BARRA DE CONTROLES (GERENCIA O MODO DE REORDENAÇÃO E SELEÇÃO) ---
    ...
    if (state.isReorderMode) {
        controlsBar.innerHTML = `
            <div class="flex w-full justify-between items-center bg-blue-900/20 border border-blue-500/50 p-2 rounded-lg animate-fade-in">
                <span class="text-blue-400 font-bold text-xs uppercase"><i class="fas fa-arrows-alt-v"></i> Modo Reordenação Ativo</span>
        ...
        const selectBtnClass = state.isSelectionMode ? "text-red-400 py-2 px-2 bg-red-900/20 rounded border border-red-900/50" : "text-yellow-500 py-2 px-2 hover:bg-yellow-900/20 rounded transition";

        controlsBar.innerHTML = `
            <div class="flex gap-2">
                <button onclick="toggleSelectionMode()" class="${selectBtnClass} text-xs font-bold uppercase">${selectBtnText}</button>
    ...
    // --- 2. HEADER DA TABELA (Dinâmico conforme o modo) ---
    ...
    const headerHTML = `
        <div class="hidden md:grid grid-cols-12 gap-2 bg-[#1f1f1f] text-gray-400 font-bold p-3 rounded-t-xl text-[10px] uppercase tracking-wider border-b border-gray-800 sticky top-0 z-20 items-center shadow-lg">
            ${state.isReorderMode ? `
                ...
                <div class="col-span-2 text-center border-r border-gray-700">Cód / Status</div>
            ...
            ` : `
                <div class="${state.isSelectionMode ? 'col-span-1 block' : 'hidden'} text-center flex items-center justify-center">
                    <input type="checkbox" onchange="toggleSelectAll(this)" ${masterCheckAttr} class="cursor-pointer rounded border-gray-600 bg-gray-800 text-yellow-500 focus:ring-0 w-4 h-4">
                </div>
                <div class="${state.isSelectionMode ? 'col-span-1' : 'col-span-2'} text-center border-r border-gray-700 cursor-pointer hover:text-white" onclick="sortProducts('code')">Cód</div>
                <div class="col-span-5 pl-2 cursor-pointer hover:text-white" onclick="sortProducts('product')">Produto</div>
            `}
            <div class="col-span-2 text-center cursor-pointer hover:text-white" onclick="sortProducts('lastmov')">Última Mov.</div>
            <div class="col-span-1 text-center cursor-pointer hover:text-white" onclick="sortProducts('sales')">Vendas</div>
            <div class="col-span-1 text-center cursor-pointer hover:text-white" onclick="sortProducts('stock')">Estoque</div>
            <div class="col-span-1 text-right pr-4 cursor-pointer hover:text-white" onclick="sortProducts('price')">Valor</div>
        </div>
    ...
    products.forEach((p, index) => {
        ...
        let bgClass = isChecked ? 'bg-[#1a233a]' : 'bg-[#151720] hover:bg-[#1c1f2b]';
        if (isInactive && !isChecked) bgClass = 'bg-[#2a1313] hover:bg-[#351818]';
        ...
        // ALTERAÇÃO DO LADO ESQUERDO DA LINHA CONFORME O MODO
        let leftControlsHTML = '';
        if (state.isReorderMode) {
            leftControlsHTML = `
                <div class="flex flex-col items-center justify-center gap-0.5 w-12 border-r border-gray-800 shrink-0 bg-black/20 h-full py-1">
                    <button onclick="event.stopPropagation(); moveProductInReorder('${p.id}', -1)" class="text-gray-500 hover:text-yellow-500 p-0.5 transition" ${isFirst ? 'disabled style="opacity:0.2"' : ''}><i class="fas fa-caret-up"></i></button>
                    <span class="text-xs font-black text-yellow-500 leading-none">${positionNum}</span>
                    <button onclick="event.stopPropagation(); moveProductInReorder('${p.id}', 1)" class="text-gray-500 hover:text-yellow-500 p-0.5 transition" ${isLast ? 'disabled style="opacity:0.2"' : ''}><i class="fas fa-caret-down"></i></button>
                </div>
                <div class="hidden md:flex flex-col w-20 items-center justify-center border-r border-gray-800 shrink-0 h-full">
            ...
            leftControlsHTML = `
                <div class="${state.isSelectionMode ? 'flex' : 'hidden'} items-center justify-center shrink-0 w-12 h-full">
                     <input type="checkbox" class="w-5 h-5 rounded border-gray-600 bg-gray-900 text-yellow-500 cursor-pointer" onclick="event.stopPropagation(); toggleProductSelection('${p.id}')" ${isChecked ? 'checked' : ''}>
                </div>
                <div class="hidden md:flex flex-col ${state.isSelectionMode ? 'w-16' : 'w-24'} items-center justify-center border-r border-gray-800 shrink-0 h-full">
                    <span class="text-base font-bold text-white font-mono opacity-80">#${p.code || '-'}</span>
        ...
        row.className = `relative overflow-hidden border-b border-gray-800 last:border-0 group select-none ${bgClass}`;
        row.ondblclick = () => editProduct(p.id);

        row.innerHTML = `
            <div class="${state.isSelectionMode || state.isReorderMode ? 'hidden' : 'absolute'} inset-y-0 right-0 w-24 bg-red-600 flex items-center justify-center cursor-pointer z-0" onclick="confirmDeleteProduct('${p.id}')">
                <i class="fas fa-trash-alt text-white text-lg"></i>
            </div>

            <div class="relative z-10 p-3 transition-transform duration-200 ease-out prod-swipe-content h-full flex flex-col md:grid md:grid-cols-12 gap-2 md:items-center ${bgClass}">
                ...
                <div class="flex items-center justify-between w-full md:contents">
                    <div class="flex items-center gap-2 md:col-span-7 w-full flex-1 min-w-0 h-full">
                        ...
                        <div class="flex items-center gap-3 flex-1 min-w-0 px-2">
                ...
                <div class="hidden md:block col-span-1 text-right pr-2">
        ...
        if (!state.isReorderMode && !state.isSelectionMode) {
