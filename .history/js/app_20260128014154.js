...
function calculateStatsMetrics() {
    ...
    if (state.statsFilterType !== 'all') {
        ...
        // O delay garante que ele rode DEPOIS dessa função e corrija qualquer valor errado
...
async function fetchGlobalStats() {
    try {
        ...
        querySnapshot.forEach((doc) => {
            const data = doc.data();
            totalPedidos++;
            // Garante que soma números, evitando erros
        ...
        // Atualiza Dashboard (Estatísticas)
        if (getEl('stat-sales-count')) getEl('stat-sales-count').innerText = totalPedidos;
        if (getEl('stat-sales-total')) getEl('stat-sales-total').innerText = formatCurrency(faturamentoTotal);
        if (getEl('stat-cost-total')) getEl('stat-cost-total').innerText = formatCurrency(custoTotal);

        const elProfit = getEl('stat-profit-total');
        if (elProfit) {
        ...
        // Atualiza Header da Lista (Apenas se não houver busca ativa)
        const searchInput = getEl('search-input'); // ou filter-search-general
        if (!searchInput || searchInput.value === '') {
            if (getEl('orders-count')) getEl('orders-count').innerText = totalPedidos;
            const headerTotal = getEl('orders-filtered-total');
            if (headerTotal) {
