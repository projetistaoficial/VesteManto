...
async function calculateDistanceByCEP(cepOrigin, cepDest) {
    const getCoords = async (cep) => {
        try {
            ...
            if (!resp.ok) throw new Error('CEP não encontrado na BrasilAPI');
    ...
    // Busca coordenadas de origem e destino em paralelo
    const [c1, c2] = await Promise.all([getCoords(cepOrigin), getCoords(cepDest)]);

    // Se falhar em encontrar qualquer um dos dois, retorna null (erro)
