...
window.openCheckoutModal = () => {
    // 1. Limpa campos anteriores (Mantido)
    ...
    // 2. RECUPERA CONFIGURAÇÕES DO PERFIL
    ...
    // 3. REFERÊNCIAS AOS ELEMENTOS (CORREÇÃO AQUI)
    // Em vez de buscar por ID na div, buscamos o INPUT e pegamos o pai (label)
    ...
    // .closest('label') encontra o cartão clicável onde o input está dentro
    const labelOnline = radioOnline ? radioOnline.closest('label') : null;
    const labelDelivery = radioDelivery ? radioDelivery.closest('label') : null;
    ...
    // B) Opção ENTREGA (Ativa somente se tiver método E logística)
    const showDeliveryOption = deliveryActive && isLogisticsActive;
    
    if (labelDelivery) {
        if (showDeliveryOption) {
            labelDelivery.classList.remove('hidden');
            labelDelivery.style.display = '';
        } else {
            labelDelivery.classList.add('hidden');
            labelDelivery.style.setProperty('display', 'none', 'important');
        }
    }

    // 5. AUTO-SELEÇÃO (Evita ficar marcado uma opção oculta)
    
    // Cenário 1: Online foi desativado -> Força marcar Entrega
    if (!onlineActive && showDeliveryOption && radioDelivery) {
        radioDelivery.checked = true;
    }
    // Cenário 2: Entrega sumiu -> Força marcar Online
    else if (onlineActive && !showDeliveryOption && radioOnline) {
        radioOnline.checked = true;
    }
    // Cenário 3: Ambos ativos -> Prioriza Online se nenhum estiver marcado
    else if (onlineActive && showDeliveryOption) {
        if (radioOnline && !radioOnline.checked && !radioDelivery.checked) {
            radioOnline.checked = true;
        }
    }

    // 6. ATUALIZAÇÕES FINAIS (Chama as rotinas de cálculo)
