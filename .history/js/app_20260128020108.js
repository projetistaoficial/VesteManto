...
function calculateStatsMetrics() {
    // 1. Capital de Giro (Sempre calcula)
    let capitalGiro = 0;
    if (state.products) {
    ...
    // 2. Se for "Visão Geral" (Tudo), chama a função global e PARA AQUI.
    // Isso impede que o código abaixo zere os números.
    if (state.statsFilterType === 'all') {
        fetchGlobalStats();
        return; // <--- O PULO DO GATO: Interrompe a função aqui para não zerar nada!
    }

    // =========================================================
    // O código abaixo só roda se você filtrar por DATA (Hoje, Mês...)
    // =========================================================
    
    if (!state.orders) return;
    let filteredOrders = state.orders; // Usa a lista local (100 itens)
