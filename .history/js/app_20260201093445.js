...
function setupDeliveryDependency() {
    ...
    // Regra: Se desligar, derruba o pagamento. Se ligar, não faz nada.
    // =================================================================
    ...
    newOwn.addEventListener('change', async (e) => {
        ...
        // DEPENDÊNCIA: Desligar Entrega -> Desliga Pagamento
        if (!isActive) {
    ...
    // =================================================================
    // 2. MEXEU NO PAGAMENTO NA ENTREGA (AQUI ESTÁ A SUA SOLICITAÇÃO)
    // =================================================================
    ...
    newPay.addEventListener('change', async (e) => {
        ...
        // --- LÓGICA DO AVISO PEDIDO ---
        const ownCheck = document.getElementById('conf-own-delivery');
        
        if (ownCheck) {
            // Se estou ATIVANDO o pagamento...
            if (isActive) {
                // ...e a Entrega está DESATIVADA
                if (!ownCheck.checked) {
                    // Ativa a entrega para corrigir o problema
                    ownCheck.checked = true;
                    if (state.storeProfile.deliveryConfig) state.storeProfile.deliveryConfig.ownDelivery = true;
                    
                    // Destrava código visualmente
                    const elReq = document.getElementById('conf-req-code');
                    if (elReq) { elReq.disabled = false; elReq.closest('label')?.classList.remove('opacity-50'); }

                    // *** O AVISO EXATO QUE VOCÊ PEDIU ***
                    showSystemModal("⚠️ Atenção:\n\nA 'Entrega Própria' estava desativada e não permitiria vender nessa modalidade.\n\nEla foi ativada automaticamente agora.", "warning");
                    
                    await autoSaveSettings('orders');
                }
            }
            // Se estou DESATIVANDO o pagamento... (Não faz nada na entrega, como pediu antes)
        }
