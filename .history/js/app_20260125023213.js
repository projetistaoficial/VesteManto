...
// --- FUNÇÃO DE AUTOSALVAMENTO (CORRIGIDA) ---
async function autoSaveSettings(type) {
    ...
    // 1. LOGÍSTICA
    if (type === 'logistics') {
        const cepEl = document.getElementById('conf-store-cep');
        const distEl = document.getElementById('conf-max-dist');
        const cep = cepEl ? cepEl.value.replace(/\D/g, '') : '';
        const dist = distEl ? (parseFloat(distEl.value) || 0) : 0;

        dataToUpdate = { cep, maxDistance: dist };
        message = 'Logística salva!';
    }
    // 2. PARCELAMENTO E PAGAMENTOS (AQUI OCORRIA O ERRO)
    else if (type === 'installments') {
        const activeEl = document.getElementById('conf-card-active');
        const maxEl = document.getElementById('conf-card-max');
        const freeEl = document.getElementById('conf-card-free');
        const rateEl = document.getElementById('conf-card-rate');

        const active = activeEl ? activeEl.checked : false;
        const max = maxEl ? (parseInt(maxEl.value) || 12) : 12;
        const free = freeEl ? (parseInt(freeEl.value) || 3) : 3;
        
        // CORREÇÃO DE SEGURANÇA: Lê como string e trata vírgula
        let rate = 0;
        if (rateEl && rateEl.value) {
            rate = parseFloat(rateEl.value.replace(/\./g, '').replace(',', '.')) || 0;
        }

        // Captura checkboxes de pagamento
        const getCheck = (id) => {
            const el = document.getElementById(id);
    ...
    // 3. PEDIDOS E FRETE
        ...
        const ownEl = document.getElementById('conf-own-delivery');
        const reqEl = document.getElementById('conf-req-code');
        const timeEl = document.getElementById('conf-cancel-time');
        const shipRuleEl = document.getElementById('conf-shipping-rule');
        const shipValEl = document.getElementById('conf-shipping-value');

        const ownDelivery = ownEl ? ownEl.checked : false;
        const reqCode = reqEl ? reqEl.checked : false;
        const cancelTime = timeEl ? (parseInt(timeEl.value) || 5) : 5;
        const shipRule = shipRuleEl ? shipRuleEl.value : 'none';
        
        let shipVal = 0;
        if (shipValEl && shipValEl.value) {
            shipVal = parseFloat(shipValEl.value.replace(/[^\d,]/g, '').replace(',', '.')) || 0;
        }
    ...
    try {
        ...
        // ATUALIZA MEMÓRIA LOCAL IMEDIATAMENTE (CRUCIAL)
        if (!state.storeProfile) state.storeProfile = {};
        
        // Mescla os dados novos com o que já existe na memória
        ...
        console.error("Erro Fatal no AutoSave:", error);
        showToast('Erro ao salvar: ' + error.message, 'error');
    }
}
...
window.openCheckoutModal = () => {
    ...
    // 2. LEITURA SEGURA DAS CONFIGURAÇÕES
    const pm = state.storeProfile?.paymentMethods || {};
    const dConfig = state.storeProfile?.deliveryConfig || {};

    // Usa '=== false' para garantir que undefined seja tratado como true (padrão)
    ...
    // Regra: "Pagar na Entrega" exige Método Ativo E Logística Própria
    const deliveryTotalActive = deliveryMethodActive && isLogisticsActive;
    ...
    // 4. APLICA VISIBILIDADE E ESTADO
    if (labelOnline) {
        if (onlineActive) {
    ...
    // 5. AUTO-CORREÇÃO DA SELEÇÃO
    // Se a opção selecionada ficou invisível, muda para a outra
    
    // Se Online visível, marca ele
    if (onlineActive) {
        if (radioOnline) radioOnline.checked = true;
    }
    // Se Online invisível mas Entrega visível, marca Entrega
    else if (!onlineActive && deliveryTotalActive) {
    ...
    // 6. Atualiza UI interna (Esconde sub-opções erradas)
...
window.togglePaymentMode = () => {
    ...
    // Se nada estiver marcado (ambos desativados), sai
    if (!modeEl) return;
    ...
    const radioCard = document.querySelector('input[name="payment-method-selection"][value="card"]');
    // Cartão tem um wrapper DIV bg-gray-800, pegamos ele
    const cardWrapper = radioCard ? radioCard.closest('.bg-gray-800') : null;
    ...
    if (mode === 'delivery') {
        ...
        const pixOn = pm.delivery?.pix !== false;
        const cardOn = pm.delivery?.card !== false;
        const cashOn = pm.delivery?.cash !== false;

        updateVis(lblPix, pixOn, radioPix);
        updateVis(cardWrapper, cardOn, radioCard);
        updateVis(cashContainer, cashOn, radioCash);

    } else {
        // ONLINE
        if (lblMethod) lblMethod.innerText = "Pagar agora com:";

        const pixOn = pm.online?.pix !== false;
        const cardOn = pm.online?.card !== false;

        updateVis(lblPix, pixOn, radioPix);
        updateVis(cardWrapper, cardOn, radioCard);
        updateVis(cashContainer, false, radioCash); // Dinheiro nunca no online
    }

    // Auto-Correção: Se a opção selecionada sumiu, marca a primeira visível
    const current = document.querySelector('input[name="payment-method-selection"]:checked');
    
    // Verifica se o atual está desabilitado ou oculto
    let isInvalid = false;
    if (!current) isInvalid = true;
    else if (current.disabled) isInvalid = true;
    else if (current.offsetParent === null) isInvalid = true;

    if (isInvalid) {
        // Busca o primeiro habilitado e visível
        const valid = document.querySelector('input[name="payment-method-selection"]:not(:disabled)');
        if (valid) {
            valid.checked = true;
            // Se for cartão ou dinheiro, chama a função de UI deles
            if (typeof window.toggleMethodSelection === 'function') window.toggleMethodSelection();
        }
    } else {
        // Apenas atualiza a UI (parcelas, troco)
        if (typeof window.toggleMethodSelection === 'function') window.toggleMethodSelection();
    }
};
