...
window.toggleSelectAll = (checkbox) => {
    // Pega os produtos que estão visíveis na tela (filtrados)
    const visibleProducts = getCurrentFilteredProducts(); 
    ...
    // Atualiza apenas a interface da lista (sem recalcular tudo do zero)
...
function renderProductsList(products) {
    ...
    // --- A. BARRA DE CONTROLES SUPERIOR ---
    const controlsBar = document.createElement('div');
    controlsBar.className = "flex justify-between items-center mb-2 px-1";
    
    // Botão de Selecionar / Cancelar
    const selectBtnText = state.isSelectionMode ? 
        '<i class="fas fa-times mr-2"></i> Cancelar Seleção' : 
        '<i class="fas fa-check-square mr-2"></i> Selecionar';
    
    const selectBtnClass = state.isSelectionMode ? 
        "text-red-400 hover:text-red-300 text-xs font-bold uppercase" : 
        "text-yellow-500 hover:text-yellow-400 text-xs font-bold uppercase";

    // Barra de Ações em Massa (Só aparece se estiver no Modo Seleção E tiver itens selecionados)
    let bulkActionsHTML = '';
    if (state.isSelectionMode && state.selectedProducts.size > 0) {
        bulkActionsHTML = `
            <div class="flex items-center gap-2 animate-fade-in">
                <span class="text-white text-xs mr-2 font-bold bg-blue-900 px-2 py-1 rounded border border-blue-700">${state.selectedProducts.size}</span>
                <select id="bulk-category-select-dynamic" class="bg-gray-900 text-white text-[10px] border border-gray-600 rounded px-2 h-7 outline-none">
                    <option value="">Mover...</option>${state.categories.map(c => `<option value="${c.name}">${c.name}</option>`).join('')}
                </select>
                <button onclick="bulkMoveDynamic()" class="bg-blue-600 hover:bg-blue-500 text-white px-3 h-7 rounded text-[10px] uppercase font-bold">Mover</button>
                <button onclick="document.getElementById('btn-bulk-delete').click()" class="bg-red-600 hover:bg-red-500 text-white px-3 h-7 rounded text-[10px] uppercase font-bold">Excluir</button>
            </div>
        `;
    }

    controlsBar.innerHTML = `
        <button onclick="toggleSelectionMode()" class="${selectBtnClass} transition-colors">
            ${selectBtnText}
        </button>
        ${bulkActionsHTML}
    `;
    ...
    // --- B. CABEÇALHO (Fixo) ---
    // A coluna de Checkbox (col-span-1) só aparece se isSelectionMode for true
    const checkColClass = state.isSelectionMode ? "col-span-1 block" : "hidden";
    const prodColClass = state.isSelectionMode ? "col-span-4" : "col-span-5"; // Ganha espaço se não tiver check

    const headerHTML = `
        <div class="hidden md:grid grid-cols-12 gap-2 bg-[#1f1f1f] text-gray-400 font-bold p-3 rounded-t-xl text-[10px] uppercase tracking-wider border-b border-gray-800 sticky top-0 z-20">
            <div class="${checkColClass} text-center"><i class="fas fa-check"></i></div>
            <div class="col-span-1 text-center border-r border-gray-700">Cód</div>
            <div class="${prodColClass} pl-2">Produto</div>
    ...
    // --- C. CONTAINER DE ROLAGEM (Scrollable Box) ---
    const scrollContainer = document.createElement('div');
    // Define altura máxima (ex: 65vh) e rolagem. 'custom-scrollbar' é opcional se você tiver CSS pra isso.
    ...
    // Adiciona cabeçalho antes do container de scroll (para o cabeçalho não rolar junto, ou dentro se preferir sticky)
    // O pedido foi "cabeçalho fixo", então coloquei sticky no headerHTML, mas precisamos injetar corretamente.
    // Vamos injetar o Header DENTRO do listEl, e as linhas DENTRO do scrollContainer.
    ...
    const metricsMap = {};
    const validSaleStatuses = ['Aprovado', 'Preparando pedido', 'Saiu para entrega', 'Entregue', 'Concluído'];
    ...
    products.forEach(p => {
        ...
        // Se estiver selecionado, muda a cor de fundo
        ...
        // Adiciona "relative" e "group" para o swipe funcionar
        row.className = `relative overflow-hidden border-b border-gray-800 last:border-0 select-none`;

        // Lógica de Duplo Clique (Desktop/Mobile)
        row.ondblclick = () => editProduct(p.id);

        // --- CONTEÚDO DA LINHA ---
        // A estrutura agora tem 2 camadas: 
        // 1. Fundo (Botão Excluir)
        // 2. Frente (Dados do Produto) - Que desliza
        row.innerHTML = `
            ...
            <div class="relative z-10 p-3 transition-transform duration-200 ease-out prod-swipe-content ${bgClass} h-full flex flex-col md:grid md:grid-cols-12 gap-2 items-center">
                ...
                <div class="flex md:contents items-center justify-between w-full">
                    <div class="flex items-center gap-3 md:col-span-6 w-full">
                        ...
                        <div class="hidden md:flex ${state.isSelectionMode ? 'md:col-span-1' : 'md:col-span-1'} items-center justify-center text-gray-500 font-mono text-xs border-r border-gray-800 h-full">
                            ${codeStr}
                        </div>

                        <div class="flex items-center gap-3 ${state.isSelectionMode ? 'md:col-span-3' : 'md:col-span-4'} min-w-0 flex-1">
                            <img src="${imgUrl}" class="w-10 h-10 rounded object-cover border border-gray-700 bg-black">
                            <div class="flex flex-col min-w-0">
                                <div class="flex items-center">
                                    <span class="md:hidden text-[9px] bg-gray-800 text-gray-400 px-1 rounded mr-1">#${codeStr}</span>
                                    <span class="text-gray-200 font-bold text-sm truncate group-hover:text-yellow-500 transition">
                                        ${p.name}
                                    </span>
                                </div>
                ...
                <div class="hidden md:block col-span-2 text-center text-gray-500 text-xs font-mono">
                    ${lastMovStr}
                </div>

                <div class="hidden md:block col-span-1 text-center text-gray-400 text-xs">
                    ${metrics.qtd > 0 ? `<span class="bg-gray-800 px-2 py-0.5 rounded text-gray-300">${metrics.qtd}</span>` : '-'}
                </div>
