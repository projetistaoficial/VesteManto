...
window.renderAdminTopics = () => {
    ...
    // Pega a caixa do formulário (a primeira div logo abaixo do body)
    const formArea = document.querySelector('#topicos-body > div');
    ...
    // ✨ NOVO: Bloqueia ou libera o formulário visualmente
    if (formArea) {
        if (isEnabled) {
...
window.saveTopicAction = async () => {
    ...
    const content = descInput.value; // Preserva quebras de linha
...
window.openClientTopic = (index) => {
    ...
    // ✨ CORREÇÃO CRÍTICA: Usar textContent em vez de innerText!
    // Como a tela está com 'display: none' neste momento, o innerText falha ao tentar aplicar formatação.
    // O textContent salva o texto bruto (incluindo os "Enters") com perfeição.
    if (elContent) elContent.textContent = topic.content;

    // Mostra a tela
    showView('topic');
    
    // Fecha a barra lateral (se estiver no mobile)
    toggleSidebar(); 
