...
function renderSalesList(orders) {
    ...
    orders.forEach(o => {
        ...
        // --- CÁLCULO DO DESCONTO/CUPOM (NOVO) ---
        ...
        const valDesconto = Math.max(0, (subTotalItens + valFrete) - valTotal);

        let discountHtml = '';
        // Mostra se tiver um cupom salvo OU se houver diferença de valor (desconto de pix, etc)
        if (valDesconto > 0.05 || o.cupom) {
            const labelCupom = o.cupom ? `Cupom: <span class="uppercase tracking-wider text-white bg-green-900/50 px-1 rounded">${o.cupom}</span>` : 'Descontos / Pix';
            discountHtml = `
                <div class="flex justify-between text-xs text-green-500 font-bold mb-1 border-b border-gray-700/50 pb-1">
                    <span>${labelCupom}</span>
