...
// =================================================================
// ===============================================
// 1. SINCRONIZADOR VISUAL (Mata a Borda Branca)
// ===============================================
window.aplicarCoresCategorias = (catName) => {
    ...
    todosBotoes.forEach(btn => {
        ...
        if (isAtivo) {
            // Fica Rosa e remove o transparente
            btn.classList.remove('bg-white/10', 'text-gray-300', 'border-transparent', 'hover:bg-white/20', 'hover:text-white');
            btn.classList.add('bg-brand-pink/80', 'text-white', 'border-brand-pink');
        } else {
            // Remove o rosa e COLOCA A BORDA TRANSPARENTE (Isso tira a borda branca)
            btn.classList.remove('bg-brand-pink/80', 'text-white', 'border-brand-pink');
...
window.filterByCat = (catName) => {
    // Avisa o carrossel se deve pausar ou andar
    ...
    // Aplica as cores sem bug de borda
    window.aplicarCoresCategorias(catName);
};

// ===============================================
// 3. O DESENHISTA DE CATEGORIAS
// ===============================================
function renderCategories() {
    ...
    if (scrollContainer) {
        ...
        // Segurança anti-fantasma: se a categoria ativa foi deletada no admin, volta pra 'Todas'
        if (activeCat !== '') {
        ...
        // Desenha todas as categorias na fonte Black arredondada (como inativas de início)
        categoriasParaMostrar.forEach(c => {
            const safeName = c.name.replace(/'/g, "\\'");
            const hasSubs = state.categories.some(sub => sub.name.startsWith(c.name + ' - '));
            const coresBase = 'bg-white/10 text-gray-300 border border-transparent backdrop-blur-md hover:bg-white/20 hover:text-white';
