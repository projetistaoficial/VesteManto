...
// Substitua sua função filterAndRenderSales inteira por esta:
window.filterAndRenderSales = function(resetLimit = true) {
    if (!state.orders) return;

    // Se resetLimit for true (veio de um filtro de data/busca), volta para 50
    if (resetLimit) {
        salesRenderLimit = 50;
    }
    ...
    const searchTerm = document.getElementById('admin-search-prod')?.value.toLowerCase() || ''; // Ajuste o ID se seu input de busca for diferente
    
    // Pegamos os valores dos inputs de data
    ...
    let filtered = state.orders.filter(order => {
        ...
        if (match && searchTerm) {
            const searchStr = `${order.code || ''} ${order.customer?.name || ''} ${order.customer?.phone || ''}`.toLowerCase();
            if (!searchStr.includes(searchTerm)) match = false;
        }

        // Filtro de Data (Sua correção de fuso horário aplicada aqui)
    ...
    // 2. ATUALIZAR TOTAIS GLOBAIS (Requisito: Mostrar total de TUDO)
    // Calculamos sobre 'filtered', que contém TODOS os resultados do filtro, não só os 50 visíveis
    const totalCount = filtered.length;
    
    // Soma apenas pedidos confirmados/entregues para o valor financeiro (ajuste conforme sua regra de negócio)
    // Se quiser somar tudo independente do status, remova o filtro interno do reduce
    const totalValue = filtered.reduce((acc, curr) => {
         // Exemplo: Somar tudo exceto cancelados/reembolsados no valor total financeiro?
         // Se quiser somar TUDO que está na lista, use apenas: return acc + (parseFloat(curr.total) || 0);
         return acc + (parseFloat(curr.total) || 0);
    }, 0);

    // Atualiza os Painéis Superiores (Dashboard)
    if (els.dashTotalItems) els.dashTotalItems.innerText = totalCount;
    if (els.dashConfirmedCount) els.dashConfirmedCount.innerText = totalCount; // Ou filtro específico
    if (els.dashTotalValue) els.dashTotalValue.innerText = formatCurrency(totalValue);

    // Atualiza barra de resumo se existir
    const totalListadoEl = document.querySelector('.total-listado-valor'); // Se tiver esse elemento no HTML
    if (totalListadoEl) totalListadoEl.innerText = formatCurrency(totalValue);


    // 3. RENDERIZAÇÃO VISUAL (Limita a 50 itens)
    // É aqui que economizamos processamento do navegador
    ...
    // Renderiza apenas os itens visíveis (paginados)
    visibleItems.forEach(order => {
        // Usa sua função existente de criar o card HTML
    ...
    if (loadMoreContainer) {
        // Se a lista total filtrada for maior que o limite atual, mostra o botão
        if (filtered.length > salesRenderLimit) {
            loadMoreContainer.classList.remove('hidden');
            
            // Opcional: Atualizar texto do botão com quantos faltam
            const remaining = filtered.length - salesRenderLimit;
            const btn = document.getElementById('btn-load-more');
            if(btn) btn.innerHTML = `<i class="fas fa-plus mr-2"></i> Carregar mais (${remaining})`;
