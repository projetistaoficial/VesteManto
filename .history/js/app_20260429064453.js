...
window.toggleStockMode = () => {
    ...
    if (isGraded) {
        ...
        // O valor que você digitou no Estoque Geral permanecerá intacto!
...
window.saveProduct = async (e) => {
    ...
    try {
        ...
        // Mantém as caixinhas na memória, mesmo que você desative a chave principal
        let finalSizes = state.tempVariations ? [...state.tempVariations] : []; 

        // Validação Blindada
        if (isGraded) {
            // Se ligou a grade, NÃO PODE salvar se não tiver criado nenhum tamanho
            if (finalSizes.length === 0) {
                ...
                return; // Bloqueia o salvamento e não apaga nada!
            }
            finalStock = finalSizes.reduce((acc, val) => acc + parseInt(val.stock || 0), 0); 
        } else {
            // Se a grade estiver desligada, usa exclusivamente o que você digitou no Estoque Geral
...
window.editProduct = (id) => {
    ...
    // ✨ A GRANDE CORREÇÃO: SEMPRE preenche o Estoque Geral com o total do banco!
    // Assim, se você desativar a grade, o campo não estará vazio, ele mostrará o último total.
    document.getElementById('prod-stock').value = p.stock || 0;

    // Carrega os tamanhos se existirem no banco
    state.tempVariations = [];
    ...
    toggleStockMode(); // Aplica a visualização correta
