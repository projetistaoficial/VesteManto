...
function renderAdminCategoryList() {
    ...
    const buildHtml = (node, level = 0) => {
        ...
        keys.forEach(key => {
            ...
            const rowContent = `
                <div class="flex items-center justify-between p-3 rounded hover:bg-white/5 transition cursor-pointer group min-h-[60px]"
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        ${hasChildren && isMain ? 
                            `<div class="w-8 h-8 rounded-full bg-black/40 flex items-center justify-center border border-gray-600 transition-transform group-open:rotate-180 group-open:bg-yellow-500/20 group-open:border-yellow-500">
            ...
            if (hasChildren) {
                html += `
                    <details class="rounded border ${selectBorder} ${bgClass} overflow-hidden group transition-all duration-300">
