...
function setupDeliveryDependency() {
    const elOwn = document.getElementById('conf-own-delivery'); // Checkbox Entrega
    const elPayDel = document.getElementById('conf-pay-delivery-active'); // Checkbox Pagamento

    if (!elOwn || !elPayDel) return;

    // =================================================================
    // 1. MEXEU NA ENTREGA PRÓPRIA (O Mestre)
    // Regra: Se desligar aqui, TEM QUE desligar o pagamento junto.
    // =================================================================
    ...
    newOwn.addEventListener('change', async (e) => {
        ...
        // 1. Atualiza Memória da Entrega IMEDIATAMENTE (Evita bug de duplo clique)
        ...
        // --- LÓGICA DE DEPENDÊNCIA (O que você pediu) ---
        if (!isActive) {
            // Se ESTÁ desligando a entrega, verifica se o pagamento está ligado
            // Como estamos usando clone, pegamos a referência do elemento pagamento de novo
            const currentPayCheck = document.getElementById('conf-pay-delivery-active');
            
            if (currentPayCheck && currentPayCheck.checked) {
                // Segurança: O Online está ligado? Se não, impede tudo.
                const onlineOn = document.getElementById('conf-pay-online-active')?.checked;
                if (!onlineOn) {
                    showSystemModal("⚠️ AÇÃO BLOQUEADA:\n\nVocê não pode desligar a Entrega Própria pois o Pagamento Online já está desativado. O cliente ficaria sem opções.", "warning");
                ...
                // 1. Desliga o pagamento VISUALMENTE
                currentPayCheck.checked = false;
                
                // 2. Desliga o pagamento na MEMÓRIA
                ...
                // 3. Atualiza opacidade do grupo
                const group = document.getElementById('group-delivery-methods');
                if (group) group.classList.add('opacity-30', 'pointer-events-none');

                // 4. Exibe o aviso
                showSystemModal("ℹ️ Como você desativou a 'Entrega Própria', a opção de 'Pagamento na Entrega' também foi desativada.", "warning");
                
                // 5. Salva o pagamento alterado
    ...
    // Regra: Se desligar aqui, NÃO mexe na entrega.
    // =================================================================
    ...
    newPay.addEventListener('change', async (e) => {
        ...
        // Validação de Segurança (Pelo menos um ativo)
        ...
        // AQUI ESTÁ A MUDANÇA: 
        // Não existe mais o bloco "if (ownCheck) ownCheck.checked = false".
        // O pagamento muda sozinho. Se ficar OFF, a função applyCheckoutVisibility
        // já sabe que não deve mostrar a aba no checkout.
