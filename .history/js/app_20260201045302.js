...
function setupEventListeners() {
    ...
    // Botão Salvar Perfil (Dentro de setupEventListeners)
    const btnSaveProfile = document.getElementById('btn-save-profile');
    if (btnSaveProfile) {
        btnSaveProfile.onclick = saveStoreProfile;
    }
}
...
function fillProfileForm() {
    ...
    // 2. LOGÍSTICA (Correção do Carregamento)
    ...
    setVal('conf-card-rate', formatMoneyForInput(inst.rate)); // Usa formatação 0,00

    // Atualiza o select de "Sem Juros" dinamicamente
    if (typeof updateFreeInstallmentsSelect === 'function') updateFreeInstallmentsSelect();
    setVal('conf-card-free', inst.freeUntil);

    // Controle visual da opacidade do parcelamento
...
async function saveStoreProfile() {
    ...
    try {
        // 1. Captura segura dos elementos pelo ID
        const getValue = (id) => {
            const el = document.getElementById(id);
        ...
        const data = {
            ...
            whatsapp: getValue('conf-store-wpp').replace(/\D/g, ''),
            ...
            // LOGÍSTICA (CEP e RAIO) - Correção aqui
            ...
            // PARCELAMENTO - Correção aqui
            installments: {
                active: getChecked('conf-card-active'), // Garante que pega o check
        ...
        // 3. Mantém configurações que não estão neste formulário (para não apagar Pix/Entrega)
        ...
        // 5. Atualiza memória local e UI
        state.storeProfile = { ...state.storeProfile, ...data };
        
        // Se mudou o parcelamento, redesenha a vitrine para atualizar os cards
        renderCatalog(state.products);
        
        if(typeof renderStoreProfile === 'function') renderStoreProfile();

        showToast('Perfil e Configurações salvos!', 'success');
