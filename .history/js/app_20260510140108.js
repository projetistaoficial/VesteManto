...
window.aplicarCoresCategorias = (catName) => {
    ...
    todosBotoes.forEach(btn => {
        ...
        if (isAtivo) {
            // ESTADO: SELECIONADA
            // Fundo: Bem mais escuro (0.7) para criar contraste forte
            btn.style.backgroundColor = 'rgba(0, 0, 0, 0.7)';
            btn.style.borderColor = 'var(--clr-accent)';
            btn.style.color = 'var(--clr-accent)';
            btn.style.opacity = '1';
            btn.style.boxShadow = '0 4px 15px rgba(0,0,0,0.3)'; // Sombra um pouco mais forte
        } else {
            // ESTADO: INATIVA (As outras)
            // Fundo: Vidro levemente escurecido
...
// ===============================================
// 2. FUNÇÃO DO CLIQUE (Filtra, muda cor e fecha menu)
// ===============================================
window.filterByCat = (catName) => {
    ...
    // Fecha o dropdown flutuante (se estiver aberto no PC) ao clicar no nome
...
// ===============================================
// 3. RENDERIZADOR (Menu Exclusivo e Botões)
// ===============================================
    ...
    if (sidebarContainer) {
        ...
        const buildHtml = (node, level = 0) => {
            ...
            keys.forEach(key => {
                ...
                if (hasChildren) {
                    // ✨ TRUQUE AQUI: name="menu-nivel-${level}" transforma as details em um Accordion Nativo!
                    // Isso força o navegador a fechar a anterior automaticamente.
                    html += `
                        <details name="menu-nivel-${level}" class="group mb-1">
