...
// =================================================================
// 🔍 SELETOR DE PRODUTOS PARA FILTRO DE VENDAS (BLINDADO)
// =================================================================
...
window.selectProductForFilter = (productName) => {
    // Pega o elemento que a lógica de vendas usa para ler o filtro
    ...
    // ✨ BLINDAGEM: Injeta tanto o .value (para a lógica) quanto o .innerText (para a parte visual)
    if (filterInput) {
        filterInput.value = productName; 
        if (filterInput.tagName !== 'INPUT') filterInput.innerText = productName; 
    }
    
    if (displayInput) {
        displayInput.value = productName;
        if (displayInput.tagName !== 'INPUT') displayInput.innerText = productName;
    }

    // Fecha o modal antes de calcular
    window.closeProductSelectorModal();
    
    // Roda o filtro na tabela
    if (typeof filterAndRenderSales === 'function') filterAndRenderSales();
};

window.clearProductFilter = (e) => {
    if(e) e.stopPropagation(); // Evita que clicar em "Limpar" acabe abrindo o modal junto
    
    const filterInput = document.getElementById('filter-search-product-value');
    const displayInput = document.getElementById('filter-search-product');
    
    // ✨ BLINDAGEM: Zera a lógica e restaura o texto visual padrão
    if (filterInput) {
        filterInput.value = ''; 
        if (filterInput.tagName !== 'INPUT') filterInput.innerText = 'Filtrar por Produto';
    }
    
    if (displayInput) {
        displayInput.value = '';
        if (displayInput.tagName !== 'INPUT') displayInput.innerText = 'Filtrar por Produto';
    }

    // Limpa também a busca interna do modal para a próxima vez que abrir
