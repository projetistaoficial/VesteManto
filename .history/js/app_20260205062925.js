...
async function initApp() {
    ...
    // 1. TRAVA VISUAL IMEDIATA (Tela Preta)
    ...
    try {
        ...
        // 2. CHECAGEM OBRIGATÓRIA NO SERVIDOR
        // Tenta buscar online. Se falhar (excluído ou sem internet), vai pro catch/erro.
        const snap = await getDocFromServer(docRef);

        // A. CENÁRIO: NÃO EXISTE (EXCLUÍDO)
        if (!snap.exists()) {
            throw new Error("SITE_DELETED"); // Força a ir para o bloco de erro
        }
        ...
        if (data.status === 'bloqueado' || data.active === false) {
            throw new Error("SITE_BLOCKED"); // Força a ir para o bloco de erro
        }

        // === SUCESSO ===
        // Se chegou aqui, o site existe e está ativo no servidor.
        ...
        // 3. TELA DE MORTE (KILL SCREEN)
        // Se caiu aqui, seja por exclusão, bloqueio ou erro de rede na validação,
        // matamos a aplicação e mostramos o aviso. NÃO CARREGAMOS CACHE.
        
        document.body.style.opacity = "1";
        document.body.style.pointerEvents = "auto";
        
        let titulo = "Loja Indisponível";
        let msg = "Não foi possível carregar a loja.";
        let icon = "fa-exclamation-triangle";
        let color = "#ef4444"; // Vermelho

        if (error.message === "SITE_DELETED" || error.code === 'permission-denied') {
            titulo = "404 - Loja Não Encontrada";
            msg = "Esta loja foi encerrada ou excluída.";
            ...
            msg = "Esta loja está temporariamente bloqueada.";
            icon = "fa-lock";
        }

        document.body.innerHTML = `
            <div style="position:fixed; top:0; left:0; width:100%; height:100vh; display:flex; flex-direction:column; align-items:center; justify-content:center; background:#000; color:${color}; z-index:99999;">
                <i class="fas ${icon}" style="font-size:3rem; margin-bottom:20px;"></i>
                <h1 style="font-size:2rem; margin:0; text-align:center; font-family:sans-serif;">${titulo}</h1>
                <p style="color:#888; margin-top:10px; font-family:sans-serif;">${msg}</p>
                <button onclick="window.location.reload()" style="margin-top:30px; padding:10px 20px; border:1px solid #333; background:transparent; color:#fff; cursor:pointer; border-radius:5px;">Atualizar Página</button>
            </div>
        `;
        return; // <--- O CÓDIGO PARA AQUI DEFINITIVAMENTE.
    }

    // ============================================================
    // CARREGAMENTO DO SISTEMA (Só roda se o Try acima deu certo)
    // ============================================================
    ...
    // 4. CHECAGEM CONTÍNUA (Expulsa quem está com a aba aberta)
    setInterval(async () => {
        try {
    ...
    // Tema e Auth (Mantidos do seu código)
    if (localStorage.getItem('theme') === 'light') toggleTheme(false);

    onAuthStateChanged(auth, (user) => {
        state.user = user;
        const btnText = user ? 'Painel' : 'Área Admin';
        if (els.menuBtnAdmin) els.menuBtnAdmin.innerHTML = `<i class="fas fa-user-shield text-white transition"></i><span class="font-bold uppercase text-sm tracking-wide">${btnText}</span>`;
    ...
    setInterval(() => {
        if (state.coupons.length > 0 && !getEl('view-admin').classList.contains('hidden')) renderAdminCoupons();
    }, 10000);
