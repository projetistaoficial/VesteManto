...
function setupDeliveryDependency() {
    ...
    // Clona para limpar listeners antigos e evitar duplicação
    ...
    newOwn.addEventListener('change', async (e) => {
        ...
        // CORREÇÃO: Atualiza a memória IMEDIATAMENTE para o check não voltar
        ...
        if (!isActive) {
            ...
            if (elPayDel.checked) {
                // Verificação de segurança (não deixar sem nenhum pagamento)
                ...
                // Aplica mudança no pagamento
                const newPayEl = document.getElementById('conf-pay-delivery-active'); // Pega referência nova
                if (newPayEl) newPayEl.checked = false;
            ...
            const newPayEl = document.getElementById('conf-pay-delivery-active');
            if (newPayEl && !newPayEl.checked) {
                newPayEl.checked = true;
                if (state.storeProfile.paymentMethods?.delivery) {
    ...
    newPay.addEventListener('change', async (e) => {
        ...
        // CORREÇÃO: Atualiza a memória IMEDIATAMENTE
        if (state.storeProfile.paymentMethods?.delivery) {
            state.storeProfile.paymentMethods.delivery.active = isActive;
        }

        // Validação Mestre (Online deve estar ativo se este for desligado)
        if (!isActive) {
            const onlineOn = document.getElementById('conf-pay-online-active')?.checked;
            if (!onlineOn) {
        ...
        // Dependência Reversa (Se mexer no Pagamento, mexe na Entrega)
        const ownCheck = document.getElementById('conf-own-delivery'); // Pega referência nova
        ...
        if (ownCheck) {
            if (!isActive) {
                ...
                if (ownCheck.checked) {
                    ...
                    if (state.storeProfile.deliveryConfig) state.storeProfile.deliveryConfig.ownDelivery = false; // Memória
                ...
                if (!ownCheck.checked) {
                    ...
                    if (state.storeProfile.deliveryConfig) state.storeProfile.deliveryConfig.ownDelivery = true; // Memória
