...
// 2. Nova Função de Busca (Usa APENAS BrasilAPI V2 que é estável e sem CORS)
async function calculateDistance_Final(cepOrigin, cepDest) {
    ...
    const getCoords_v2 = async (cep) => {
        try {
            ...
            console.warn("Erro ao buscar coordenadas na BrasilAPI:", e);
            return null;
        }
    };
...
// 3. Listener do Campo CEP (handleCheckoutCep) - Substitui a lógica antiga
// ATENÇÃO: Esta função substitui o listener 'blur' do seu input de CEP.
window.handleCheckoutCep = async () => {
    const cepInput = document.getElementById('checkout-cep');
    if (!cepInput) return;
    ...
    if (elDistDisplay) {
        elDistDisplay.innerText = "Calculando...";
        elDistDisplay.className = "text-yellow-500 font-bold text-xs mt-1 block"; // Reseta classes
    }

    try {
        // A. Preenche endereço (ViaCEP é ótimo para texto de rua/bairro)
        const viaCepRes = await fetch(`https://viacep.com.br/ws/${cep}/json/`);
        const data = await viaCepRes.json();
        ...
        if (config.ownDelivery === true) {
            ...
            if (dist === null || isNaN(dist)) {
                ...
                // Libera venda mesmo sem cálculo exato
        ...
        if (elLoading) elLoading.classList.add('hidden');
        // Chama funções globais se existirem
