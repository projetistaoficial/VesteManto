...
function loadAdminSales() {
    // Usa o caminho correto do seu banco de dados
    ...
    onSnapshot(q, (snapshot) => {
        ...
        // Isso garante que o número seja real
...
// =================================================================
// FUNÇÃO QUE FALTAVA: ABRIR DETALHES E MARCAR COMO VISTO
// =================================================================
window.openAdminOrderDetail = async (order) => {
    // 1. Se o pedido for "NOVO", marca como visto no banco de dados
    // Isso fará a bolinha vermelha e o aviso "NOVO" sumirem automaticamente
    if (!order.viewed) {
        try {
            const orderRef = doc(db, `sites/${state.siteId}/sales`, order.id);
            // Atualiza sem travar a tela (o onSnapshot atualiza a UI depois)
            updateDoc(orderRef, { viewed: true }); 
        } catch (error) {
            console.error("Erro ao dar baixa na visualização:", error);
        }
    }

    // 2. Preenche os dados no Modal de Detalhes do Admin
    // Certifique-se de que esses IDs existem no seu HTML do modal '#modal-admin-order'
    ...
    const addressEl = document.getElementById('admin-client-address');
    ...
    if (nameEl) nameEl.innerText = order.customer?.name || 'Cliente';
    if (phoneEl) phoneEl.innerText = order.customer?.phone || '-';
    
    if (addressEl) {
        // Usa sua função auxiliar de formatação se disponível, senão texto simples
        addressEl.innerHTML = typeof formatarEnderecoAdmin === 'function' 
            ? formatarEnderecoAdmin(order.customer) 
    ...
    if (itemsEl) {
        itemsEl.innerHTML = order.items.map(i => `
            <div class="flex justify-between items-center py-2 border-b border-gray-700 last:border-0">
                <div class="text-sm">
                    <span class="font-bold text-yellow-500">${i.qty}x</span> 
                    <span class="text-gray-300">${i.name}</span>
                    ${i.size ? `<span class="text-xs text-gray-500">(${i.size})</span>` : ''}
                </div>
    ...
    // Configura o Select de Status para o status atual do pedido
    if (statusSelect) {
        statusSelect.value = order.status;
        // Salva o ID do pedido no botão de salvar/atualizar status para saber qual alterar
        statusSelect.setAttribute('onchange', `handleStatusChange(this, '${order.id}')`);
    }

    // Configura botões de ação (Cancelar/Finalizar) se existirem no modal
...
window.filterAndRenderSales = () => {
    // 1. Captura o local onde a lista será desenhada
    const list = document.getElementById('admin-sales-list');
    
    // Se não existir o local da lista, não tem o que fazer (provavelmente não está na tela de admin)
    if (!list) return;

    // 2. Captura Inputs de forma segura (sem travar se não existirem)
    ...
    // Valores Tratados (com fallback para vazio se o elemento não existir)
    ...
    // 3. Filtragem
    // Se state.orders estiver vazio ou indefinido, usa array vazio para não dar erro
    ...
    if (filtered.length === 0) {
        list.innerHTML = '<div class="text-gray-500 text-center py-8"><i class="fas fa-search text-2xl mb-2 opacity-50"></i><p>Nenhuma venda encontrada.</p></div>';
    } else {
        filtered.forEach(order => {
            ...
            div.onclick = () => {
                ...
                else console.error("Função openAdminOrderDetail não encontrada!");
            };
