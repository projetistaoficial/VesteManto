...
async function dangerousFactoryReset() {
    const code = prompt("☢️ ZONA NUCLEAR ☢️\n\nIsso apagará TUDO:\n- Produtos e Categorias\n- Todos os Pedidos e Clientes\n- Cupons e Estatísticas\n\nSua loja ficará VAZIA (como nova).\n\nPara confirmar, digite 'ZERAR LOJA':");
    ...
    try {
        const btn = document.activeElement;
        if(btn) {
            btn.innerText = "DESTRUINDO DADOS...";
            btn.disabled = true;
        }
        ...
        // 1. Apaga todas as coleções de dados
        await Promise.all([
            wipeCollection('products'),
            wipeCollection('categories'),
            wipeCollection('coupons'),
            wipeCollection('sales'),      // Pedidos
            wipeCollection('dailyStats')  // Estatísticas de visita/venda
        ]);

        // 2. Reseta os Contadores (Para o próximo produto/pedido ser #1)
        ...
        // 3. Limpeza Local
        localStorage.clear(); // Limpa carrinho e sessões locais

        alert("SUCESSO: A loja foi resetada para o estado inicial.");
        window.location.reload();
