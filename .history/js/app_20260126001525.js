...
function renderCatalog(productsToRender) {
    ...
    filtered.forEach(p => {
        ...
        // --- LÓGICA DE EXIBIÇÃO DO PIX ---
        let pixHtml = '';

        // 1. Verifica se não está tudo bloqueado
        if (!pixGlobal.disableAll) {
            
            // A) Regra Global Ativa (Prioridade Máxima)
            if (pixGlobal.active && pixGlobal.value > 0) {
                // Calcula o valor visualmente, mesmo que a regra seja aplicar no total
                const valDesconto = currentPrice * (pixGlobal.value / 100);
                const finalPix = currentPrice - valDesconto;
                
                // Exibe o valor calculado com um indicador sutil que é global
                pixHtml = `<p class="text-green-500 text-[10px] font-bold mt-1"><i class="fas fa-bolt mr-1"></i>${formatCurrency(finalPix)} no Pix <span class="opacity-60 font-normal">(Global)</span></p>`;
            }
            
            // B) Regra Individual (Fallback: Só se Global estiver OFF)
...
function setupEventListeners() {
    ...
    if (chkDisablePix) {
        chkDisablePix.addEventListener('change', (e) => {
            togglePixGlobalUI();
            autoSaveSettings('installments'); // Salva no grupo de pagamentos
        });
    }
    ...
    if (chkGlobalPix) {
        chkGlobalPix.addEventListener('change', (e) => {
            ...
            // --- MODAL DE AVISO (O que você pediu) ---
            if (isActive) {
                showSystemModal("⚠️ ATENÇÃO: PADRONIZAÇÃO ATIVA\n\nAo ativar esta opção, qualquer desconto Pix configurado individualmente dentro dos produtos será IGNORADO.\n\nValerá a regra global definida aqui.");
            }
    ...
    radiosPixMode.forEach(r => {
        r.addEventListener('change', () => autoSaveSettings('installments'));
    });
}
