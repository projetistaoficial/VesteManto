...
function setupEventListeners() {
    ...
    // --- CORREÇÃO 2: Removido 'window.' dos Botões Principais ---
    // Carrinho Desktop
    const btnCart = document.getElementById('cart-btn');
    if (btnCart) {
        btnCart.onclick = openCart; // <--- CORRIGIDO
    }

    // Carrinho Mobile
    const btnCartMob = document.getElementById('cart-btn-mobile');
    if (btnCartMob) {
        btnCartMob.onclick = openCart; // <--- CORRIGIDO
    }
    ...
    // Sidebar e UI Geral
    // --- CORREÇÃO 3: Removido 'window.' da Sidebar ---
    ...
    // --- CORREÇÃO 4: Removido 'window.' do Checkout ---
...
function startBackgroundListeners() {
    // PROTEÇÃO: Se a lista de pedidos não existir, não faz nada e evita o erro fatal
    if (!state.myOrders || !Array.isArray(state.myOrders) || state.myOrders.length === 0) {
        checkActiveOrders(); // Atualiza a bolinha mesmo sem pedidos
        return;
    }

    // Limpa ouvintes antigos para não duplicar
    if (window.activeListeners) {
        window.activeListeners.forEach(unsubscribe => unsubscribe());
    }
    window.activeListeners = [];

    // Inicia os novos ouvintes com segurança
    state.myOrders.forEach(localOrder => {
        if (!localOrder || !localOrder.id) return; // Pula itens inválidos
        ...
        const unsub = onSnapshot(doc(db, `sites/${state.siteId}/sales`, localOrder.id), (docSnap) => {
            if (docSnap.exists()) {
                ...
                if (index !== -1) {
                    ...
                    // Se a lista de pedidos estiver aberta, atualiza ela
            ...
            console.warn("Aviso silencioso no rastreio:", error);
        });
...
// ============================================================
// CONECTOR GLOBAL (NECESSÁRIO PARA O HTML ENXERGAR AS FUNÇÕES)
// ============================================================
// --- ZONA DE CONEXÃO GLOBAL (Necessária para HTML) ---
...
// Se o usuário já estiver logado, força o carregamento das estatísticas
if (state.user) {
    if (typeof loadAdminSales === 'function') loadAdminSales();
