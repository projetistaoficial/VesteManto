...
function setupEventListeners() {
    ...
    if (btnCart) {
        btnCart.onclick = openCart; // <--- CORRIGIDO: Chama função local
    }

    // Carrinho Mobile
    const btnCartMob = document.getElementById('cart-btn-mobile');
    if (btnCartMob) {
        btnCartMob.onclick = openCart; // <--- CORRIGIDO: Chama função local
    }
...
// --- GARANTIA FINAL (Cole no fim do arquivo) ---
window.updateStoreStatusUI = typeof updateStoreStatusUI !== 'undefined' ? updateStoreStatusUI : null;
window.goToCheckoutView = typeof goToCheckoutView !== 'undefined' ? goToCheckoutView : null;
window.submitOrder = typeof submitOrder !== 'undefined' ? submitOrder : null;