...
function loadSettings() {
    ...
    onSnapshot(docRef, (docSnap) => {
        if (docSnap.exists()) {
            // Se o arquivo existe (loja configurada), usa os dados de lá
            ...
            // O código antigo tentava usar "setDoc" aqui. ISSO TRAVA O SITE.
            // Agora, apenas usamos a memória do navegador.
            console.log("Settings não encontrados no banco. Usando memória temporária.");
            
            // Forçamos TRUE para que os produtos apareçam mesmo sem estoque/configuração
            state.globalSettings = { allowNoStock: true }; 
        }
        
        // Continua o carregamento normalmente
        if (typeof setupDeliveryDependency === 'function') setupDeliveryDependency();
        renderCatalog(state.products);

    }, (error) => {
        // Se der erro de permissão (comum quando a pasta settings não existe), cai aqui
        console.warn("Aviso: Não foi possível ler as configurações.", error);
        
        // Fallback de segurança para o site abrir
        state.globalSettings = { allowNoStock: true };
        renderCatalog(state.products);
    });
}
