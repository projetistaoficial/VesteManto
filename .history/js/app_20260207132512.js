import { 
    ...
    onSnapshot, query, where, orderBy, signInWithEmailAndPassword, 
    signOut, onAuthStateChanged, getDocsCheck, setDoc, getDocs, 
    getDoc, runTransaction, 
    getDocFromServer // Importado do seu config corrigido
...
// Elementos globais (ajuste conforme seu HTML real)
const els = {
...
async function initApp() {
    ...
    // TRAVA VISUAL: Começa invisível para não mostrar conteúdo antigo
    document.body.style.opacity = "0";
    document.body.style.pointerEvents = "none";

    if (!siteId) {
        showKillScreen("Loja não identificada", "URL inválida.");
        return;
    }
    ...
    try {
        console.log("🔒 Verificando integridade da loja...");
        const docRef = doc(db, "sites", siteId);
        
        // 2. CHECAGEM NO SERVIDOR (IGNORA CACHE)
        const snap = await getDocFromServer(docRef);

        // A. NÃO EXISTE (Excluído)
        if (!snap.exists()) {
        ...
        // ============================================================
        // 3. CARREGAMENTO DO SISTEMA (SÓ RODA AQUI DENTRO)
        // ============================================================
        console.log("✅ Loja verificada. Carregando sistema...");

        // Funções do seu sistema (adapte se os nomes forem diferentes)
        if(typeof loadSiteStats === 'function') loadSiteStats();
        if(typeof incrementVisitsCounter === 'function') incrementVisitsCounter();
        if(typeof loadSettings === 'function') loadSettings();
        if(typeof loadCategories === 'function') loadCategories();
        if(typeof loadProducts === 'function') loadProducts();
        
        // Renderiza perfil visual da loja
        if(typeof renderStoreProfile === 'function') renderStoreProfile();

        if(typeof loadCoupons === 'function') loadCoupons();
        if(typeof updateCartUI === 'function') updateCartUI();
        
        if(typeof startBackgroundListeners === 'function') startBackgroundListeners();
        if(typeof initStatsModule === 'function') initStatsModule();
        if(typeof loadTheme === 'function') loadTheme();

        // 4. MONITORAMENTO EM TEMPO REAL (Expulsa se excluir durante o uso)
        setInterval(async () => {
            try {
                const check = await getDocFromServer(docRef);
                if (!check.exists() || check.data().status === 'bloqueado') {
                    window.location.reload(); 
                }
            } catch(e) { /* Ignora erros de rede no background */ }
        }, 30000);
        ...
        // Limpa cache local para evitar fantasmas
        localStorage.removeItem('vestemanto_cart'); 
        localStorage.removeItem('site_orders_history');

        // Mostra tela de erro baseada no tipo
        document.body.style.opacity = "1"; // Mostra o corpo para exibir o erro
        
        if (error.message === "SITE_DELETED" || error.code === 'permission-denied') {
            showKillScreen("404 - Loja Não Encontrada", "Esta loja foi encerrada ou excluída.");
        } else if (error.message === "SITE_BLOCKED") {
            showKillScreen("Acesso Suspenso", "Esta loja está temporariamente indisponível.");
        } else {
            // Erro de conexão/técnico (Opcional: liberar cache se quiser modo offline)
            showKillScreen("Erro de Conexão", "Não foi possível verificar a loja. Verifique sua internet.");
        }
    }
}

// --- FUNÇÃO DE TELA DE MORTE (KILL SCREEN) ---
function showKillScreen(titulo, msg) {
    document.body.innerHTML = `
        <div style="position:fixed; top:0; left:0; width:100%; height:100vh; display:flex; flex-direction:column; align-items:center; justify-content:center; background:#000; color:#ef4444; z-index:99999; font-family:sans-serif;">
            <i class="fas fa-exclamation-circle" style="font-size:4rem; margin-bottom:20px;"></i>
            <h1 style="font-size:2rem; margin:0; text-align:center; color:white;">${titulo}</h1>
            <p style="color:#888; margin-top:10px; font-size:1.2rem; text-align:center; max-width:80%;">${msg}</p>
            <button onclick="window.location.reload()" style="margin-top:30px; padding:12px 25px; border:1px solid #333; background:#111; color:#fff; cursor:pointer; border-radius:8px; font-weight:bold;">
                Tentar Novamente
            </button>
        </div>
    `;
    // Garante que o body esteja visível para mostrar o erro
    document.body.style.opacity = "1";
    document.body.style.pointerEvents = "auto";
}
...
function setupAuthListener() {
    onAuthStateChanged(auth, (user) => {
        ...
        if (els.menuBtnAdmin) {
            els.menuBtnAdmin.innerHTML = `<i class="fas fa-user-shield"></i> <span class="font-bold text-sm ml-2">${btnText}</span>`;
        }
...
function setupCouponTimer() {
    setInterval(() => {
        const viewAdmin = getEl('view-admin');
        if (state.coupons.length > 0 && viewAdmin && !viewAdmin.classList.contains('hidden')) {
...
function checkActiveOrders() {
    ...
    // Se você tiver uma função que atualiza o ícone da moto, chame aqui
}
