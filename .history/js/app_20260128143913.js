...
function filterAndRenderSales() {
    ...
    let filtered = state.orders.filter(o => {
       ...
       // E. Pagamento (LÓGICA REFINADA)
        let matchPayment = true;
        if (payment) {
            // Normaliza para minúsculo para evitar erros de digitação/banco
            ...
            if (payment === 'pix') {
                ...
                // Procura por crédito, MAS ignora se tiver "débito" junto (caso legado)
                // OU aceita se for o legado explícito, dependendo da sua preferência.
                // Aqui vamos ser estritos: Crédito PURO.
                const isCredit = method.includes('crédito') || method.includes('credito') || method.includes('credit');
                const isMixed = method.includes('/') || (method.includes('crédito') && method.includes('débito'));
                
                // Se quiser mostrar o legado junto com crédito, remova o "&& !isMixed"
                matchPayment = isCredit; 
            } 
            else if (payment === 'debit') {
                // Procura por débito
                const isDebit = method.includes('débito') || method.includes('debito') || method.includes('debit');
                
                // O PULO DO GATO: Se tiver "crédito" ou barra "/" junto, NÃO é débito puro.
                // Isso remove o "Cartão Crédito/Débito" da lista de Débito.
                const isMixed = method.includes('crédito') || method.includes('credito') || method.includes('/');
                
                matchPayment = isDebit && !isMixed;
            } 
