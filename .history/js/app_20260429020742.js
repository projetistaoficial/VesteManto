...
window.openProductModal = (productId) => {
    ...
    // Apenas a barra de rolagem (sem gambiarras de imagem aqui)
    ...
    // Modal base com altura máxima de 90vh (não passa da tela do PC)
    ...
    // ✨ 1. COLUNA DA IMAGEM: A div que segura a foto ganha altura total e overflow escondido.
    if (imgCol) {
        imgCol.className = "w-full md:w-1/2 h-[40vh] md:h-full shrink-0 relative overflow-hidden bg-black";
    }

    // ✨ 2. A MAGIA ACONTECE AQUI: Pega a tag <img> em si e estica!
    const modalImg = document.getElementById('modal-img');
    if (modalImg) {
        // Remove classes velhas que possam limitar o tamanho
        modalImg.className = "w-full h-full object-cover object-top transition-opacity duration-300";
    }

    // Posicionamento das miniaturas flutuantes
    const thumbnailsContainer = document.getElementById('modal-thumbnails');
    if (thumbnailsContainer) {
        thumbnailsContainer.className = "absolute bottom-4 left-0 w-full flex justify-center z-10 gap-2 px-4";
    }
