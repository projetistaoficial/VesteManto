...
window.renderAdminTopics = () => {
    ...
    state.storeProfile.customTopics.forEach((topic, index) => {
        ...
        const titleColor = topic.titleColor || '#ffffff';
        ...
        listContainer.innerHTML += `
            <div class="flex items-center justify-between border rounded-md px-4 py-3 ${bgClass} transition-colors cursor-pointer group" ondblclick="editTopic(${index})">
                <div class="flex items-center gap-3 flex-1 min-w-0">
                    ...
                    <span class="font-bold text-sm truncate pr-2" style="color: ${titleColor};">${topic.title}</span>
...
window.checkTopicChanges = () => {
    ...
    const titleColor = document.getElementById('topico-cor-titulo').value; // COR DO TÍTULO CAPTURADA
    ...
    // Inclui a cor na validação do estado para ativar o botão de Salvar
...
window.handleTopicAction = async () => {
    ...
    const icon = document.getElementById('topico-icone').value;
    const titleColor = document.getElementById('topico-cor-titulo').value; // COR DO TÍTULO
    ...
    try {
        if (state.editingTopicIndex >= 0) {
            state.storeProfile.customTopics[state.editingTopicIndex] = { 
                ...
                content, 
                icon,
                titleColor // SALVA A COR NO BANCO
            };
        } else {
...
document.addEventListener('DOMContentLoaded', () => {
    const titleInput = document.getElementById('topico-titulo');
    const richDesc = document.getElementById('topico-descricao-rich');
    const colorTitle = document.getElementById('topico-cor-titulo'); 
    ...
    if (colorTitle) colorTitle.addEventListener('input', checkTopicChanges); 
...
// APLICAÇÃO DA COR NO CLIENTE (SIDEBAR E VITRINE)
window.renderSidebarTopics = () => {
    ...
    if (activeTopics.length > 0) {
        ...
        activeTopics.forEach((topic) => {
            ...
            // Aplica a cor também no menu lateral
            const tColor = topic.titleColor || '#ffffff';

            container.innerHTML += `
                <button onclick="openClientTopic(${originalIndex})" class="w-full text-left py-2.5 px-4 text-sm font-bold hover:bg-gray-800 rounded transition flex items-center gap-3" style="color: ${tColor};">
                    <i class="fas ${iconClass} w-5 text-center opacity-70"></i> ${topic.title}
                </button>
            `;
        });
    }
