...
window.handleCheckoutCep = async () => {
    ...
    // 1. BLOQUEIO INICIAL (Impede clicar enquanto calcula)
    if (btnFinish) {
    ...
    try {
        ...
        if (config.ownDelivery === true) {
            ...
            // CORREÇÃO CRÍTICA: Se a API falhar (retornar null ou NaN), lança erro
...
async function calculateDistanceByCEP(cepOrigin, cepDest) {
    // Função auxiliar para buscar coordenadas via BrasilAPI (V2)
    const getCoords = async (cep) => {
        try {
            const cleanCep = cep.replace(/\D/g, '');
            // Busca na BrasilAPI (V2 inclui coordenadas latitude/longitude)
            const url = `https://brasilapi.com.br/api/cep/v2/${cleanCep}`;
            const r = await fetch(url);
            
            if (!r.ok) throw new Error('CEP não encontrado na BrasilAPI');
            ...
            // Fallback silencioso (só usa se a BrasilAPI falhar)
            return getCoordsNominatim(cep);
        }
    };

    // Fallback antigo (Nominatim) - Só é acionado em emergência
    const getCoordsNominatim = async (c) => {
        try {
            // Removido header User-Agent que causava problemas em localhost
    ...
    // Executa as duas buscas em paralelo (Origem e Destino)
    const [c1, c2] = await Promise.all([getCoords(cepOrigin), getCoords(cepDest)]);

    // Se falhar em encontrar qualquer um dos dois, retorna null
