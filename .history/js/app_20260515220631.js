...
window.renderIconSelector = (selectedIcon = 'fa-file-alt') => {
    ...
    // Agora são botõezinhos redondos menores para caberem todos no espaço
    container.innerHTML = availableIcons.map(icon => {
        ...
        return `
            <button type="button" onclick="selectTopicIcon('${icon}')" class="w-6 h-6 rounded border flex items-center justify-center transition ${color} text-[10px]">
                <i class="fas ${icon}"></i>
            </button>
        `;
    }).join('');
