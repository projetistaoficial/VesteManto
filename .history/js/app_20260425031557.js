...
const startApplication = async () => {
    ...
    // ============================================================
    // 1. O SEGREDO: Salvar na memória ANTES do Firebase carregar
    // ============================================================
    if (window.location.search.includes('admin=true') || window.location.hash.includes('admin')) {
        // Grava no "Cofre" do navegador que o lojista quer logar
        sessionStorage.setItem('openAdminPanel', 'true');
        
        // Limpa a URL visualmente para o cliente não ver
        window.history.replaceState(null, '', window.location.pathname);
    }

    // 2. Tenta Validar o Acesso
    ...
    if (acessoPermitido) {
        ...
        // ============================================================
        // 3. O GATILHO INFALÍVEL (Abre o modal se o cofre permitir)
        // ============================================================
        if (sessionStorage.getItem('openAdminPanel') === 'true') {
            
            // Apaga a chave do cofre para não ficar abrindo sozinho ao dar F5
            sessionStorage.removeItem('openAdminPanel'); 
            
            setTimeout(() => {
                if (state.user) {
                    // Se o navegador já lembrar do lojista, entra no painel direto
                    if (typeof showView === 'function') showView('admin');
                } else {
                    // Força a abertura bruta do Modal de Senha
        ...
        console.log("⛔ Acesso Negado. Site permanece oculto.");
    }
};
