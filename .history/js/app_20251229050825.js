...
function renderAdminCategoryList() {
    ...
    // 1. Mapeia Nome -> ID para facilitar a busca na árvore
    // (Como a árvore é construída por nomes, precisamos recuperar o ID original para edição/exclusão)
    const catMap = {};
    state.categories.forEach(c => catMap[c.name] = c.id);

    // 2. Função para contar produtos (Inclui subcategorias)
    const getCount = (catName) => {
        return state.products.filter(p => 
            p.category === catName || (p.category && p.category.startsWith(catName + ' - '))
        ).length;
    };

    // 3. Monta a estrutura da Árvore
    const tree = {};
    // Ordena alfabeticamente antes de montar
    ...
    // 4. Função Recursiva de Renderização
    const buildHtml = (node, level = 0) => {
        let html = '';
        const keys = Object.keys(node).sort(); // Ordena chaves do nível atual

        keys.forEach(key => {
            const item = node[key];
            const hasChildren = Object.keys(item._children).length > 0;
            const fullPath = item._path;
            const id = catMap[fullPath]; // Recupera o ID do Firebase
            const count = getCount(fullPath); // Quantidade de produtos

            // ESTILIZAÇÃO
            // Nível 0 (Principal): Cinza mais claro, borda amarela se selecionado
            // Nível 1+ (Filho): Mais escuro, recuado, borda esquerda
            const isMain = level === 0;
            
            // Verifica se está selecionado para inserção (lógica antiga de "Adicionar em...")
            ...
            const bgClass = isMain 
                ? 'bg-[#1f2937] mb-2' // Cor Principal (Cinza Azulado)
                : 'bg-black/30 mt-1 ml-4 border-l-2 border-gray-700'; // Cor Filho (Escuro + Recuo)

            // Conteúdo da Linha (Nome, Contador, Botões)
            const rowContent = `
                <div class="flex items-center justify-between p-3 rounded hover:bg-white/5 transition cursor-pointer group"
                     onclick="selectParentCategory('${id}', '${fullPath}')">
                    
                    <div class="flex items-center gap-3">
                        ${hasChildren && isMain ? '<span class="text-yellow-500 text-xs transition-transform group-open:rotate-180">▼</span>' : ''}
                        
                        <div class="flex flex-col">
                            <span class="${isMain ? 'text-white font-bold text-sm' : 'text-gray-300 text-sm'}">
                                ${key}
                            </span>
            ...
            if (hasChildren) {
                // SE TEM FILHOS: Usa <details> (Acordeão)
                html += `
                    <details class="rounded border ${selectBorder} ${bgClass} overflow-hidden group">
                        <summary class="list-none select-none">
                            ${rowContent}
                        </summary>
                        <div class="pb-2 pr-2 border-t border-gray-700/50">
                            ${buildHtml(item._children, level + 1)}
                        </div>
                    </details>
                `;
            } else {
                // SE NÃO TEM FILHOS: Div simples
                html += `
...
// Helper pequeno para selecionar o pai ao clicar (mantendo a funcionalidade antiga de criar subcategoria)
window.selectParentCategory = (id, name) => {
    ...
    if (state.selectedCategoryParent === name) {
        ...
        // Pega apenas o último nome para exibir no placeholder
        const displayName = name.split(' - ').pop();
        if(els.newCatName) els.newCatName.placeholder = `Adicionar em: ${displayName} > ...`;
    }
    // Re-renderiza para atualizar as bordas amarelas
    renderAdminCategoryList();
};
