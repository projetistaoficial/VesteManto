...
async function autoSaveSettings(type) {
    ...
    try {
        if (type === 'installments') {
            ...
            // Tratamento especial para Taxa (troca vírgula por ponto)
            let rate = 0;
            if (elRate) {
        ...
        // Outros tipos (logística, etc) podem ficar aqui...
        // else if (type === 'logistics') { ... }

        // 4. Envia para o Firebase (Merge: true não apaga o resto)
        await setDoc(docRef, dataToUpdate, { merge: true });

        // 5. Atualiza a memória local (State)
        if (!state.storeProfile) state.storeProfile = {};
        state.storeProfile.installments = dataToUpdate.installments;

        // 6. Atualiza a Vitrine (para os cards exibirem "12x" imediatamente)
