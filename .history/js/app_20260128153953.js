...
window.openCheckoutModal = () => {
    ...
    // 2. APLICA VISIBILIDADE E SELEÇÃO (Configura o visual inicial)
    ...
    // 4. FORÇA A VALIDAÇÃO AGORA (O Segredo está aqui)
    // Como limpamos os campos lá em cima, essa função vai rodar, ver que está vazio
    // e aplicar o BLOQUEIO (opacity-50 e pointer-events-none) automaticamente.
    if (typeof validateCheckoutForm === 'function') {
        validateCheckoutForm();
    } else {
        // Fallback de segurança caso a função ainda não tenha carregado
...
// Função que TRAVA o pagamento até preencher tudo
function validateCheckoutForm() {
    // 1. Pega os valores
    const name = document.getElementById('checkout-name')?.value.trim();
    const phone = document.getElementById('checkout-phone')?.value.trim();
    ...
    // 3. Regra: Tem que ter Rua (via CEP), Número, Nome e Telefone
