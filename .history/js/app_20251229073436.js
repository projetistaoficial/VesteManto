...
function updateCartUI() {
    ...
    // 2. Calcula Subtotal dos Produtos
    let subtotal = 0;
    ...
    if (dConfig.shippingActive && dConfig.shippingValue > 0) {
        shippingCost = dConfig.shippingValue;
    }
    ...
    if (state.currentCoupon) {
        couponHTML = `
            <div class="bg-green-900/10 border border-green-500/30 p-3 rounded-lg flex justify-between items-center animate-fade-in">
                ...
                <button onclick="removeCoupon()" class="text-gray-500 hover:text-red-500 transition w-8 h-8 flex items-center justify-center">
                    <i class="fas fa-trash-alt text-xs"></i>
                </button>
            </div>`;
    } else {
        couponHTML = `
            <div class="relative flex gap-2">
                ...
                <input type="text" id="cart-coupon-input-dynamic" placeholder="CUPOM DE DESCONTO" 
                       class="bg-[#0f111a] border border-gray-700 text-white text-xs rounded-lg pl-9 pr-3 h-10 flex-1 outline-none focus:border-yellow-500 uppercase transition font-bold tracking-wide"
    ...
    summaryDiv.innerHTML = `
        ...
        <div class="space-y-1 pt-2">
            ...
            ${state.currentCoupon ? `
            <div class="flex justify-between text-green-500 text-xs font-bold">
                <span>Desconto</span>
                <span>- ${formatCurrency(discount)}</span>
            </div>` : ''}

            ${shippingCost > 0 ? `
            <div class="flex justify-between text-yellow-500 text-xs font-bold">
...
window.calcCheckoutTotal = () => {
    ...
    // Valor APENAS dos produtos com desconto
    ...
    if (method === 'pix') {
        ...
        const select = document.getElementById('checkout-installments');
        // Se tem parcelamento, o total já vem calculado (com juros e COM FRETE, pois atualizamos a função acima)
        if (select && select.options.length > 0) {
            const selectedOpt = select.options[select.selectedIndex];
            if (selectedOpt && selectedOpt.dataset.total) {
                // Aqui o frete já está embutido na parcela
                finalTotal = parseFloat(selectedOpt.dataset.total);
            }
        }
    }

    // Se não for cartão online (ex: Pix, Dinheiro, ou Entrega), soma o frete manualmente ao final
    if (!(method === 'card' && payMode === 'online')) {
    ...
    // Cria/Atualiza aviso visual de frete
    ...
    if (shippingFee > 0) {
        elShipDisplay.innerText = `+ Frete: ${formatCurrency(shippingFee)}`;
