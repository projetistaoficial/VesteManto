...
function setupEventListeners() {
  ...
  // Listener Ordenação
    ...
    const sortLabel = document.getElementById('sort-label-display');
    
    if (sortSelect) {
        sortSelect.addEventListener('change', (e) => {
            // Atualiza o texto do botão visualmente (ex: "Menor Preço")
            const text = e.target.options[e.target.selectedIndex].text;
            // Remove o emoji para ficar limpo no botão pequeno, se quiser
            if(sortLabel) sortLabel.innerText = text.replace(/^[^\w\sÁ-ú]+/, '').trim(); 
