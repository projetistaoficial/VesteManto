...
window.calcCheckoutTotal = () => {
    ...
    // Só calcula se o endereço/entrega for válido (checkoutState definido no app.js)
    if (typeof checkoutState !== 'undefined' && checkoutState.isValidDelivery && shipValue > 0) {
        if (shipRule === 'both') appliedShipping = shipValue;
        else if (shipRule === 'online' && payMode === 'online') appliedShipping = shipValue;
        else if (shipRule === 'delivery' && payMode === 'delivery') appliedShipping = shipValue;
    }
    ...
    // Valor base dos produtos com desconto do cupom
    ...
    if (method === 'pix') {
        ...
        // Reaplica cupom sobre novo total reduzido
    ...
    // B. CARTÃO DE CRÉDITO (ONLINE) - AQUI ESTAVA O ERRO
    // Mudamos de 'card' para 'credit'
    else if (method === 'credit' && payMode === 'online') {
        const select = document.getElementById('checkout-installments');

        // Se tem opção selecionada no dropdown, ela JÁ CONTÉM O FRETE + JUROS
        // (O frete é somado na função populateInstallments antes de calcular os juros)
        if (select && select.options.length > 0) {
    ...
    // C. DÉBITO, DINHEIRO OU CARTÃO NA ENTREGA
    else {
        finalTotal = productsTotal + appliedShipping;
    }
