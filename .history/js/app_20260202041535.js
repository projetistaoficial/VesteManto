...
async function loadStoreProfile() {
    try {
        // 1. CHECAGEM DE SEGURANÇA NO SERVIDOR (Pula o Cache Local)
        // Isso força o site a perguntar pro Firebase: "O site principal ainda existe?"
        const siteRef = doc(db, "sites", state.siteId);
        
        // A opção { source: 'server' } é vital! Ela ignora o cache do navegador e vai buscar a verdade no banco.
        // Se der erro de rede (offline), cai no catch e tenta usar o cache como fallback ou bloqueia.
        let siteSnap;
        try {
            siteSnap = await getDoc(siteRef, { source: 'server' });
        } catch (e) {
            // Se falhar ao conectar no servidor (ex: sem internet), tenta ler do cache para não deixar o cliente na mão
            siteSnap = await getDoc(siteRef);
        }

        // 2. SE O SITE FOI EXCLUÍDO (Não existe mais o doc pai)
        if (!siteSnap.exists()) {
            document.body.innerHTML = `
                <div class="flex flex-col items-center justify-center h-screen bg-gray-900 text-white text-center p-4">
                    <h1 class="text-6xl font-bold text-red-500 mb-4">404</h1>
                    <h2 class="text-2xl font-bold mb-2">Loja Não Encontrada</h2>
                    <p class="text-gray-400">Esta loja foi desativada ou não existe.</p>
                </div>
            `;
            // Lança um erro proposital para parar o resto do código
            throw new Error("SITE_DELETED");
        }

        const siteData = siteSnap.data();

        // 3. SE O SITE ESTIVER BLOQUEADO (Pausado pelo Master)
        if (siteData.active === false) {
            document.body.innerHTML = `
                <div class="flex flex-col items-center justify-center h-screen bg-gray-900 text-white text-center p-4">
                    <div class="text-6xl mb-4">🔒</div>
                    <h1 class="text-3xl font-bold text-white mb-2">Loja Temporariamente Indisponível</h1>
                    <p class="text-gray-400 max-w-md">
                        O acesso a esta loja está suspenso. Entre em contato com a administração.
                    </p>
                </div>
            `;
        ...
        // === SE PASSOU DA SEGURANÇA, CARREGA O RESTO ===

        // Ouve as configurações visuais (Cores, Logo, etc)
        ...
        onSnapshot(settingsRef, (docSnap) => {
            if (docSnap.exists()) {
                ...
                // Configuração padrão se o arquivo de settings ainda não foi criado
                state.storeProfile = { 
            ...
            // Força atualização da vitrine (para aplicar juros/regras novas)
        ...
        // Se foi um dos nossos erros de bloqueio, só ignora (já limpamos a tela)
        if (error.message === "SITE_DELETED" || error.message === "SITE_BLOCKED") return;
