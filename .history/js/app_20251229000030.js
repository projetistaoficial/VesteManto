...
function renderProductsList(products, preCalcMetrics = null) {
    ...
    // --- 1. BARRA DE CONTROLES SUPERIOR (Responsiva) ---
    ...
    // --- LÓGICA DA BARRA DE AÇÃO (Mobile Friendly) ---
    let bulkActionsHTML = '';
    if (state.isSelectionMode && state.selectedProducts.size > 0) {
        // Removi o 'hidden md:block' dos elementos para aparecerem no celular
        bulkActionsHTML = `
    ...
    products.forEach(p => {
        ...
        const bgClass = isChecked ? 'bg-blue-900/20 border-blue-500/30' : 'bg-[#151720] border-gray-800 hover:bg-[#1c1f2b]';
        const imgUrl = (p.images && p.images.length > 0) ? p.images[0] : 'https://placehold.co/100?text=Sem+Foto';
        const codeStr = p.code ? p.code : '-';

        // CORREÇÃO: Esconder o fundo vermelho se estiver selecionando
        // Se isSelectionMode for true, o fundo vermelho (lixeira) recebe 'hidden'
