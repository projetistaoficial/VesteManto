...
function renderStoreProfile() {
    ...
    // --- ATUALIZA A NAVBAR (LOGO CENTRALIZADA) ---
    ...
    if (navLogo && navText) {
        if (p.logo) {
            ...
            // Se não tem logo, exibe o nome em texto
    ...
    // --- ATUALIZA A TELA INICIAL (VITRINE) ---
    // (Opcional: Se quiser manter a logo grande na vitrine também, deixe este bloco. 
    // Se quiser tirar da vitrine e deixar só na navbar, pode apagar este bloco abaixo)
    ...
    if (homeLogo && homeTitle) {
        if (p.logo) {
            ...
            if (document.getElementById('home-store-name-text')) {
                document.getElementById('home-store-name-text').innerText = p.name || 'SUA LOJA';
            }
            homeTitle.classList.remove('hidden');
        }
    }

    // --- ATUALIZA SIDEBAR (LINKS SOCIAIS) ---
    const updateLink = (id, val, prefix = '') => {
        const el = document.getElementById(id);
        if (!el) return;
        if (val) {
    ...
    updateLink('sidebar-link-whatsapp', p.whatsapp, 'https://wa.me/'); 
    ...
    // Atualiza status (Bolinha Aberto/Fechado)
