...
window.openCheckoutModal = () => {
    ...
    // 2. RECUPERA CONFIGURAÇÕES (Com Logs de Debug)
    const pm = state.storeProfile.paymentMethods || {};
    const dConfig = state.storeProfile.deliveryConfig || {};

    // Verifica Financeiro
    // Se for undefined (não configurado), assume TRUE. Se for false, é false.
    ...
    console.log("Pagamento Online Ativo?", onlineActive);
    console.log("Pagamento Entrega Ativo?", deliveryActive);
    console.log("Logística Própria Ativa?", isLogisticsActive);
    ...
    // --- A. CONTROLE DE VISIBILIDADE (NUCLEAR) ---

    // 1. Opção ONLINE
    if (labelOnline) {
        if (onlineActive) {
            labelOnline.classList.remove('hidden');
            labelOnline.classList.add('flex'); // Devolve o Flex
            labelOnline.style.display = '';    // Limpa style inline
        } else {
            labelOnline.classList.add('hidden');
            labelOnline.classList.remove('flex'); // REMOVE O FLEX PARA NÃO DAR CONFLITO
            labelOnline.style.setProperty('display', 'none', 'important'); // Força bruta
        }
    } else {
        console.error("ERRO: Elemento 'label-pay-online' não encontrado no HTML.");
    }
    ...
    console.log("Mostrar Opção Entrega?", showDeliveryOption);
    ...
    if (containerDelivery) {
        if (showDeliveryOption) {
            ...
            // Como é uma div w-full, não costuma ter flex, mas garantimos:
            containerDelivery.style.setProperty('display', 'none', 'important');
        }
    } else {
        console.error("ERRO: Elemento 'container-delivery-option' não encontrado no HTML.");
    }
