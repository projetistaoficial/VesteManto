...
function filterAndRenderSales() {
    ...
    // 4. Renderiza (COM O VISUAL DE PEDIDO NOVO - ALTERADO AQUI)
...
window.openAdminOrderDetail = async (order) => {
    // 1. Lógica do Contador: Se for novo, marca como visto no Banco
    if (!order.viewed) {
        try {
            const ref = doc(db, `sites/${state.siteId}/sales`, order.id);
            updateDoc(ref, { viewed: true }); 
            // O firebase avisa o app automaticamente, removendo o "NOVO" e diminuindo o contador
        } catch (e) {
            console.error("Erro ao marcar visto:", e);
        }
    }

    // 2. Abre o Modal (Aqui você usa sua lógica atual de abrir modal)
    // Se você já tem código que abre o modal, mantenha-o. 
    // Exemplo genérico:
    const modal = document.getElementById('modal-admin-order');
    if (modal) {
        modal.classList.remove('hidden');
        // ... aqui você preenche os dados do modal (nome, itens, etc) ...
    }
};
