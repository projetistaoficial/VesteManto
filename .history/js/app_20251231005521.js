...
async function autoSaveSettings(type) {
    ...
    // 2. PARCELAMENTO E PAGAMENTO
        ...
        // Captura os valores dos novos toggles
    ...
    // Grava no Firebase e Atualiza Memória
    try {
        await setDoc(docRef, dataToUpdate, { merge: true });

        // --- ATUALIZAÇÃO IMEDIATA DO ESTADO (CORREÇÃO AQUI) ---
        if (state.storeProfile) {
            
            // ESSA É A PARTE QUE FALTAVA NO SEU CÓDIGO:
            if (dataToUpdate.paymentMethods) {
                state.storeProfile.paymentMethods = JSON.parse(JSON.stringify(dataToUpdate.paymentMethods));
            }
...
window.openCheckoutModal = () => {
    // 1. Limpeza
    ['checkout-cep', 'checkout-number', 'checkout-comp'].forEach(id => { const el = document.getElementById(id); if (el) el.value = ''; });
    ['address-details', 'delivery-error'].forEach(id => { const el = document.getElementById(id); if (el) el.classList.add('hidden'); });

    // 2. Leitura de Configuração (Segura)
    ...
    // Compatibilidade com a trava de logística
    const logisticsActive = state.storeProfile.deliveryConfig?.ownDelivery !== false;

    // 3. Referências HTML (Pelo ID que colocamos)
    ...
    if (labelOnline) {
        if (!onlineActive) {
            // Se DESATIVADO: Força sumir
            labelOnline.classList.remove('flex'); // Remove flex para não brigar
            labelOnline.classList.add('hidden');
            labelOnline.style.setProperty('display', 'none', 'important'); 
        } else {
            // Se ATIVADO: Mostra
            labelOnline.classList.remove('hidden');
            labelOnline.classList.add('flex');
            labelOnline.style.display = ''; // Remove o inline style
        }
    }

    // --- B. CONTROLE DE VISIBILIDADE DA ENTREGA ---
    // Só aparece se estiver ativado no financeiro E na logística
    ...
    // --- C. AUTO-SELEÇÃO DO RADIO (Para não ficar sem seleção) ---
    // Se o Online está visível, ele ganha o check.
    // Se o Online sumiu, mas Entrega está visível, Entrega ganha o check.
    if (onlineActive) {
