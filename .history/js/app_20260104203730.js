...
window.filterAndRenderSales = () => {
    // 1. Tenta achar a lista pelos dois IDs comuns para garantir que encontre
    const list = document.getElementById('admin-sales-list') || document.getElementById('orders-list');
    ...
    // Filtros (Com segurança contra null)
    const termCode = document.getElementById('filter-search-code')?.value.trim() || ''; 
    const termGeneral = document.getElementById('filter-search-general')?.value.toLowerCase().trim() || '';
    ...
    let filtered = state.orders.filter(o => {
        let matchCode = true;
        // Converte para string para a busca funcionar mesmo se for número
        ...
        return matchCode && matchGeneral && matchStatus && matchPayment;
    });
    ...
    // Renderização
    if (filtered.length === 0) {
        list.innerHTML = '<div class="text-gray-500 text-center py-8"><p>Nenhum pedido encontrado.</p></div>';
    } else {
        filtered.forEach(order => {
            // --- VISUAL DE PEDIDO NOVO ---
            const isNew = !order.viewed; 

            // Estilos: Borda Verde se for novo
            ...
            const div = document.createElement('div');
            div.className = `p-4 rounded-xl border ${borderClass} flex justify-between items-center group hover:bg-[#1a1c25] transition cursor-pointer mb-2 relative overflow-hidden`;
            
            // Clique abre detalhes
            div.onclick = () => {
                if (typeof openAdminOrderDetail === 'function') openAdminOrderDetail(order);
            };

            // Dados
            const dateDisplay = order.date ? new Date(order.date).toLocaleString() : '-';
            const totalDisplay = parseFloat(order.total || 0).toFixed(2);
            const customerDisplay = order.customer?.name || order.customerName || 'Cliente';
            
            // --- CORREÇÃO DO ERRO .slice ---
            // Forçamos "String(...)" para garantir que seja texto antes de usar .slice
            const rawCode = order.code || order.id || '';
            const idDisplay = String(rawCode).slice(0, 6); 

            div.innerHTML = `
                <div class="flex flex-col gap-1 relative z-10">
                    <div class="flex items-center">
                        <span class="font-bold text-white text-sm">#${idDisplay}</span>
                        ${badgeHtml}
                    </div>
    ...
    // Atualiza resumo colorido se existir
    if (typeof renderOrdersSummary === 'function') renderOrdersSummary(filtered);
};
