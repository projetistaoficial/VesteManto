        ...
        if (!pixGlobal.disableAll) {
            ...
            if (pixGlobal.active && pixGlobal.value > 0) {
                ...
                if (pixGlobal.mode === 'total') {
                    // MODO TOTAL: Exibe APENAS a etiqueta
                    pixHtml = `<p class="text-green-500 text-[10px] font-bold mt-1"><i class="fas fa-tag mr-1"></i>${labelOff} no Pix (Total)</p>`;
                } else {
                    // MODO PRODUTO: Calcula
                    let valDesconto = 0;
                    if (isFixed) valDesconto = pixGlobal.value;
                    else valDesconto = currentPrice * (pixGlobal.value / 100);
...
window.openProductModal = (productId) => {
    ...
    // --- LÓGICA DE EXIBIÇÃO PIX (CORRIGIDA) ---
    let pixHtml = '';
    ...
    if (!pixGlobal.disableAll) {
        ...
        if (pixGlobal.active && pixGlobal.value > 0) {
            ...
            // Cria os textos de exibição
            const labelOff = isFixed ? `R$ ${formatCurrency(pixGlobal.value)} OFF` : `${pixGlobal.value}% OFF`;
            const labelText = isFixed ? `R$ ${formatCurrency(pixGlobal.value)}` : `${pixGlobal.value}%`;
            ...
            if (pixGlobal.mode === 'total') {
                ...
                <div class="flex flex-col gap-1 mt-1">
                    <div class="flex items-center gap-2 text-sm text-gray-300">
                        ...
                        <span class="text-green-400 font-bold">${labelText} de Desconto</span>
                    </div>
                    <p class="text-[10px] text-gray-500 pl-6 italic">* Aplicado no valor total da venda.</p>
                </div>`;
            } else {
                // MODO PRODUTO: Calcula e exibe o preço com desconto
                let valDesconto = 0;
                ...
                <div class="flex items-center gap-2 text-sm text-gray-300 mt-1">
                    <i class="fab fa-pix text-green-400 text-lg"></i>
                    <span><b>${formatCurrency(finalPix)}</b> no Pix <span class="text-green-400 text-[10px] font-bold bg-green-900/30 px-1.5 py-0.5 rounded ml-1">${labelOff}</span></span>
                </div>`;
            }
