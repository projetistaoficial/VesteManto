...
let salesRenderLimit = 50; // Começa exibindo 50
...
window.filterAndRenderSales = function(resetLimit = true) {
    // Verificação de segurança: se as vendas não carregaram, para aqui.
    ...
    if (resetLimit) {
        salesRenderLimit = 50; // Reseta se for uma nova busca
    }

    // --- 2. PEGAR VALORES DOS FILTROS ---
    const searchInput = document.getElementById('admin-search-prod'); // Verifique se o ID é esse ou 'search-input'
    ...
    const statusFilter = document.getElementById('filter-order-status')?.value || 'Todos';
    ...
    let filtered = state.orders.filter(order => {
        ...
        // Filtro de Status
        if (statusFilter !== 'Todos' && order.status !== statusFilter) match = false;

        // Filtro de Texto (Nome, ID, Telefone)
        if (match && searchTerm) {
            // Garante que as propriedades existam para não dar erro
            const code = (order.code || order.id || '').toString().toLowerCase();
            const name = (order.customer?.name || '').toLowerCase();
            const phone = (order.customer?.phone || '').toLowerCase();
            const searchStr = `${code} ${name} ${phone}`;
            
            if (!searchStr.includes(searchTerm)) match = false;
        }

        // Filtro de Data
        if (match && (dateStartVal || dateEndVal)) {
            // Converte a data do pedido (timestamp ou string) para objeto Date
            const oDate = order.date && order.date.seconds 
                          ? new Date(order.date.seconds * 1000) 
                          : new Date(order.date);

            if (dateStartVal) {
                const [ano, mes, dia] = dateStartVal.split('-').map(Number);
                const s = new Date(ano, mes - 1, dia, 0, 0, 0, 0);
                if (oDate < s) match = false;
            }
            if (dateEndVal) {
                const [ano, mes, dia] = dateEndVal.split('-').map(Number);
                const e = new Date(ano, mes - 1, dia, 23, 59, 59, 999);
                if (oDate > e) match = false;
            }
        }
    ...
    // --- 4. ATUALIZAR TOTAIS (GLOBAL) ---
    // Calcula totais baseados em TUDO que foi filtrado (não só o que aparece na tela)
    const totalCount = filtered.length;
    const totalValue = filtered.reduce((acc, curr) => acc + (parseFloat(curr.total) || 0), 0);

    // Atualiza elementos da tela (se existirem)
    const elTotalCount = document.getElementById('orders-count'); // Título da lista
    const elDashItems = document.getElementById('dash-total-items'); // Dashboard
    const elDashValue = document.getElementById('dash-total-value'); // Dashboard
    
    if (elTotalCount) elTotalCount.innerText = totalCount;
    if (elDashItems) elDashItems.innerText = totalCount;
    if (elDashValue) elDashValue.innerText = totalValue.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });

    // --- 5. RENDERIZAÇÃO (PAGINADA) ---
    // Limpa a tela
    container.innerHTML = '';
    ...
    // Pega apenas a fatia (slice) baseada no limite atual (50, 100, 150...)
    const visibleItems = filtered.slice(0, salesRenderLimit);

    visibleItems.forEach(order => {
        // Chama a função createOrderCard (que vou fornecer abaixo)
        const cardHTML = createOrderCard(order);
        container.insertAdjacentHTML('beforeend', cardHTML);
    });

    // --- 6. CONTROLE DO BOTÃO CARREGAR MAIS ---
    if (loadMoreContainer) {
        if (filtered.length > salesRenderLimit) {
            loadMoreContainer.classList.remove('hidden');
            const remaining = filtered.length - salesRenderLimit;
            if (btnLoadMore) {
                btnLoadMore.innerHTML = `Carregar mais (+${remaining})`;
            }
        } else {
...
// Função auxiliar para desenhar o card do pedido
function createOrderCard(order) {
    // Formata a data
    let dateStr = 'Data inválida';
    if (order.date) {
        const d = order.date.seconds ? new Date(order.date.seconds * 1000) : new Date(order.date);
    ...
    let statusBg = 'bg-gray-800'; // Cor de fundo da badge (opcional)
    
    const st = (order.status || '').toLowerCase();
    if (st.includes('concluído') || st.includes('entregue')) statusColor = 'text-green-500';
    else if (st.includes('cancelado')) statusColor = 'text-red-500';
    else if (st.includes('pendente')) statusColor = 'text-yellow-500';
    else if (st.includes('reembolsado')) statusColor = 'text-purple-400';
    else if (st.includes('enviado')) statusColor = 'text-blue-400';

    // Se tiver ID curto ou Code, usa
    const displayId = order.code ? `#${order.code}` : `#${order.id.slice(0,6)}`;
    const customerName = order.customer?.name || 'Cliente';

    // HTML do Card (Estilo Acordeão)
    return `
    <li class="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden hover:border-gray-700 transition-colors duration-200">
        <div class="p-4 flex items-center justify-between cursor-pointer" 
             onclick="openAdminOrderDetail({id: '${order.id}', code: '${order.code || ''}', status: '${order.status}', customer: {name: '${customerName}', phone: '${order.customer?.phone||''}'}, total: '${order.total}', items: [] /* Simplificado para clique */ });">
            
            <div class="flex flex-col gap-1">
                <div class="flex items-center gap-3">
                    <span class="font-bold text-yellow-500 text-lg">${displayId}</span>
                    <span class="text-xs font-bold uppercase ${statusColor} border border-gray-700 px-2 py-0.5 rounded">
            ...
            <div class="flex items-center gap-4">
                ...
                <button class="text-gray-400 hover:text-white transition" onclick="openAdminOrderDetail(state.orders.find(o => o.id === '${order.id}'))">
   ...
   ['checkout-name', 'checkout-phone', 'checkout-number'].forEach(id => {
    ...
    if (btnLoadMore) {
        btnLoadMore.addEventListener('click', () => {
            salesRenderLimit += 50; // Aumenta o limite
            filterAndRenderSales(false); // Chama a renderização sem resetar filtros
        });
    }

    // IMPORTANTE: Nos seus filtros existentes (Data, Nome, Status), 
    // sempre que chamar filterAndRenderSales(), você deve resetar o limite.
    // Exemplo de como deve ficar o listener do input de busca:
    // searchInput.addEventListener('input', () => {
    //     salesRenderLimit = 50; // <--- Reseta para 50 ao pesquisar
    //     filterAndRenderSales();
    // });
}
