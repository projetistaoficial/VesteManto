...
window.openCheckoutModal = () => {
    // 1. Limpeza de campos (CEP, Erros, etc)
    ...
    // 2. LEITURA DA CONFIGURAÇÃO (Segura)
    // Se 'paymentMethods' não existir, cria um objeto vazio
    const pm = state.storeProfile.paymentMethods || {};
    
    // Verifica se está ATIVO. 
    // A lógica é: Se for estritamente 'false', está desligado. Qualquer outra coisa (undefined, null, true) considera ligado.
    const isOnlineEnabled = pm.online?.active !== false; 
    const isDeliveryEnabled = pm.delivery?.active !== false;

    // Compatibilidade com a configuração antiga de logística (opcional, mas recomendada)
    const oldOwnDelivery = state.storeProfile.deliveryConfig?.ownDelivery !== false;

    // 3. SELEÇÃO DOS ELEMENTOS HTML
    // Pega o botão de rádio "Online" e encontra o LABEL pai dele (que é o botão visual inteiro)
    const radioOnline = document.querySelector('input[name="pay-mode"][value="online"]');
    const labelOnline = radioOnline ? radioOnline.closest('label') : null;
    
    // Pega o container e o rádio da "Entrega"
    ...
    // 4. APLICAÇÃO DE VISIBILIDADE (FORÇA BRUTA)

    // --- Controla "Pagar Online" ---
    if (labelOnline) {
        if (!isOnlineEnabled) {
            // Desativa: Esconde e garante com CSS inline
            labelOnline.classList.add('hidden');
            labelOnline.style.setProperty('display', 'none', 'important');
        } else {
            // Ativa: Remove hidden e limpa o CSS inline para o flexbox voltar
            labelOnline.classList.remove('hidden');
            labelOnline.style.display = ''; 
        }
    }

    // --- Controla "Pagar na Entrega" ---
    // Só mostra se estiver ativado no financeiro E na logística
    const showDelivery = isDeliveryEnabled && oldOwnDelivery;
    ...
    // 5. AUTO-SELEÇÃO (Evita ficar sem opção marcada)
    // Se o Online está visível, marcamos ele.
    // Se não, e a Entrega está visível, marcamos ela.
    if (isOnlineEnabled && radioOnline) {
        radioOnline.checked = true;
    } else if (showDelivery && radioDelivery) {
        radioDelivery.checked = true;
    }

    // 6. Atualizações Finais de UI
    // Chama as funções para esconder/mostrar Pix/Cartão/Dinheiro baseado na escolha acima
    ...
    // 7. Navegação entre Telas (Esconde Carrinho -> Mostra Checkout)
    ...
    if (btnFinish) {
        ...
        btnFinish.disabled = true; // Desabilita botão até preencher CEP
    }
};
