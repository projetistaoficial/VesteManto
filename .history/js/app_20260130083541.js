...
function setupEventListeners() {
    ...
    // Usamos addEventListener e a função direta 'openCart'
    ...
    if (btnCartMob) {
        const newBtnMob = btnCartMob.cloneNode(true);
        btnCartMob.parentNode.replaceChild(newBtnMob, btnCartMob);
        newBtnMob.addEventListener('click', openCart);
    }

    // Botões dentro do carrinho
    const btnCloseCart = document.getElementById('close-cart'); // X para fechar
    ...
    // --- MENU LATERAL (CORREÇÃO CRÍTICA) ---
    const btnMob = document.getElementById('mobile-menu-btn');
    if (btnMob) btnMob.onclick = toggleSidebar;

    const btnCloseSide = document.getElementById('close-sidebar');
    if (btnCloseSide) btnCloseSide.onclick = toggleSidebar;

    const overlay = document.getElementById('sidebar-overlay');
    if (overlay) overlay.onclick = toggleSidebar;
    
    // Botão "Ir para Loja" no Admin
    const viewCatalogBtn = document.getElementById('view-catalog'); // Se tiver botão específico
    if (els.menuLinkHome) {
    ...
    if (btnTrack) {
        btnTrack.onclick = openTrackModal;
    }
    ...
    if (modalBackdrop) modalBackdrop.onclick = closeProductModal;

    // Fecha com ESC
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape') {
            if (!document.getElementById('image-viewer').classList.contains('hidden')) closeImageViewer();
            else if (!document.getElementById('product-modal').classList.contains('hidden')) closeProductModal();
            else if (!document.getElementById('cart-modal').classList.contains('hidden')) closeCartModal();
        }
    });

    // --- ADMIN / LOGIN ---
    const btnAdmin = document.getElementById('menu-btn-admin');
    if (btnAdmin) {
        btnAdmin.onclick = () => {
            toggleSidebar();
            if (state.user) showView('admin');
            else document.getElementById('login-modal').showModal();
        };
    }
    ...
    if (btnLoginSubmit) {
        btnLoginSubmit.onclick = async () => {
            ...
            // Login Suporte
            if (typeof checkAndActivateSupport === 'function' && checkAndActivateSupport(pass)) {
                modal.close();
                showView('admin'); // Vai pro admin, mas com flag de suporte
                showView('support'); // Ou vai direto pro suporte
                return;
            }
    ...
    if (btnLoginCancel) btnLoginCancel.onclick = () => document.getElementById('login-modal').close();

    const btnLogout = document.getElementById('btn-logout');
    if (btnLogout) btnLogout.onclick = () => signOut(auth);


    // --- PRODUTOS (FILTROS E AÇÕES) ---
    if (els.searchInput) {
        els.searchInput.addEventListener('input', (e) => {
            const term = e.target.value.toLowerCase();
            const filtered = state.products.filter(p => p.name.toLowerCase().includes(term) || (p.description && p.description.toLowerCase().includes(term)));
    ...
    // --- FORMULÁRIOS DE CONFIGURAÇÃO (LISTENERS DE UI) ---
    // Parcelamento UI
    const confCardActive = document.getElementById('conf-card-active');
    if (confCardActive) {
        confCardActive.addEventListener('change', (e) => {
    ...
    if (chkDisablePix) chkDisablePix.addEventListener('change', togglePixGlobalUI);
    if (chkGlobalPix) chkGlobalPix.addEventListener('change', (e) => {
        togglePixGlobalUI();
    ...
    if(btnBulkDel) {
         btnBulkDel.onclick = async () => {
            if(!confirm(`Excluir ${state.selectedProducts.size} itens?`)) return;
            // Lógica inline ou chamar função externa se existir
            // ... (Mantenha sua lógica original aqui se quiser, ou chame function)
            alert("Use a função individual por segurança enquanto testamos.");
         };
    }
