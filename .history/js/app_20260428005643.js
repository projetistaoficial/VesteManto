...
window.openProductModal = (productId) => {
    ...
    // ✨ CORREÇÃO 1: DESCRIÇÃO VISÍVEL NO MOBILE
    ...
    if (elDesc) {
        ...
        if (fullText.length > 150 || (fullText.match(/\n/g) || []).length > 2) {
            ...
            // O Segredo pro mobile: força o normal quando está cortado
            ...
            btnMore.onclick = () => {
                if (elDesc.style.webkitLineClamp === '3') {
                    ...
                    elDesc.style.whiteSpace = 'pre-line'; // Restaura as quebras de linha reais
                    ...
                    elDesc.style.maxHeight = 'unset';
                    elDesc.style.overflowY = 'hidden';
                    elDesc.style.whiteSpace = 'normal'; // Esconde as quebras pro box funcionar
                    elDesc.classList.remove('thin-scroll');
                    btnMore.innerText = 'Ver mais';
                    elDesc.scrollTop = 0;
                }
            ...
            elDesc.style.webkitLineClamp = 'unset';
            elDesc.style.whiteSpace = 'pre-line'; // Se for curto, quebra normalmente
        }
    }
    ...
    // ✨ CORREÇÃO 2: SCROLL SEGURO SEM DESTRUIR AS CLASSES DO MOBILE
    const rightCol = card.children[2];
    if (rightCol) {
        // Apenas adicionamos as classes necessárias, sem sobrescrever (className = ...)
        rightCol.classList.add('min-h-0', 'hide-scroll');
        if (rightCol.children[1]) {
            rightCol.children[1].classList.add('min-h-0');
        }
    }

    // ✨ CORREÇÃO 3: FORÇA A ORDEM CORRETA (TAMANHO EM CIMA DO BOTÃO)
    ...
    if (btnAdd && sizesWrapper && btnAdd.parentElement) {
        ...
        // Dá uma margenzinha pro botão não ficar colado
