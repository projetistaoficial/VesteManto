...
// VERSÃO FINAL: Sem orderBy (Não precisa criar índices manuais)
async function updateRealTotals() {
    ...
    // Pega os elementos do Dashboard Financeiro
    ...
    try {
        // Referência simples à coleção de vendas
        const salesRef = collection(db, `sites/${state.siteId}/sales`);

        // --- AQUI ESTÁ A CORREÇÃO ---
        // Removemos o 'query' e o 'orderBy'. 
        // Passamos 'salesRef' direto. O Firebase soma tudo sem exigir índice complexo.
        const snapshot = await getAggregateFromServer(salesRef, {
            totalPedidos: count(),
            faturamentoTotal: sum('total'),
            custoTotalAgregado: sum('totalCost')
        });

        const { totalPedidos, faturamentoTotal, custoTotalAgregado } = snapshot.data();

        // Cálculo do Lucro (Venda - Custo)
        const lucroLiquido = faturamentoTotal - custoTotalAgregado;

        // --- Atualiza o Cabeçalho (Topo da Lista) ---
        if (headerCount) {
            // Verifica se NÃO tem busca digitada. Se não tiver, mostra o totalzão (315).
            const searchInput = document.getElementById('filter-search-general');
            if (!searchInput || searchInput.value === '') {
        ...
        if (headerTotalValue) {
            ...
            // Remove o texto "Total Listado" se quiser, ou mude para "Total Global"
        ...
        if (dashTotalProfit) {
            ...
            // Pinta de verde se lucro, vermelho se prejuízo
            dashTotalProfit.className = lucroLiquido >= 0 
                ? "text-3xl font-bold text-green-500" 
                : "text-3xl font-bold text-red-500";
        }

        console.log("Totais atualizados (Sem índice):", totalPedidos, faturamentoTotal);

    } catch (error) {
        console.error("Erro ao buscar totais:", error);
        // Se ainda der erro, avisa na tela
        if (headerTotalValue) headerTotalValue.innerText = "Erro Conexão";
    }
}