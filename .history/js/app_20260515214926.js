...
function setupEventListeners() {
    ...
    // 2. OUVINTE DO BOTÃO DE AÇÃO (Novo / Cancelar / Salvar)
    const btnAcaoTopico = document.getElementById('btn-topico-acao');
    if (btnAcaoTopico) {
        const newBtn = btnAcaoTopico.cloneNode(true);
        btnAcaoTopico.parentNode.replaceChild(newBtn, btnAcaoTopico);
        newBtn.addEventListener('click', () => {
            if (typeof handleTopicAction === 'function') handleTopicAction();
        });
    }
