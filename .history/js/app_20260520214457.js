...
window.saveProduct = async (e) => {
    ...
    try {
        ...
        const nameExists = state.products.some(p => {
            // Se estamos editando, ignora o nome do PRÓPRIO produto para não bloquear o salvamento
            if (id && p.id === id) return false;
            // Compara os nomes ignorando maiúsculas/minúsculas e espaços vazios nas bordas
            return p.name.toLowerCase().trim() === prodName.toLowerCase();
        });

        if (nameExists) {
            showToast("Já existe um produto com este nome!", "error");
            // Devolve o botão ao estado normal e para a função
        ...
        if (!id) {
            ...
            // ✨ A CORREÇÃO: Verifica se o ouvinte em tempo real do Firebase já adicionou o produto
            const index = state.products.findIndex(p => p.id === docRef.id);
            if (index === -1) {
                // Se o radar ainda não agiu, nós empurramos para a tela
                state.products.push(productData);
            } else {
                // Se o radar já agiu, nós apenas atualizamos os dados para evitar duplicidade
