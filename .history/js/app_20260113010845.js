...
function updateCartUI() {
    ...
    // 1. Atualiza contadores (Bolinha vermelha)
    ...
    state.cart.forEach((item, index) => {
        ...
        // --- CORREÇÃO DA IMAGEM ---
        // 1. Busca o produto original no cadastro para garantir a imagem mais atual
        const originalProduct = state.products.find(p => p.id === item.id);
        
        let imgUrl = 'https://placehold.co/100?text=Sem+Foto'; // Placeholder padrão

        // Prioridade 1: Imagem do produto original (Cadastro)
        if (originalProduct && originalProduct.images && originalProduct.images.length > 0) {
            imgUrl = originalProduct.images[0];
        } 
        // Prioridade 2: Imagem salva no item do carrinho (caso produto tenha sido deletado)
        else if (item.image && item.image.length > 10) {
            imgUrl = item.image;
        }

        const imageHTML = `<img src="${imgUrl}" class="w-full h-full object-cover group-hover:scale-110 transition duration-500">`;

        const div = document.createElement('div');
        // Layout do Card melhorado
        div.className = "group flex items-start gap-4 bg-[#151720] p-3 rounded-xl border border-gray-800 shadow-sm relative overflow-hidden transition hover:border-gray-600 mb-3";
        
        div.innerHTML = `
            <div class="w-24 h-24 shrink-0 rounded-lg overflow-hidden border border-gray-700 bg-black relative shadow-lg">
                ${imageHTML}
            </div>

            <div class="flex-1 flex flex-col justify-between h-24 py-0.5">
                
                <div class="flex justify-between items-start">
                    <div class="pr-2">
                        <h4 class="text-white font-bold text-base leading-tight line-clamp-2">${item.name}</h4>
                        <div class="flex items-center gap-2 mt-1.5">
                            ${item.size !== 'U' ? `<span class="text-xs bg-gray-700 border border-gray-600 px-2 py-0.5 rounded text-white font-bold shadow-sm">${item.size}</span>` : ''}
                        </div>
                    </div>
                    
                    <button onclick="changeQty(${index}, -${item.qty})" class="text-gray-400 hover:text-red-500 transition p-2 -mr-2 -mt-2 rounded-full hover:bg-red-500/10">
                ...
                <div class="flex justify-between items-end mt-1">
                    <div class="flex flex-col">
                        <span class="text-[10px] text-gray-500 uppercase font-bold">Total Item</span>
                        <span class="text-yellow-400 font-extrabold font-mono text-base tracking-wide">
                            ${formatCurrency(item.price * item.qty)}
                        </span>
                    </div>

                    <div class="flex items-center bg-black border border-gray-600 rounded-lg h-9 shadow-inner">
                        <button onclick="changeQty(${index}, -1)" 
                            class="w-9 h-full flex items-center justify-center text-gray-300 hover:text-white hover:bg-gray-800 rounded-l-lg transition active:bg-gray-700 border-r border-gray-800">
                            <i class="fas fa-minus text-xs"></i>
                        </button>
                        
                        <span class="w-8 text-center text-white text-sm font-bold font-mono select-none">${item.qty}</span>
                        
                        <button onclick="changeQty(${index}, 1)" 
                            class="w-9 h-full flex items-center justify-center text-gray-300 hover:text-white hover:bg-gray-800 rounded-r-lg transition active:bg-gray-700 border-l border-gray-800">
    ...
    // 5. Renderiza Resumo e Cupom
    const summaryDiv = document.createElement('div');
    summaryDiv.className = "mt-6 pt-4 border-t border-dashed border-gray-700 space-y-4 pb-20 md:pb-4"; // pb-20 para não cortar no mobile

    // HTML do Cupom (Responsivo)
    let couponHTML = state.currentCoupon ?
        `<div class="bg-green-900/10 border border-green-500/30 p-3 rounded-lg flex justify-between items-center animate-fade-in shadow-lg">
            <div class="flex items-center gap-3">
            ...
            <button onclick="removeCoupon()" class="text-gray-500 hover:text-red-500 transition w-8 h-8 flex items-center justify-center hover:bg-red-900/20 rounded-full">
                <i class="fas fa-trash-alt text-xs"></i>
            </button>
        </div>` :
