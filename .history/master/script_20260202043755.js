...
// --- MODAL ---
// --- FUNÇÃO PARA ABRIR O MODAL E CONFIGURAR OS BOTÕES ---
async function openClientModal(docId = null) {
    currentDocId = docId; // GUARDA O ID GLOBALMENTE (Importante para o delete)
    ...
    if (docId) {
        ...
        if (client) {
            ...
            // Preenche dados (mantido do seu código anterior)
            ...
            // === LÓGICA DOS BOTÕES (AQUI É A NOVIDADE) ===
            ...
            if (client.status === 'bloqueado' || client.active === false) {
                ...
                // Configura botão verde "Desbloquear"
                btnReactivate.className = "w-full bg-green-600 hover:bg-green-500 text-white font-bold py-3 rounded transition shadow-lg shadow-green-900/50 flex justify-center items-center gap-2";
            ...
            console.error("Cliente não encontrado na lista local");
        }
    } else {
        // === MODO NOVO ===
        document.getElementById('modal-title').innerText = 'Novo Cliente';
        if (typeof getNextCode === 'function') document.getElementById('inp-id').value = getNextCode();
        ...
        // Novo cliente sempre nasce com botões de "Bloquear/Pausar" visíveis
        actionsActive.classList.remove('hidden');
        actionsInactive.classList.add('hidden');
...
// --- FUNÇÃO DE MUDAR STATUS (Bloquear, Pausar, ATIVAR) ---
async function changeClientStatus(action) {
    ...
    // Define o que vai ser gravado no banco
    if (action === 'block') {
        newStatus = 'bloqueado';
        isActive = false;
        actionText = "BLOQUEAR";
    ...
    try {
        ...
        await updateDoc(doc(db, "sites", currentDocId), { 
            active: isActive,   // Mantém compatibilidade
            status: newStatus   // Novo sistema de status
        });
        
        alert(`Sucesso! Status alterado para: ${newStatus.toUpperCase()}`);
        closeClientModal();
        loadClients(); // Recarrega a lista para atualizar as cores
    } catch (err) {
...
async function deleteCurrentClient() {
    ...
    if (!currentDocId) {
        alert("Erro: Nenhum cliente selecionado para exclusão.");
        return;
    }
    
    const confirmInput = prompt(`ATENÇÃO: EXCLUSÃO PERMANENTE!\n\nPara confirmar, digite o código da loja: ${currentDocId}`);
    
    if (confirmInput !== currentDocId) {
        alert("Código incorreto. A exclusão foi cancelada.");
        return;
    }

    try {
        // Deleta o documento principal em 'sites'
        await deleteDoc(doc(db, "sites", currentDocId));
        
        // Opcional: Aqui você poderia deletar as subcoleções também, 
        // mas o Firestore não faz isso nativamente via Client SDK de forma recursiva simples.
        // Como sua loja checa a existência do documento pai, apenas deletar o pai já "mata" a loja.
