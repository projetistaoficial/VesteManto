...
function renderClients(clients) {
    ...
    clients.forEach(client => {
        ...
        const code = client.code || '#'; // Código Numérico
        const displayId = client.docId;  // Slug (veste-manto)
        ...
        row.onclick = (e) => {
            ...
            if (isSelectionMode) {
                ...
                openClientModal(displayId); // Passamos o slug como ID
            }
        };
        ...
        row.innerHTML = `
            ...
            <div class="col-span-3 text-center">
                <a href="${fullLink}" target="_blank" class="link-btn truncate block w-full text-xs" onclick="event.stopPropagation()">
                    ${displayId}
                </a>
            </div>
...
async function openClientModal(docId = null) {
    ...
    // Resetar campos
    document.getElementById('inp-site-slug').disabled = false; // Habilita edição se for novo
    ...
    if (docId) {
        ...
        if (client) {
            ...
            document.getElementById('inp-site-slug').disabled = true; // Não muda slug depois de criado
            document.getElementById('preview-url').innerText = `${PRODUCTION_DOMAIN}?site=${client.docId}`;
            
            // Carrega dados extras (owner, status, etc)
...
async function changeClientStatus(action) {
    ...
    try {
        await updateDoc(doc(db, "sites", currentDocId), { active: isActive });
        closeClientModal();
        loadClients();
    } catch (e) { alert("Erro."); }
}
