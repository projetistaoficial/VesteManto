...
function renderClients(clients) {
    ...
    clients.forEach(client => {
        ...
        // Verifica qual código o usuário escolheu ver e mostra ele
        const displayCode = currentCodeDisplay === 'code' ? (client.code || '#') : (client.altCode || '#');

        const firstCol = isClientSelectionMode 
            ? `<div class="col-span-1 flex justify-center"><input type="checkbox" class="w-5 h-5 cursor-pointer pointer-events-none rounded border-gray-600 text-blue-500" ${isChecked}></div>` 
