...
async function deleteCurrentClient() {
    ...
    // 1. TRAVA DE SEGURANÇA (Evita erro 'indexOf null')
    const idParaDeletar = currentDocId; 

    const confirmacao = prompt(`DIGITE "DELETAR" para apagar permanentemente a loja: ${idParaDeletar}`);
    if (confirmacao !== "DELETAR") return alert("Cancelado.");

    try {
        console.log(`Iniciando exclusão de: ${idParaDeletar}`);
        
        // 2. APAGAR SUBCOLEÇÕES (PRODUTOS, VENDAS, ETC)
        const subcollections = ['products', 'categories', 'sales', 'orders', 'coupons', 'settings', 'dailyStats'];
        
        for (const sub of subcollections) {
            const snapshot = await getDocs(collection(db, `sites/${idParaDeletar}/${sub}`));
            if (!snapshot.empty) {
                const batch = writeBatch(db);
                snapshot.docs.forEach(doc => batch.delete(doc.ref));
                await batch.commit();
                console.log(`Limpou ${sub}`);
            }
        }

        // 3. APAGAR O DOCUMENTO DO SITE
        await deleteDoc(doc(db, "sites", idParaDeletar));

        // 4. LIMPAR VARIÁVEIS PARA NÃO RE-CRIAR
        currentDocId = null;
        
        alert("SITE EXCLUÍDO COM SUCESSO!");
        window.location.reload(); // Recarrega o painel para limpar tudo
