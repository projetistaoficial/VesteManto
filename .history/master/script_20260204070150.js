...
// --- VARIÁVEL GLOBAL PARA ARMAZENAR DADOS BRUTOS ---
// Isso evita ter que ir no banco de dados toda vez que filtra a data
let currentClientOrders = []; 

// --- CARREGAR FINANCEIRO (Busca do Banco) ---
async function loadFinancials(siteId) {
    // 1. Reseta a tela para "Carregando..."
    resetFinancialUI();

    try {
        // 2. Busca TODOS os pedidos da subcoleção 'orders'
        // (Firestore não cobra caro por leitura de texto, então trazemos tudo e filtramos no JS)
        const q = await getDocs(collection(db, `sites/${siteId}/orders`));
        
        currentClientOrders = []; // Limpa cache local
        
        q.forEach(doc => {
            // Guardamos os dados crus + a data formatada para JS
            const data = doc.data();
            currentClientOrders.push({
                ...data,
                // Tenta converter createdAt (seja string ISO ou Timestamp) para objeto Date
                dateObj: data.createdAt ? new Date(data.createdAt) : new Date() 
            });
        });

        // 3. Aplica o cálculo inicial (sem filtro de data, ou seja, TUDO)
