...
window.toggleLeadsBlock = () => {
    ...
    if (container.classList.contains('h-20')) {
        ...
        // Adiciona estado TELA CHEIA SÓ NA DIREITA (absolute no topo direito, 100% de altura)
        // Como o HTML já tem 'w-1/2', ele vai cobrir exatamente 50% da tela do lado direito!
        container.classList.add('absolute', 'top-0', 'right-0', 'h-full', 'z-[100]', 'border-l-2', 'border-yellow-500/50', 'shadow-[-15px_0_30px_rgba(0,0,0,0.6)]');
        arrow.classList.add('rotate-180');
    } else {
        // FECHA: Remove o estado grandão absoluto
        container.classList.remove('absolute', 'top-0', 'right-0', 'h-full', 'z-[100]', 'border-l-2', 'border-yellow-500/50', 'shadow-[-15px_0_30px_rgba(0,0,0,0.6)]');
        // Volta a ser só o cabeçalho encaixado no layout
        container.classList.add('h-20', 'border-gray-800');
        arrow.classList.remove('rotate-180');
    }
};

// Aba Ativa com Cores Vibrantes via Tailwind
window.setLeadTab = (tab) => {
    ...
    document.querySelectorAll('.lead-tab-btn').forEach(btn => {
        ...
        if (btn.dataset.tab === tab) {
            // Estilo ABA ATIVA (Rosa, sombreada e maior)
            btn.className = "lead-tab-btn px-4 py-2.5 rounded-lg text-xs font-bold uppercase transition-all flex items-center gap-2 bg-[#ea106b] text-white shadow-[0_0_15px_rgba(234,16,107,0.3)] transform scale-105 border border-transparent";
            if(span) span.className = "bg-white/20 text-white px-2 py-0.5 rounded-full count";
        } else {
            // Estilo ABA INATIVA (Escura e cinza)
            ...
            if(span) span.className = "bg-gray-800 text-gray-400 px-2 py-0.5 rounded-full count";
        }
    });
...
function atualizarContadoresLeads() {
    ...
    // Força a atualização da cor da aba atual
