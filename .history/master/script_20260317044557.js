...
window.confirmInvoicePayment = async () => {
    ...
    try {
        ...
        // ✨ NOVO: Busca os dados da fatura ATUAL antes de atualizar
        const snapFatura = await getDocFromServer(faturaRef);
        const dadosFaturaAtual = snapFatura.data();
        const dataVencimentoDestaFatura = dadosFaturaAtual.vencimento; 
        ...
        // 2. Gera a PRÓXIMA fatura usando a data de vencimento da que foi paga como base
...
// Calcula a próxima data baseada em Meses Calendário e cria nova fatura
async function gerarProximaFatura(clientId) {
    ...
    // Quebra a data 'YYYY-MM-DD' ignorando o relógio, para não dar erro de fuso horário
    const [ano, mes, dia] = ultimoVencimento.split('-').map(Number);
    let d = new Date(ano, mes - 1, dia); // O mês em JS começa no 0

    // Adiciona o período de forma inteligente
    if (periodoDias === 7) d.setDate(d.getDate() + 7);
    else if (periodoDias === 30) d.setMonth(d.getMonth() + 1); // Exatamente 1 mês pra frente
    else if (periodoDias === 180) d.setMonth(d.getMonth() + 6); // Exatamente 6 meses
    else if (periodoDias === 365) d.setFullYear(d.getFullYear() + 1); // Vira o ano perfeitamente
    else d.setDate(d.getDate() + periodoDias);

    // Formata a string local (Sem usar toISOString para não bugar a virada de ano)
    const y = d.getFullYear();
    const m = String(d.getMonth() + 1).padStart(2, '0');
    const day = String(d.getDate()).padStart(2, '0');
