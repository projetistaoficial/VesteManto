...
async function loadFinancials(siteId) {
    ...
    try {
        currentClientOrders = []; 
        currentClientProducts = []; // Zera os produtos para o novo cliente
        ...
        // Força a UI a começar exibindo "TUDO" e faz o cálculo inicial
...
function calculateAndRenderStats(startDate = null, endDate = null) {
    ...
    // (O capital de giro é independente de data de venda, é apenas o estoque atual)
    currentClientProducts.forEach(p => {
        const stock = parseInt(p.stock) || 0;
        if (stock > 0) {
            const price = parseFloat(p.promoPrice || p.price || 0);
            capitalGiro += (stock * price);
        }
    });
    ...
    currentClientOrders.forEach(order => {
        ...
        totalOrders++; // Contabiliza intenção de compra
...
window.setFinancialFilterType = (type) => {
    ...
    if (type === 'tudo') {
        ...
        // Esconde container de datas
        if(dateContainer) {
            dateContainer.classList.add('hidden');
            dateContainer.classList.remove('flex');
        }

        // Limpa os inputs de data e recalcula Geral
        ...
        // Mostra container de datas
        if(dateContainer) {
            dateContainer.classList.remove('hidden');
            dateContainer.classList.add('flex');
        }
        
        // Tenta aplicar o filtro se já houver algo preenchido
        applyFinancialFilter();
    }
};

window.applyFinancialFilter = () => {
    // Se o filtro "Tudo" estiver ativo (datas ocultas), não faz nada
    const dateContainer = document.getElementById('fin-date-container');
    ...
    // Só calcula por data se ambos os campos estiverem preenchidos
    if(!s || !e) {
        calculateAndRenderStats(null, null);
        return;
    }
    ...
    // Ajusta para pegar até 23:59:59 do último dia selecionado
