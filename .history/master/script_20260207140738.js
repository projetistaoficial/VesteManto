...
async function deleteCurrentClient() {
    ...
    const confirmacao = prompt(`ATENÇÃO: Isso apagará TODOS os produtos, pedidos e dados da loja "${currentDocId}".\nEssa ação é irreversível.\n\nDigite DELETAR para confirmar:`);
    
    if (confirmacao !== "DELETAR") return alert("Cancelado.");

    // Mostra que está trabalhando (pode demorar um pouco se tiver muitos produtos)
    const btnExcluir = document.querySelector('#action-buttons-container button:last-child'); // Ou use um ID específico se tiver
    if(btnExcluir) btnExcluir.innerText = "Apagando dados...";

    try {
        const siteRef = doc(db, "sites", currentDocId);
        
        // 1. APAGAR SUBCOLEÇÕES (Limpeza Nuclear)
        // Isso impede o site de "ressuscitar" com lixo antigo
        const subcollections = ['products', 'categories', 'orders', 'coupons'];
        
        for (const subColName of subcollections) {
            console.log(`Limpando ${subColName}...`);
            const subColRef = collection(db, `sites/${currentDocId}/${subColName}`);
            const snapshot = await getDocs(subColRef);
            
            // Apaga documento por documento
            const deletePromises = snapshot.docs.map(doc => deleteDoc(doc.ref));
            await Promise.all(deletePromises);
        }

        // 2. APAGAR O DOCUMENTO DO SITE
        await deleteDoc(siteRef);

        // 3. LIMPEZA DE MEMÓRIA (Crucial para não salvar produtos no limbo)
        const idDeletado = currentDocId;
        currentDocId = null; 
        
        // Reseta tudo visualmente
        document.querySelectorAll('#client-modal input').forEach(i => i.value = '');
        closeClientModal();
        await loadClients(); 

        alert(`Site '${idDeletado}' e todos os seus dados foram removidos do banco de dados.`);

    } catch (e) {
        console.error(e);
        alert("Erro ao excluir: " + e.message);
        if(btnExcluir) btnExcluir.innerText = "Tentar Novamente";
    }
}
