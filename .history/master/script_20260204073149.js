...
function renderClients(clients) {
    ...
    clients.forEach(client => {
        ...
        // Lógica de Status (Pausado > Bloqueado > Ativo)
...
// --- ABRIR MODAL (COM CORREÇÃO PARA DADOS ANTIGOS) ---
async function openClientModal(docId = null) {
    ...
    if (docId) {
        ...
        if (client) {
            ...
            // === LÓGICA HÍBRIDA (VELHO + NOVO) ===
            // Tenta ler do objeto novo (ownerData). Se não tiver, tenta ler da raiz (legacy).
...
// --- FINANCEIRO (BUSCA GLOBAL COM CORREÇÃO DE ERRO) ---
async function loadFinancials(siteId) {
    resetFinanceiroUI();
    
    // Tenta buscar no banco (agora vai funcionar se você arrumou as Regras no Firebase)
    try {
        // 1. Tenta subcoleção
        let ordersRef = collection(db, `sites/${siteId}/orders`);
        let snap = await getDocs(ordersRef);

        // 2. Se vazio, tenta global
        if (snap.empty) {
            // OBS: Isso requer índice composto ou regra permissiva. 
            // Se der erro de permissão aqui, é porque você não fez o PASSO 1 da resposta.
        ...
        // Não mostra erro na tela para não assustar, mas loga no console
...
// --- BOILERPLATE ---
// function resetFinancialUI() { ['fin-pedidos','fin-vendas'].forEach(i=>document.getElementById(i).innerText='0'); ['fin-faturamento','fin-custos','fin-lucro'].forEach(i=>document.getElementById(i).innerText='R$ 0,00'); }
