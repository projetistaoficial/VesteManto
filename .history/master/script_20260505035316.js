...
// ✨ CRIA UM APP SECUNDÁRIO: Permite criar contas sem deslogar o Master!
...
// =================================================================
// 📢 SISTEMA DE ENVIO EM MASSA (BROADCAST)
// =================================================================

window.openBroadcastModal = () => {
    document.getElementById('broadcast-message-text').value = '';
    document.getElementById('modal-broadcast-alert').showModal();
};

window.submitBroadcastAlert = async () => {
    const msg = document.getElementById('broadcast-message-text').value.trim();
    if (!msg) return alert("Digite a mensagem para todos os lojistas.");

    if (!confirm(`Confirmar envio para TODAS as lojas?\n\nEsta ação enviará a mensagem para ${allClients.length} lojistas.`)) return;
    ...
    try {
        ...
        // Criamos o objeto da mensagem
        const aviso = {
            mensagem: msg,
            data: new Date().toISOString(),
            lido: false,
            tipo: 'geral' // Identificador caso queira estilizar diferente na loja
        };

        // Percorremos a lista de todos os clientes carregada na memória (allClients)
        const promessas = allClients.map(async (cliente) => {
            try {
                // Adiciona na subcoleção 'avisos' de cada loja
                const avisosRef = collection(db, `sites/${cliente.docId}/avisos`);
                await addDoc(avisosRef, aviso);
                enviados++;
            } catch (err) {
                console.error(`Erro ao enviar para ${cliente.docId}:`, err);
            }
        });
        ...
        showToast(`Sucesso! Mensagem enviada para ${enviados} lojas.`);
        document.getElementById('modal-broadcast-alert').close();

    } catch (e) {
        console.error("Erro no broadcast:", e);
        alert("Erro ao disparar mensagens em massa.");
    } finally {
...
function renderLeads() {
    ...
    if (filtered.length === 0) {
        list.innerHTML = `<div class="text-center py-10 text-gray-500 text-sm italic">Nenhum lead em "${currentLeadTab}"</div>`;
        return;
    }

    // Aumentamos os paddings e os tamanhos dos textos (Nome, Loja, Faturamento)
    list.innerHTML = filtered.map(lead => `
        <div onclick="abrirDetalheLead('${lead.id}')" class="bg-[#161821] p-5 rounded-xl border ${lead.status === 'Novo' ? 'border-green-500/50 ring-1 ring-green-500/20' : 'border-gray-700'} hover:border-gray-500 transition cursor-pointer relative group">
            <div class="flex justify-between items-start mb-3">
                <span class="text-xs text-gray-500 font-bold uppercase">${new Date(lead.dataCadastro).toLocaleDateString('pt-BR')}</span>
                <button onclick="event.stopPropagation(); excluirLead('${lead.id}')" class="text-gray-600 hover:text-red-500 transition opacity-0 group-hover:opacity-100" title="Excluir Lead">
                    <i class="fas fa-trash-alt text-lg"></i>
                </button>
            </div>
            
            <h4 class="text-white font-extrabold text-xl tracking-wide">${lead.nome}</h4>
            <p class="text-yellow-500 text-sm font-bold uppercase mt-1">${lead.nomeLoja}</p>
            
            <div class="mt-4 flex justify-between items-center">
                <span class="text-gray-400 text-xs italic">${lead.segmento || 'Segmento n/i'}</span>
                <span class="text-white font-black text-sm bg-gray-900 px-3 py-1.5 rounded border border-gray-800">${lead.faturamento || ''}</span>
            </div>
        </div>
    `).join('');
}
...
window.excluirLead = async (id) => {
    if (confirm("⚠️ Tem certeza que deseja apagar permanentemente este lead?")) {
