...
async function openClientModal(docId = null) {
    ...
    if (docId) {
        ...
        if (client) {
            ...
            // DADOS DO DONO (Recuperando os dados antigos)
            ...
            // ENDEREÇO (Recuperando do banco)
            const address = client.address || {};
            document.getElementById('inp-cep').value = address.cep || '';
            document.getElementById('inp-rua').value = address.rua || '';
            document.getElementById('inp-numero').value = address.numero || '';
            document.getElementById('inp-complemento').value = address.complemento || '';
            document.getElementById('inp-bairro').value = address.bairro || '';
            document.getElementById('inp-cidade').value = address.cidade || '';
            document.getElementById('inp-uf').value = address.uf || '';
...
async function saveClientData() {
    ...
    // Objeto mestre com todos os dados empacotados corretamente
    const data = {
        name: name,
        ownerData: {
    ...
    // Apenas gera a data de criação e status se for um site NOVO
    if (!currentDocId) {
    ...
    try {
        // O { merge: true } garante que não apague nenhum histórico do cliente!
        await setDoc(doc(db, "sites", docId), data, { merge: true });
        
        // CHAMA A NOTIFICAÇÃO MODERNA
        showToast("Dados salvos com sucesso!");
        
        closeClientModal();
        loadClients(); // Atualiza a lista por trás
    } catch (e) { 
...
window.changeClientStatus = (action) => {
    ...
    showToast("Status alterado. Clique em SALVAR para confirmar a mudança.");
};
...
window.filterClients = (searchTerm = '') => {
    ...
    const filtered = allClients.filter(c => {
        ...
        const textMatch = term === '' || matchName || matchSlug || matchCode;
        ...
        // Só exibe se passar nas duas regras
