...
function renderClients(clients) {
    ...
    clients.forEach(client => {
        ...
        // --- GERADOR DAS DUAS URLs (CORRIGIDO E BLINDADO) ---
        const baseUrl = PRODUCTION_DOMAIN.endsWith('/') ? PRODUCTION_DOMAIN.slice(0, -1) : PRODUCTION_DOMAIN;
        
        // Em vez de jogar o ? direto após o nome, vamos garantir que funcione se for subdiretório:
        const linkVitrine = `${baseUrl}/index.html?site=${client.docId}`;
        const linkAdmin = `${baseUrl}/index.html?site=${client.docId}&admin=true`;
