...
// Gera a fatura do mês seguinte baseado na data da fatura paga
async function gerarProximaFatura(clientId, dataBase) {
    ...
    // Quebra a data da fatura paga (YYYY-MM-DD)
    const [ano, mes, dia] = dataBase.split('-').map(Number);
    let d = new Date(ano, mes - 1, dia); // Mês no JS começa em 0
    ...
    // Atualiza o cadastro do cliente com a NOVA data de vencimento
    await updateDoc(doc(db, "sites", clientId), {
        "plan.nextDue": novoVencimento,
        "manualOverride": false 
    });

    // Cria o registro da nova fatura pendente
    const novaFatura = {
    ...
    console.log(`✅ Sucesso: Fatura de ${dataBase} paga. Próxima gerada para ${novoVencimento}`);
}
