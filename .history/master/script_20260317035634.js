...
window.runLazyPenaltyCheck = async () => {
    ...
    try {
        ...
        for (const documento of snap.docs) {
            ...
            // 1. FOFOCA: Diz qual loja está olhando
            console.log(`🔎 Olhando loja: ${c.name || c.docId} | Status atual: '${c.status}' | Active: ${c.active}`);

            if (c.plan && c.plan.nextDue) {
                
                // 2. Ajustamos a regra: Pega quem for 'ativo', 'Ativo' ou tiver active: true
                if (c.status?.toLowerCase() === 'ativo' || c.active === true) {
                    ...
                    if (hoje > venc) {
                        ...
                        // 3. Força a leitura correta de verdadeiro/falso
                        ...
                        console.log(`   ⚠️ Dias Atrasado: ${diffDays} | Limite da Carência: ${carenciaDias}`);

                        if (diffDays > carenciaDias) {
                            const action = c.plan.carenciaAction || 'pausado';
                            ...
                            console.log(`   ✅ Salvo pela carência.`);
                        }
                    } else {
                        console.log(`   ✅ Vencimento está no futuro.`);
                    }
                } else {
                    console.log(`   ⏭️ Ignorado: Não está ativo no sistema ou já foi punido.`);
                }
            } else {
                console.log(`   ⏭️ Ignorado: Não tem plano cadastrado.`);
            }
        }
