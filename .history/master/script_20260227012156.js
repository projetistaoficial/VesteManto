...
function calculateAndRenderStats(startDate = null, endDate = null) {
    ...
    // LISTA DE STATUS QUE SOMAM DINHEIRO NO PAINEL (Híbrida: Novo + Antigo)
    const validStatuses = [
        'aguardando aprovação', // Novos
        'pendente',             // Antigo (Legado)
        'aprovado',             // Novos
        'confirmado',           // Antigo (Legado - É AQUI QUE ESTÃO SUAS 2 VENDAS!)
        'preparando pedido',    // Novos
        'saiu para entrega',    // Novos
        'entregue',             // Universal
        'concluído',            // Universal
        'concluido'             // Sem acento (Garantia)
    ];
    ...
    currentClientOrders.forEach(order => {
        ...
        totalOrders++; // Conta a quantidade total de pedidos na bolha
        const status = (order.status || '').toLowerCase().trim();

        // Se o status da venda estiver na lista de aprovados acima, soma o dinheiro
        if (validStatuses.includes(status)) {
            confirmedSales++; // Conta como venda efetuada
