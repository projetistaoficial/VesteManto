...
window.addEventListener('DOMContentLoaded', () => {
    ...
    window.copyToClipboard = copyToClipboard; // Expondo função de copiar
...
function renderClients(clients) {
    ...
    clients.forEach(client => {
        ...
        // Clique na linha abre o modal (exceto se clicar no link ou botão de copiar)
        row.onclick = (e) => {
        ...
        // --- CORREÇÃO AQUI: Link clicável + Botão Copiar ---
        row.innerHTML = `
            <div class="col-span-2 text-center text-white font-bold">${client.code || '#'}</div>
            <div class="col-span-4 font-bold text-white truncate text-sm">${client.name}</div>
            ...
            <div class="col-span-3 text-center flex items-center justify-center gap-2 bg-[#0f1014] border border-gray-700 rounded px-2 py-1">
                 <a href="${fullLink}" target="_blank" class="text-blue-400 text-[10px] truncate hover:underline hover:text-blue-300 w-full text-left">
                    vestemanto.../?site=${client.docId}
                 </a>
                 <button onclick="copyToClipboard('${fullLink}')" class="text-gray-500 hover:text-white transition" title="Copiar Link">
                    <i class="far fa-copy text-xs"></i>
                 </button>
            </div>

            <div class="col-span-2 text-center text-gray-400 text-xs">${client.plan?.name || '30 dias (Mensal)'}</div>
            <div class="col-span-1 text-center text-[10px] font-bold uppercase ${badgeColor}">${statusText}</div>
        `;
        listContainer.appendChild(row);
    });
}

// --- ABRIR MODAL (CORRIGIDO PREENCHIMENTO DO LINK) ---
async function openClientModal(docId = null) {
    ...
    if (docId) {
        ...
        if (client) {
            ...
            // --- CORREÇÃO AQUI: Preenche o link completo no modal ---
            const fullLink = `${PRODUCTION_DOMAIN}?site=${client.docId}`;
...
// --- GERAR LINK (CORRIGIDO ATUALIZAÇÃO DO CAMPO LINK) ---
window.generateSiteLink = () => {
    const name = document.getElementById('inp-name').value;
    if (!name) return alert("Digite o nome da loja primeiro!");
    ...
    // --- CORREÇÃO AQUI: Preenche também o link completo visualmente ---
    ...
    if (status === 'bloqueado' || active === false) {
        ...
        container.innerHTML = `
            <div class="flex gap-2">
                <button onclick="changeClientStatus('block')" class="bg-red-600 hover:bg-red-500 text-white px-4 py-3 rounded font-bold uppercase text-xs w-1/2 transition">Bloquear</button>
                <button onclick="changeClientStatus('pause')" class="bg-yellow-500 hover:bg-yellow-400 text-black px-4 py-3 rounded font-bold uppercase text-xs w-1/2 transition flex items-center justify-center gap-2">Pausar <i class="far fa-calendar-alt"></i></button>
            </div>
        `;
    }
}
...
async function saveClientData() {
    ...
    if (!slug || !name) return alert("Preencha o Nome e o Slug");
...
async function changeClientStatus(action) {
    ...
    try {
        ...
        loadClients(); // Atualiza a lista atrás
...
async function deleteCurrentClient() {
    if (!currentDocId) return;
    const confirmCode = prompt(`Para excluir, digite o código da loja: ${currentDocId}`);
    if(confirmCode !== currentDocId) return alert("Código incorreto.");
...
async function loadFinancials(siteId) {
    ...
    try {
        ...
        document.getElementById('fin-sales').innerText = total; // Ajuste conforme lógica de vendas confirmadas
