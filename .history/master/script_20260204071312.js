...
// --- CARREGAR FINANCEIRO (Busca do Banco) ---
// --- VERSÃO DE DIAGNÓSTICO (DEBUG) ---
async function loadFinancials(siteId) {
    // Limpa visual
    document.getElementById('fin-orders').innerText = "Verificando...";
    document.getElementById('fin-sales').innerText = "...";
    document.getElementById('fin-revenue').innerText = "...";

    console.group("🕵️‍♂️ DIAGNÓSTICO FINANCEIRO");
    console.log(`1. Buscando dados para o ID do Site: "${siteId}"`);

    try {
        // Tenta buscar na coleção 'orders'
        const ordersRef = collection(db, `sites/${siteId}/orders`);
        console.log(`2. Caminho da coleção: sites/${siteId}/orders`);
        
        const q = await getDocs(ordersRef);
        console.log(`3. Resultado da busca: ${q.empty ? "NENHUM documento encontrado ❌" : q.size + " documentos encontrados ✅"}`);

        if (q.empty) {
            console.warn("⚠️ AVISO: A coleção 'orders' está vazia ou não existe neste caminho.");
            console.log("Dica: Verifique no seu Firebase se os pedidos estão salvos em 'sites > ID_DO_SITE > orders'.");
            console.groupEnd();
            document.getElementById('fin-orders').innerText = "0 (Vazio)";
            return;
        }
        ...
        // Lista os primeiros pedidos para analisarmos a estrutura
        console.log("4. Analisando os primeiros pedidos encontrados:");

        q.forEach(doc => {
            const data = doc.data();
            totalOrders++;
            
            // Loga o primeiro pedido para você ver os campos exatos
            if (totalOrders === 1) {
                console.log(">>> ESTRUTURA DO PRIMEIRO PEDIDO:", data);
                console.log(" - Campo 'status':", data.status);
                console.log(" - Campo 'total':", data.total, "(Tipo:", typeof data.total, ")");
                console.log(" - Campo 'totalCost' ou 'cost':", data.totalCost || data.cost);
            }
            ...
            // Verifica status
            // Adicionei mais variações para garantir
            const validStatuses = ['approved', 'aprovado', 'paid', 'pago', 'delivered', 'entregue', 'shipped', 'enviado', 'completed', 'concluido'];
            ...
            if (validStatuses.includes(status)) {
                ...
                // Trata Valor (Receita)
                let val = 0;
                if (typeof data.total === 'number') val = data.total;
                else if (typeof data.total === 'string') {
                    // Remove R$, espaços e converte vírgula para ponto
                    val = parseFloat(data.total.replace(/[^\d,.-]/g, '').replace(',', '.')) || 0;
                }
                totalRevenue += val;
        ...
        console.log(` - Pedidos Totais: ${totalOrders}`);
        console.log(` - Vendas Confirmadas: ${confirmedSales}`);
        console.log(` - Receita Calculada: ${totalRevenue}`);
        console.log(` - Custos Calculados: ${totalCosts}`);
        console.groupEnd();

        // Atualiza UI
        document.getElementById('fin-orders').innerText = totalOrders;
        document.getElementById('fin-sales').innerText = confirmedSales;
        document.getElementById('fin-revenue').innerText = totalRevenue.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });
        document.getElementById('fin-costs').innerText = totalCosts.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });
        ...
        console.error("❌ ERRO FATAL AO BUSCAR:", e);
        alert("Erro de permissão ou conexão. Veja o console (F12).");
    }
}
