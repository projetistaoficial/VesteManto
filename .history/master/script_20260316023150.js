...
let currentCodeDisplay = 'code'; // Controla qual código aparece ('code' ou 'altCode')
let currentSortDirection = 'asc'; // Controla a ordem ('asc' = crescente, 'desc' = decrescente)
...
async function saveClientData() {
    ...
    // Se deixou em branco, mantém o antigo ou gera o próximo da fila
    ...
    const data = {
        name: name,
        altCode: inputAltCode, // <--- SALVA EXATAMENTE O QUE VOCÊ DIGITOU
    ...
    try {
        ...
        // CHAMA O MOTOR PARA EMPURRAR OS OUTROS SE NECESSÁRIO
...
async function deleteCurrentClient() {
    ...
    // Guarda o código de quem vai ser apagado para tampar o buraco depois
    ...
    try {
        ...
        // Puxa todo mundo pra baixo pra tampar o buraco
...
// --- BUSCA HÍBRIDA E ORDENAÇÃO DINÂMICA ---
window.filterClients = (searchTerm = '') => {
    ...
    filtered.sort((a, b) => {
        // Pega o valor dependendo do que está selecionado (Cód normal ou Alternativo)
        let valA = parseInt(a[currentCodeDisplay]) || 0;
        let valB = parseInt(b[currentCodeDisplay]) || 0;
        
        // Retorna crescente ou decrescente
...
window.bulkDeleteClients = async () => {
    ...
    try {
        ...
        allClients = allClients.filter(c => !selectedClients.has(c.docId));
...
async function shiftAltCodes(docIdToIgnore, oldCode, newCode) {
    let updates = [];
    // Pega todos os clientes, exceto o que estamos salvando/excluindo agora
    ...
    for (let c of others) {
        ...
        // 1. Se apagou ou moveu alguém de baixo, desce pra tampar o buraco
        if (oldCode !== null && currentCode > oldCode) {
            modifiedCode -= 1;
        }

        // 2. Se inseriu alguém aqui, empurra o resto pra cima
        if (newCode !== null && modifiedCode >= newCode) {
            modifiedCode += 1;
        }

        // Se o número dessa loja mudou na dança das cadeiras, atualiza no banco
        if (modifiedCode !== currentCode) {
            c.altCode = modifiedCode; // Atualiza na memória
...
// =================================================================
// FUNÇÕES DE ALTERNAR CÓDIGO E ORDENAÇÃO
// =================================================================
...
window.toggleSortDirection = () => {
    // Alterna a variável de direção
    currentSortDirection = currentSortDirection === 'asc' ? 'desc' : 'asc';
    
    // Muda o ícone (seta para baixo ou seta para cima)
