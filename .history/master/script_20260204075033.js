...
async function loadFinancials(siteId) {
    ...
    try {
        // Busca na subcoleção 'orders' dentro do site
        let ordersRef = collection(db, `sites/${siteId}/orders`);
        let snap = await getDocs(ordersRef);

        // Se vazio, tenta global (apenas se a subcoleção falhar)
        if (snap.empty) {
        ...
        document.getElementById('fin-revenue').innerText = "Erro Permissão";
    }
}
