...
// =================================================================
// ⚙️ MOTOR DE AUTOMAÇÃO E PUNIÇÕES (O "XERIFE")
// =================================================================

window.runLazyPenaltyCheck = async () => {
    console.log("🤠 [Xerife] Iniciando ronda de verificação de atrasos...");
    let sofreuAlteracao = false;

    try {
        const snap = await getDocs(collection(db, "sites"));
        
        // Usando um loop 'for...of' para garantir que ele espere cada atualização
        for (const documento of snap.docs) {
            const c = { docId: documento.id, ...documento.data() };
            
            // 1. Verifica se tem plano, vencimento e se ainda está 'ativo'
            if (c.plan && c.plan.nextDue && c.status === 'ativo') {
                ...
                if (hoje > venc) {
                    ...
                    // ✨ CORREÇÃO AQUI: Se a carência estiver desligada, o limite de dias é ZERO.
                    const carenciaAtiva = c.plan.carenciaActive === true;
                    ...
                    if (diffDays > carenciaDias) {
                        const action = c.plan.carenciaAction || 'pausado'; // pausar ou bloquear
                        
                        console.log(`🚨 [Xerife] Punindo: ${c.name} | Atraso: ${diffDays} dias | Carência Máxima: ${carenciaDias} dias | Punição: ${action.toUpperCase()}`);
                        ...
                        // Atualiza a lista local na memória para não precisar dar F5
        ...
        // Se o Xerife prendeu alguém, atualiza a tela automaticamente
        if (sofreuAlteracao) {
            console.log("🔄 [Xerife] Punições aplicadas! Atualizando a lista na tela...");
            // Chama a função que desenha os cards dos clientes no seu painel principal
            // (Assumindo que sua função de renderizar a tela inicial se chama renderClients)
        ...
        console.error("❌ [Xerife] Erro na ronda:", e);
    }
};

// Dispara o verificador automaticamente 2 segundos após você entrar no Painel Master
setTimeout(() => {
