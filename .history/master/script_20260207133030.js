...
async function deleteCurrentClient() {
    ...
    const confirmacao = prompt("Para confirmar a exclusão permanente, digite: DELETAR");
    if (confirmacao !== "DELETAR") return alert("Ação cancelada.");

    try {
        // EM VEZ DE DELETAR, MARCAMOS COMO 'EXCLUIDO'
        // Isso impede que ele ressuscite se você adicionar produtos sem querer
        ...
        alert("Cliente marcado como excluído!");
        closeClientModal();
        loadClients(); // Recarrega a lista para sumir ou mostrar vermelho
        
        // Limpa a memória para evitar salvar produtos nesse ID
        currentDocId = null; 
