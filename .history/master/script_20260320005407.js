...
async function openClientModal(docId = null) {
    ...
    if (docId) {
        ...
        document.getElementById('plan-next-due').value = `${y}-${m}-${day}`;
...
async function saveClientData() {
    ...
    try {
        ...
        // ✨ SINCRONIZADOR ABSOLUTO: Atualiza qualquer fatura pendente para ter os dados que você acabou de salvar no topo
        if (currentDocId) {
            ...
            pendentesSnap.forEach(async (docSnap) => {
...
window.editInvoiceValue = async (faturaId, valorAtual) => {
    ...
    try {
        // 1. Atualiza o valor na Fatura
        await updateDoc(doc(db, `sites/${currentDocId}/faturas`, faturaId), {
            valor: novoValor
        });
        
        // 2. ✨ SINCRONIZA o Plano lá no topo para acompanharem juntos
        await updateDoc(doc(db, "sites", currentDocId), {
            "plan.value": novoValor
        });
        document.getElementById('plan-value').value = formatForInput(novoValor);
