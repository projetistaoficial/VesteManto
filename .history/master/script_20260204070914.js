...
// --- CARREGAR FINANCEIRO (Versão Robusta) ---
async function loadFinancials(siteId) {
    ...
    try {
        console.log(`🔍 Buscando pedidos para o site: ${siteId}...`);
        
        // Busca na subcoleção 'orders' do site específico
        const q = await getDocs(collection(db, `sites/${siteId}/orders`));
        
        // Cache local para o filtro de data usar depois
        currentClientOrders = [];
        
        console.log(`📄 Encontrados ${q.size} documentos de pedidos.`);

        q.forEach(doc => {
            const data = doc.data();
            
            // Adiciona ao array local com tratamento de data
            let dateObj = new Date();
            if (data.createdAt) {
                // Tenta converter se for Timestamp do Firestore ou String ISO
        ...
        document.getElementById('fin-revenue').innerText = "Erro";
    }
}

// --- MOTOR DE CÁLCULO (Atualizado para aceitar Português) ---
function calculateAndRenderStats(startDate = null, endDate = null) {
    ...
    // Adicionei variações comuns em português e inglês, minúsculas
    const validStatuses = [
        'approved', 'aprovado', 
        'paid', 'pago', 
        'delivered', 'entregue', 
    ...
    currentClientOrders.forEach(order => {
        // 1. Filtro de Data (Se aplicável)
        if (startDate && endDate) {
            if (order.dateObj < startDate || order.dateObj > endDate) return;
        }

        // 2. Contagem de Pedidos (Qualquer status conta como pedido feito)
        totalOrders++;

        // Normaliza o status para minúsculo para comparar
        ...
        if (validStatuses.includes(orderStatus)) {
            ...
            // --- TRATAMENTO DE VALORES (AQUI É O SEGREDO) ---
            
            // Receita (Total do Pedido)
            let revenueVal = 0;
            if (typeof order.total === 'number') {
                revenueVal = order.total;
            } else if (typeof order.total === 'string') {
                // Remove "R$", espaços e troca vírgula por ponto
                revenueVal = parseFloat(order.total.replace('R$', '').replace(/\./g, '').replace(',', '.').trim()) || 0;
            }
            totalRevenue += revenueVal;

            // Custos (Custo do Produto + Taxas)
            let costVal = 0;
            // Verifica se existe o campo de custo no pedido
            if (order.totalCost || order.cost) {
                const rawCost = order.totalCost || order.cost;
                if (typeof rawCost === 'number') {
                    costVal = rawCost;
                } else if (typeof rawCost === 'string') {
                    costVal = parseFloat(rawCost.replace('R$', '').replace(/\./g, '').replace(',', '.').trim()) || 0;
                }
            }
            totalCosts += costVal;
        } else {
            // Debug: Se o status não for aceito, avisa no console
            console.warn(`⚠️ Pedido ${order.id} ignorado. Status: "${orderStatus}" não está na lista de aprovados.`);
        }
    });
    ...
    // Cor do Lucro
    if (profit >= 0) {
        profitEl.className = "text-xl font-bold text-green-400";
    } else {
        profitEl.className = "text-xl font-bold text-red-400";
    }
}
