    ...
    clients.forEach(client => {
        ...
        row.innerHTML = `
            ...
            <div class="col-span-3 text-center">
                 <span class="text-gray-500 text-[10px] bg-black px-2 py-1 rounded border border-gray-800">
                    vestemanto...
                 </span>
            </div>
            <div class="col-span-3 text-center text-[10px] font-bold uppercase ${badgeColor}">${statusText}</div>
        `;
        listContainer.appendChild(row);
    });
}

// --- ABRIR MODAL (Dentro da div esquerda) ---
async function openClientModal(docId = null) {
    currentDocId = docId;
    
    // Animação de entrada (remove o translate-y)
    ...
    if (docId) {
        ...
        if (client) {
            ...
            // loadFinancials(client.docId); // Implementar se tiver orders
        }
    } else {
        // NOVO
...
window.closeClientModal = () => {
    // Animação de saída (adiciona o translate-y)
    ...
    if (status === 'bloqueado' || active === false) {
        ...
        container.innerHTML = `
            <div class="flex gap-2">
                ...
                <button onclick="changeClientStatus('pause')" class="bg-yellow-500 hover:bg-yellow-400 text-black px-4 py-3 rounded font-bold uppercase text-xs w-1/2 transition">Pausar</button>
            </div>
        `;
    }
}
