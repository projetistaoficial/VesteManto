...
async function deleteCurrentClient() {
    ...
    const confirmacao = prompt(`ATENÇÃO: EXCLUSÃO TOTAL!\n\nIsso apagará a loja "${currentDocId}" e TODOS os seus produtos, vendas e clientes.\n\nDigite DELETAR para confirmar:`);
    ...
    if(btnDelete) btnDelete.innerText = "Apagando dados... (Aguarde)";

    try {
        console.log(` iniciando protocolo de exclusão para: ${currentDocId}`);

        // 2. LISTA DE TODAS AS SUBCOLEÇÕES DO SEU SISTEMA
        // Se você tiver criado outras, adicione os nomes aqui
        const subcollections = [
            'products', 
            'categories', 
            'sales',       // ou 'orders', dependendo de como você nomeou no banco
            'orders',      // coloco os dois por garantia
            'coupons', 
            'settings',
            'dailyStats'
        ];

        // 3. APAGA AS SUBCOLEÇÕES (ITEM POR ITEM)
        for (const subColName of subcollections) {
            const colRef = collection(db, `sites/${currentDocId}/${subColName}`);
            const snapshot = await getDocs(colRef);

            if (!snapshot.empty) {
                console.log(`Apagando ${snapshot.size} itens de ${subColName}...`);
                
                // Cria um lote de promessas para apagar rápido
                const deletePromises = snapshot.docs.map(doc => deleteDoc(doc.ref));
                await Promise.all(deletePromises);
            }
        }

        // 4. AGORA SIM, APAGA O DOCUMENTO DO SITE
        console.log("Apagando documento principal...");
        await deleteDoc(doc(db, "sites", currentDocId));

        // 5. LIMPA A MEMÓRIA DO PAINEL
        // Isso impede que o painel tente salvar algo num site que não existe mais
        ...
        alert(`SUCESSO: A loja '${idDeletado}' foi totalmente removida.`);
