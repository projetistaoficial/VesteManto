...
function renderLeads() {
    ...
    if (filtered.length === 0) {
        list.innerHTML = `<div class="text-center py-10 text-gray-600 text-xs italic">Nenhum lead em "${currentLeadTab}"</div>`;
        return;
    }

    list.innerHTML = filtered.map(lead => `
        <div onclick="abrirDetalheLead('${lead.id}')" class="bg-[#161821] p-4 rounded-xl border ${lead.status === 'Novo' ? 'border-green-500/50 ring-1 ring-green-500/20' : 'border-gray-800'} hover:border-gray-600 transition cursor-pointer relative group">
            <div class="flex justify-between items-start mb-2">
                <span class="text-[10px] text-gray-500 font-bold uppercase">${new Date(lead.dataCadastro).toLocaleDateString('pt-BR')}</span>
                <button onclick="event.stopPropagation(); excluirLead('${lead.id}')" class="text-gray-700 hover:text-red-500 transition opacity-0 group-hover:opacity-100"><i class="fas fa-trash-alt"></i></button>
            </div>
            <h4 class="text-white font-bold text-sm">${lead.nome}</h4>
            <p class="text-yellow-500 text-[10px] font-bold uppercase">${lead.nomeLoja}</p>
            <div class="mt-3 flex justify-between items-center">
                <span class="text-gray-400 text-[10px] italic">${lead.segmento || 'Segmento n/i'}</span>
                <span class="text-white font-black text-[10px]">${lead.faturamento || ''}</span>
            </div>
        </div>
    `).join('');
}
