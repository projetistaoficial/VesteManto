...
function renderClients(clients) {
    ...
    clients.forEach(client => {
        ...
        // Ajustamos os tamanhos: col-3 (Nome), col-2 (Documento), col-2 (Link), col-2 (Plano), col-2 (Status) = 12 colunas no total
        row.innerHTML = `
            ${firstCol}
            <div class="col-span-3 font-bold text-white truncate text-sm" title="${client.name || ''}">${client.name || 'Sem Nome'}</div>
            
            <div class="col-span-2 text-center text-gray-400 text-[10px] font-mono truncate" title="${docText}">${docText}</div>
            
            <div class="col-span-2 text-center flex items-center justify-center gap-1 bg-[#0f1014] border border-gray-700 rounded px-2 py-1">
            ...
            <div class="col-span-2 text-center text-gray-400 text-[10px] truncate">${client.plan?.name || '30 dias (Mensal)'}</div>
            <div class="col-span-2 text-center"><span class="${badgeColor} border px-2 py-1 rounded text-[10px] font-bold uppercase block w-full max-w-[90px] mx-auto truncate">${statusText}</span></div>
        `;
