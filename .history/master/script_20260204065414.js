...
// --- RENDERIZAR BOTÕES DE AÇÃO (Lógica Corrigida) ---
function renderActionButtons(status, active = true) {
    const container = document.getElementById('action-buttons-container');
    container.innerHTML = '';

    // 1º: Verifica se está PAUSADO (Prioridade Alta)
    if (status === 'pausado') {
        container.innerHTML = `
    ...
    // 2º: Verifica se está BLOQUEADO ou INATIVO (Active false sem ser pausado)
    ...
    // 3º: Se está ATIVO (Mostra opções de bloquear/pausar)
    else {
        container.innerHTML = `
